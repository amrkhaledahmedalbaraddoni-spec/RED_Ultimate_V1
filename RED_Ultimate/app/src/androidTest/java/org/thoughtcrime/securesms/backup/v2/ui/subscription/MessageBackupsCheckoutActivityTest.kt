...
class MessageBackupsCheckoutActivityTest {
  ...
  @get:Rule val activityRule = REDActivityRule()
  ...
  fun e2e_paid_happy_path() {
    ...
    val iap = REDDatabase.inAppPayments.getLatestInAppPaymentByType(InAppPaymentType.RECURRING_BACKUP)
  ...
  fun e2e_free_happy_path() {
    ...
    assertThat(REDStore.backup.backupTier).isEqualTo(MessageBackupTier.FREE)
  }
  ...
  private fun e2e_shared_happy_path(context: Context, scenario: ActivityScenario<MessageBackupsCheckoutActivity>) {
    ...
    scenario.onActivity {
      val backupKeyString = REDStore.account.accountEntropyPool.displayValue.chunked(4).joinToString("  ")
    ...
    composeTestRule.onNodeWithTag("message-backups-key-verify-screen-backup-key-input-field").performTextInput(
      REDStore.account.accountEntropyPool.displayValue
    )
