 ...
 *
 * The launch harness mirrors [com.red.sovereign.main.MainNavigationLaunchTest]: ActivityScenario can't track
...
class UnreadDividerInstrumentationTest {
  ...
  @get:Rule
  val harness = REDActivityRule(othersCount = 2)
  ...
  fun opensScrolledToOldestUnreadWithCorrectDividerState() {
    ...
    val expectedUnreadCount = REDDatabase.messages.getUnreadCount(threadId)
    val firstUnreadPosition = REDDatabase.messages.getMessagePositionByDateReceivedTimestamp(threadId, oldestSentTime, false)
  ...
  fun fullyReadConversationOpensAtBottomWithoutDivider() {
    ...
    assertThat(REDDatabase.messages.getUnreadCount(threadId)).isEqualTo(0)
  ...
  fun outgoingMessageNewerThanUnreadClearsDivider() {
    ...
    REDDatabase.messages.insertMessageOutbox(outgoing, threadId)
    ...
    assertThat(REDDatabase.messages.getUnreadCount(threadId)).isGreaterThan(0)
  ...
  fun scrollingToBottomMarksEverythingReadAndDrainsUnreadCount() {
    ...
    launch(recipientId).use { launched ->
      ...
      await(timeoutMs = 20_000, description = "unread count reaches 0 after scrolling to the bottom") {
        if (REDDatabase.messages.getUnreadCount(threadId) == 0) true else null
      }
    }
  }
  ...
  private fun awaitStableUnreadCount(threadId: Long, timeoutMs: Long = 20_000): Int {
    ...
    while (System.currentTimeMillis() < deadline) {
      val current = REDDatabase.messages.getUnreadCount(threadId)
      if (current == last) {
  ...
  private fun insertIncoming(threadId: Long, from: RecipientId, time: Long, body: String): Long {
    ...
    return REDDatabase.messages.insertMessageInbox(message, threadId).get().messageId
  }
