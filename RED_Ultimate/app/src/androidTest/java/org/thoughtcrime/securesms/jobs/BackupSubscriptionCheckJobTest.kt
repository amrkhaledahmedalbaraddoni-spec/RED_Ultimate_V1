...
class BackupSubscriptionCheckJobTest {
  ...
  fun givenDeletionStateIsNotNone_whenIRun_thenIExpectSuccessAndEarlyExit() {
    DeletionState.entries.filter { it != DeletionState.NONE }.forEach { deletionState ->
      REDStore.backup.deletionState = deletionState
  ...
  fun givenAPendingPayment_whenIRun_thenIExpectSuccessAndEarlyExit() {
    ...
    assertThat(REDStore.backup.subscriptionStateMismatchDetected).isFalse()
  }
  ...
  fun givenInactiveSubscription_whenIRun_thenIExpectStateMismatchDetected() {
    ...
    assertThat(result.isSuccess).isTrue()
    assertThat(REDStore.backup.subscriptionStateMismatchDetected).isTrue()
  }
  ...
  @Test
  fun givenActiveREDSubscriptionWithTokenMismatch_whenIRun_thenIExpectTokenRedemption() {
    mockActivePurchase()
  ...
  @Test
  fun givenGooglePlayBillingCanceledWithoutActiveREDSubscription_whenIRun_thenIExpectValidCancelState() {
  ...
  @Test
  fun givenGooglePlayBillingCanceledWithFailedREDSubscription_whenIRun_thenIExpectValidCancelState() {
    mockCanceledPurchase()
