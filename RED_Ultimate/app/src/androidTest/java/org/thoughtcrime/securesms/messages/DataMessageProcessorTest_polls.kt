...
class DataMessageProcessorTest_polls {
  ...
  fun handlePollCreate_whenIHaveAValidPollProto_createPoll() {
    ...
    val poll = REDDatabase.polls.getPoll(insertResult!!.messageId)
    assert(poll != null)
  ...
  fun handlePollTerminate_whenIHaveValidProto_endPoll() {
    ...
    val poll = REDDatabase.polls.getPoll(pollMessageId)
    assert(poll != null)
  ...
  private fun insertPoll(allowMultiple: Boolean = true): Long {
    ...
    val messageId = REDDatabase.messages.insertMessageInbox(pollMessage).get()
    REDDatabase.polls.insertPoll("question?", allowMultiple, listOf("a", "b", "c"), alice.id.toLong(), messageId.messageId)
