...
class EditMessageSyncProcessorTest {
  ...
  fun textMessage() {
    ...
    for (i in 1..10) {
      ...
      REDDatabase.recipients.setExpireMessages(toRecipient.id, content.dataMessage?.expireTimer ?: 0, content.dataMessage?.expireTimerVersion ?: 1)
      ...
      REDDatabase.recipients.setExpireMessages(toRecipient.id, (content.dataMessage?.expireTimer ?: 0) / 1000, content.dataMessage?.expireTimerVersion ?: 1)
      ...
      val originalMessageId = REDDatabase.messages.insertMessageOutbox(originalTextMessage, threadId, false, null).messageId
      REDDatabase.messages.markAsSent(originalMessageId, true)
      if ((content.dataMessage?.expireTimer ?: 0) > 0) {
        REDDatabase.messages.markExpireStarted(originalMessageId, originalTimestamp)
      }
      ...
      val editMessageId = REDDatabase.messages.insertMessageOutbox(editMessage, threadId, false, null).messageId
      REDDatabase.messages.markAsSent(editMessageId, true)

      if ((content.dataMessage?.expireTimer ?: 0) > 0) {
        REDDatabase.messages.markExpireStarted(editMessageId, originalTimestamp)
      }
  ...
  private inner class TestResults {
    ...
    fun cleanup() {
      REDDatabase.rawDatabase.withinTransaction { db ->
