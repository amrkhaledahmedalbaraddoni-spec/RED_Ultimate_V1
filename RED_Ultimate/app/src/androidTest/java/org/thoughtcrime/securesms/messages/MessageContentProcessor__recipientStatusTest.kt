...
import com.red.sovereign.messages.REDServiceProtoUtil.buildWith
...
class MessageContentProcessor__recipientStatusTest {
  ...
  fun syncGroupSentTextMessageWithRecipientUpdateFollowup() {
    ...
    val threadId = REDDatabase.threads.getThreadIdFor(groupRecipientId)!!
    val firstSyncMessages = MessageTableTestUtils.getMessages(threadId)
    val firstMessageId = firstSyncMessages[0].id
    val firstReceiptInfo = REDDatabase.groupReceipts.getGroupReceiptInfo(firstMessageId)
    ...
    val secondReceiptInfo = REDDatabase.groupReceipts.getGroupReceiptInfo(firstMessageId)
