...
import com.red.sovereign.testing.GroupTestingUtils
 ...
 */
class MessageHelper(private val harness: REDActivityRule, var startTime: Long = System.currentTimeMillis()) {
  ...
  init {
    ...
    every { ThreadUpdateJob.enqueue(capture(threadIdSlot), any()) } answers {
      REDDatabase.threads.update(threadIdSlot.captured, false)
    }
  }
  ...
  fun outgoingText(conversationId: RecipientId = alice, successfulSend: Boolean = true, updateMessage: ((OutgoingMessage) -> OutgoingMessage)? = null): MessageData {
    ...
    val messageId = REDDatabase.messages.insertMessageOutbox(message, threadId, false, null).messageId

    if (successfulSend) {
      REDDatabase.messages.markAsSent(messageId, true)
    }
  ...
  fun outgoingGroupChange(): MessageData {
    ...
    val decryptedGroupV2Context = DecryptedGroupV2Context(
      ...
      groupState = REDDatabase.groups.getGroup(group.groupId).get().requireV2GroupProperties().decryptedGroup
    )

    val updateDescription = GV2UpdateDescription.Builder()
      .gv2ChangeDescription(decryptedGroupV2Context)
      .groupChangeUpdate(GroupsV2UpdateMessageConverter.translateDecryptedChange(REDStore.account.getServiceIds(), decryptedGroupV2Context))
      .build()
