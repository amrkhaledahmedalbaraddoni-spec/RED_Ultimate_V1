...
class SyncMessageProcessorTest_attachmentBackfill {
  ...
  private var originalDeviceId: Int = REDServiceAddress.DEFAULT_DEVICE_ID

  @Before
  fun setUp() {
    messageHelper = MessageHelper(harness)
    originalDeviceId = REDStore.account.deviceId
  ...
  @After
  fun tearDown() {
    REDStore.account.deviceId = originalDeviceId
  ...
  fun fresh_pointer_updates_row_and_resets_transfer_state() {
    ...
    val refreshed = REDDatabase.attachments.getAttachmentsForMessage(messageId).single()
  ...
  fun multi_attachment_response_matches_positionally_with_mixed_status() {
    ...
    val body = REDDatabase.attachments.getAttachmentsForMessage(messageId).sortedBy { it.displayOrder }
    assertThat(body.size).isEqualTo(2)
    body.forEach { REDDatabase.attachments.setTransferProgressFailed(it.attachmentId, messageId) }
    ...
    val refreshed = REDDatabase.attachments.getAttachmentsForMessage(messageId).sortedBy { it.displayOrder }
  ...
  fun long_text_slot_is_applied_independently_of_the_body() {
    ...
    val all = REDDatabase.attachments.getAttachmentsForMessage(messageId)
    all.forEach { REDDatabase.attachments.setTransferProgressFailed(it.attachmentId, messageId) }
    ...
    val refreshed = REDDatabase.attachments.getAttachmentsForMessage(messageId)
  ...
  fun remote_attachment_list_longer_than_local_skips_extras() {
    ...
    val attachmentId = REDDatabase.attachments.getAttachmentsForMessage(messageId).single().attachmentId
    REDDatabase.attachments.setTransferProgressFailed(attachmentId, messageId)
  ...
  private fun insertIncomingMediaMessage(sender: RecipientId): Pair<Long, AttachmentId> {
    ...
    val messageId = REDDatabase.messages.getMessageIdOrNull(syncMessageId)
    assertThat(messageId, name = "messageId").isNotNull()

    val attachment = REDDatabase.attachments.getAttachmentsForMessage(messageId!!).single()
  ...
  private fun insertIncomingMessageWith(sender: RecipientId, pointers: List<AttachmentPointer>): Long {
    ...
    val messageId = REDDatabase.messages.getMessageIdOrNull(MessageTable.SyncMessageId(sender, sentTimestamp))
    assertThat(messageId, name = "messageId").isNotNull()
    return messageId!!
  }
  ...
  private fun sentTimestampFor(messageId: Long): Long {
    return REDDatabase.messages.getMessageRecord(messageId).dateSent
  }
