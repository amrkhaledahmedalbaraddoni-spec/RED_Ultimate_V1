...
/**
 * Declares remote config values a test needs. The [REDTestRunner] reads this off the
 * about-to-run test (class and/or method) and stages the values into [TestRemoteConfig], which
 * [com.red.sovereign.REDInstrumentationApplicationContext] seeds into the real
...
/**
 * Process-static bridge between [REDTestRunner] (which knows the running test) and
 * [com.red.sovereign.REDInstrumentationApplicationContext] (which seeds the config).
...
object TestRemoteConfig {
  ...
  /**
   * The staged config as a JSON string ready to write into `REDStore.remoteConfig`. Mirrors
