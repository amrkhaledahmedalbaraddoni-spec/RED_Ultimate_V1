...
object IncomingMessageObserverAssertions {
  ...
  fun OtherClient.recipientId(): RecipientId = Recipient.externalPush(REDServiceAddress(serviceId, e164)).id

  fun findIncomingMessage(from: OtherClient, body: String): MessageRecord? {
    val threadId = REDDatabase.threads.getThreadIdFor(from.recipientId()) ?: return null
    return REDDatabase.messages.getConversation(threadId).use { cursor ->
  ...
  fun findIncomingGroupMessage(from: OtherClient, group: GroupHandle, body: String): MessageRecord? {
    val threadId = REDDatabase.threads.getThreadIdFor(group.recipientId) ?: return null
    return REDDatabase.messages.getConversation(threadId).use { cursor ->
  ...
  fun assertNoMessagesInThread(recipientId: RecipientId) {
    val threadId = REDDatabase.threads.getThreadIdFor(recipientId) ?: return
    val count = REDDatabase.messages.getConversation(threadId).use { cursor -> cursor.count }
  ...
  fun assertDeliveryReceipt(outgoingMessageId: Long) {
    val record = REDDatabase.messages.getMessageRecord(outgoingMessageId)
