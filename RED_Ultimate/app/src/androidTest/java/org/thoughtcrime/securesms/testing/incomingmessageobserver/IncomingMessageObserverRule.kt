...
/**
 * JUnit rule that drives [com.red.sovereign.messages.IncomingMessageObserver] from
 * instrumentation tests. Sets up self, registers [peerCount] simulated peers from
 * [Harness.otherClients], establishes a RED double-ratchet session with each, and exposes a
 ...
 * Mutually exclusive with `REDDatabaseRule` / `REDActivityRule` — all three claim the
 * local identity.
 */
class IncomingMessageObserverRule(
  ...
  companion object {
    ...
    private fun Envelope.toWebSocketPayload(): WebSocketRequestMessage = WebSocketRequestMessage(
      ...
      path = "/api/v1/message",
      id = Random.nextLong(),
      headers = listOf("X-RED-Timestamp: $serverTimestamp"),
