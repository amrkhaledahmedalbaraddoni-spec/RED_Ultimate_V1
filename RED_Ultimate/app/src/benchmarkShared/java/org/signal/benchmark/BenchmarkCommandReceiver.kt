...
class BenchmarkCommandReceiver : BroadcastReceiver() {
  ...
  private fun handleDeleteThread() {
    val threadId = REDDatabase.rawDatabase.rawQuery("SELECT thread_id, COUNT(*) AS msg_count FROM message GROUP BY thread_id ORDER BY msg_count DESC LIMIT 1").use { cursor ->
    ...
    val recipientName = REDDatabase.threads.getRecipientForThreadId(threadId)
  ...
  private fun Envelope.toWebSocketPayload(): WebSocketRequestMessage {
    return WebSocketRequestMessage(
      ...
      path = "/api/v1/message",
      id = Random.nextLong(),
      headers = listOf("X-RED-Timestamp: ${this.serverTimestamp}"),
