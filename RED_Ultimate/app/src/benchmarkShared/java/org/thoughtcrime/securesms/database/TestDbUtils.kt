...
object TestDbUtils {
  ...
  fun setMessageReceived(messageId: Long, timestamp: Long) {
    val database: SQLiteDatabase = REDDatabase.messages.databaseHelper.signalWritableDatabase
  ...
  fun insertReactionsForThread(threadId: Long, authorId: RecipientId, moduloFilter: Int) {
    val db = REDDatabase.messages.databaseHelper.signalWritableDatabase
    db.execSQL(
      """
  ...
  fun getOutgoingMessageTimestamps(threadId: Long, selfRecipientId: Long): List<Long> {
    ...
    REDDatabase.messages.databaseHelper.signalReadableDatabase.query(
