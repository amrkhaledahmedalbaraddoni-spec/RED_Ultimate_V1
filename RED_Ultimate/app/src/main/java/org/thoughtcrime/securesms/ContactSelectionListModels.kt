...
import com.red.sovereign.contacts.paged.ContactSearchModels.EmptyModel
...
import com.red.sovereign.util.adapter.mapping.compose.MappingEntryProviderBuilder

/**
 * Holds the [MappingModel]s and [MappingViewHolder]s used by [ContactSelectionListAdapter] on top of
 * the base set in [com.red.sovereign.contacts.paged.ContactSearchModels], along with helpers
...
object ContactSelectionListModels {
  ...
  fun registerInviteToRED(mappingAdapter: MappingAdapter, onClick: () -> Unit) {
    mappingAdapter.registerFactory(
      InviteToREDModel::class.java,
      LayoutFactory({ InviteToREDViewHolder(it, onClick) }, R.layout.contact_selection_invite_action_item)
    )
  }
  ...
  fun composeEntries(
    ...
    return MappingEntryProviderBuilder<Any>().apply {
      ...
      viewHolder<InviteToREDModel> { context ->
        LayoutFactory(
          { view -> InviteToREDViewHolder(view, callback::onInviteToREDClicked) },
  ...
  class InviteToREDModel : MappingModel<InviteToREDModel> {
    override fun areItemsTheSame(newItem: InviteToREDModel): Boolean = true
    override fun areContentsTheSame(newItem: InviteToREDModel): Boolean = true
  }
  ...
  private class InviteToREDViewHolder(itemView: View, onClickListener: () -> Unit) : MappingViewHolder<InviteToREDModel>(itemView) {
    init {
      itemView.setOnClickListener { onClickListener() }
    }

    override fun bind(model: InviteToREDModel) = Unit
  }
