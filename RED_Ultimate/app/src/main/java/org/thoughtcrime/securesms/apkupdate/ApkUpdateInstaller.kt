...
package com.red.sovereign.apkupdate
...
import com.red.sovereign.jobs.ApkUpdateJob
...
object ApkUpdateInstaller {
  ...
  fun installOrPromptForInstall(context: Context, downloadId: Long, userInitiated: Boolean) {
    if (downloadId != REDStore.apkUpdate.downloadId) {
      Log.w(TAG, "DownloadId doesn't match the one we're waiting for (current: $downloadId, expected: ${REDStore.apkUpdate.downloadId})! We likely have newer data. Ignoring.")
      ApkUpdateNotifications.dismissInstallPrompt(context)
      AppDependencies.jobManager.add(ApkUpdateJob())
      return
    }

    val digest = REDStore.apkUpdate.digest
    if (digest == null) {
    ...
    if (!userInitiated && !shouldAutoUpdate()) {
      ...
      Log.w(TAG, "Not user-initiated and not eligible for auto-update. Prompting. (API=${Build.VERSION.SDK_INT}, Foreground=${AppForegroundObserver.isForegrounded()}, AutoUpdate=${REDStore.apkUpdate.autoUpdate})")
...
//
//    return Build.VERSION.SDK_INT >= 31 && REDStore.apkUpdate.autoUpdate && !AppForegroundObserver.isForegrounded() && !isCallActive
  }
}
