...
import com.red.sovereign.components.ProgressCardDialogFragmentArgs
...
import com.red.sovereign.util.SaveAttachmentUtil.SaveAttachment
import com.red.sovereign.util.SaveAttachmentUtil.SaveAttachmentsResult
...
class AttachmentSaver(private val host: Host) {
  ...
  private suspend fun checkIsSaveWarningAccepted(attachmentCount: Int): SaveToStorageWarningResult {
    if (REDStore.uiHints.hasDismissedSaveStorageWarning()) {
  ...
  private suspend fun saveToStorage(attachments: Set<SaveAttachment>): SaveAttachmentsResult {
    ...
    return try {
      ...
      withContext(REDDispatchers.Main) {
  ...
  data class FragmentHost(private val fragment: Fragment) : Host {
    ...
    override suspend fun showSaveToStorageWarning(attachmentCount: Int): SaveToStorageWarningResult = withContext(REDDispatchers.Main) {
      ...
      if (result == AlertDialogResult.POSITIVE) {
        ...
        if (dontShowAgainCheckbox.isChecked) {
          REDStore.uiHints.markDismissedSaveStorageWarning()
        }
    ...
    override suspend fun requestWriteExternalStoragePermission(): RequestPermissionResult = withContext(REDDispatchers.Main) {
