...
object AttachmentUploadUtil {
  ...
  /**
   * Builds a [REDServiceAttachmentStream] from the provided data, which can then be provided to various upload methods.
   */
  @Throws(IOException::class)
  fun buildREDServiceAttachmentStream(
    context: Context,
    attachment: Attachment,
    cancellationRED: (() -> Boolean)? = null,
    progressListener: ProgressListener? = null
  ): REDServiceAttachmentStream {
    val inputStream = PartAuthority.getAttachmentStream(context, attachment.uri!!)
    val builder = REDServiceAttachment.newStreamBuilder()
      ...
      .withCancelationRED(cancellationRED)
