...
class PointerAttachment : Attachment {
  ...
  companion object {
    ...
    @JvmStatic
    fun forPointers(pointers: Optional<List<REDServiceAttachment>>): List<Attachment> {
    ...
    @JvmOverloads
    fun forPointer(
      pointer: Optional<REDServiceAttachment>,
    ...
    fun forPointer(quotedAttachment: DataMessage.Quote.QuotedAttachment): Optional<Attachment> {
      val thumbnail: REDServiceAttachment? = try {
        if (quotedAttachment.thumbnail != null) {
          AttachmentPointerUtil.createREDAttachmentPointer(quotedAttachment.thumbnail)
        } else {
          null
        }
