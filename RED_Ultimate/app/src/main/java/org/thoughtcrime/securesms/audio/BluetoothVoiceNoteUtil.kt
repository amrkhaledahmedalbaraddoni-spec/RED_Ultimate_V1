...
import com.red.sovereign.webrtc.audio.REDAudioHandler
...
private class BluetoothVoiceNoteUtilLegacy(val context: Context, val listener: (Boolean) -> Unit, val bluetoothPermissionDeniedHandler: () -> Unit) : BluetoothVoiceNoteUtil {
  private val commandAndControlThread: HandlerThread = REDExecutors.getAndStartHandlerThread("voice-note-audio", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
  private val uiThreadHandler = Handler(context.mainLooper)
  private val audioHandler: REDAudioHandler = REDAudioHandler(commandAndControlThread.looper)
  ...
  private val signalBluetoothManager: REDBluetoothManager = REDBluetoothManager(context, deviceUpdatedListener, audioHandler)
  ...
  override fun connectBluetoothScoConnection() {
    audioHandler.post {
      ...
      if (currentState == REDBluetoothManager.State.AVAILABLE) {
        Log.d(TAG, "Bluetooth manager state is AVAILABLE. Starting SCO connection.")
        signalBluetoothManager.startScoAudio()
      } else {
        Log.d(TAG, "Recording from phone mic because bluetooth state was " + currentState + ", not " + REDBluetoothManager.State.AVAILABLE)
        uiThreadHandler.post {
          if (currentState == REDBluetoothManager.State.PERMISSION_DENIED && !hasWarnedAboutBluetooth) {
  ...
  override fun disconnectBluetoothScoConnection() {
    audioHandler.post {
      if (signalBluetoothManager.state == REDBluetoothManager.State.CONNECTED) {
