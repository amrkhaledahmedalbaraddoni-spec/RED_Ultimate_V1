 ...
 * reports that to the [com.red.sovereign.webrtc.audio.REDAudioManager], and then handles connecting/disconnecting
 * to the device if requested by [com.red.sovereign.webrtc.audio.REDAudioManager].
 */
@SuppressLint("MissingPermission") // targetSdkVersion is still 30 (https://issuetracker.google.com/issues/201454155)
class REDBluetoothManager(
  private val context: Context,
  private val audioDeviceUpdatedListener: AudioDeviceUpdatedListener,
  private val handler: REDAudioHandler
) {
  ...
  companion object {
    private val TAG = Log.tag(REDBluetoothManager::class.java)
