...
object BackupVerifier {
  ...
  fun verifyFile(cipherStream: InputStream, passphrase: String, expectedCount: Long, cancellationRED: FullBackupExporter.BackupCancellationRED): Boolean {
    ...
    cipherStream.use {
      while (frame.end != true && !cancellationRED.isCanceled) {
        val verified = when {
