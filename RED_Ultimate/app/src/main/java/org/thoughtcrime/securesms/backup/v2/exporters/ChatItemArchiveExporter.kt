...
package com.red.sovereign.backup.v2.exporters
...
import com.red.sovereign.backup.v2.BackupMode
...
import com.red.sovereign.backup.v2.util.clampToValidBackupRange
import com.red.sovereign.backup.v2.util.toRemoteFilePointer
...
import com.red.sovereign.util.mb
...
class ChatItemArchiveExporter(
  ...
  private fun fetchExtraMessageData(db: REDDatabase, messageIds: Set<Long>): ExtraMessageData {
...
private fun BackupMessageRecord.toRemoteGroupUpdate(): ChatUpdateMessage? {
  ...
  body?.let { body ->
    return try {
      ...
      ChatUpdateMessage(
        groupChange = GroupsV2UpdateMessageConverter.translateDecryptedChange(selfIds = REDStore.account.getServiceIds(), context)
      )
    } catch (e: IOException) {
...
private fun BackupMessageRecord.toRemotePaymentNotificationUpdate(db: REDDatabase): PaymentNotification {
