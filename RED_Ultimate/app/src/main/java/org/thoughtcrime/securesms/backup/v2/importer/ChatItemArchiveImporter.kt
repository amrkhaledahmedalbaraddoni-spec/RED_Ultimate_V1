...
import com.red.sovereign.database.model.databaseprotos.PaymentTombstone
...
import com.red.sovereign.payments.CryptoValueUtil
import com.red.sovereign.payments.Direction
import com.red.sovereign.payments.FailureReason
import com.red.sovereign.payments.State
import com.red.sovereign.payments.proto.PaymentMetaData
...
class ChatItemArchiveImporter(
  ...
  private fun ChatItem.toMessageInsert(fromRecipientId: RecipientId, chatRecipientId: RecipientId, threadId: Long): MessageInsert {
    ...
    if (updateMessage != null) {
      ...
      if (individualCall != null && individualCall.callId != null) {
        ...
        followUps += { endPollMessageId ->
          val pollMessageId = REDDatabase.messages.getMessageFor(pollTerminate.targetSentTimestamp, fromRecipientId)?.id ?: -1
          val pollId = REDDatabase.polls.getPollId(pollMessageId)
          ...
          if (pollId != null) {
            REDDatabase.polls.endPoll(pollId = pollId, endingMessageId = endPollMessageId)
          }
        }
        ...
        followUps += { pinUpdateMessageId ->
          ...
          if (targetAuthorId != null && targetAuthorAci != null) {
            val pinnedMessageId = REDDatabase.messages.getMessageFor(pinMessage.targetSentTimestamp, targetAuthorId)?.id ?: -1
      ...
      if (contact != null) {
        ...
        followUps += { messageRowId ->
          val attachmentMap = if (contactAttachment != null) {
            REDDatabase.attachments.insertAttachmentsForMessage(messageRowId, listOf(contactAttachment), emptyList())
          } else {
            emptyMap()
          }
          db.update(
            ...
            contentValuesOf(
              MessageTable.SHARED_CONTACTS to REDDatabase.messages.getSerializedSharedContacts(attachmentMap, listOf(contact))
            ),
            "${MessageTable.ID} = ?",
    ...
    if (directStoryReplyMessage != null) {
      ...
      if (longTextAttachment != null) {
        followUps += { messageRowId ->
          val ids = REDDatabase.attachments.insertAttachmentsForMessage(messageRowId, listOf(longTextAttachment), emptyList())
          ids.values.firstOrNull()?.let { attachmentId ->
            REDDatabase.attachments.setTransferState(messageRowId, attachmentId, AttachmentTable.TRANSFER_PROGRESS_DONE)
      ...
      if (hasAttachments || linkPreviews.isNotEmpty()) {
        followUps += { messageRowId ->
          val attachmentMap = if (hasAttachments) {
            REDDatabase.attachments.insertAttachmentsForMessage(messageRowId, attachments + linkPreviewAttachments + longTextAttachment.asList(), quoteAttachments)
          } else {
            emptyMap()
          }
          ...
          if (linkPreviews.isNotEmpty()) {
            db.update(MessageTable.TABLE_NAME)
              .values(MessageTable.LINK_PREVIEWS to REDDatabase.messages.getSerializedLinkPreviews(attachmentMap, linkPreviews))
              .where("${MessageTable.ID} = ?", messageRowId)
              .run()
          }
        }
    ...
    if (stickerMessage != null) {
      ...
      if (attachment != null) {
        followUps += { messageRowId ->
          REDDatabase.attachments.insertAttachmentsForMessage(messageRowId, listOf(attachment), emptyList())
        }
      }
    }
    ...
    if (poll != null) {
      ...
      followUps += { messageRowId ->
        val pollId = REDDatabase.polls.insertPoll(
        ...
        val localOptionIds = REDDatabase.polls.getPollOptionIds(pollId)
        poll.options.forEachIndexed { index, option ->
          ...
          REDDatabase.polls.addPollVotes(pollId = pollId, optionId = localOptionIds[index], voters = localVoters)
        }
        ...
        if (poll.hasEnded) {
          ...
          REDDatabase.polls.endPoll(pollId = pollId, endingMessageId = -1)
        }
      }
    }
  ...
  private fun tryRestorePayment(chatItem: ChatItem, chatRecipientId: RecipientId): UUID? {
    ...
    return REDDatabase.payments.restoreFromBackup(
      chatRecipientId,
  ...
  private fun ChatItem.toGroupReceiptContentValues(messageId: Long, chatBackupRecipientId: Long): List<ContentValues> {
    ...
    // SIGNAL_INHERITED: TODO [backup] This seems like an indirect/bad way to detect if this is a 1:1 or group convo
  ...
  private fun ContentValues.addFailedPaymentNotification(chatItem: ChatItem, amount: Money, fee: Money, chatRecipientId: RecipientId) {
    val uuid = REDDatabase.payments.restoreFromBackup(
      chatRecipientId,
      0,
      0,
      0,
  ...
  private fun ContentValues.addIdentityKeyMismatches(chatItem: ChatItem, importState: ImportState) {
    ...
    val mismatches = outgoing.sendStatus
      ...
      .mapNotNull { status -> importState.remoteToLocalRecipientId[status.recipientId] }
      .map { recipientId -> IdentityKeyMismatch(recipientId, null) } // SIGNAL_INHERITED: TODO We probably want the actual identity key in this status situation?
      .toSet()
  ...
  private fun LinkPreview.toLocalLinkPreview(): com.red.sovereign.linkpreview.LinkPreview {
    return com.red.sovereign.linkpreview.LinkPreview(
      this.url,
      this.title ?: "",
      this.description ?: "",
      this.date ?: 0,
