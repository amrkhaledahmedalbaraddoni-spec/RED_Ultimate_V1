...
import com.red.sovereign.backup.v2.ImportResult
...
object LocalArchiver {
   ...
   */
  fun export(snapshotFileSystem: SnapshotFileSystem, filesFileSystem: FilesFileSystem, stopwatch: Stopwatch, cancellationRED: () -> Boolean = { false }): ArchiveResult {
    ...
    try {
      ...
      BackupRepository.exportForLocalBackup(mainStream, LocalExportProgressListener(), cancellationRED) { attachment, source ->
        if (cancellationRED() || createFailures.size > MAX_CREATE_FAILURES) {
        ...
        for (attachment in attachments) {
          ...
          try {
            ...
            contentResolver.openOutputStream(mediaFile.uri)?.use { out ->
              REDDatabase.attachments.getAttachmentStream(attachment).use { input ->
  ...
  private fun getEncryptedBackupId(): Metadata.EncryptedBackupId {
    val metadataKey = REDStore.backup.messageBackupKey.deriveLocalBackupMetadataKey()
    val iv = Util.getSecretBytes(12)
    val backupId = REDStore.backup.messageBackupKey.deriveBackupId(REDStore.account.requireAci())
