...
import com.red.sovereign.backup.v2.database.restoreSelfFromBackup
import com.red.sovereign.backup.v2.database.restoreWallpaperAttachment
import com.red.sovereign.backup.v2.util.ChatStyleConverter
import com.red.sovereign.backup.v2.util.isValid
import com.red.sovereign.backup.v2.util.isValidUsername
import com.red.sovereign.backup.v2.util.parseChatWallpaper
import com.red.sovereign.backup.v2.util.toLocal
import com.red.sovereign.backup.v2.util.toLocalAttachment
...
object AccountDataArchiveProcessor {
  ...
  fun export(db: REDDatabase, signalStore: REDStore, exportState: ExportState, emitter: BackupFrameEmitter) {
    ...
    val chatColors = REDStore.chatColors.chatColors
    val chatWallpaper = REDStore.wallpaper.currentRawWallpaper
    ...
    emitter.emit(
      Frame(
        account = AccountData(
          ...
          ).takeUnless { Environment.IS_INSTRUMENTATION && REDStore.backup.importedEmptyAndroidSettings },
  ...
  fun import(accountData: AccountData, selfId: RecipientId, importState: ImportState) {
    REDDatabase.recipients.restoreSelfFromBackup(accountData, selfId)

    REDStore.account.setRegistered(true)
    if (accountData.svrPin.isNotBlank()) {
      REDStore.svr.setPin(accountData.svrPin)
    }
    ...
    if (androidSpecificSettings != null) {
      REDStore.settings.isPreferSystemEmoji = androidSpecificSettings.useSystemEmoji
      TextSecurePreferences.setScreenSecurityEnabled(context, androidSpecificSettings.screenshotSecurity)
      REDStore.settings.useCompactNavigationBar = androidSpecificSettings.navigationBarSize.toLocalNavigationBarSize()
    } else if (Environment.IS_INSTRUMENTATION) {
      REDStore.backup.importedEmptyAndroidSettings = true
    }

    if (accountData.bioText.isNotBlank() || accountData.bioEmoji.isNotBlank()) {
      REDDatabase.recipients.setAbout(selfId, accountData.bioText.takeIf { it.isNotBlank() }, accountData.bioEmoji.takeIf { it.isNotBlank() })
    }
    ...
    if (usernameLink != null) {
      ...
      REDStore.misc.usernameQrCodeColorScheme = usernameLink.color.toLocalUsernameColor()
    } else {
      REDStore.account.usernameLink = null
    }

    REDDatabase.recipients.clearSelfKeyTransparencyData()

    REDDatabase.runPostSuccessfulTransaction { ProfileUtil.handleSelfProfileKeyChange() }
  ...
  private fun importSettings(context: Context, settings: AccountData.AccountSettings, importState: ImportState) {
    ...
    REDStore.settings.isLinkPreviewsEnabled = settings.linkPreviews
    REDStore.phoneNumberPrivacy.phoneNumberDiscoverabilityMode = if (settings.notDiscoverableByPhoneNumber) PhoneNumberDiscoverabilityMode.NOT_DISCOVERABLE else PhoneNumberDiscoverabilityMode.DISCOVERABLE
    REDStore.phoneNumberPrivacy.phoneNumberSharingMode = settings.phoneNumberSharingMode.toLocalPhoneNumberMode()
    REDStore.settings.isPreferSystemContactPhotos = settings.preferContactAvatars
    REDStore.settings.universalExpireTimer = settings.universalExpireTimerSeconds
    REDStore.emoji.reactions = settings.preferredReactionEmoji
    REDStore.inAppPayments.setDisplayBadgesOnProfile(settings.displayBadgesOnProfile)
    REDStore.settings.setKeepMutedChatsArchived(settings.keepMutedChatsArchived)
    REDStore.story.userHasBeenNotifiedAboutStories = settings.hasSetMyStoriesPrivacy
    REDStore.story.userHasViewedOnboardingStory = settings.hasViewedOnboardingStory
    REDStore.story.isFeatureDisabled = settings.storiesDisabled
    REDStore.story.userHasSeenGroupStoryEducationSheet = settings.hasSeenGroupStoryEducationSheet
    REDStore.story.viewedReceiptsEnabled = settings.storyViewReceiptsEnabled ?: settings.readReceipts
    REDStore.backup.optimizeStorage = !importState.backupMode.isLinkAndSync && settings.optimizeOnDeviceStorage
    REDStore.backup.backupTier = MessageBackupTier.fromBackupLevel(settings.backupTier)
    REDStore.settings.sentMediaQuality = settings.defaultSentMediaQuality.toLocalSentMediaQuality()
    REDStore.settings.setTheme(settings.appTheme.toLocalTheme())
    REDStore.settings.setCallDataMode(settings.callsUseLessDataSetting.toLocalCallDataMode())
    REDStore.settings.automaticVerificationEnabled = settings.allowAutomaticKeyVerification
    ...
    if (screenLockTimeoutMinutes != null) {
      REDStore.settings.screenLockTimeout = screenLockTimeoutMinutes.minutes.inWholeSeconds
    }

    val pinReminders = settings.pinReminders
    if (pinReminders != null) {
      REDStore.pin.setPinRemindersEnabled(pinReminders)
    }
    ...
    settings.customChatColors
      ...
      .forEach { chatColor ->
        ...
        val saved = REDDatabase.chatColors.saveChatColors(chatColor.withId(ChatColors.Id.NotSet))
    ...
    if (defaultChatStyle != null) {
      ...
      val wallpaperAttachmentId: AttachmentId? = defaultChatStyle.wallpaperPhoto?.let { filePointer ->
        filePointer.toLocalAttachment()?.let {
          REDDatabase.attachments.restoreWallpaperAttachment(it)
        }
      }

      REDStore.wallpaper.wallpaper = defaultChatStyle.parseChatWallpaper(wallpaperAttachmentId)
    } else {
      REDStore.chatColors.chatColors = null
      REDStore.wallpaper.wallpaper = null
    }
