...
import com.red.sovereign.database.ChatFolderTables.ChatFolderMembershipTable
import com.red.sovereign.database.ChatFolderTables.ChatFolderTable
import com.red.sovereign.database.ChatFolderTables.MembershipType
...
object ChatFolderArchiveProcessor {
  ...
  fun import(chatFolder: ChatFolderProto, importState: ImportState) {
    ...
    includedChatsQueries.forEach {
      REDDatabase.writableDatabase.execSQL(it.where, it.whereArgs)
    }
...
private fun List<Long>.toMembershipInsertQueries(chatFolderId: Long, importState: ImportState, membershipType: MembershipType): List<SqlUtil.Query> {
  val values = this
    ...
    .map { recipientId -> importState.recipientIdToLocalThreadId[recipientId] ?: REDDatabase.threads.getOrCreateThreadIdFor(recipientId, importState.recipientIdToIsGroup[recipientId] == true) }
    .map { threadId ->
      contentValuesOf(
