...
object BackupAlertDelegate {
    ...
    lifecycle.coroutineScope.launch {
      lifecycle.repeatOnLifecycle(Lifecycle.State.RESUMED) {
        if (BackupRepository.shouldDisplayBackupFailedSheet()) {
          ...
          BackupAlertBottomSheet.create(BackupAlert.CouldNotCompleteBackup(daysSinceLastBackup = REDStore.backup.daysSinceLastBackup)).show(fragmentManager, FRAGMENT_TAG)
  ...
  private suspend fun displayBackupDownloadNotifier(fragmentManager: FragmentManager) {
    val downloadYourBackupToday = withContext(REDDispatchers.Default) { BackupRepository.getDownloadYourBackupData() }
