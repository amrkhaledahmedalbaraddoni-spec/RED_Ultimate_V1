...
import com.red.sovereign.components.settings.app.subscription.DonationEvent
...
class GiftFlowViewModel(
  ...
  private val store = RxStore(
    GiftFlowState(
      currency = REDStore.inAppPayments.getOneTimeCurrency()
    )
  )
  ...
  fun refresh() {
    disposables.clear()
    disposables += REDStore.inAppPayments.observableOneTimeCurrency.subscribe { currency ->
      store.update {
        it.copy(
          currency = currency
        )
