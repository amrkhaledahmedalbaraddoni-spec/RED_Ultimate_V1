...
class OutdatedBuildBanner : Banner<Int>() {
  ...
  override val enabled: Boolean
    get() {
      val daysUntilExpiry = Util.getTimeUntilBuildExpiry(REDStore.misc.estimatedServerTime).milliseconds.inWholeDays.toInt()
      return daysUntilExpiry <= MAX_DAYS_UNTIL_EXPIRE
    }

  override val dataFlow: Flow<Int>
    get() = flowOf(Util.getTimeUntilBuildExpiry(REDStore.misc.estimatedServerTime).milliseconds.inWholeDays.toInt())
