...
import com.red.sovereign.database.CallTable.Direction
import com.red.sovereign.database.CallTable.Event
import com.red.sovereign.database.CallTable.Type
...
class CallEventCache(
  private val executor: Executor = REDExecutors.newCachedSingleThreadExecutor("call-event-cache", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
) {
  companion object {
    ...
    private fun readDataFromDatabase(): List<CacheRecord> {
      ...
      val events = REDDatabase.calls.getCallsForCache(10_000).readToList { it.readCacheRecord() }
    ...
    private fun canUserBeginCall(peer: Recipient, decryptedGroup: ByteArray?): CallLogRow.CanStartCall {
      if (peer.isGroup && decryptedGroup != null) {
        ...
        when {
          ...
          DecryptedGroupUtil.findMemberByAci(proto.members, REDStore.account.requireAci()).isEmpty -> return CallLogRow.CanStartCall.NOT_A_MEMBER
          proto.isAnnouncementGroup == EnabledState.ENABLED && proto.members.firstOrNull { it.aciBytes == REDStore.account.aci?.toByteString() }?.role != Member.Role.ADMINISTRATOR -> return CallLogRow.CanStartCall.ADMIN_ONLY
        }
      }
