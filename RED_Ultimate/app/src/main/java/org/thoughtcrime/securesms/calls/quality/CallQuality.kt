...
package com.red.sovereign.calls.quality
...
object CallQuality {
  ...
  private fun handleCallSummary(callSummary: CallSummary, callType: CallType) {
    if (isCallQualitySurveyRequired(callSummary)) {
      REDStore.callQuality.surveyRequest = SubmitCallQualitySurveyRequest.Builder()
      ...
      REDStore.callQuality.surveyRequest = null
    }
  }
  ...
  fun consumeQualityRequest(): SubmitCallQualitySurveyRequest? {
    val request = REDStore.callQuality.surveyRequest ?: return null
    REDStore.callQuality.surveyRequest = null
  ...
  private fun isCallQualitySurveyRequired(callSummary: CallSummary): Boolean {
    ...
    val lastFailure = REDStore.callQuality.lastFailureReportTime ?: 0.milliseconds
    val failureDelta = now - lastFailure

    if (!isSuccess && (failureDelta < 1.days)) {
      REDStore.callQuality.lastFailureReportTime = now
      return true
    }

    if (isSuccess) {
      val lastSurveyPromptTime = REDStore.callQuality.lastSurveyPromptTime ?: 0.milliseconds
  ...
  private fun isFeatureEnabled(): Boolean {
    return RemoteConfig.callQualitySurvey && REDStore.callQuality.isQualitySurveyEnabled
  }
