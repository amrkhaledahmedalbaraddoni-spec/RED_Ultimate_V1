...
import com.red.sovereign.util.dualsim.SubscriptionInfoCompat;
import com.red.sovereign.util.dualsim.SubscriptionManagerCompat;
    ...
    if (messageRecord.getExpiresIn() > 0 && !messageRecord.isPending()) {
      ...
      if (messageRecord.getExpireStarted() > 0) {
        ...
        REDExecutors.BOUNDED.execute(() -> {
          ...
          REDDatabase.messages().markExpireStarted(id, now);
  ...
  private void presentDeliveryStatus(@NonNull MessageRecord messageRecord) {
    ...
    if (previousMessageId == newMessageId && deliveryStatusView.isPending() && !messageRecord.isPending()) {
      if (messageRecord.getToRecipient().isGroup()) {
        REDLocalMetrics.GroupMessageSend.onUiUpdated(messageRecord.getId());
      } else {
        REDLocalMetrics.IndividualMessageSend.onUiUpdated(messageRecord.getId());
      }
    }
