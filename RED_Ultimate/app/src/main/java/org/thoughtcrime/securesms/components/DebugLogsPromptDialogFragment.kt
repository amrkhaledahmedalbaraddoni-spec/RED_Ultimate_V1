...
import com.red.sovereign.databinding.PromptLogsBottomSheetBinding
...
class DebugLogsPromptDialogFragment : FixedRoundedCornerBottomSheetDialogFragment() {
  ...
  companion object {
    ...
    fun show(activity: AppCompatActivity, purpose: Purpose) {
      ...
      if (NetworkUtil.isConnected(activity) && activity.supportFragmentManager.findFragmentByTag(BottomSheetUtil.STANDARD_BOTTOM_SHEET_FRAGMENT_TAG) == null) {
        ...
        when (purpose) {
          Purpose.NOTIFICATIONS -> REDStore.uiHints.lastNotificationLogsPrompt = System.currentTimeMillis()
          Purpose.CRASH -> REDStore.uiHints.lastCrashPrompt = System.currentTimeMillis()
          Purpose.CONNECTIVITY_WARNING -> REDStore.misc.lastConnectivityWarningTime = System.currentTimeMillis()
        }
      }
    }
  }
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    binding.submit.setOnClickListener {
      val progressDialog = REDProgressDialog.show(requireContext())
    ...
    binding.decline.setOnClickListener {
      if (purpose == Purpose.NOTIFICATIONS) {
        REDStore.uiHints.markDeclinedShareNotificationLogs()
      }
