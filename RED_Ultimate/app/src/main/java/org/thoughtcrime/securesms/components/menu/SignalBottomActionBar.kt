 ...
 *
 * Overflow items are rendered in a [REDContextMenu].
 */
class REDBottomActionBar(context: Context, attributeSet: AttributeSet?) : LinearLayout(context, attributeSet) {
    ...
    if (overflowItems.isNotEmpty()) {
      ...
      bindItem(
        ...
        ActionItem(
          ...
          title = context.getString(R.string.REDBottomActionBar_more),
          action = {
            REDContextMenu.Builder(view, parent as ViewGroup)
...
fun REDBottomActionBar(
  ...
  AnimatedVisibility(
    ...
    AndroidView(
      factory = { context ->
        REDBottomActionBar(context, null)
          .apply {
            elevation = 0f
            setItems(items)
          }
