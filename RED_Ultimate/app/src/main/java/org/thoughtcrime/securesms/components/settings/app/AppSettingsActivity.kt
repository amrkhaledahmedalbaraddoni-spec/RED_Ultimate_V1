...
import com.red.sovereign.profiles.manage.UsernameEditMode
...
class AppSettingsActivity : DSLSettingsActivity(), GooglePayComponent {
  ...
  override fun onCreate(savedInstanceState: Bundle?, ready: Boolean) {
    ...
    val startingAction: NavDirections? = if (intent?.categories?.contains(NOTIFICATION_CATEGORY) == true) {
      ...
      when (val appSettingsRoute: AppSettingsRoute? = intent?.getParcelableExtraCompat(START_ROUTE, AppSettingsRoute::class.java)) {
        ...
        is AppSettingsRoute.BackupsRoute.Local -> {
          if (REDStore.backup.newLocalBackupsEnabled || (Environment.Backups.isNewFormatSupportedForLocalBackup() && (!REDStore.settings.isBackupEnabled || appSettingsRoute.triggerUpdateFlow))) {
        ...
        AppSettingsRoute.AccountRoute.Account -> if (REDStore.account.isPrimaryDevice) {
    ...
    REDStore.settings.onConfigurationSettingChanged.observe(this) { key ->
      ...
      when (intent.getStringExtra(EXTRA_PERFORM_ACTION_ON_CREATE)) {
        ACTION_CHANGE_NUMBER_SUCCESS -> {
          MaterialAlertDialogBuilder(this)
            .setMessage(getString(R.string.ChangeNumber__your_phone_number_has_changed_to_s, REDE164Util.prettyPrint(Recipient.self().requireE164())))
