...
import com.red.sovereign.pin.RegistrationLockV2Dialog
  ...
  private fun setPinRemindersEnabled(enabled: Boolean) {
    if (!enabled) {
      ...
      turnOffButton.setOnClickListener {
        ...
        val correct = PinHashUtil.verifyLocalPinHash(REDStore.svr.localPinHash!!, pin)
        if (correct) {
          REDStore.pin.setPinRemindersEnabled(false)
          viewModel.refreshState()
          dialog.dismiss()
        } else {
      ...
      REDStore.pin.setPinRemindersEnabled(true)
      viewModel.refreshState()
    }
  }
