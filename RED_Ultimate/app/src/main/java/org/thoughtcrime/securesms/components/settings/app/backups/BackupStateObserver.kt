...
class BackupStateObserver(
  ...
  init {
    scope.launch(REDDispatchers.IO) {
    ...
    scope.launch(REDDispatchers.Default) {
    ...
    scope.launch(REDDispatchers.Default) {
      REDStore.backup.subscriptionStateMismatchDetectedFlow.collect {
        requestBackupStateRefresh()
      }
    }

    scope.launch(REDDispatchers.Default) {
      REDStore.backup.deletionStateFlow.collect {
        requestBackupStateRefresh()
      }
    }
  }
  ...
  private fun getDatabaseBackupState(): BackupState {
    ...
    val latestPayment = REDDatabase.inAppPayments.getLatestInAppPaymentByType(InAppPaymentType.RECURRING_BACKUP)
    ...
    val isPending = REDDatabase.inAppPayments.hasPendingBackupRedemption()
    if (isPending) {
    ...
    if (REDStore.backup.subscriptionStateMismatchDetected) {
  ...
  private suspend fun performFullBackupStateRefresh() {
    ...
    withContext(REDDispatchers.IO) {
      val latestInAppPayment = REDDatabase.inAppPayments.getLatestInAppPaymentByType(InAppPaymentType.RECURRING_BACKUP)
  ...
  private suspend fun getNetworkBackupState(lastPurchase: InAppPaymentTable.InAppPayment?): BackupState {
    ...
    if (REDStore.backup.subscriptionStateMismatchDetected) {
      ...
      val googlePlayBillingSubscriptionIsActiveAndWillRenew = when (purchaseResult) {
        ...
        REDStore.backup.backupTierInternalOverride == MessageBackupTier.PAID
    ...
    return when (REDStore.backup.latestBackupTier) {
          ...
          when {
            ...
            subscription.isActive -> {
              ...
              BackupState.ActivePaid(
                messageBackupsType = subscriberType,
                price = FiatMoney.fromREDNetworkAmount(subscription.amount, Currency.getInstance(subscription.currency)),
                renewalTime = subscription.endOfCurrentPeriod.seconds
              )
            }
  ...
  private suspend fun getFreeBackupState(): BackupState {
    ...
    val backupState = if (REDStore.backup.areBackupsEnabled) {
  ...
  private fun buildPaidTypeFromSubscription(subscription: ActiveSubscription.Subscription): MessageBackupsType.Paid? {
    ...
    val price = FiatMoney.fromREDNetworkAmount(subscription.amount, Currency.getInstance(subscription.currency))
