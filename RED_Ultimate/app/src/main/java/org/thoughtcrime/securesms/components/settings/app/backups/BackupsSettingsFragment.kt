...
class BackupsSettingsFragment : ComposeFragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    if (savedInstanceState == null && args.launchCheckoutFlow && REDStore.account.isPrimaryDevice) {
  ...
  override fun FragmentContent() {
    ...
    BackupsSettingsContent(
      ...
      onOnDeviceBackupsRowClick = {
        if (state.isLinkedDevice || REDStore.backup.newLocalBackupsEnabled || (Environment.Backups.isNewFormatSupportedForLocalBackup() && !REDStore.settings.isBackupEnabled)) {
      ...
      onLearnMoreClick = { CommunicationActions.openBrowserLink(requireContext(), "https://support.red.local/hc/articles/360007059752") }
    )
  }
}
  ...
  Rows.TextRow(
    ...
    icon = {
      Box(
        ...
        Icon(
          painter = REDIcons.Backup.painter,
...
private fun InternalBackupOverrideRow(
  ...
  val deletionState by REDStore.backup.deletionStateFlow.collectAsStateWithLifecycle(REDStore.backup.deletionState)
