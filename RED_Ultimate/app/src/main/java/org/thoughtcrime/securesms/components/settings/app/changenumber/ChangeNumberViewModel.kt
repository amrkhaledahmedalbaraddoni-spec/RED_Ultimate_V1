...
import com.red.sovereign.registration.viewmodel.NumberViewState
...
class ChangeNumberViewModel : ViewModel() {
  ...
  private val initialLocalNumber = REDStore.account.e164
  private val password = REDStore.account.servicePassword!!
  ...
  init {
    try {
      val countryCode: Int = PhoneNumberUtil.getInstance()
        .parse(REDStore.account.e164!!, null)
        .countryCode
  ...
  fun reattemptChangeLocalNumber(onSuccess: () -> Unit, onError: (Throwable) -> Unit) {
    require(REDStore.misc.pendingChangeNumberMetadata != null)
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        val remoteMatchesLocal = remoteE164 == REDStore.account.requireE164()
  ...
  private suspend fun changeNumberWithRecoveryPassword(): ChangeLocalNumberOutcome {
    ...
    REDStore.svr.recoveryPassword?.let { recoveryPassword ->
  ...
  private suspend fun getRegistrationData(context: Context): RegistrationData {
    ...
    val recoveryPassword = if (currentState.sessionId == null) REDStore.svr.recoveryPassword else null
