...
class DataAndStorageSettingsViewModel(
  ...
  private fun applyForceWebsocketMode(enabled: Boolean) {
    REDStore.settings.forceWebsocketMode = if (enabled) ForceWebsocketMode.ENABLED_BY_USER else ForceWebsocketMode.DISABLED
    if (!enabled) {
  ...
  private fun getState() = DataAndStorageSettingsState(
    ...
    callDataMode = REDStore.settings.callDataMode,
    isProxyEnabled = REDStore.proxy.isProxyEnabled,
    sentMediaQuality = REDStore.settings.sentMediaQuality,
    forceWebsocketMode = REDStore.settings.forceWebsocketMode.isEnabled,
