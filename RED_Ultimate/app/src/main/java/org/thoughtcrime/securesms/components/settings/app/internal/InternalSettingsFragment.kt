...
import com.red.sovereign.components.settings.app.privacy.advanced.AdvancedPrivacySettingsRepository
...
import com.red.sovereign.payments.DataExportUtil
import com.red.sovereign.recipients.Recipient
import com.red.sovereign.recipients.RecipientId
import com.red.sovereign.registration.data.QuickstartCredentialExporter
...
class InternalSettingsFragment : DSLSettingsFragment(R.string.preferences__internal_preferences, R.menu.internal_settings) {
  ...
  override fun onPause() {
    ...
    if (firstVisiblePosition != null) {
      REDStore.internal.lastScrollPosition = firstVisiblePosition
    }
  }

  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    super.onViewCreated(view, savedInstanceState)
    scrollToPosition = REDStore.internal.lastScrollPosition
  ...
  private fun getConfiguration(state: InternalSettingsState): DSLConfiguration {
    return configure {
      ...
      clickPref(
        ...
        onClick = {
          REDStore.misc.completedCollapsedEventsMigration = false
        ...
        onClick = {
          MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(android.R.string.ok) { _, _ ->
              REDDatabase.signedPreKeys.debugDeleteAll()
              REDDatabase.oneTimePreKeys.debugDeleteAll()
              REDDatabase.kyberPreKeys.debugDeleteAll()
      ...
      switchPref(
        ...
        isChecked = REDStore.internal.anrDetectionCrashes,
        onClick = {
          REDStore.internal.anrDetectionCrashes = !REDStore.internal.anrDetectionCrashes
        }
      )
      ...
      // SIGNAL_INHERITED: TODO [alex] -- db access on main thread!
      ...
      if (state.hasPendingOneTimeDonation) {
        clickPref(
          ...
          onClick = {
            REDStore.inAppPayments.setPendingOneTimeDonation(null)
          }
        )
      } else {
        clickPref(
      ...
      clickPref(
        ...
        onClick = {
          REDStore.releaseChannel.highestVersionNoteReceived = max(REDStore.releaseChannel.highestVersionNoteReceived - 10, 0)
        }
      )
      ...
      clickPref(
        ...
        onClick = {
          REDStore.releaseChannel.previousManifestMd5 = ByteArray(0)
      ...
      clickPref(
        ...
        isEnabled = true,
        onClick = {
          REDStore.story.userHasBeenNotifiedAboutStories = false
        }
      )
      ...
      clickPref(
        ...
        isEnabled = true,
        onClick = {
          REDStore.story.userHasSeenFirstNavView = false
        }
      )
        ...
        onClick = {
          MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(android.R.string.ok) { _, _ ->
              ...
              REDStore.account.username = random
              REDDatabase.recipients.setUsername(Recipient.self().id, random)
          ...
          MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(android.R.string.ok) { _, _ ->
              REDStore.account.usernameLink = UsernameLinkComponents(
                ...
                serverId = REDStore.account.usernameLink?.serverId ?: UUID.randomUUID()
              )
              StorageSyncHelper.scheduleSyncForDataChange()
              Toast.makeText(context, "Done", Toast.LENGTH_SHORT).show()
            }
      ...
      clickPref(
        ...
        onClick = {
          REDStore.uiHints.resetNeverDisplayPullToRefreshCount()
        }
      )
  ...
  private fun onUnregisterClicked() {
    MaterialAlertDialogBuilder(requireContext())
      .setTitle("Unregister?")
      .setMessage("Are you sure? You'll have to re-register to use RED again -- no promises that the process will go smoothly.")
      .setPositiveButton(android.R.string.ok) { _, _ ->
  ...
  private fun clearAllServiceIds() {
    MaterialAlertDialogBuilder(requireContext())
      ...
      .setPositiveButton(android.R.string.ok) { _, _ ->
        REDDatabase.recipients.debugClearServiceIds()
  ...
  private fun clearAllProfileKeys() {
    MaterialAlertDialogBuilder(requireContext())
      ...
      .setMessage("Are you sure? Never do this on a non-test device.")
      .setPositiveButton(android.R.string.ok) { _, _ ->
        REDDatabase.recipients.debugClearProfileData()
  ...
  private fun logPreKeyIds() {
    SimpleTask.run({
      val oneTimePreKeys = REDDatabase.rawDatabase
  ...
  private fun promptUserForSentTimestamp() {
    ...
    MaterialAlertDialogBuilder(requireContext())
      ...
      .setPositiveButton(android.R.string.ok) { _, _ ->
        ...
        val messages = REDDatabase.messages.getMessagesBySentTimestamp(number)
        if (messages.isEmpty()) {
        ...
        val startingPosition = REDDatabase.messages.getMessagePositionInConversation(message.threadId, message.dateReceived)
