...
package com.red.sovereign.components.settings.app.internal.backup
...
import com.red.sovereign.backup.v2.DebugBackupMetadata
...
import com.red.sovereign.database.AttachmentTable.DebugAttachmentStats
...
class InternalBackupPlaygroundViewModel : ViewModel() {
  ...
  private val _state: MutableState<ScreenState> = mutableStateOf(
    ScreenState(
      canReadWriteBackupDirectory = REDStore.settings.signalBackupDirectory?.let {
  ...
  fun validateBackup() {
    ...
    disposables += Single
      .fromCallable {
        ...
        ArchiveValidator.validateLocalOrLinking(tempFile, REDStore.backup.messageBackupKey, forTransfer = false)
      }
      .subscribeOn(Schedulers.io())
      .observeOn(AndroidSchedulers.mainThread())
      .subscribe { result ->
  ...
  fun importEncryptedBackup(length: Long, inputStreamFactory: () -> InputStream) {
    ...
    val backupKey = customCredentials?.messageBackupKey ?: REDStore.backup.messageBackupKey
  ...
  fun fetchRemoteBackupAndWritePlaintext(outputStream: OutputStream?) {
    ...
    REDExecutors.BOUNDED_IO.execute {
      ...
      val forwardSecrecyToken = when (val result = REDNetwork.svrB.restore(svrBAuth, REDStore.backup.messageBackupKey, forwardSecrecyMetadata)) {
  ...
  fun wipeAllDataAndRestoreFromRemote(afterDbRestoreCallback: () -> Unit) {
    REDExecutors.BOUNDED_IO.execute {
      REDStore.backup.restoreWithCellular = false
  ...
  fun onImportConfirmed(aci: String, backupKey: String): Boolean {
    ...
    _state.value = state.value.copy(
      customBackupCredentials = ImportCredentials(
        messageBackupKey = parsedBackupKey ?: REDStore.backup.messageBackupKey,
        aci = parsedAci ?: REDStore.account.aci!!
      )
    )
  ...
  fun loadStats() {
    viewModelScope.launch(Dispatchers.Default) {
      launch {
        var stats = REDDatabase.attachments.debugGetAttachmentStats()
