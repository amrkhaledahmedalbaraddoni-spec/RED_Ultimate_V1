...
package com.red.sovereign.components.settings.app.internal.storage
...
import com.red.sovereign.components.settings.app.internal.storage.InternalStorageServicePlaygroundViewModel.OneOffEvent
import com.red.sovereign.components.settings.app.internal.storage.InternalStorageServicePlaygroundViewModel.StorageInsights
...
fun Screen(
  ...
  storageRecords: List<REDStorageRecord>,
  storageInsights: StorageInsights,
  oneOffEvent: OneOffEvent,
...
fun ToolScreen() {
  Column(
    ...
    ActionRow("Set initial master key", "Initializes it to something random. Will cause a decryption failure.") {
      REDStore.svr.masterKeyForInitialDataRestore = MasterKey(Util.getSecretBytes(32))
    }
...
@Composable
private fun ManifestRow(manifest: REDStorageManifest) {
  Column {
...
@Composable
private fun StorageRecordRow(record: REDStorageRecord) {
  Row(modifier = Modifier.fillMaxWidth()) {
    when {
...
fun ScreenPreview() {
  Previews.Preview {
    Screen(
      manifest = REDStorageManifest.EMPTY,
