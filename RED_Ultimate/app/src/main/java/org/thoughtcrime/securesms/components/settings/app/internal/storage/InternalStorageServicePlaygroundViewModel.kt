...
class InternalStorageServicePlaygroundViewModel : ViewModel() {
  ...
  private val _manifest: MutableState<REDStorageManifest> = mutableStateOf(REDStorageManifest.EMPTY)
  val manifest: State<REDStorageManifest>
    get() = _manifest

  private val _storageItems: MutableState<List<REDStorageRecord>> = mutableStateOf(emptyList())
  val storageRecords: State<List<REDStorageRecord>>
    get() = _storageItems
  ...
  fun onViewTabSelected() {
    viewModelScope.launch {
      withContext(Dispatchers.IO) {
        ...
        val storageKey = REDStore.storageService.storageKeyForInitialDataRestore ?: REDStore.storageService.storageKey
        ...
        // SIGNAL_INHERITED: TODO get total manifest size -- we need the raw proto, which we don't have
  ...
  private fun REDStorageRecord.sizeInBytes(): Int {
