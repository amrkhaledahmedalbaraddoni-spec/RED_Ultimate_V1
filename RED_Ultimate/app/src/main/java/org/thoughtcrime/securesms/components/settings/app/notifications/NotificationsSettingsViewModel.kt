package com.red.sovereign.components.settings.app.notifications
...
import com.red.sovereign.notifications.SlowNotificationHeuristics
...
class NotificationsSettingsViewModel(private val sharedPreferences: SharedPreferences) : ViewModel() {
  ...
  init {
    if (NotificationChannels.supported()) {
      REDStore.settings.messageNotificationSound = NotificationChannels.getInstance().messageRingtone
      REDStore.settings.isMessageVibrateEnabled = NotificationChannels.getInstance().messageVibrate
    }
  ...
  fun setCallNotificationsEnabled(enabled: Boolean) {
    REDStore.settings.isCallNotificationsEnabled = enabled
    refresh()
  }

  fun setCallRingtone(ringtone: Uri?) {
    REDStore.settings.callRingtone = ringtone ?: Uri.EMPTY
    refresh()
  }

  fun setCallVibrateEnabled(enabled: Boolean) {
    REDStore.settings.isCallVibrateEnabled = enabled
    refresh()
  }

  fun setNotifyWhenContactJoinsRED(enabled: Boolean) {
    REDStore.settings.isNotifyWhenContactJoinsRED = enabled
    refresh()
  }
  ...
  private fun getState(currentState: NotificationsSettingsState? = null, calculateSlowNotifications: Boolean = false): NotificationsSettingsState = NotificationsSettingsState(
    messageNotificationsState = MessageNotificationsState(
      notificationsEnabled = REDStore.settings.isMessageNotificationsEnabled && canEnableNotifications(),
      canEnableNotifications = canEnableNotifications(),
      sound = REDStore.settings.messageNotificationSound,
      vibrateEnabled = REDStore.settings.isMessageVibrateEnabled,
      ledColor = REDStore.settings.messageLedColor,
      ledBlink = REDStore.settings.messageLedBlinkPattern,
      inChatSoundsEnabled = REDStore.settings.isMessageNotificationsInChatSoundsEnabled,
      repeatAlerts = REDStore.settings.messageNotificationsRepeatAlerts,
      messagePrivacy = REDStore.settings.messageNotificationsPrivacy.toString(),
    ...
    callNotificationsState = CallNotificationsState(
      notificationsEnabled = REDStore.settings.isCallNotificationsEnabled && canEnableNotifications(),
      canEnableNotifications = canEnableNotifications(),
      ringtone = REDStore.settings.callRingtone,
      vibrateEnabled = REDStore.settings.isCallVibrateEnabled
    ),
    notifyWhenContactJoinsRED = REDStore.settings.isNotifyWhenContactJoinsRED
  )
