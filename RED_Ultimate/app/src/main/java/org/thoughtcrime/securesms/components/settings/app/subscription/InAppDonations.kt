...
object InAppDonations {
  ...
  fun isDonationsPaymentSourceAvailable(paymentSourceType: PaymentSourceType, inAppPaymentType: InAppPaymentType): Boolean {
    ...
    return when (paymentSourceType) {
      PaymentSourceType.PayPal -> isPayPalAvailableForDonateToREDType(inAppPaymentType)
      PaymentSourceType.Stripe.CreditCard -> isCreditCardAvailable()
      PaymentSourceType.Stripe.GooglePay -> isGooglePayAvailable()
      PaymentSourceType.Stripe.SEPADebit -> isSEPADebitAvailableForDonateToREDType(inAppPaymentType)
      PaymentSourceType.Stripe.IDEAL -> isIDEALAvailbleForDonateToREDType(inAppPaymentType)
      PaymentSourceType.GooglePlayBilling -> false
      PaymentSourceType.Unknown -> false
    }
  }

  private fun isPayPalAvailableForDonateToREDType(inAppPaymentType: InAppPaymentType): Boolean {
  ...
  fun isGooglePayAvailable(): Boolean {
    return REDStore.inAppPayments.isGooglePayReady && !LocaleRemoteConfig.isGooglePayDisabled()
  }
   ...
   * and donation type.
   */
  fun isSEPADebitAvailableForDonateToREDType(inAppPaymentType: InAppPaymentType): Boolean {
   ...
   * donation type
   */
  fun isIDEALAvailbleForDonateToREDType(inAppPaymentType: InAppPaymentType): Boolean {
