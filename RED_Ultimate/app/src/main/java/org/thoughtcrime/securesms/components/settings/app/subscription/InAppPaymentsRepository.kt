...
import com.red.sovereign.components.settings.app.subscription.manage.NonVerifiedMonthlyDonation
...
object InAppPaymentsRepository {
  ...
  fun updateInAppPaymentWithCancelation(activeSubscription: ActiveSubscription, subscriberType: InAppPaymentSubscriberRecord.Type) {
    if (activeSubscription.isCanceled || (subscriberType == InAppPaymentSubscriberRecord.Type.BACKUP && activeSubscription.willCancelAtPeriodEnd()) || activeSubscription.isFailedPayment) {
      ...
      val latestPayment = REDDatabase.inAppPayments.getLatestBySubscriberId(subscriber.subscriberId) ?: return
      if (latestPayment.state == InAppPaymentTable.State.END && latestPayment.data.cancellation == null) {
        synchronized(subscriber.type.lock) {
          val payment = REDDatabase.inAppPayments.getLatestBySubscriberId(subscriber.subscriberId) ?: return
  ...
  fun updateInAppPayment(inAppPayment: InAppPaymentTable.InAppPayment): Completable {
    return Completable.fromAction {
      REDDatabase.inAppPayments.update(inAppPayment)
  ...
  fun observeLatestBackupPayment(): Flow<InAppPaymentTable.InAppPayment> {
    return callbackFlow {
      fun refresh() {
        val latest = REDDatabase.inAppPayments.getLatestInAppPaymentByType(InAppPaymentType.RECURRING_BACKUP)
        if (latest != null) {
  ...
  fun requireInAppPayment(inAppPaymentId: InAppPaymentTable.InAppPaymentId): Single<InAppPaymentTable.InAppPayment> {
    return Single.fromCallable {
      REDDatabase.inAppPayments.getById(inAppPaymentId) ?: throw Exception("Not found.")
    }.subscribeOn(Schedulers.io())
  }
  ...
  fun observeUpdates(inAppPaymentId: InAppPaymentTable.InAppPaymentId): Flowable<InAppPaymentTable.InAppPayment> {
    return Flowable.create({ emitter ->
      ...
      REDDatabase.inAppPayments.getById(inAppPaymentId)?.also {
  ...
  fun resolveLock(inAppPaymentId: InAppPaymentTable.InAppPaymentId): Lock {
    val payment = REDDatabase.inAppPayments.getById(inAppPaymentId) ?: error("Not found")
   ...
   * back on information in REDStore. This information is utilized in some error presentation as well as in subscription
   * updates.
   */
  @WorkerThread
  fun getLatestPaymentMethodType(subscriberType: InAppPaymentSubscriberRecord.Type): InAppPaymentData.PaymentMethodType {
    ...
    return if (paymentMethodType != InAppPaymentData.PaymentMethodType.UNKNOWN) {
      ...
      REDStore.inAppPayments.getSubscriptionPaymentSourceType().toPaymentMethodType()
    } else {
  ...
  fun isUserManuallyCancelled(subscriberType: InAppPaymentSubscriberRecord.Type): Boolean {
    return if (subscriberType == InAppPaymentSubscriberRecord.Type.DONATION) {
      ...
      REDStore.inAppPayments.isBackupSubscriptionManuallyCancelled()
    }
  }
  ...
  fun getFallbackLastEndOfPeriod(subscriberType: InAppPaymentSubscriberRecord.Type): Duration {
    return if (subscriberType == InAppPaymentSubscriberRecord.Type.DONATION) {
      REDStore.inAppPayments.getLastEndOfPeriod().seconds
    } else {
      0.seconds
    }
  }
  ...
  fun setShouldCancelSubscriptionBeforeNextSubscribeAttempt(subscriberType: InAppPaymentSubscriberRecord.Type, subscriberId: SubscriberId?, shouldCancel: Boolean) {
    if (subscriberType == InAppPaymentSubscriberRecord.Type.DONATION) {
      REDStore.inAppPayments.shouldCancelSubscriptionBeforeNextSubscribeAttempt = shouldCancel
    }
    ...
    REDDatabase.inAppPaymentSubscribers.setRequiresCancel(
   ...
   * type. This method will first check the database, and then fall back on the deprecated REDStore value. This method
  ...
  fun getShouldCancelSubscriptionBeforeNextSubscribeAttempt(subscriberType: InAppPaymentSubscriberRecord.Type): Boolean {
    ...
    val localState = latestSubscriber?.requiresCancel ?: if (subscriberType == InAppPaymentSubscriberRecord.Type.DONATION) {
      REDStore.inAppPayments.shouldCancelSubscriptionBeforeNextSubscribeAttempt
    } else {
      false
    }
  ...
  fun getRecurringDonationSubscriber(currency: Currency): InAppPaymentSubscriberRecord? {
    val subscriber = REDDatabase.inAppPaymentSubscribers.getByCurrencyCode(currency.currencyCode)

    return subscriber ?: REDStore.inAppPayments.getSubscriber(currency)
  }
  ...
  fun getSubscriber(type: InAppPaymentSubscriberRecord.Type): InAppPaymentSubscriberRecord? {
    if (type == InAppPaymentSubscriberRecord.Type.BACKUP) {
      return REDDatabase.inAppPaymentSubscribers.getBackupsSubscriber()
    }

    val currency = REDStore.inAppPayments.getRecurringDonationCurrency()
  ...
  fun setSubscriber(subscriber: InAppPaymentSubscriberRecord) {
    REDDatabase.inAppPaymentSubscribers.insertOrReplace(subscriber)
  }
  ...
  fun hasPendingDonation(): Boolean {
    return REDDatabase.inAppPayments.hasPendingDonation()
  }
  ...
  fun observeInAppPaymentRedemption(type: InAppPaymentType): Observable<DonationRedemptionJobStatus> {
    val fromDatabase: Observable<DonationRedemptionJobStatus> = Observable.create { emitter ->
      val observer = InAppPaymentObserver {
        val latestInAppPayment = REDDatabase.inAppPayments.getLatestInAppPaymentByType(type)
