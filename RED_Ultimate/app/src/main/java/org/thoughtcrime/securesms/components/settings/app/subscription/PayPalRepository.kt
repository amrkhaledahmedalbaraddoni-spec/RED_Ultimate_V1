...
class PayPalRepository(private val donationsService: DonationsService) {
  ...
  /**
   * Confirms a one-time payment via the RED Service to complete a donation.
   */
  @WorkerThread
  ...
  /**
   * Sets the default payment method via the RED Service.
   */
  @WorkerThread
  fun setDefaultPaymentMethod(subscriberType: InAppPaymentSubscriberRecord.Type, paymentMethodId: String) {
    ...
    REDDatabase.inAppPaymentSubscribers.setPaymentMethod(
      subscriber.subscriberId,
