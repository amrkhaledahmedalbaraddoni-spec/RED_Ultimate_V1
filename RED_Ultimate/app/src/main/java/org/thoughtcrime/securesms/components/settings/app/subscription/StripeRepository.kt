...
import com.red.sovereign.components.settings.app.subscription.errors.DonationError.OneTimeDonationError
 ...
 * 1. Generate and send a SubscriberId, which is a 32 byte ID representing this user, to RED Service, which creates a Stripe Customer
 ...
 * 1. Set the default PaymentMethod for the customer, using the PaymentMethod id, via the RED service
 *
...
object StripeRepository : StripeApi.PaymentIntentFetcher, StripeApi.SetupIntentHelper {
  ...
  override fun fetchPaymentIntent(price: FiatMoney, level: Long, sourceType: PaymentSourceType.Stripe): StripeIntentAccessor {
    Log.d(TAG, "Fetching payment intent from RED service for $price... (Locale.US minimum precision: ${price.minimumUnitPrecisionString})")
  ...
  override fun fetchSetupIntent(inAppPaymentType: InAppPaymentType, sourceType: PaymentSourceType.Stripe): StripeIntentAccessor {
    ...
    Log.d(TAG, "Got setup intent from RED service!")
  ...
  /**
   * Sets the default payment method for the given subscriber type via RED Service
  ...
  fun setDefaultPaymentMethod(
    ...
    Log.d(TAG, "Setting default payment method via RED service...")
    ...
    Log.d(TAG, "Set default payment method via RED service!")
    Log.d(TAG, "Storing the subscription payment source type locally.")
    REDDatabase.inAppPaymentSubscribers.setPaymentMethod(subscriber.subscriberId, paymentSourceType.toPaymentMethodType())
  }
  ...
  /**
   * Creates the PaymentMethod via the RED Service. Note that if the operation fails with a 409,
