...
import com.red.sovereign.components.settings.app.subscription.boost.Boost
import com.red.sovereign.components.settings.app.subscription.donate.gateway.GatewaySelectorBottomSheet
import com.red.sovereign.components.settings.app.subscription.thanks.ThanksForYourSupportBottomSheetDialogFragment
import com.red.sovereign.components.settings.app.subscription.ui.CurrencySelection
import com.red.sovereign.components.settings.app.subscription.ui.NetworkFailure
...
import com.red.sovereign.databinding.DonateToREDFragmentBinding
...
class DonateToREDFragment :
  ...
  companion object {
    private val TAG = Log.tag(DonateToREDFragment::class.java)
  }
  ...
  private val args: DonateToREDFragmentArgs by navArgs()
  private val viewModel: DonateToREDViewModel by viewModels(factoryProducer = {
    DonateToREDViewModel.Factory(args.startType)
  })

  private val disposables = LifecycleDisposable()
  private val binding by ViewBinderDelegate(DonateToREDFragmentBinding::bind)

  private val supportTechSummary: CharSequence by lazy {
    SpannableStringBuilder(SpanUtil.color(ContextCompat.getColor(requireContext(), CoreUiR.color.signal_colorOnSurfaceVariant), requireContext().getString(R.string.DonateToREDFragment__private_messaging)))
      .append("\n")
      .append(
        SpanUtil.readMore(requireContext(), ContextCompat.getColor(requireContext(), CoreUiR.color.signal_colorPrimary)) {
          findNavController().safeNavigate(DonateToREDFragmentDirections.actionDonateToREDFragmentToSubscribeLearnMoreBottomSheetDialog())
        }
      )
  }
  ...
  override fun bindAdapter(adapter: MappingAdapter) {
    ...
    disposables += viewModel.actions.subscribe { action ->
      when (action) {
        is DonateToREDAction.DisplayCurrencySelectionDialog -> {
          val navAction = DonateToREDFragmentDirections.actionDonateToREDFragmentToSetCurrencyFragment(
        ...
        is DonateToREDAction.DisplayGatewaySelectorDialog -> {
          Log.d(TAG, "Presenting gateway selector for ${action.inAppPayment.id}")
          val navAction = DonateToREDFragmentDirections.actionDonateToREDFragmentToGatewaySelectorBottomSheetDialog(action.inAppPayment.id)

          findNavController().safeNavigate(navAction)
        }

        is DonateToREDAction.CancelSubscription -> {
          val navAction = DonateToREDFragmentDirections.actionDonateToREDFragmentToStripePaymentInProgressFragment(
        ...
        is DonateToREDAction.UpdateSubscription -> {
          if (action.inAppPayment.data.paymentMethodType == InAppPaymentData.PaymentMethodType.PAYPAL) {
            val navAction = DonateToREDFragmentDirections.actionDonateToREDFragmentToPaypalPaymentInProgressFragment(
  ...
  private fun getConfiguration(state: DonateToREDState): DSLConfiguration {
    return configure {
      space(12.dp)
      ...
      noPadTextPref(
        title = DSLSettingsText.from(
          R.string.DonateToREDFragment__privacy_over_profit,
      ...
      if (state.inAppPaymentType == InAppPaymentType.RECURRING_DONATION && (state.monthlyDonationState.isSubscriptionActive || state.monthlyDonationState.isSubscriptionInProgress)) {
        ...
        primaryButton(
          text = DSLSettingsText.from(R.string.DonateToREDFragment__continue),
  ...
  private fun showDonationPendingDialog(state: DonateToREDState) {
    val message = if (state.inAppPaymentType == InAppPaymentType.ONE_TIME_DONATION) {
      if (state.oneTimeDonationState.isOneTimeDonationLongRunning) {
        R.string.DonateToREDFragment__bank_transfers_usually_take_1_business_day_to_process_onetime
      } else if (state.oneTimeDonationState.isNonVerifiedIdeal) {
        R.string.DonateToREDFragment__your_ideal_wero_payment_is_still_processing
      } else {
        R.string.DonateToREDFragment__your_payment_is_still_being_processed_onetime
      }
    } else {
      if (state.monthlyDonationState.activeSubscription?.paymentMethod == ActiveSubscription.PaymentMethod.SEPA_DEBIT) {
        R.string.DonateToREDFragment__bank_transfers_usually_take_1_business_day_to_process_monthly
      } else if (state.monthlyDonationState.nonVerifiedMonthlyDonation != null) {
        R.string.DonateToREDFragment__your_ideal_wero_payment_is_still_processing
      } else {
        R.string.DonateToREDFragment__your_payment_is_still_being_processed_monthly
      }
    }

    MaterialAlertDialogBuilder(requireContext())
      .setTitle(R.string.DonateToREDFragment__you_have_a_donation_pending)
      .setMessage(message)
      .setPositiveButton(android.R.string.ok, null)
      .show()
  }

  private fun DSLConfiguration.displayOneTimeSelection(areFieldsEnabled: Boolean, state: DonateToREDState.OneTimeDonationState) {
    when (state.donationStage) {
      DonateToREDState.DonationStage.INIT -> customPref(Boost.LoadingModel())
      DonateToREDState.DonationStage.FAILURE -> customPref(NetworkFailure.Model { viewModel.retryOneTimeDonationState() })
      DonateToREDState.DonationStage.READY -> {
        customPref(
  ...
  private fun DSLConfiguration.displayMonthlySelection(areFieldsEnabled: Boolean, state: DonateToREDState.MonthlyDonationState) {
    when (state.donationStage) {
      DonateToREDState.DonationStage.INIT -> customPref(Subscription.LoaderModel())
      DonateToREDState.DonationStage.FAILURE -> customPref(NetworkFailure.Model { viewModel.retryMonthlyDonationState() })
      else -> {
        state.subscriptions.forEach { subscription ->
          ...
          val activePrice = state.activeSubscription?.let { sub ->
            FiatMoney.fromREDNetworkAmount(sub.amount, Currency.getInstance(sub.currency))
          }
  ...
  private fun showSepaEuroMaximumDialog(sepaEuroMaximum: FiatMoney) {
    ...
    MaterialAlertDialogBuilder(requireContext())
      .setTitle(R.string.DonateToRED__donation_amount_too_high)
      .setMessage(getString(R.string.DonateToREDFragment__you_can_send_up_to_s_via_bank_transfer, max))
      .setPositiveButton(android.R.string.ok, null)
      .show()
  }
  ...
  override fun navigateToStripePaymentInProgress(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(
      DonateToREDFragmentDirections.actionDonateToREDFragmentToStripePaymentInProgressFragment(
  ...
  override fun navigateToPayPalPaymentInProgress(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(
      DonateToREDFragmentDirections.actionDonateToREDFragmentToPaypalPaymentInProgressFragment(
        InAppPaymentProcessorAction.PROCESS_NEW_IN_APP_PAYMENT,
        inAppPayment.id
      )
    )
  ...
  override fun navigateToCreditCardForm(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToREDFragmentDirections.actionDonateToREDFragmentToCreditCardFragment(inAppPayment.id))
  }

  override fun navigateToIdealDetailsFragment(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToREDFragmentDirections.actionDonateToREDFragmentToIdealTransferDetailsFragment(inAppPayment.id))
  }

  override fun navigateToBankTransferMandate(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToREDFragmentDirections.actionDonateToREDFragmentToBankTransferMandateFragment(inAppPayment.id))
  }

  override fun onPaymentComplete(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToREDFragmentDirections.actionDonateToREDFragmentToThanksForYourSupportBottomSheetDialog(Badges.fromDatabaseBadge(inAppPayment.data.badge!!)))
  }
  ...
  override fun onProcessorActionProcessed() {
    // SIGNAL_INHERITED: TODO [alex] - what did this used to do?
  }
  ...
  override fun navigateToDonationPending(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToREDFragmentDirections.actionDonateToREDFragmentToDonationPendingBottomSheet(inAppPayment.id))
  }
