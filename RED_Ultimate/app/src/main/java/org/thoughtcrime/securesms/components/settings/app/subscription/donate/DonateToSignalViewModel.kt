...
import com.red.sovereign.components.settings.app.subscription.OneTimeInAppPaymentRepository
...
import com.red.sovereign.components.settings.app.subscription.manage.DonationRedemptionJobStatus
...
import com.red.sovereign.database.model.isExpired
 ...
 */
class DonateToREDViewModel(
  startType: InAppPaymentType
) : ViewModel() {

  companion object {
    private val TAG = Log.tag(DonateToREDViewModel::class.java)
  }

  private val store = RxStore(DonateToREDState(inAppPaymentType = startType))
  ...
  private val _actions = PublishSubject.create<DonateToREDAction>()
  ...
  val actions: Observable<DonateToREDAction> = _actions.observeOn(AndroidSchedulers.mainThread())
  ...
  fun retryMonthlyDonationState() {
    if (!monthlyDonationDisposables.isDisposed && store.state.monthlyDonationState.donationStage == DonateToREDState.DonationStage.FAILURE) {
      store.update { it.copy(monthlyDonationState = it.monthlyDonationState.copy(donationStage = DonateToREDState.DonationStage.INIT)) }
      initializeMonthlyDonationState(RecurringInAppPaymentRepository)
    }
  }

  fun retryOneTimeDonationState() {
    if (!oneTimeDonationDisposables.isDisposed && store.state.oneTimeDonationState.donationStage == DonateToREDState.DonationStage.FAILURE) {
      store.update { it.copy(oneTimeDonationState = it.oneTimeDonationState.copy(donationStage = DonateToREDState.DonationStage.INIT)) }
      initializeOneTimeDonationState(OneTimeInAppPaymentRepository)
    }
  }
  ...
  fun requestChangeCurrency() {
    ...
    if (snapshot.canSetCurrency) {
      _actions.onNext(DonateToREDAction.DisplayCurrencySelectionDialog(snapshot.inAppPaymentType, snapshot.selectableCurrencyCodes))
    }
  }
  ...
  fun requestSelectGateway() {
    ...
    if (snapshot.areFieldsEnabled) {
      actionDisposable += createInAppPayment(snapshot).subscribeBy {
        _actions.onNext(DonateToREDAction.DisplayGatewaySelectorDialog(it))
      }
    }
  }
  ...
  fun updateSubscription() {
    ...
    if (snapshot.areFieldsEnabled) {
      actionDisposable += createInAppPayment(snapshot).subscribeBy {
        _actions.onNext(DonateToREDAction.UpdateSubscription(it, snapshot.isUpdateLongRunning))
      }
    }
  }
  ...
  fun cancelSubscription() {
    val snapshot = store.state
    if (snapshot.areFieldsEnabled) {
      _actions.onNext(DonateToREDAction.CancelSubscription)
    }
  }
  ...
  private fun createInAppPayment(snapshot: DonateToREDState): Single<InAppPaymentTable.InAppPayment> {
    ...
    return Single.fromCallable {
      ...
      REDDatabase.inAppPayments.getById(id)!!
    }
  }

  private fun getAmount(snapshot: DonateToREDState): FiatMoney {
  ...
  private fun getOneTimeAmount(snapshot: DonateToREDState.OneTimeDonationState): FiatMoney {
  ...
  private fun initializeOneTimeDonationState(oneTimeInAppPaymentRepository: OneTimeInAppPaymentRepository) {
    ...
    val oneTimeDonationFromStore: Observable<Optional<PendingOneTimeDonation>> = REDStore.inAppPayments.observablePendingOneTimeDonation
    ...
    val oneTimeCurrency: Observable<Currency> = REDStore.inAppPayments.observableOneTimeCurrency
    ...
    oneTimeDonationDisposables += Observable.combineLatest(boostsAndBadge, oneTimeCurrency) { (boostMap, badge), currency ->
      val boostList = if (currency in boostMap) {
        ...
        REDStore.inAppPayments.setOneTimeCurrency(PlatformCurrencyUtil.USD)
        listOf()
      }
        ...
        store.update { state ->
          state.copy(
            oneTimeDonationState = state.oneTimeDonationState.copy(
              ...
              donationStage = DonateToREDState.DonationStage.READY,
      ...
      onError = {
        ...
        store.update { it.copy(oneTimeDonationState = it.oneTimeDonationState.copy(donationStage = DonateToREDState.DonationStage.FAILURE)) }
      }
    )
  }
        ...
        store.update { state ->
          state.copy(
            monthlyDonationState = state.monthlyDonationState.copy(
              ...
              transactionState = DonateToREDState.TransactionState(jobStatus.isInProgress(), levelUpdateProcessing, jobStatus is DonationRedemptionJobStatus.PendingKeepAlive)
            )
          )
        }
      }
    ...
    monthlyDonationDisposables += Observable.combineLatest(allSubscriptions.toObservable(), _activeSubscription, ::Pair).subscribeBy(
      ...
      onError = {
        store.update { state ->
          state.copy(
            monthlyDonationState = state.monthlyDonationState.copy(
              donationStage = DonateToREDState.DonationStage.FAILURE
            )
          )
        }
      }
    ...
    monthlyDonationDisposables += allSubscriptions.subscribeBy(
      onSuccess = { subscriptions ->
        if (subscriptions.isNotEmpty()) {
          ...
          val selectedCurrency = REDStore.inAppPayments.getRecurringDonationCurrency()
  ...
  private fun monitorSubscriptionCurrency() {
    monthlyDonationDisposables += REDStore.inAppPayments.observableRecurringDonationCurrency.subscribe {
      store.update { state ->
  ...
  class Factory(
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
      return modelClass.cast(DonateToREDViewModel(startType)) as T
    }
  }
}
