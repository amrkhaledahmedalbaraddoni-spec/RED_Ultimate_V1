...
import com.red.sovereign.components.settings.app.subscription.donate.card.CreditCardFragment
import com.red.sovereign.components.settings.app.subscription.donate.paypal.PayPalPaymentInProgressFragment
...
import com.red.sovereign.components.settings.app.subscription.errors.DonationErrorDialogs
import com.red.sovereign.components.settings.app.subscription.errors.DonationErrorParams
    ...
    if (result.action == InAppPaymentProcessorAction.CANCEL_SUBSCRIPTION) {
      ...
      fragment.lifecycleScope.launch {
        val inAppPayment = withContext(REDDispatchers.Default) {
          REDDatabase.inAppPayments.getById(result.inAppPaymentId!!)!!
        }
  ...
  class ErrorHandler : DefaultLifecycleObserver {
    ...
    fun attach(
      ...
      disposables += inAppPaymentIdSource
        ...
        .doOnNext {
          REDDatabase.inAppPayments.update(it.copy(notified = true))
        }
      ...
      disposables += inAppPaymentIdSource
        ...
        .concatMapSingle { (id, err) -> Single.fromCallable { REDDatabase.inAppPayments.getById(id)!! to err } }
        .subscribeOn(Schedulers.io())
        .observeOn(AndroidSchedulers.mainThread())
