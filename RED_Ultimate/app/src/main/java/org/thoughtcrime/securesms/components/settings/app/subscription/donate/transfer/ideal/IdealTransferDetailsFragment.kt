...
import com.red.sovereign.components.TemporaryScreenshotSecurity
import com.red.sovereign.components.settings.app.subscription.DonationSerializationHelper.toFiatMoney
import com.red.sovereign.components.settings.app.subscription.donate.InAppPaymentCheckoutDelegate
import com.red.sovereign.components.settings.app.subscription.donate.InAppPaymentProcessorAction
import com.red.sovereign.components.settings.app.subscription.donate.InAppPaymentProcessorActionResult
import com.red.sovereign.components.settings.app.subscription.donate.gateway.createInAppPaymentPreview
import com.red.sovereign.components.settings.app.subscription.donate.stripe.StripePaymentInProgressFragment
import com.red.sovereign.components.settings.app.subscription.donate.stripe.StripePaymentInProgressViewModel
import com.red.sovereign.components.settings.app.subscription.donate.transfer.BankTransferRequestKeys
import com.red.sovereign.components.settings.app.subscription.donate.transfer.DonationTransferTestTags
import com.red.sovereign.components.settings.app.subscription.donate.transfer.ideal.IdealTransferDetailsViewModel.Field
...
class IdealTransferDetailsFragment : ComposeFragment(), InAppPaymentCheckoutDelegate.ErrorHandlerCallback {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    setFragmentResultListener(StripePaymentInProgressFragment.REQUEST_KEY) { _, bundle ->
      ...
      if (result.status == InAppPaymentProcessorActionResult.Status.SUCCESS) {
        findNavController().popBackStack(R.id.donateToREDFragment, false)
  ...
  override fun FragmentContent() {
    ...
    val donateLabel = remember(iap) {
      if (iap.type.recurring) { // SIGNAL_INHERITED: TODO [message-request] -- Handle backups
        getString(
  ...
  private fun onDonateClick() {
    ...
    if (state.inAppPayment!!.type.recurring) { // SIGNAL_INHERITED: TODO [message-requests] -- handle backup
  ...
  override fun onUserLaunchedAnExternalApplication() {
    viewLifecycleOwner.lifecycle.addObserver(object : DefaultLifecycleObserver {
      override fun onResume(owner: LifecycleOwner) {
        findNavController().popBackStack(R.id.donateToREDFragment, true)
      }
    })
  }
