...
private fun TabButton(label: String, active: Boolean, onClick: () -> Unit, modifier: Modifier = Modifier) {
  val colors = if (active) {
    ...
    ButtonDefaults.buttonColors(
      containerColor = REDTheme.colors.colorSurface2,
  ...
  Previews.Preview {
    MainScreen(
      state = UsernameLinkSettingsState(
        ...
        usernameLinkState = UsernameLinkState.Present("https://red.local"),
  ...
  try {
    ByteArrayOutputStream().use { byteArrayOutputStream ->
      ...
      val shareUri = AppDependencies.blobs
        ...
        .withFileName("REDUsernameQr.png")
