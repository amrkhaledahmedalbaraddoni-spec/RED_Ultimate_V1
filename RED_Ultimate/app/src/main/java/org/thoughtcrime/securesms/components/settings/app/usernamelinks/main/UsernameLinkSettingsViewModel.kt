...
import com.red.sovereign.components.settings.app.usernamelinks.drawQr
import com.red.sovereign.components.settings.app.usernamelinks.main.UsernameLinkSettingsState.ActiveTab
...
class UsernameLinkSettingsViewModel : ViewModel() {
  ...
  private val _state = mutableStateOf(
    UsernameLinkSettingsState(
      ...
      usernameLinkState = REDStore.account.usernameLink?.let { UsernameLinkState.Present(it.toLink()) } ?: UsernameLinkState.NotSet,
      qrCodeState = QrCodeState.Loading,
      qrCodeColorScheme = REDStore.misc.usernameQrCodeColorScheme
    )
  )
  val state: State<UsernameLinkSettingsState> = _state

  private val disposable: CompositeDisposable = CompositeDisposable()
  private val usernameLink: BehaviorSubject<Optional<UsernameLinkComponents>> = BehaviorSubject.createDefault(Optional.ofNullable(REDStore.account.usernameLink))
  ...
  init {
    ...
    if (REDStore.account.usernameLink == null) {
      onUsernameLinkReset()
    }
  }
