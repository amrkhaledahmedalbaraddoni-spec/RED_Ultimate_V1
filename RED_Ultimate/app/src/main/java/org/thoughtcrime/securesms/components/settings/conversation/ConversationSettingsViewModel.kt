...
sealed class ConversationSettingsViewModel(
  ...
  protected val store = Store(
    ConversationSettingsState(
      specificSettingsState = specificSettingsState,
      isDeprecatedOrUnregistered = REDStore.misc.isClientDeprecated || TextSecurePreferences.isUnauthorizedReceived(AppDependencies.application)
    )
  )
  ...
  init {
    ...
    val sharedMedia: LiveData<List<MediaTable.MediaRecord>> = LiveDataUtil.mapAsync(REDExecutors.BOUNDED, updater) { tId ->
  ...
  private class GroupSettingsViewModel(
      ...
      store.update(liveGroup.groupRecipient) { recipient, state ->
        state.copy(
          ...
          buttonStripState = ButtonStripPreference.State(
            ...
            isSearchAvailable = callMessageIds.isEmpty(),
            isAddToStoryAvailable = recipient.isPushV2Group && !recipient.isBlocked && recipient.isActiveGroup && !REDStore.story.isFeatureDisabled
          ),
    ...
    private fun loadMemberLabels(v2GroupId: GroupId.V2, groupMembers: List<GroupMemberEntry.FullMember>) = viewModelScope.launch(REDDispatchers.Default) {
    ...
    private fun loadCanSetMemberLabel(groupId: GroupId.V2) = viewModelScope.launch(REDDispatchers.Default) {
