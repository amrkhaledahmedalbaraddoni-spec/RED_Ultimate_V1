...
import com.red.sovereign.webrtc.audio.AudioDeviceMapping
...
class WebRtcAudioPicker31(private val audioOutputChangedListener: OnAudioOutputChangedListener, private val outputState: ToggleButtonOutputState, private val stateUpdater: AudioStateUpdater) {
  ...
  companion object {
    ...
    private fun WebRtcAudioOutput.toREDAudioDevice(): REDAudioManager.AudioDevice {
      return when (this) {
        WebRtcAudioOutput.HANDSET -> REDAudioManager.AudioDevice.EARPIECE
        WebRtcAudioOutput.SPEAKER -> REDAudioManager.AudioDevice.SPEAKER_PHONE
        WebRtcAudioOutput.BLUETOOTH_HEADSET -> REDAudioManager.AudioDevice.BLUETOOTH
        WebRtcAudioOutput.WIRED_HEADSET -> REDAudioManager.AudioDevice.WIRED_HEADSET
      }
    }
  ...
  fun cycleToNextDevice() {
    ...
    if (index != -1) {
      ...
      val targetDeviceType = nextOutput.toREDAudioDevice()
  ...
  private fun AudioOutputOption.toWebRtcAudioOutput(): WebRtcAudioOutput {
    return when (this.deviceType) {
      REDAudioManager.AudioDevice.WIRED_HEADSET -> WebRtcAudioOutput.WIRED_HEADSET
      REDAudioManager.AudioDevice.EARPIECE -> WebRtcAudioOutput.HANDSET
      REDAudioManager.AudioDevice.BLUETOOTH -> WebRtcAudioOutput.BLUETOOTH_HEADSET
      REDAudioManager.AudioDevice.SPEAKER_PHONE, REDAudioManager.AudioDevice.NONE -> WebRtcAudioOutput.SPEAKER
    }
  }
}
