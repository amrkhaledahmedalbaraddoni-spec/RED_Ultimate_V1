...
import com.red.sovereign.calls.links.EditCallLinkNameDialogFragment
...
import com.red.sovereign.components.webrtc.controls.CallInfoView
import com.red.sovereign.components.webrtc.controls.ControlsAndInfoViewModel
import com.red.sovereign.components.webrtc.controls.RaiseHandSnackbar
...
class ComposeCallScreenMediator(private val activity: WebRtcCallActivity, viewModel: WebRtcCallViewModel) : CallScreenMediator, AdditionalActionsListener {
  ...
  class CallScreenViewModel : ViewModel() {
    ...
    val callScreenState = MutableStateFlow(
      CallScreenState(
        reactions = REDStore.emoji.reactions.map {
          REDStore.emoji.getPreferredVariation(it)
        }.toPersistentList()
      )
    )
