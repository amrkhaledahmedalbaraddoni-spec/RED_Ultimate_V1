...
public class ContactRepository {
  ...
  private static final List<Pair<String, ValueMapper>> SEARCH_CURSOR_MAPPERS = new ArrayList<Pair<String, ValueMapper>>() {{
    ...
    add(new Pair<>(NUMBER_COLUMN, cursor -> {
      ...
      if (phone != null) {
        phone = REDE164Util.prettyPrint(phone);
      }
  ...
  @WorkerThread
  public @NonNull Cursor queryREDContacts(@NonNull String query) {
    return queryREDContacts(new RecipientTable.ContactSearchQuery(query, new RecipientTable.IncludeSelfMode.IncludeWithRemap(selfTitle), ContactSearchSortOrder.NATURAL));
  }

  @WorkerThread
  public @NonNull Cursor queryREDContacts(@NonNull RecipientTable.ContactSearchQuery contactSearchQuery) {
    Cursor cursor = TextUtils.isEmpty(contactSearchQuery.getQuery()) ? recipientTable.getREDContacts(contactSearchQuery.getIncludeSelfMode())
                                                                     : recipientTable.queryREDContacts(contactSearchQuery);
