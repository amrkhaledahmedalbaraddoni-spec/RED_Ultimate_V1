...
import com.red.sovereign.jobs.SyncSystemContactLinksJob
...
object ContactDiscovery {
  ...
  fun refreshAll(context: Context, notifyOfNewUsers: Boolean) {
    if (TextUtils.isEmpty(REDStore.account.e164)) {
    ...
    if (!REDStore.registration.isRegistrationComplete) {
      if (REDStore.account.isRegistered && REDStore.svr.lastPinCreateFailed()) {
  ...
  private fun phoneNumberFormatter(): (String) -> String? {
    return { REDE164Util.formatNonShortCodeAsE164(it) }
  }
  ...
  private fun refreshRecipients(
    ...
    val preExistingRegisteredIds: Set<RecipientId> = REDDatabase.recipients.getRegistered()
    ...
    if (hasContactsPermissions(context)) {
      ...
      if (TextSecurePreferences.hasSuccessfullyRetrievedDirectory(context) && notifyOfNewUsers) {
        val systemContacts: Set<RecipientId> = REDDatabase.recipients.getSystemContacts().toSet()
  ...
  private fun notifyNewUsers(context: Context, newUserIds: Collection<RecipientId>) {
    if (!REDStore.settings.isNotifyWhenContactJoinsRED) return
    ...
    Recipient.resolvedList(newUserIds)
      ...
      .map {
        ...
        REDDatabase.messages.insertMessageInbox(message)
      }
      .filter { it.isPresent }
      .map { it.get() }
      .forEach { result ->
  ...
  private fun syncRecipientsWithSystemContacts(
    ...
    val localNumber: String = REDStore.account.e164 ?: ""
    ...
    if (contactInfos.isNotEmpty()) {
      val handle = REDDatabase.recipients.beginBulkSystemContactUpdate(clearInfoForMissingContacts)
      try {
  ...
  private fun hasSession(id: RecipientId): Boolean {
    ...
    val protocolAddress = Recipient.resolved(id).requireServiceId().toProtocolAddress(REDServiceAddress.DEFAULT_DEVICE_ID)
