package com.red.sovereign.contacts.sync
...
import com.red.sovereign.contacts.sync.FuzzyPhoneNumberHelper.InputResult
import com.red.sovereign.contacts.sync.FuzzyPhoneNumberHelper.OutputResult
import com.red.sovereign.database.RecipientTable.CdsV2Result
...
object ContactDiscoveryRefreshV2 {
  ...
  fun refreshAll(context: Context, timeoutMs: Long? = null): ContactDiscovery.RefreshResult {
    val recipientE164s: Set<String> = REDDatabase.recipients.getAllE164s().sanitize()
    ...
    return refreshInternal(
      ...
      inputPreviousE164s = REDDatabase.cds.getAllE164s(),
  ...
  fun lookupE164(e164: String): ContactDiscovery.LookupResult? {
    val result = REDNetwork.cdsApi.getRegisteredUsers(
      previousE164s = emptySet(),
      newE164s = setOf(e164),
      serviceIds = REDDatabase.recipients.getAllServiceIdProfileKeyPairs(),
      ...
      is NetworkResult.StatusCodeError -> {
        when (val e = result.exception) {
          is CdsiResourceExhaustedException -> {
            ...
            REDStore.misc.cdsBlockedUtil = System.currentTimeMillis() + e.retryAfterSeconds.seconds.inWholeMilliseconds
            throw e
          }
    ...
    return response.results[e164]?.let { item ->
      val id = REDDatabase.recipients.processIndividualCdsLookup(e164 = e164, aci = item.aci.orElse(null), pni = item.pni)
  ...
  private fun refreshInternal(
    ...
    val previousE164s: Set<String> = if (REDStore.misc.cdsToken != null && !isPartialRefresh) inputPreviousE164s else emptySet()
    ...
    if (newE164s.size > RemoteConfig.cdsHardLimit) {
      ...
      REDStore.misc.markCdsPermanentlyBlocked()
      throw IOException("New contacts over the CDS hard limit!")
    }

    val token: ByteArray? = if (previousE164s.isNotEmpty() && !isPartialRefresh) REDStore.misc.cdsToken else null
    ...
    val result = REDNetwork.cdsApi.getRegisteredUsers(
      ...
      if (!isPartialRefresh) {
        REDStore.misc.cdsToken = tokenToSave
        REDDatabase.cds.updateAfterFullCdsQuery(previousE164s + newE164s, allE164s + newE164s)
        Log.d(TAG, "Token saved!")
      } else {
        REDDatabase.cds.updateAfterPartialCdsQuery(newE164s)
      ...
      is NetworkResult.StatusCodeError -> {
        when (val e = result.exception) {
          ...
          is CdsiInvalidTokenException -> {
            ...
            REDStore.misc.cdsToken = null
            REDDatabase.cds.clearAll()
            throw e
          }
    ...
    if (!isPartialRefresh && REDStore.misc.isCdsBlocked) {
      Log.i(TAG, "Successfully made a request while blocked -- clearing blocked state.")
      REDStore.misc.clearCdsBlocked()
    }
    ...
    REDDatabase.recipients.rewritePhoneNumbers(fuzzyOutput.rewrites)
    stopwatch.split("rewrite-e164")

    registeredIds += REDDatabase.recipients.bulkProcessCdsResult(fuzzyOutput.numbers)
    rewrites += fuzzyOutput.rewrites
    stopwatch.split("process-result")

    val existingIds: Set<RecipientId> = REDDatabase.recipients.getAllPossiblyRegisteredByE164(recipientE164s + rewrites.values)
    ...
    REDDatabase.recipients.updatePhoneNumberDiscoverability(registeredIds, missingFromCds)

    REDDatabase.recipients.bulkUpdatedRegisteredStatus(registeredIds, inactiveIds)
  ...
  private fun hasCommunicatedWith(recipient: Recipient): Boolean {
    val localAci = REDStore.account.requireAci()
    return REDDatabase.threads.hasActiveThread(recipient.id) || (recipient.hasServiceId && REDDatabase.sessions.hasSessionFor(localAci, recipient.requireServiceId().toString()))
  }
  ...
  private fun Set<String>.toE164s(): Set<String> {
    return this.mapNotNull { REDE164Util.formatAsE164(it) }.toSet()
  }
