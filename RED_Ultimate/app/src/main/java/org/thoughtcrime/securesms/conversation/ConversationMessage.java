...
import com.red.sovereign.conversation.mutiselect.Multiselect;
import com.red.sovereign.conversation.mutiselect.MultiselectCollection;
import com.red.sovereign.conversation.v2.computed.FormattedDate;
  ...
  public static class ConversationMessageFactory {
                                                                        ...
                                                                        @Nullable Map<RecipientId, MemberLabel> prefetchedLabels)
    {
      ...
      if (messageRecord.isEditMessage()) {
        try {
          originalMessage = REDDatabase.messages().getMessageRecord(messageRecord.getOriginalMessageId().getId());
      ...
      if (CollapsedState.isHead(messageRecord.getCollapsedState())) {
        collapsedSize = REDDatabase.messages().getCollapsedCount(messageRecord.getId());
        if (CollapsibleEvents.getCollapsibleType(messageRecord.getType(), messageRecord.getMessageExtras()) == CollapsibleEvents.CollapsibleType.DISAPPEARING_TIMER && collapsedSize > 1) {
          collapsedExpirationInMs = REDDatabase.messages().getDisappearingTimerStateForCollapsedSet(messageRecord.getId());
        }
      }
    ...
    public static @NonNull ConversationMessage createWithUnresolvedData(@NonNull Context context, @NonNull MessageRecord messageRecord, boolean hasBeenQuoted, @NonNull Recipient threadRecipient) {
      List<Mention> mentions = messageRecord.isMms() ? REDDatabase.mentions().getMentionsForMessage(messageRecord.getId())
                                                     : null;
    ...
    public static @NonNull ConversationMessage createWithUnresolvedData(@NonNull Context context, @NonNull MessageRecord messageRecord, @NonNull CharSequence body, @NonNull Recipient threadRecipient) {
      boolean       hasBeenQuoted = REDDatabase.messages().isQuoted(messageRecord);
      List<Mention> mentions      = REDDatabase.mentions().getMentionsForMessage(messageRecord.getId());
