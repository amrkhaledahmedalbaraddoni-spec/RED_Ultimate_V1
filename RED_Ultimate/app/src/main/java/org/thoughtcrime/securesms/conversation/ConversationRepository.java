...
public class ConversationRepository {
  ...
  public @NonNull ConversationData getConversationData(long threadId, @NonNull Recipient conversationRecipient, int jumpToPosition) {
    ThreadTable.ConversationMetadata    metadata                       = REDDatabase.threads().getConversationMetadata(threadId);
    int                                 threadSize                     = REDDatabase.messages().getMessageCountForThread(threadId);
    MessageTable.OldestUnread           oldestUnread                   = metadata.getUnreadCount() > 0 ? REDDatabase.messages().getOldestUnread(threadId) : null;
    ...
    if (firstUnreadDateReceived > 0) {
      firstUnreadPosition = REDDatabase.messages().getMessagePositionByDateReceivedTimestamp(threadId, firstUnreadDateReceived, false);
    }
    ...
    if (firstUnreadDateReceived == 0 && lastScrolled > 0) {
      lastScrolledPosition = REDDatabase.messages().getMessagePositionByDateReceivedTimestamp(threadId, lastScrolled, true);
    }
    ...
    if (!isMessageRequestAccepted) {
      ...
      if (conversationRecipient.isGroup()) {
        Optional<GroupRecord> group = REDDatabase.groups().getGroup(conversationRecipient.getId());
        if (group.isPresent()) {
  ...
  public void markGiftBadgeRevealed(long messageId) {
    REDExecutors.BOUNDED_IO.execute(() -> {
      List<MessageTable.MarkedMessageInfo> markedMessageInfo = REDDatabase.messages().setOutgoingGiftsRevealed(Collections.singletonList(messageId));
  ...
  public Single<ConversationMessage> resolveMessageToEdit(@NonNull ConversationMessage message) {
    return Single.fromCallable(() -> {
                   ...
                   if (latestRevisionId != null) {
                     MessageRecord latestRecord = REDDatabase.messages().getMessageRecordOrNull(latestRevisionId.getId());
