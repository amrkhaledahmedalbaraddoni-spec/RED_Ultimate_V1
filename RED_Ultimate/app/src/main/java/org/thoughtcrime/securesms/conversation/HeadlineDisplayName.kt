...
import org.signal.core.ui.fonts.REDSymbols.REDSymbol
...
/**
 * Compose-native version of [com.red.sovereign.recipients.Recipient.getDisplayNameForHeadline].
 */
@Composable
fun HeadlineDisplayName(
  ...
  val chevronGlyph = if (isLtr) REDSymbols.Glyph.CHEVRON_RIGHT else REDSymbols.Glyph.CHEVRON_LEFT
  ...
  Emojifier(text = displayName) { emojiText, emojiInlineContent ->
    val styledText = buildAnnotatedString {
      if (!isLtr) {
        if (showChevron) {
          REDSymbol(chevronGlyph, fontSize = 18.sp, color = outlineColor, baselineShift = BaselineShift(0.1f))
          append("\u00A0")
        }
        if (showVerified) {
          ...
          REDSymbol(REDSymbols.Glyph.PERSON_CIRCLE, fontSize = 18.sp, baselineShift = BaselineShift(0.1f))
          append("\u00A0")
        }
      }
