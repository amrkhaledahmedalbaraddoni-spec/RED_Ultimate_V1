...
public final class MenuState {
                                       ...
                                       boolean canEditGroupInfo)
  {
    ...
    for (MultiselectPart part : selectedParts) {
      ...
      if (REDStore.labs().getStarredMessages() && !messageRecord.isUpdate() && !messageRecord.isRemoteDelete() && !messageRecord.isStarred()) {
        canStarMessage = true;
      }

      if (REDStore.labs().getStarredMessages() && messageRecord.isStarred()) {
    ...
    return builder.shouldShowCopyAction(!actionMessage && !remoteDelete && hasText && !hasGift && !hasPayment && !hasPoll)
                  ...
                  .shouldShowPaymentDetails(hasPayment && REDStore.account().isPrimaryDevice())
