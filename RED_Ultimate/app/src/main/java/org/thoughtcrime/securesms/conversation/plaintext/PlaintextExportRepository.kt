...
package com.red.sovereign.conversation.plaintext
...
object PlaintextExportRepository {
  ...
  fun export(
    ...
    try {
      ZipOutputStream(BufferedOutputStream(outputFile.outputStream())).use { zipOut ->
        ...
        MessageTable.mmsReaderFor(REDDatabase.messages.getConversation(threadId, dateReceiveOrderBy = "ASC")).use { reader ->
          while (true) {
            if (cancellationRED.isCancelled()) return false
        ...
        if (includeMedia) {
          ...
          for (pending in pendingAttachments) {
            ...
            try {
              ...
              REDDatabase.attachments.getAttachmentStream(pending.attachment.attachmentId, 0).use { input ->
                input.copyTo(zipOut)
              }
              zipOut.closeEntry()
            } catch (e: Exception) {
  ...
  internal fun getExportStats(threadId: Long): ExportStats {
    val messageCount = REDDatabase.messages.getMessageCountForThread(threadId)
    val attachmentCount = REDDatabase.attachments.getPlaintextExportableAttachmentCountForThread(threadId)
  ...
  internal fun formatUpdateMessage(context: Context, message: MmsMessageRecord, timestamp: String): String {
    return when {
      ...
      message.isJoined -> "$timestamp ${getSenderName(context, message)} joined RED"
