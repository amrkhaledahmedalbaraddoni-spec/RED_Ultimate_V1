...
object EditMessageHistoryRepository {
  ...
  fun getEditHistory(messageId: Long): Observable<List<ConversationMessage>> {
    return Observable.create { emitter ->
      val threadId: Long = REDDatabase.messages.getThreadIdForMessage(messageId)
      if (threadId < 0) {
  ...
  fun markRevisionsRead(messageId: Long) {
    REDExecutors.BOUNDED.execute {
      MarkReadReceiver.process(REDDatabase.messages.setAllEditMessageRevisionsRead(messageId))
    }
  }

  private fun getEditHistorySync(messageId: Long): List<ConversationMessage> {
    val context = AppDependencies.application
    val records = REDDatabase
      .messages
    ...
    val threadRecipient: Recipient = requireNotNull(REDDatabase.threads.getRecipientForThreadId(records[0].threadId))
