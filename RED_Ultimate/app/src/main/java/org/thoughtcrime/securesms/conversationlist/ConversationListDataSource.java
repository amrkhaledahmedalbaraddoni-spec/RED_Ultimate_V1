...
import com.red.sovereign.conversationlist.model.ConversationReader;
...
abstract class ConversationListDataSource implements PagedDataSource<Long, Conversation> {
  ...
  @Override
  public @NonNull List<Conversation> load(int start, int length, int totalSize, @NonNull CancellationRED cancellationRED) {
    REDTrace.beginSection("ConversationListDataSource#load");
    ...
    try (ConversationReader reader = new ConversationReader(getCursor(start, length))) {
      ...
      while ((record = reader.getNext()) != null && !cancellationRED.isCanceled()) {
