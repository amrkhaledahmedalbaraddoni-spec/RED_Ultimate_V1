...
import com.red.sovereign.OverlayTransformation;
...
import com.red.sovereign.components.DeliveryStatusView;
import com.red.sovereign.components.FromTextView;
import com.red.sovereign.components.TypingIndicatorView;
...
import com.red.sovereign.glide.targets.GlideLiveDataTarget;
...
import static com.red.sovereign.database.model.LiveUpdateMessage.recipientToStringAsync;
                                                                         ...
                                                                         @NonNull GlideLiveDataTarget thumbTarget)
  {
    ...
    if (!thread.isMessageRequestAccepted()) {
      ...
      if (thread.getRecipient().getE164().isPresent()) {
        return emphasisAdded(context, context.getString(R.string.ThreadRecord_s_belongs_to_s, REDE164Util.prettyPrint(thread.getRecipient().requireE164()), thread.getRecipient().getDisplayName(context)), defaultTint);
      } else {
        return emphasisAdded(context, context.getString(R.string.ThreadRecord_safety_number_changed), defaultTint);
      }
