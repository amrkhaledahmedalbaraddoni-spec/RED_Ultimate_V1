...
sealed class ConversationListViewModel(
  ...
  init {
    ...
    conversationListDataSource = saveableStateFlowable
      ...
      .map { (filterRequest, folder) ->
        ConversationListDataSource.create(
          ...
          isArchived,
          REDStore.uiHints.canDisplayPullToFilterTip() && filterRequest.source === ConversationFilterSource.OVERFLOW
        )
      }
      .replay(1)
      .refCount()
    ...
    val pinnedCount = RxDatabaseObserver
      .conversationList
      .map { REDDatabase.threads.getPinnedConversationListCount(ConversationFilter.OFF) }
      .distinctUntilChanged()
    ...
    hasNoConversations = store
      ...
      .map { (filterRequest, conversations) ->
        if (conversations.isNotEmpty()) {
          ...
          val archivedCount = REDDatabase.threads.getArchivedConversationListCount(filterRequest.filter)
          val unarchivedCount = REDDatabase.threads.getUnarchivedConversationListCount(filterRequest.filter)
  ...
  fun onUpdateMute(chatFolder: ChatFolderRecord, until: Long) {
    viewModelScope.launch(Dispatchers.Default) {
      val ids = REDDatabase.threads.getRecipientIdsByChatFolder(chatFolder)
  ...
  fun markChatFolderRead(chatFolder: ChatFolderRecord) {
    viewModelScope.launch(Dispatchers.IO) {
      val ids = REDDatabase.threads.getThreadIdsByChatFolder(chatFolder)
      val messageIds = REDDatabase.threads.setRead(ids)
  ...
  fun removeChatFromFolder(threadId: Long) {
    viewModelScope.launch(Dispatchers.Default) {
      REDDatabase.chatFolders.removeFromFolder(currentFolder.id, threadId)
  ...
  fun addToFolder(folderId: Long, threadIds: List<Long>) {
    viewModelScope.launch(Dispatchers.Default) {
      ...
      REDDatabase.chatFolders.addToFolder(folderId, threadIdsNotIncluded)
