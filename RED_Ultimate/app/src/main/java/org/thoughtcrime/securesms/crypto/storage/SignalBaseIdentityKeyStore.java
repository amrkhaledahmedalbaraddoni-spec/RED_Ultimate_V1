...
import com.red.sovereign.crypto.storage.REDIdentityKeyStore.SaveResult;
 ...
 * So this class represents the core store, and we can create multiple {@link REDIdentityKeyStore} that use this same instance, changing only what each of
 * those reports as their own identity key.
 */
public class REDBaseIdentityKeyStore {

  private static final String TAG = Log.tag(REDBaseIdentityKeyStore.class);
  ...
  public REDBaseIdentityKeyStore(@NonNull Context context) {
    this(context, REDDatabase.identities());
  }

  REDBaseIdentityKeyStore(@NonNull Context context, @NonNull IdentityTable identityDatabase) {
    this.context = context;
    this.cache   = new Cache(identityDatabase);
  }

  public int getLocalRegistrationId() {
    return REDStore.account().getRegistrationId();
  }

  public IdentityKeyStore.IdentityChange saveIdentity(REDProtocolAddress address, IdentityKey identityKey) {
  ...
  public @NonNull SaveResult saveIdentity(REDProtocolAddress address, IdentityKey identityKey, boolean nonBlockingApproval) {
    try (REDSessionLock.Lock unused = ReentrantSessionLock.INSTANCE.acquire()) {
      ...
      RecipientId         recipientId      = RecipientId.from(ServiceId.fromLibRED(address.getServiceId()));
      ...
      if (identityKeyChanged && Recipient.self().getId().equals(recipientId) && Objects.equals(REDStore.account().getAci(), ServiceId.parseOrNull(address.getName()))) {
        ...
        REDDatabase.senderKeyShared().deleteAllFor(recipientId);
  ...
  public boolean isTrustedIdentity(REDProtocolAddress address, IdentityKey identityKey, IdentityKeyStore.Direction direction) {
    boolean isSelf = address.getName().equals(REDStore.account().requireAci().toString()) ||
                     address.getName().equals(REDStore.account().requirePni().toString()) ||
                     address.getName().equals(REDStore.account().getE164());

    if (isSelf) {
      return identityKey.equals(REDStore.account().getAciIdentityKey().getPublicKey());
    }
  ...
  private static final class Cache {
    ...
    private void withWriteLock(Runnable runnable) {
      SQLiteDatabase db = REDDatabase.getRawDatabase();
      db.beginTransaction();
      try {
        synchronized (this) {
          runnable.run();
