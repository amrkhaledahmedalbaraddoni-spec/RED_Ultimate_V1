...
import com.red.sovereign.attachments.LocalBackupKey
import com.red.sovereign.attachments.LocalStickerAttachment
import com.red.sovereign.attachments.WallpaperAttachment
import com.red.sovereign.audio.AudioHash
...
import com.red.sovereign.database.AttachmentTable.Companion.DATA_HASH_END
import com.red.sovereign.database.AttachmentTable.Companion.PREUPLOAD_MESSAGE_ID
import com.red.sovereign.database.AttachmentTable.Companion.REMOTE_KEY
import com.red.sovereign.database.AttachmentTable.Companion.TRANSFER_PROGRESS_DONE
import com.red.sovereign.database.MessageTable.SyncMessageId
...
import com.red.sovereign.database.model.databaseprotos.AudioWaveFormData
import com.red.sovereign.dependencies.AppDependencies
import com.red.sovereign.jobs.AttachmentDownloadJob
import com.red.sovereign.jobs.AttachmentUploadJob
import com.red.sovereign.jobs.GenerateAudioWaveFormJob
...
import com.red.sovereign.util.FileUtils
import com.red.sovereign.util.ImageCompressionUtil
import com.red.sovereign.util.MediaUtil
import com.red.sovereign.util.RemoteConfig
import com.red.sovereign.video.EncryptedMediaDataSource
...
class AttachmentTable(
  ...
  fun deleteAttachmentsForMessage(mmsId: Long): Boolean {
    ...
    val deleteCount = writableDatabase.withinTransaction { db ->
      ...
      REDDatabase.attachmentMetadata.cleanupById(metadataIdsToCleanup)
  ...
  fun deleteAttachmentFilesForViewOnceMessage(messageId: Long) {
    ...
    if (threadId > 0) {
      REDDatabase.threads.updateSnippetContentTypeSilently(threadId, messageId, MediaUtil.VIEW_ONCE)
    }
    ...
    writableDatabase.withinTransaction { db ->
      db.select(DATA_FILE, CONTENT_TYPE, MESSAGE_ID, METADATA_ID)
        ...
        .use { cursor ->
          ...
          REDDatabase.attachmentMetadata.cleanupById(listOfNotNull(metadataId))
  ...
  fun deleteAbandonedAttachmentFiles(): Int {
    ...
    filesInDb += REDDatabase.stickers.getAllStickerFiles()
  ...
  fun deleteAllAttachments() {
    ...
    REDDatabase.attachmentMetadata.cleanup()
  ...
  fun updateAttachmentData(
    ...
    // SIGNAL_INHERITED: TODO We don't dedupe here because we're assuming that we should have caught any dupe scenarios on first insert. We could consider doing dupe checks here though.
    ...
    readableDatabase
      ...
      .forEach { cursor ->
        ...
        val mediaId = MediaName.fromPlaintextHashAndRemoteKey(plaintextHash, remoteKey)
          .toMediaId(REDStore.backup.mediaRootBackupKey)
          .encode()
  ...
  private fun insertArchivedAttachment(messageId: Long, attachment: ArchivedAttachment, quote: Boolean, quoteTargetContentType: String?): AttachmentId {
    ...
    val attachmentId: AttachmentId = writableDatabase.withinTransaction { db ->
      ...
      val metadataId: Long? = if (plaintextHash != null && attachment.localBackupKey != null) {
        REDDatabase.attachmentMetadata.insert(plaintextHash, attachment.localBackupKey)
      } else {
        null
      }
  ...
  private fun buildAttachmentsThatCanArchiveQuery(archiveTransferStateFilter: String, includeOptimized: Boolean = false): String {
    val notReleaseChannelClause = REDStore.releaseChannel.releaseChannelRecipientId?.let {
  ...
  fun getMediaObjectsThatCantBeFound(objects: Set<ArchivedMediaObject>): Set<ArchivedMediaObject> {
    ...
    readableDatabase
      ...
      .forEach { cursor ->
        ...
        val mediaIdThumbnail = MediaName.fromPlaintextHashAndRemoteKeyForThumbnail(plaintextHash, remoteKey).toMediaId(REDStore.backup.mediaRootBackupKey).encode()
  ...
  fun debugGetAttachmentStats(): DebugAttachmentStats {
    ...
    val lastSnapshotFullSizeCount = REDDatabase.backupMediaSnapshots.debugGetFullSizeAttachmentCountForMostRecentSnapshot()
    val lastSnapshotThumbnailCount = REDDatabase.backupMediaSnapshots.debugGetThumbnailAttachmentCountForMostRecentSnapshot()
