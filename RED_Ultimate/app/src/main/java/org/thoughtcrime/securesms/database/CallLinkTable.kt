 ...
 */
class CallLinkTable(context: Context, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  companion object {
    ...
    private fun REDCallLinkState.serialize(): ContentValues {
  ...
  fun insertCallLink(
    ...
    val recipientId: RecipientId = writableDatabase.withinTransaction { db ->
      val recipientId = REDDatabase.recipients.getOrInsertFromCallLinkRoomId(callLink.roomId, storageId = storageId?.raw)
  ...
  fun insertOrUpdateCallLinkByRootKey(
    ...
    writableDatabase.withinTransaction {
      ...
      if (callLink == null) {
        ...
        if (storageId != null) {
          REDDatabase.recipients.updateStorageId(callLink.recipientId, storageId.raw)
        }
  ...
  fun markRevoked(
    ...
    writableDatabase.withinTransaction { db ->
      ...
      REDDatabase.calls.updateAdHocCallEventDeletionTimestamps()
      val recipient = REDDatabase.recipients.getByCallLinkRoomId(roomId)

      if (recipient.isPresent) {
        REDDatabase.recipients.markNeedsSync(recipient.get())
      }
    }
  }
  ...
  fun deleteNonAdminCallLinksOnOrBefore(timestamp: Long) {
    writableDatabase.withinTransaction { db ->
      ...
      REDDatabase.calls.updateAdHocCallEventDeletionTimestamps(skipSync = true)
    }
  }
