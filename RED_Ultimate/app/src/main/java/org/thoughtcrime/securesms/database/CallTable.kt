...
import com.red.sovereign.database.CallTable.Companion.TIMESTAMP
 ...
 */
class CallTable(context: Context, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  fun markAllCallEventsRead(timestamp: Long = Long.MAX_VALUE, readAt: Long = System.currentTimeMillis()) {
    ...
    val expiringCalls = REDDatabase.messages.getUnstartedExpirations(allUnreadMissedCalls)

    if (expiringCalls.isNotEmpty()) {
      Log.i(TAG, "Found ${expiringCalls.size} calls that needs expiring.")
      REDDatabase.messages.markExpireStarted(expiringCalls.map { it.key to readAt })
  ...
  fun markAllCallEventsWithPeerBeforeTimestampRead(peer: RecipientId, timestamp: Long, readAt: Long = System.currentTimeMillis()): Call? {
    val latestCallAsOfTimestamp = writableDatabase.withinTransaction { db ->
      ...
      val expiring = REDDatabase.messages.getUnstartedExpirations(unreadMissedCalls)

      if (expiring.isNotEmpty()) {
        Log.i(TAG, "Found ${expiring.size} calls that needs expiring.")
        REDDatabase.messages.markExpireStarted(expiring.map { it.key to readAt })
  ...
  fun insertOneToOneCall(callId: Long, timestamp: Long, peer: RecipientId, type: Type, direction: Direction, event: Event, fromSync: Boolean = false) {
    ...
    writableDatabase.withinTransaction {
      val result = REDDatabase.messages.insertOneToOneCallLog(peer, messageType, timestamp, direction == Direction.OUTGOING, fromSync)
    ...
    return writableDatabase.withinTransaction {
      ...
      if (call != null) {
        ...
        if (call.messageId == null) {
          ...
          REDDatabase.messages.updateOneToOneCallLog(call.messageId, call.messageType)
        }
  ...
  fun markCallDeletedFromSyncEvent(call: Call) {
    ...
    writableDatabase.withinTransaction { db ->
      ...
      if (call.messageId != null) {
        REDDatabase.messages.deleteMessage(call.messageId)
      }
    }
  ...
  fun insertAcceptedGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      val messageId: MessageId? = if (type == Type.GROUP_CALL) {
        REDDatabase.messages.insertGroupCall(
  ...
  private fun handleGroupUpdate(
    ...
    writableDatabase.withinTransaction {
      ...
      val messageId: MessageId = if (call != null) {
        ...
        REDDatabase.messages.updateGroupCall(
          call.messageId,
          peekGroupCallEraId,
          peekJoinedUuids,
          isCallFull,
  ...
  fun updateGroupCallFromPeek(
    ...
    val recipientId = REDDatabase.threads.getRecipientIdForThreadId(threadId)
    ...
    REDDatabase.messages.updatePreviousGroupCall(
      threadId = threadId,
  ...
  private fun createEventFromRingState(
    ...
    writableDatabase.withinTransaction { db ->
      val messageId = REDDatabase.messages.insertGroupCall(
  ...
  fun setTimestamp(callId: Long, recipientId: RecipientId, timestamp: Long) {
    writableDatabase.withinTransaction { db ->
      ...
      if (call.messageId != null) {
        REDDatabase.messages.updateCallTimestamps(call.messageId, timestamp)
      }
    }
  ...
  fun deleteNonAdHocCallEvents(callRowIds: Set<Long>) {
    ...
    REDDatabase.messages.deleteCallUpdates(messageIds)
  ...
  fun markRingingCallsAsMissed() {
    writableDatabase.withinTransaction { db ->
      ...
      REDDatabase.messages.clearIsRingingOnLocalDeviceFlag(messageIds)
    }
  }
