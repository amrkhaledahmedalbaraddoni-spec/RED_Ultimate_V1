...
class ChatColorsTable(context: Context, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  private fun updateChatColors(chatColors: ChatColors): ChatColors {
    ...
    if (REDStore.chatColors.chatColors?.id == chatColors.id) {
      REDStore.chatColors.chatColors = chatColors
    }

    REDDatabase.recipients.onUpdatedChatColors(chatColors)
    notifyListeners()
  ...
  fun deleteChatColors(chatColors: ChatColors) {
    ...
    REDDatabase.recipients.onDeletedChatColors(chatColors)
    notifyListeners()
  }
