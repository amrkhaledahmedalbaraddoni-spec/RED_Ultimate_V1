...
import com.red.sovereign.database.ThreadTable.Companion.ID
 ...
 */
class ChatFolderTables(context: Context?, databaseHelper: REDDatabase?) : DatabaseTable(context, databaseHelper), ThreadIdDatabaseReference {
  ...
  fun getUnreadCountAndEmptyAndMutedStatusForFolders(folders: List<ChatFolderRecord>): HashMap<Long, Triple<Int, Boolean, Boolean>> {
    ...
    folders.map { folder ->
      val unreadCount = REDDatabase.threads.getUnreadCountByChatFolder(folder)
      val isEmpty = !REDDatabase.threads.hasChatInFolder(folder)
      val isMuted = !REDDatabase.threads.hasUnmutedChatsInFolder(folder)
   ...
   */
  fun insertChatFolderFromStorageSync(record: REDChatFolderRecord) {
   ...
   */
  fun updateChatFolderFromStorageSync(record: REDChatFolderRecord) {
  ...
  /**
   * Parses a remote chat folder [REDChatFolderRecord] into a local folder [ChatFolderRecord]
   */
  private fun remoteChatFolderRecordToLocal(record: REDChatFolderRecord): ChatFolderRecord {
    ...
    return ChatFolderRecord(
      ...
      includedChats = record.proto.includedRecipients
        ...
        .map { recipient -> REDDatabase.threads.getOrCreateThreadIdFor(recipient) },
