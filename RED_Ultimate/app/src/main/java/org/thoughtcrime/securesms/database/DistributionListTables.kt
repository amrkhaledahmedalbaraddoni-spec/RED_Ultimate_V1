 ...
 */
class DistributionListTables constructor(context: Context?, databaseHelper: REDDatabase?) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  fun createList(
    ...
    try {
      ...
      val recipientId = REDDatabase.recipients.getOrInsertFromDistributionListId(DistributionListId.from(id), storageId)
    ...
    return when (privacyMode) {
      DistributionListPrivacyMode.ALL -> {
        REDDatabase.recipients
          .getREDContacts(RecipientTable.IncludeSelfMode.Exclude)
  ...
  fun getPrivacyData(listId: DistributionListId): DistributionListPrivacyData {
    ...
    readableDatabase.withinTransaction {
      ...
      totalContactCount = REDDatabase.recipients.getREDContactsCount(RecipientTable.IncludeSelfMode.Exclude)
    }
  ...
  fun getRecipientIdForSyncRecord(record: REDStoryDistributionListRecord): RecipientId? {
  ...
  fun applyStorageSyncStoryDistributionListInsert(insert: REDStoryDistributionListRecord) {
  ...
  fun applyStorageSyncStoryDistributionListUpdate(update: StorageRecordUpdate<REDStoryDistributionListRecord>) {
    ...
    REDDatabase.recipients.updateStorageId(recipientId, update.new.id.raw)
