...
import com.red.sovereign.groups.GroupId.Push
...
class GroupTable(context: Context?, databaseHelper: REDDatabase?) :
  ...
  class MembershipTable(context: Context, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper) {
    companion object {
  ...
  @CheckReturnValue
  fun create(groupId: GroupId.V1, title: String?, members: Collection<RecipientId>, avatar: REDServiceAttachmentPointer?): Boolean {
  ...
  private fun create(
    ...
    avatar: REDServiceAttachmentPointer?,
    groupMasterKey: GroupMasterKey?,
    groupState: DecryptedGroup?,
    ...
    if (avatar != null) {
      values.put(AVATAR_ID, (avatar.remoteId as REDServiceAttachmentRemoteId.V2).cdnId)
    ...
    if (groupId.isV2) {
      REDDatabase.nameCollisions.handleGroupNameCollisions(groupId.requireV2(), members.toSet())
    }

    return true
  }

  fun update(groupId: GroupId.V1, title: String?, avatar: REDServiceAttachmentPointer?) {
    val contentValues = ContentValues().apply {
      ...
      if (avatar != null) {
        put(AVATAR_ID, (avatar.remoteId as REDServiceAttachmentRemoteId.V2).cdnId)
  ...
  fun update(
    ...
    if (groupId.isV2 && addedMembers.isNotEmpty()) {
      REDDatabase.nameCollisions.handleGroupNameCollisions(groupId.requireV2(), addedMembers.toSet())
    }
  }
  ...
  private fun clearGroupIfLeftAndDeleted(record: GroupRecord?) {
    ...
    if (REDDatabase.threads.hasActiveThread(record.recipientId)) {
    ...
    val keepGroupIdentifier = REDStore.account.isMultiDevice || recipients.isBlocked(record.recipientId)
    ...
    writableDatabase.withinTransaction { db ->
      ...
      record.distributionId?.let { distributionId ->
        REDDatabase.senderKeys.deleteAllFor(distributionId)
        REDDatabase.senderKeyShared.deleteAllFor(distributionId)
      }

      REDDatabase.threads.deleteThread(record.recipientId)
  ...
  fun getGroupSendFullToken(threadId: Long, recipientId: RecipientId): GroupSendFullToken? {
    val threadRecipient = REDDatabase.threads.getRecipientForThreadId(threadId)
  ...
  fun getGroupSendFullToken(groupId: GroupId.V2, recipientId: RecipientId): GroupSendFullToken? {
    val groupRecord = REDDatabase.groups.getGroup(groupId).filter { it.hasV2GroupProperties }.orElse(null) ?: return null
    val endorsement = REDDatabase.groups.getGroupSendEndorsement(groupId, recipientId) ?: return null
  ...
  class V2GroupProperties(val groupMasterKey: GroupMasterKey, val groupRevision: Int, private val decryptedGroupBytes: ByteArray) {
    ...
    fun getMemberRecipientIds(memberSet: MemberSet): List<RecipientId> {
      ...
      val selfAci = REDStore.account.requireAci()
