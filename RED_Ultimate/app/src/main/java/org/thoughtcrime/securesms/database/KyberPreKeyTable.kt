...
/**
 * A table for storing data related to [com.red.sovereign.crypto.storage.REDKyberPreKeyStore].
 */
class KyberPreKeyTable(context: Context, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun handleMarkKyberPreKeyUsed(serviceId: ServiceId, kyberPreKeyId: Int, signedPreKeyId: Int, baseKey: ECPublicKey) {
    writableDatabase.withinTransaction { db ->
      ...
      if (lastResortRowId < 0) {
        ...
        REDDatabase.lastResortKeyTuples.insert(
