...
import com.red.sovereign.util.MediaUtil.SlideType

@SuppressLint("RecipientIdDatabaseReferenceUsage", "ThreadIdDatabaseReferenceUsage") // Not a real table, just a view
class MediaTable internal constructor(context: Context?, databaseHelper: REDDatabase?) : DatabaseTable(context, databaseHelper) {
  ...
  data class MediaRecord constructor(
    ...
    companion object {
      ...
      fun from(cursor: Cursor): MediaRecord {
        ...
        return MediaRecord(
          attachment = if (hasAttachment) REDDatabase.attachments.getAttachment(cursor) else null,
