...
import com.red.sovereign.attachments.DatabaseAttachment.DisplayOrderComparator
import com.red.sovereign.backup.v2.exporters.ChatItemArchiveExporter
import com.red.sovereign.contactshare.Contact
import com.red.sovereign.conversation.MessageStyler
import com.red.sovereign.database.CallTable.Event
import com.red.sovereign.database.EarlyDeliveryReceiptCache.Receipt
import com.red.sovereign.database.MentionUtil.UpdatedBodyAndMentions
import com.red.sovereign.database.REDDatabase.Companion.attachments
import com.red.sovereign.database.REDDatabase.Companion.calls
import com.red.sovereign.database.REDDatabase.Companion.distributionLists
import com.red.sovereign.database.REDDatabase.Companion.groupReceipts
import com.red.sovereign.database.REDDatabase.Companion.groups
import com.red.sovereign.database.REDDatabase.Companion.mentions
import com.red.sovereign.database.REDDatabase.Companion.messages
import com.red.sovereign.database.REDDatabase.Companion.polls
import com.red.sovereign.database.REDDatabase.Companion.reactions
import com.red.sovereign.database.REDDatabase.Companion.recipients
import com.red.sovereign.database.REDDatabase.Companion.storySends
import com.red.sovereign.database.REDDatabase.Companion.threads
import com.red.sovereign.database.documents.Document
import com.red.sovereign.database.documents.IdentityKeyMismatch
import com.red.sovereign.database.documents.IdentityKeyMismatchSet
import com.red.sovereign.database.documents.NetworkFailure
import com.red.sovereign.database.documents.NetworkFailureSet
import com.red.sovereign.database.model.GroupCallUpdateDetailsUtil
import com.red.sovereign.database.model.Mention
import com.red.sovereign.database.model.MessageExportStatus
...
import com.red.sovereign.database.model.StoryResult
import com.red.sovereign.database.model.StoryType
import com.red.sovereign.database.model.StoryType.Companion.fromCode
import com.red.sovereign.database.model.StoryViewState
import com.red.sovereign.database.model.databaseprotos.AdminDeleteStatus
import com.red.sovereign.database.model.databaseprotos.BodyRangeList
import com.red.sovereign.database.model.databaseprotos.DecryptedGroupV2Context
...
import com.red.sovereign.database.model.databaseprotos.MessageExportState
...
import com.red.sovereign.database.model.databaseprotos.ProfileChangeDetails
...
import com.red.sovereign.jobs.OptimizeMessageSearchIndexJob
import com.red.sovereign.jobs.ThreadUpdateJob
...
import com.red.sovereign.mms.MessageGroupContext
...
import com.red.sovereign.notifications.v2.DefaultMessageNotifier.StickyThread
import com.red.sovereign.polls.Poll
import com.red.sovereign.polls.PollRecord
import com.red.sovereign.recipients.Recipient
import com.red.sovereign.recipients.RecipientId
import com.red.sovereign.revealable.ViewOnceExpirationInfo
import com.red.sovereign.sms.GroupV2UpdateMessageUtil
import com.red.sovereign.stories.Stories.isFeatureEnabled
...
open class MessageTable(context: Context?, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper), MessageTypes, RecipientIdDatabaseReference, ThreadIdDatabaseReference {
  ...
  fun updateGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      ...
      val updateDetail = GroupCallUpdateDetailsUtil.parse(message.body)
      val containsSelf = joinedUuids.contains(REDStore.account.requireAci().rawUuid)
      val selfCreated = updateDetail.startedCallUuid == REDStore.account.requireAci().rawUuid.toString()
  ...
  fun updatePreviousGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      ...
      MmsReader(cursor).use { reader ->
        ...
        val containsSelf = peekJoinedUuids.contains(REDStore.account.requireAci().rawUuid)
        // Treat empty eraId from ring requests as matching for updating
  ...
  fun insertProfileNameChangeMessages(recipient: Recipient, newProfileName: String, previousProfileName: String) {
    writableDatabase.withinTransaction { db ->
      ...
      groupRecords.filter { it.hasV2GroupProperties }.forEach {
        REDDatabase.nameCollisions.handleGroupNameCollisions(it.id.requireV2(), setOf(recipient.id))
      }
    }
  }
  ...
  fun insertLearnedProfileNameChangeMessage(recipient: Recipient, e164: String?, username: String?) {
    ...
    val threadId: Long? = REDDatabase.threads.getThreadIdFor(recipient.id)
    ...
    return mmsReaderFor(cursor).use { reader ->
      reader.mapNotNull {
        if (!it.isMms) {
          ...
          REDDatabase.payments.updateMessageWithPayment(it) as MmsMessageRecord
        } else {
  ...
  private fun markAsRemoteDeleteInternal(messageId: Long, deletedBy: RecipientId) {
    ...
    writableDatabase.withFtsSecureDelete(SearchTable.FTS_TABLE_NAME) {
      writableDatabase.withinTransaction { db ->
        ...
        REDDatabase.messageLog.deleteAllRelatedToMessage(messageId)
  ...
  fun insertMessageOutbox(
    ...
    contentValues.put(FROM_DEVICE_ID, REDStore.account.deviceId)
  ...
  private fun insertMediaMessage(
    ...
    val (messageId, insertedAttachments) = writableDatabase.withinTransaction { db ->
      ...
      REDDatabase.mentions.insert(threadId, messageId, mentions)

      val insertedAttachments = REDDatabase.attachments.insertAttachmentsForMessage(messageId, allAttachments, quoteAttachments)
  ...
  fun deleteMessagesInThread(threadIds: Collection<Long>, extraWhere: String = ""): Int {
    ...
    REDTrace.beginSection("MessageTable#deleteMessagesInThread")
    for (threadId in threadsWithPossibleDeletes) {
      ...
      do {
        deletedCount = writableDatabase.withinTransaction { db ->
          ...
          REDTrace.beginSection("deleteMessages")
  ...
  fun flushBulkDeleteNotifications(touchedThreadIds: Set<Long>) {
    touchedThreadIds.forEach { threadId ->
      REDDatabase.threads.update(threadId, unarchive = false)
      notifyConversationListeners(threadId)
    }
  ...
  private fun getReleaseChannelThreadId(hasSeenReleaseChannelStories: Boolean): Long {
    ...
    val releaseChannelRecipientId = REDStore.releaseChannel.releaseChannelRecipientId ?: return -1L
   ...
   *
   * Heavily lifted from [com.red.sovereign.database.helpers.migration.V191_UniqueMessageMigrationV2].
   */
...
fun MessageRecord.withAttachments(): MessageRecord {
  ...
  val fetchedAttachments = REDDatabase.attachments.getAttachmentsForMessage(id)
...
fun List<MessageRecord>.withAttachments(): List<MessageRecord> {
  ...
  val allAttachments = REDDatabase.attachments.getAttachmentsForMessages(map { it.id })
  return map { record ->
