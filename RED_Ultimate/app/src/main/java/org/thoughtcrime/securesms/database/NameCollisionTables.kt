...
class NameCollisionTables(
  ...
  fun markCollisionsForThreadRecipientDismissed(threadRecipientId: RecipientId) {
    writableDatabase.withinTransaction { db ->
      val threadId = REDDatabase.threads.getThreadIdFor(threadRecipientId) ?: return@withinTransaction
  ...
  fun getCollisionsForThreadRecipientId(recipientId: RecipientId): List<ReviewRecipient> {
    val threadId = REDDatabase.threads.getThreadIdFor(recipientId) ?: return emptyList()
    ...
    val groupMembers: Optional<List<RecipientId>> = REDDatabase.groups.getGroup(recipientId).map { it.members }
  ...
  fun handleIndividualNameCollision(recipientId: RecipientId) {
    writableDatabase.withinTransaction { db ->
      val similarRecipients = REDDatabase.recipients.getSimilarRecipientIds(Recipient.resolved(recipientId))
      ...
      if (similarRecipients.size == 1) {
        val threadId = REDDatabase.threads.getThreadIdFor(recipientId) ?: -1
        if (threadId > 0L) {
  ...
  fun handleGroupNameCollisions(groupId: GroupId.V2, changed: Set<RecipientId>) {
    writableDatabase.withinTransaction {
      val threadRecipientId = REDDatabase.recipients.getByGroupId(groupId).orNull() ?: return@withinTransaction
      handleNameCollisions(
        threadRecipientId = threadRecipientId,
  ...
  private fun collisionExists(threadRecipientId: RecipientId): Boolean {
    val threadId = REDDatabase.threads.getThreadIdFor(threadRecipientId) ?: return false
  ...
  private fun updateCollision(collision: NameCollision) {
    ...
    collision.members.forEach { member ->
      writableDatabase
        ...
        .run(conflictStrategy = com.red.sovereign.database.SQLiteDatabase.CONFLICT_IGNORE)
    }
  }
  ...
  private fun getDuplicatedGroupRecipients(groupId: V2, toCheck: Set<RecipientId>): List<ReviewRecipient> {
    ...
    val members: MutableList<Recipient> = REDDatabase.groups.getGroupMembers(groupId, GroupTable.MemberSet.FULL_MEMBERS_INCLUDING_SELF).toMutableList()
  ...
  private fun getProfileChangeRecordsForGroup(groupId: V2): List<MessageRecord> {
    val groupRecipientId = REDDatabase.recipients.getByGroupId(groupId).get()
    val groupThreadId = REDDatabase.threads.getThreadIdFor(groupRecipientId)

    return if (groupThreadId == null) {
      emptyList()
    } else {
      REDDatabase.messages.getProfileChangeDetailsRecords(
        groupThreadId,
