 ...
 */
class PendingPniSignatureMessageTable(context: Context, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
    ...
    pendingPniSignaturesCacheLock.writeLock().withLock {
      writableDatabase.withinTransaction { db ->
        ...
        if (!stillPending) {
          ...
          REDDatabase.recipients.clearNeedsPniSignature(recipientId)
