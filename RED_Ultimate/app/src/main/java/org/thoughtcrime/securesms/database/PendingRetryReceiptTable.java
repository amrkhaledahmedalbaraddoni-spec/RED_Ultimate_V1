...
public final class PendingRetryReceiptTable extends DatabaseTable implements RecipientIdDatabaseReference, ThreadIdDatabaseReference {
  ...
  PendingRetryReceiptTable(Context context, REDDatabase databaseHelper) {
  ...
  @NonNull PendingRetryReceiptModel insert(@NonNull RecipientId author, int authorDevice, long sentTimestamp, long receivedTimestamp, long threadId) {
    ...
    long id = databaseHelper.getREDWritableDatabase().insertWithOnConflict(TABLE_NAME, null, values, SQLiteDatabase.CONFLICT_REPLACE);
  ...
  void delete(@NonNull PendingRetryReceiptModel model) {
    databaseHelper.getREDWritableDatabase().delete(TABLE_NAME, ID_WHERE, SqlUtil.buildArgs(model.getId()));
  }
  ...
  public void onDeletedRecipient(@NonNull RecipientId recipientId) {
    int count = databaseHelper.getREDWritableDatabase().delete(TABLE_NAME, AUTHOR + " = ?", SqlUtil.buildArgs(recipientId.serialize()));
    AppDependencies.getPendingRetryReceiptCache().clear();
  ...
  public void onDeletedGroupThread(long threadId) {
    int count = databaseHelper.getREDWritableDatabase().delete(TABLE_NAME, THREAD_ID + " = ?", SqlUtil.buildArgs(threadId));
    AppDependencies.getPendingRetryReceiptCache().clear();
