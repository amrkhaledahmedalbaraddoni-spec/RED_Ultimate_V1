...
import com.red.sovereign.badges.Badges.toDatabaseBadge
import com.red.sovereign.badges.models.Badge
import com.red.sovereign.color.MaterialColor
import com.red.sovereign.contacts.paged.ContactSearchSortOrder
...
import com.red.sovereign.database.GroupTable.LegacyGroupInsertException
...
import com.red.sovereign.database.RecipientTableCursorUtil.getRecipientExtras
import com.red.sovereign.database.REDDatabase.Companion.groups
import com.red.sovereign.database.REDDatabase.Companion.identities
import com.red.sovereign.database.REDDatabase.Companion.runPostSuccessfulTransaction
import com.red.sovereign.database.REDDatabase.Companion.sessions
...
import com.red.sovereign.database.model.databaseprotos.DeviceLastResetTime
import com.red.sovereign.database.model.databaseprotos.ExpiringProfileKeyCredentialColumnData
...
import com.red.sovereign.database.model.databaseprotos.Wallpaper
...
import com.red.sovereign.groups.GroupId.V1
import com.red.sovereign.groups.GroupId.V2
import com.red.sovereign.groups.v2.ProfileKeySet
...
import com.red.sovereign.wallpaper.ChatWallpaperFactory
...
open class RecipientTable(context: Context, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  companion object {
    ...
    @JvmStatic
    fun maskCapabilitiesToLong(capabilities: REDServiceProfile.Capabilities): Long {
      var value: Long = 0
  ...
  fun getAndPossiblyMerge(aci: ACI?, pni: PNI?, e164: String?, pniVerified: Boolean = false, changeSelf: Boolean = false): RecipientId {
    ...
    val e164 = e164?.let { REDE164Util.formatAsE164(it) }
  ...
  fun applyStorageSyncContactInsert(insert: REDContactRecord, rotateProfileKeyOnBlock: Boolean): Boolean {
    val db = writableDatabase
  ...
  fun applyStorageSyncContactUpdate(update: StorageRecordUpdate<REDContactRecord>, rotateProfileKeyOnBlock: Boolean): Boolean {
    val db = writableDatabase
  ...
  fun applyStorageSyncGroupV1Insert(insert: REDGroupV1Record) {
  ...
  fun applyStorageSyncGroupV1Update(update: StorageRecordUpdate<REDGroupV1Record>) {
  ...
  fun applyStorageSyncGroupV2Insert(insert: REDGroupV2Record) {
  ...
  fun applyStorageSyncGroupV2Update(update: StorageRecordUpdate<REDGroupV2Record>) {
  ...
  fun applyStorageSyncAccountUpdate(update: StorageRecordUpdate<REDAccountRecord>) {
  ...
  fun setCapabilities(id: RecipientId, capabilities: REDServiceProfile.Capabilities) {
    val values = ContentValues(1).apply {
  ...
  fun setCallNotificationSetting(id: RecipientId, setting: NotificationSetting) {
    ...
    if (update(id, values)) {
      // SIGNAL_INHERITED: TODO rotate storageId once this is actually synced in storage service
//      rotateStorageId(id)
      AppDependencies.databaseObserver.notifyRecipientChanged(id)
      StorageSyncHelper.scheduleSyncForDataChange()
    }
...
//      rotateStorageId(id)
  ...
  fun markHidden(id: RecipientId, clearProfileKey: Boolean = false, showMessageRequest: Boolean = false) {
    ...
    if (updated) {
      REDDatabase.distributionLists.removeMemberFromAllLists(id)
      REDDatabase.messages.deleteStoriesForRecipient(id)
      rotateStorageId(id)
      AppDependencies.databaseObserver.notifyRecipientChanged(id)
      StorageSyncHelper.scheduleSyncForDataChange()
    } else {
  ...
  fun updateSelfE164(e164: String, pni: PNI) {
    ...
    try {
      ...
      val newId = getAndPossiblyMerge(aci = REDStore.account.requireAci(), pni = pni, e164 = e164, pniVerified = true, changeSelf = true)
      ...
      KeyTransparencyApi.reset(aci = REDStore.account.requireAci().libREDAci, field = KeyTransparency.AccountDataField.E164, keyTransparencyStore = KeyTransparencyStore)

      REDDatabase.pendingPniSignatureMessages.deleteAll()
  ...
  private fun markUnregisteredAndSplit(id: RecipientId, record: RecipientRecord) {
    ...
    if (!REDE164Util.isPotentialE164(record.e164)) {
  ...
  fun splitForStorageSyncIfNecessary(aci: ACI) {
    ...
    val pni = record.pni
    val e164 = record.e164?.takeIf { REDE164Util.isPotentialNonShortCodeE164(it) }
      ...
      when (operation) {
        ...
        is PnpOperation.SessionSwitchoverInsert -> {
          if (hadThreadMerge) {
            ...
            if (threadId != null) {
              ...
              try {
                REDDatabase.messages.insertSessionSwitchoverEvent(operation.recipientId, threadId, event)
              } catch (e: Exception) {
                ...
                if (record.aci != null && record.aci == REDStore.account.aci) {
                  if (pnisWithSessions.contains(REDStore.account.pni!!)) {
                ...
                if (record.pni != null && record.pni == REDStore.account.pni) {
                  if (pnisWithSessions.contains(REDStore.account.pni!!)) {
                ...
                if (record.e164 != null && record.e164 == REDStore.account.e164) {
                  if (pnisWithSessions.contains(REDStore.account.pni!!)) {
                ...
                if (pnisWithSessions.isEmpty()) {
                  ...
                  if (pnisWithSessions.first() == REDStore.account.pni) {
        ...
        is PnpOperation.ChangeNumberInsert -> {
          if (changeSet.id is PnpIdResolver.PnpNoopId) {
            REDDatabase.messages.insertNumberChangeMessages(changeSet.id.recipientId)
          } else {
  ...
  private fun notSelf(e164: String?, pni: PNI?, aci: ACI?): Boolean {
    return (e164 == null || e164 != REDStore.account.e164) &&
      (pni == null || pni != REDStore.account.pni) &&
      (aci == null || aci != REDStore.account.aci)
  }
  ...
  fun getREDContacts(includeSelfMode: IncludeSelfMode): Cursor {
    return getREDContacts(includeSelfMode, "$SORT_NAME, $SYSTEM_JOINED_NAME, $SEARCH_PROFILE_NAME, $USERNAME, $E164")
  }

  fun getREDContactsCount(includeSelfMode: IncludeSelfMode): Int {
    return getREDContacts(includeSelfMode).count
  }
  ...
  private fun getREDContacts(includeSelfMode: IncludeSelfMode, orderBy: String? = null): Cursor {
  ...
  fun queryREDContacts(contactSearchQuery: ContactSearchQuery): Cursor? {
  ...
  fun queryREDContactLetterHeaders(inputQuery: String, includeSelfMode: IncludeSelfMode, includePush: Boolean, includeSms: Boolean): Map<RecipientId, String> {
    val searchSelection = ContactSearchSelection.Builder()
  ...
  fun rotateStorageId(recipientId: RecipientId, logFailure: Boolean = false) {
    ...
    if (recipientId != selfId && recipientId == REDStore.releaseChannel.releaseChannelRecipientId) {
  ...
  private fun merge(primaryId: RecipientId, secondaryId: RecipientId, newPni: PNI? = null, pniVerified: Boolean): MergeResult {
    ...
    if (threadMerge.neededMerge) {
      ...
      REDDatabase.messages.insertThreadMergeEvent(primaryRecord.id, threadMerge.threadId, mergeEvent.build())
    }
  ...
  private fun buildContentValuesForNewUser(e164: String?, pni: PNI?, aci: ACI?, pniVerified: Boolean): ContentValues {
    ...
    val values = contentValuesOf(
      E164 to e164.nullIfBlank()?.let { REDE164Util.formatAsE164(it) },
      ACI_COLUMN to aci?.toString(),
      PNI_COLUMN to pni?.toString(),
      PNI_SIGNATURE_VERIFIED to pniVerified.toInt(),
  ...
  private fun getValuesForStorageContact(contact: REDContactRecord, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
      ...
      put(E164, contact.proto.e164.nullIfBlank()?.let { REDE164Util.formatAsE164(it) })
      put(PROFILE_GIVEN_NAME, profileName.givenName)
      put(PROFILE_FAMILY_NAME, profileName.familyName)
      put(PROFILE_JOINED_NAME, profileName.toString())
  ...
  private fun getValuesForStorageGroupV1(groupV1: REDGroupV1Record, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
  ...
  private fun getValuesForStorageGroupV2(groupV2: REDGroupV2Record, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
  ...
  fun updatePendingSelfData(selfId: RecipientId) {
    REDDatabase.messages.updatePendingSelfData(RecipientId.from(PLACEHOLDER_SELF_ID), selfId)
...
//    const val VERSIONED_EXPIRATION_TIMER = 10
  ...
  data class ProfileUpdate(
    ...
    val capabilities: REDServiceProfile.Capabilities? = null,
