...
class RemappedRecords {
  ...
  void deleteThread(long oldId) {
    ...
    REDDatabase.remappedRecords().deleteThreadMapping(oldId);
  }
  ...
  void addRecipient(@NonNull RecipientId oldId, @NonNull RecipientId newId) {
    ...
    REDDatabase.remappedRecords().addRecipientMapping(oldId, newId);
  }
  ...
  void addThread(long oldId, long newId) {
    ...
    REDDatabase.remappedRecords().addThreadMapping(oldId, newId);
  }
  ...
  private void ensureRecipientMapIsPopulated() {
    if (recipientMap == null) {
      Map<RecipientId, RecipientId> loaded = REDDatabase.remappedRecords().getAllRecipientMappings();
      synchronized (this) {
        if (recipientMap == null) {
          recipientMap = loaded;
        }
  ...
  private void ensureThreadMapIsPopulated() {
    if (threadMap == null) {
      Map<Long, Long> loaded = REDDatabase.remappedRecords().getAllThreadMappings();
      synchronized (this) {
        if (threadMap == null) {
          threadMap = loaded;
        }
  ...
  private void scheduleStaleTrimIfNeeded(boolean loadedMapWasEmpty) {
    if (!loadedMapWasEmpty && staleTrimScheduled.compareAndSet(false, true)) {
      REDExecutors.BOUNDED.execute(() -> REDDatabase.remappedRecords().trimStaleMappings());
    }
  }
