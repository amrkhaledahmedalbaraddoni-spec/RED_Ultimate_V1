...
import android.os.CancellationRED;
...
public class SQLiteDatabase implements SupportSQLiteDatabase {
  ...
  @Override
  public Cursor query(SupportSQLiteQuery query, CancellationRED cancellationRED) {
    DatabaseMonitor.onSql(query.getSql(), null);
    return traceSql("query(SupportSQLiteQuery, CancellationRED)", null, query.getSql(), false, () -> wrapped.query(query, cancellationRED));
  }
