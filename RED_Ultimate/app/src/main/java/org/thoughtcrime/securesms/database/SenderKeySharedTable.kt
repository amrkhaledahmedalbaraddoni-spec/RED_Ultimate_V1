 ...
 */
class SenderKeySharedTable internal constructor(context: Context?, databaseHelper: REDDatabase?) : DatabaseTable(context, databaseHelper) {
  companion object {
...
/**
   ...
   */
  fun markAsShared(distributionId: DistributionId, addresses: Collection<REDProtocolAddress>) {
   ...
   */
  fun getSharedWith(distributionId: DistributionId): Set<REDProtocolAddress> {
   ...
   */
  fun delete(distributionId: DistributionId, addresses: Collection<REDProtocolAddress>) {
    writableDatabase.withinTransaction { db ->
      for (address in addresses) {
   ...
   */
  fun deleteAllFor(addresses: Collection<REDProtocolAddress>) {
    writableDatabase.withinTransaction { db ->
      for (address in addresses) {
        db.delete(TABLE_NAME)
