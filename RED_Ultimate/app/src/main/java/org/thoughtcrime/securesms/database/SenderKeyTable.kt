 ...
 */
class SenderKeyTable internal constructor(context: Context?, databaseHelper: REDDatabase?) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun store(address: REDProtocolAddress, distributionId: DistributionId, record: SenderKeyRecord) {
  ...
  fun load(address: REDProtocolAddress, distributionId: DistributionId): SenderKeyRecord? {
   ...
   */
  fun getCreatedTime(address: REDProtocolAddress, distributionId: DistributionId): Long {
    return readableDatabase
      .select(CREATED_AT)
      .from(TABLE_NAME)
      .where("$ADDRESS = ? AND $DEVICE = ? AND $DISTRIBUTION_ID = ?", address.name, address.deviceId, distributionId)
  ...
  fun getAllCreatedBySelf(): Cursor {
    return readableDatabase
      ...
      .from(TABLE_NAME)
      .where("$ADDRESS = ?", REDStore.account.requireAci())
