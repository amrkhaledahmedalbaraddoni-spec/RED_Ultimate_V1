...
class SessionTable(context: Context, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun store(serviceId: ServiceId, address: REDProtocolAddress, record: SessionRecord) {
  ...
  fun load(serviceId: ServiceId, address: REDProtocolAddress): SessionRecord? {
  ...
  fun load(serviceId: ServiceId, addresses: List<REDProtocolAddress>): List<SessionRecord?> {
    ...
    val sessions: HashMap<REDProtocolAddress, SessionRecord?> = LinkedHashMap(addresses.size)
      ...
      readableDatabase.query(TABLE_NAME, projection, combinedQuery.where, combinedQuery.whereArgs, null, null, null).use { cursor ->
        while (cursor.moveToNext()) {
          ...
          try {
            ...
            sessions[REDProtocolAddress(address, device)] = record
          } catch (e: IOException) {
            Log.w(TAG, e)
          }
        }
  ...
  fun getSubDevices(serviceId: ServiceId, addressName: String): List<Int> {
    ...
    val selection = "$ACCOUNT_ID = ? AND $ADDRESS = ? AND $DEVICE != ?"
    val args = SqlUtil.buildArgs(serviceId, addressName, REDServiceAddress.DEFAULT_DEVICE_ID)
  ...
  fun delete(serviceId: ServiceId, address: REDProtocolAddress) {
