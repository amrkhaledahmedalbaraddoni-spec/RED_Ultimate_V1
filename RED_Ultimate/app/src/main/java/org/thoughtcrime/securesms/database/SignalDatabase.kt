...
import com.red.sovereign.crypto.DatabaseSecret
...
open class REDDatabase(private val context: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret, name: String = DATABASE_NAME) :
  SQLiteOpenHelper(
    context,
    name,
    databaseSecret.asString(),
    null,
    REDDatabaseMigrations.DATABASE_VERSION,
    0,
    SqlCipherErrorHandler(context, name),
    SqlCipherDatabaseHook(),
    true
  ),
  REDDatabaseOpenHelper {
  ...
  override fun onCreate(db: net.zetetic.database.sqlcipher.SQLiteDatabase) {
    val signalDb = REDSQLiteDatabase(db)
  ...
  @VisibleForTesting
  fun onCreateTablesIndexesAndTriggers(db: REDSQLiteDatabase) {
  ...
  override fun onUpgrade(db: net.zetetic.database.sqlcipher.SQLiteDatabase, oldVersion: Int, newVersion: Int) {
    ...
    try {
      ...
      REDDatabaseMigrations.migrate(context, REDSQLiteDatabase(db), oldVersion, newVersion)
    } finally {
      db.setForeignKeyConstraintsEnabled(true)
    ...
    REDDatabaseMigrations.migratePostTransaction(context, oldVersion)
    Log.i(TAG, "Upgrade complete. Took " + (System.currentTimeMillis() - startTime) + " ms.")
  }

  override val readableDatabase: net.zetetic.database.sqlcipher.SQLiteDatabase
    get() = throw UnsupportedOperationException("Call getREDReadableDatabase() instead!")

  override val writableDatabase: net.zetetic.database.sqlcipher.SQLiteDatabase
    get() = throw UnsupportedOperationException("Call getREDWritableDatabase() instead!")
  ...
  open val signalReadableDatabase: REDSQLiteDatabase
    get() = REDSQLiteDatabase(super.readableDatabase)

  open val signalWritableDatabase: REDSQLiteDatabase
    get() = REDSQLiteDatabase(super.writableDatabase)
  ...
  open fun markCurrent(db: net.zetetic.database.sqlcipher.SQLiteDatabase) {
    db.version = REDDatabaseMigrations.DATABASE_VERSION
  }
  ...
  companion object {
    private val TAG = Log.tag(REDDatabase::class.java)
    const val DATABASE_NAME = "signal.db"

    @JvmStatic
    @Volatile
    var instance: REDDatabase? = null
      private set

    @JvmStatic
    fun init(application: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret) {
      if (instance == null) {
        synchronized(REDDatabase::class.java) {
          if (instance == null) {
            instance = REDDatabase(application, databaseSecret, attachmentSecret)
          }
        }
      }
    }
    ...
    @JvmStatic
    @VisibleForTesting
    fun setREDDatabaseInstanceForTesting(signalDatabase: REDDatabase) {
    ...
    @JvmStatic
    val readableDatabase: REDSQLiteDatabase
      get() = instance!!.signalReadableDatabase

    @JvmStatic
    val writableDatabase: REDSQLiteDatabase
    ...
    @JvmStatic
    fun <T> runInTransaction(block: (REDSQLiteDatabase) -> T): T {
     ...
     */
    @JvmStatic
    fun tryRunInTransaction(block: (REDSQLiteDatabase) -> Unit): Boolean {
      var committed = false
