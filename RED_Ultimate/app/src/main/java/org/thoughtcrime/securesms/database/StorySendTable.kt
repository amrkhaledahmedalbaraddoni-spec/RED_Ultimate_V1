 ...
 */
class StorySendTable(context: Context, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  fun applySentStoryManifest(remoteManifest: SentStorySyncManifest, sentTimestamp: Long) {
    writableDatabase.withinTransaction {
      ...
      messagesWithoutAnyReceivers.forEach {
        REDDatabase.messages.markAsDeleteBySelf(it)
        REDDatabase.messages.deleteRemotelyDeletedStory(it)
      }
    }
  }
