...
import org.signal.core.util.JsonUtils.SaneJSONObject
...
import com.red.sovereign.database.REDDatabase.Companion.messageLog
...
import com.red.sovereign.database.ThreadBodyUtil.ThreadBody
import com.red.sovereign.database.model.MessageRecord
import com.red.sovereign.database.model.MmsMessageRecord
import com.red.sovereign.database.model.ThreadRecord
...
import com.red.sovereign.database.model.serialize
...
import com.red.sovereign.util.isScheduled
...
import org.whispersystems.signalservice.api.storage.toREDServiceAddress
...
class ThreadTable(context: Context, databaseHelper: REDDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  private fun allowedToUnarchive(threadId: Long): Boolean {
    if (!REDStore.settings.shouldKeepMutedChatsArchived()) {
      return true
    }
  ...
  fun trimAllThreads(length: Int, trimBeforeDate: Long) {
    ...
    val syncThreadTrimDeletes = REDStore.settings.shouldSyncThreadTrimDeletes()
  ...
  private fun trimThreadInternal(
    ...
    if (finalTrimBeforeDate != NO_TRIM_BEFORE_DATE_SET) {
      ...
      if (deletes > 0) {
        ...
        REDDatabase.calls.updateCallEventDeletionTimestamps()
  ...
  fun getRecentConversationList(limit: Int, includeInactiveGroups: Boolean, individualsOnly: Boolean, groupsOnly: Boolean, hideV1Groups: Boolean, hideSms: Boolean, hideSelf: Boolean): Cursor {
    ...
    if (REDStore.releaseChannel.releaseChannelRecipientId != null) {
      where += " AND $TABLE_NAME.$RECIPIENT_ID != ${REDStore.releaseChannel.releaseChannelRecipientId!!.toLong()}"
    }
  ...
  fun deleteConversations(selectedConversations: Set<Long>, syncThreadDeletes: Boolean = true) {
    REDTrace.beginSection("ThreadTable#deleteConversations")
  ...
  fun deleteAllConversations() {
    writableDatabase.withinTransaction { db ->
      ...
      REDDatabase.calls.deleteAllCalls()
      synchronized(threadIdCache) {
        threadIdCache.clear()
      }
    }
  ...
  fun applyStorageSyncUpdate(recipientId: RecipientId, record: REDContactRecord) {
    applyStorageSyncUpdate(recipientId, record.proto.archived, record.proto.markedUnread, isGroup = false)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: REDGroupV1Record) {
    applyStorageSyncUpdate(recipientId, record.proto.archived, record.proto.markedUnread, isGroup = true)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: REDGroupV2Record) {
    applyStorageSyncUpdate(recipientId, record.proto.archived, record.proto.markedUnread, isGroup = true)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: REDAccountRecord) {
    writableDatabase.withinTransaction { db ->
      ...
      for (pinned: AccountRecord.PinnedConversation in record.proto.pinnedConversations) {
        val pinnedRecipient: Recipient? = if (pinned.contact != null) {
          if (ServiceId.parseOrNull(pinned.contact!!.serviceId, pinned.contact!!.serviceIdBinary) != null) {
            Recipient.externalPush(pinned.contact!!.toREDServiceAddress())
          } else {
            Log.w(TAG, "Failed to parse serviceId!")
            null
          }
        } else if (pinned.releaseNotes != null) {
          REDStore.releaseChannel.releaseChannelRecipientId?.let { Recipient.resolved(it) }
  ...
  private fun hasMoreRecentDraft(threadId: Long, timestamp: Long): Boolean {
    val drafts: DraftTable.Drafts = REDDatabase.drafts.getDrafts(threadId)
