...
public class UnknownStorageIdTable extends DatabaseTable {
  ...
  public UnknownStorageIdTable(Context context, REDDatabase databaseHelper) {
  ...
  public List<StorageId> getAllUnknownIds() {
    ...
    try (Cursor cursor = databaseHelper.getREDReadableDatabase().query(TABLE_NAME, null, null, null, null, null, null)) {
  ...
  public List<StorageId> getAllWithTypes(List<Integer> types) {
    ...
    try (Cursor cursor = databaseHelper.getREDReadableDatabase().query(TABLE_NAME, null, query.getWhere(), query.getWhereArgs(), null, null, null)) {
      while (cursor != null && cursor.moveToNext()) {
        String keyEncoded = CursorUtil.requireString(cursor, STORAGE_ID);
        int    type       = CursorUtil.requireInt(cursor, TYPE);
        try {
  ...
  public @Nullable REDStorageRecord getById(@NonNull byte[] rawId) {
    String   query = STORAGE_ID + " = ?";
    String[] args  = new String[] { Base64.encodeWithPadding(rawId) };

    try (Cursor cursor = databaseHelper.getREDReadableDatabase().query(TABLE_NAME, null, query, args, null, null, null)) {
      if (cursor != null && cursor.moveToFirst()) {
        int type = CursorUtil.requireInt(cursor, TYPE);
        return REDStorageRecord.forUnknown(StorageId.forType(rawId, type));
      } else {
        return null;
      }
    }
  ...
  public void insert(@NonNull Collection<REDStorageRecord> inserts) {
    SQLiteDatabase db = databaseHelper.getREDWritableDatabase();

    Preconditions.checkArgument(db.inTransaction(), "Must be in a transaction!");

    for (REDStorageRecord insert : inserts) {
  ...
  public void deleteAll() {
    databaseHelper.getREDWritableDatabase().delete(TABLE_NAME, null, null);
  }
}
