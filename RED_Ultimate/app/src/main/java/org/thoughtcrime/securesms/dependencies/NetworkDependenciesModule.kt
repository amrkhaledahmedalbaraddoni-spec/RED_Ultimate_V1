...
import com.red.sovereign.groups.GroupsV2AuthorizationMemoryValueCache
...
import com.red.sovereign.push.REDServiceTrustStore
...
class NetworkDependenciesModule(
  ...
  val signalServiceNetworkAccess: REDServiceNetworkAccess by lazy {
    provider.provideREDServiceNetworkAccess()
  }

  private val _protocolStore = resettableLazy {
    provider.provideProtocolStore()
  }
  val protocolStore: REDServiceDataStoreImpl by _protocolStore

  private val _signalServiceMessageSender = resettableLazy {
    provider.provideREDServiceMessageSender(protocolStore, pushServiceSocket, messageApi, keysApi)
  }
  val signalServiceMessageSender: REDServiceMessageSender by _signalServiceMessageSender
  ...
  val signalRestClient: REDRestClient by lazy {
    provider.provideREDRestClient(signalServiceNetworkAccess.getConfiguration())
  }

  val signalServiceAccountManager: REDServiceAccountManager by lazy {
    provider.provideREDServiceAccountManager(authWebSocket, accountApi, pushServiceSocket, groupsV2Operations)
  }
  ...
  val authWebSocket: REDWebSocket.AuthenticatedWebSocket by lazy {
  ...
  val unauthWebSocket: REDWebSocket.UnauthenticatedWebSocket by lazy {
  ...
  val groupsV2Authorization: GroupsV2Authorization by lazy {
    val authCache: GroupsV2Authorization.ValueCache = GroupsV2AuthorizationMemoryValueCache(REDStore.groupsV2AciAuthorizationCache)
  ...
  val signalServiceMessageReceiver: REDServiceMessageReceiver by lazy {
    provider.provideREDServiceMessageReceiver(pushServiceSocket)
  }
  ...
  val signalOkHttpClient: OkHttpClient by lazy {
    try {
      ...
      val trustStore: TrustStore = REDServiceTrustStore(application)
