package com.red.sovereign.emoji
...
import com.red.sovereign.components.emoji.parsing.EmojiDrawInfo
import com.red.sovereign.emoji.protos.JumbomojiPack
import com.red.sovereign.jobmanager.impl.AutoDownloadEmojiConstraint
import com.red.sovereign.keyvalue.REDStore
import com.red.sovereign.util.ListenableFutureTask
import com.red.sovereign.util.SoftHashMap
...
object JumboEmoji {
  ...
  private val executor = REDExecutors.newCachedSingleThreadExecutor("jumbo-emoji", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
  ...
  fun updateCurrentVersion(context: Context) {
    REDExecutors.BOUNDED.execute {
      ...
      if (EmojiFiles.getLatestEmojiData(context, version)?.format != null) {
        ...
        ThreadUtil.runOnMain { downloadedJumbos.addAll(REDStore.emoji.getJumboEmojiSheets(version.version)) }
      }
    }
  }
  ...
  fun loadJumboEmoji(context: Context, drawInfo: EmojiDrawInfo): LoadResult {
    ...
    val newTask = ListenableFutureTask<Bitmap> {
      ...
      if (uuid == null) {
        ...
        REDStore.emoji.addJumboEmojiSheet(version.version, drawInfo.jumboSheet)
      }
