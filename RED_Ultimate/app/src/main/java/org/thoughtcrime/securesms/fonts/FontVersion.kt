package com.red.sovereign.fonts
...
import com.red.sovereign.util.EncryptedStreamUtils
import java.io.File
...
data class FontVersion(val id: Long, val path: String) {
  ...
  companion object {
    ...
    fun get(context: Context): FontVersion {
      ...
      val version: FontVersion = if (System.currentTimeMillis() - REDStore.story.lastFontVersionCheck > VERSION_CHECK_INTERVAL) {
        ...
        if (fromDisk == null && fromNetwork == null) {
          ...
          if (fromDisk.id < fromNetwork.id) {
            ...
            REDStore.story.lastFontVersionCheck = System.currentTimeMillis()
            fromDisk
          }
        } else {
