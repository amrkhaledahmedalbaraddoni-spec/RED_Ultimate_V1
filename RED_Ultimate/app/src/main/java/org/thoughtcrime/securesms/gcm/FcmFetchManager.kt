...
import com.red.sovereign.messages.WebSocketDrainer
...
object FcmFetchManager {
  ...
  private val EXECUTOR = SerialMonoLifoExecutor(REDExecutors.UNBOUNDED)
  ...
  fun startBackgroundService(context: Context) {
    ...
    REDLocalMetrics.FcmServiceStartSuccess.onFcmStarted()
  }
  ...
  fun startForegroundService(context: Context) {
    ...
    if (FcmFetchForegroundService.startServiceIfNecessary(context)) {
      ...
      REDLocalMetrics.FcmServiceStartFailure.onFcmFailedToStart()
    }
  }
  ...
  private fun fetch(context: Context) {
    ...
    val metricId = REDLocalMetrics.PushWebsocketFetch.startFetch()
    val success = retrieveMessages(context)
    if (success) {
      REDLocalMetrics.PushWebsocketFetch.onDrained(metricId)
      cancelMayHaveMessagesNotification(context)
    } else {
      REDLocalMetrics.PushWebsocketFetch.onTimedOut(metricId)
