...
public final class GroupManager {
      ...
      throws GroupChangeBusyException, GroupChangeFailedException, IOException
  {
    try (GroupManagerV2.GroupEditor edit = new GroupManagerV2(context).edit(groupId.requireV2())) {
      edit.terminateGroup();
      REDDatabase.groups().setTerminatedBy(groupId, Recipient.self().getId());
      ...
      throws GroupChangeBusyException, GroupChangeFailedException, IOException
  {
    ...
    REDDatabase.recipients().getByGroupId(groupId).ifPresent(id -> REDDatabase.messages().deleteScheduledMessages(id));
    REDDatabase.groups().clearGroupIfLeftAndDeleted(groupId);
  }
  ...
  {
    try (GroupManagerV2.GroupUpdater updater = new GroupManagerV2(context).updater(groupMasterKey)) {
      return updater.updateLocalToServerRevision(revision, timestamp, groupRecord, groupSecretParams, signedGroupChange, serverGuid, receivedTime);
    }
  }
      ...
      throws GroupChangeBusyException, GroupChangeFailedException, GroupNotAMemberException, GroupInsufficientRightsException, IOException
  {
    try (GroupManagerV2.GroupEditor editor = new GroupManagerV2(context).edit(groupId.requireV2())) {
      editor.acceptInvite();
      REDDatabase.groups().setMember(groupId, true);
    }
  }
