package com.red.sovereign.groups.ui.invitesandrequests.invited;
...
final class PendingMemberInvitesRepository {
  ...
  public void getInvitees(@NonNull Consumer<InviteeResult> onInviteesLoaded) {
    executor.execute(() -> {
      ...
      ByteString                                   self               = REDStore.account().requireAci().toByteString();
  ...
  boolean revokeInvites(@NonNull Collection<UuidCiphertext> uuidCipherTexts) {
    try {
      GroupManager.revokeInvites(context, REDStore.account().requireAci(), groupId, uuidCipherTexts);
      return true;
