...
package com.red.sovereign.groups.v2.processing
...
import com.red.sovereign.database.model.GroupsV2UpdateMessageConverter
...
import com.red.sovereign.groups.GroupMutation
...
import com.red.sovereign.groups.v2.processing.GroupsV2StateProcessor.Companion.LATEST
import com.red.sovereign.jobs.AvatarGroupsV2DownloadJob
import com.red.sovereign.jobs.ConversationShortcutUpdateJob
import com.red.sovereign.jobs.DirectoryRefreshJob
import com.red.sovereign.jobs.LeaveGroupV2Job
...
class GroupsV2StateProcessor private constructor(
  ...
  fun updateGroupSendEndorsements() {
    ...
    if (receivedGroupSendEndorsements != null) {
      ...
      REDDatabase.groups.updateGroupSendEndorsements(groupId, receivedGroupSendEndorsements)
    } else {
  ...
  fun updateLocalGroupToRevision(
    ...
    groupRecord: Optional<GroupRecord> = REDDatabase.groups.getGroup(groupId),
  ...
  private fun canApplyP2pChange(
    ...
    if (REDStore.internal.gv2IgnoreP2PChanges) {
      ...
      if (groupRecord.map { it.isMember }.orNull() == false && updatedGroupState != null && updatedGroupState == remoteGroupStateDiff.previousGroupState) {
        ...
        if (currentLocalState != null && applyGroupStateDiffResult.remainingRemoteGroupChanges.isEmpty()) {
          ...
          if (endorsements != null) {
            ...
            REDDatabase.groups.updateGroupSendEndorsements(groupId, endorsements)
          }
        }
  ...
  private fun localIsAtLeast(groupRecord: Optional<GroupRecord>, revision: Int): Boolean {
    if (revision == LATEST || groupRecord.isEmpty || REDDatabase.groups.isUnknownGroup(groupRecord)) {
      return false
    }
  ...
  private fun saveGroupUpdate(
    ...
    if (forceSave && updatedGroupState != null) {
      ...
      if (groupSendEndorsements != null) {
        ...
        REDDatabase.groups.updateGroupSendEndorsements(groupId, groupSendEndorsements)
      }
  ...
  private fun saveGroupState(
    ...
    val needsAvatarFetch = if (previousGroupState == null) {
      ...
      val groupId = REDDatabase.groups.create(groupMasterKey, updatedGroupState, groupSendEndorsements, verifiedNameHash)

      if (groupId == null) {
        Log.w(TAG, "$logPrefix Group create failed, trying to update")
        REDDatabase.groups.update(groupMasterKey, updatedGroupState, groupSendEndorsements, terminatorRecipientId, selfAuthoredTitle)
      }
  ...
  internal class ProfileAndMessageHelper(private val aci: ACI, private val masterKey: GroupMasterKey, private val groupId: GroupId.V2) {
    ...
    fun setProfileSharing(groupStateDiff: GroupStateDiff, newLocalState: DecryptedGroup, needsAvatarFetch: Boolean) {
      ...
      if (REDDatabase.recipients.isProfileSharing(groupId)) {
    ...
    fun leaveGroupLocally(serviceIds: ServiceIds) {
      if (!REDDatabase.groups.isActive(groupId)) {
        Log.w(TAG, "Group $groupId has already been left.")

        val groupRecipient = Recipient.externalGroupExact(groupId)
        val threadId = REDDatabase.threads.getThreadIdFor(groupRecipient.id)

        if (threadId != null) {
          REDDatabase.drafts.clearDrafts(threadId)
      ...
      val decryptedGroup = REDDatabase
        .groups
      ...
      try {
        ...
        val id = REDDatabase.messages.insertMessageOutbox(leaveMessage, threadId, false, null).messageId
      ...
      REDDatabase.groups.setMember(groupId, false)
      REDDatabase.groups.remove(groupId, Recipient.self().id)
    }

    fun markTerminatedLocally() {
      val group = REDDatabase.groups.getGroup(groupId).orNull()
      ...
      try {
        val threadId = REDDatabase.threads.getOrCreateThreadIdFor(groupRecipient)
        val id = REDDatabase.messages.insertMessageOutbox(terminateMessage, threadId, false, null).messageId
        REDDatabase.messages.markAsSent(id)
        REDDatabase.threads.update(threadId, unarchive = false, allowDeletion = false)
      } catch (e: MmsException) {
      ...
      REDDatabase.groups.update(masterKey, simulatedGroupState, null)
      ConversationShortcutUpdateJob.enqueue()
    }
    ...
    fun markJoinRequestRejectedLocally() {
      ...
      try {
        val threadId = REDDatabase.threads.getOrCreateThreadIdFor(groupRecipient)
        val id = REDDatabase.messages.insertMessageOutbox(rejectedMessage, threadId, false, null).messageId
        REDDatabase.messages.markAsSent(id)
        REDDatabase.threads.update(threadId, unarchive = false, allowDeletion = false)
      } catch (e: MmsException) {
    ...
    fun stopAllTypingForGroup() {
      if (TextSecurePreferences.isTypingIndicatorsEnabled(AppDependencies.application)) {
        val threadId = REDDatabase.threads.getThreadIdFor(REDDatabase.recipients.getOrInsertFromGroupId(groupId))
        if (threadId != null) {
    ...
    fun persistLearnedProfileKeys(profileKeys: ProfileKeySet) {
      val updated = REDDatabase.recipients.persistProfileKeySet(profileKeys)
    ...
    fun storeMessage(decryptedGroupV2Context: DecryptedGroupV2Context, timestamp: Long, serverGuid: String?, receivedTime: Long? = null) {
      ...
      val serviceIds = REDStore.account.getServiceIds()
      ...
      if (outgoing) {
        ...
        try {
          val recipientId = REDDatabase.recipients.getOrInsertFromGroupId(groupId)
          ...
          val messageId = REDDatabase.messages.insertMessageOutbox(outgoingMessage, threadId, false, null).messageId
        ...
        try {
          ...
          val insertResult = REDDatabase.messages.insertMessageInbox(groupMessage)

          if (insertResult.isPresent) {
            REDDatabase.threads.update(insertResult.get().threadId, unarchive = false, allowDeletion = false)
    ...
    fun scheduleStorageServiceSync() {
      ...
      REDDatabase.recipients.rotateStorageId(groupRecipientId)
