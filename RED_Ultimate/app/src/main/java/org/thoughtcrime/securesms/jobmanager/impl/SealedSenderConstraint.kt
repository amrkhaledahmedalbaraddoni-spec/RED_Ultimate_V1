...
object SealedSenderConstraint : Constraint {
  ...
  fun checkAndSetValidity() {
    try {
      val requiredTypes = REDStore.phoneNumberPrivacy.getRequiredCertificateTypes()

      for (certificateType in requiredTypes) {
        val certificateBytes = REDStore.certificate.getUnidentifiedAccessCertificate(certificateType)
