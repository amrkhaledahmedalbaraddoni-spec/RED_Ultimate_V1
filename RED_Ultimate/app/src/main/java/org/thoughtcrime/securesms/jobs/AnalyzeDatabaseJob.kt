...
class AnalyzeDatabaseJob private constructor(
  ...
  override fun run(): Result {
    val tables = REDDatabase.rawDatabase.getAllTables()
      .sorted()
    ...
    logTime(TAG, "analyze-$table", decimalPlaces = 2) {
      REDDatabase.rawDatabase.rawQuery("PRAGMA analysis_limit=1000").close()
      REDDatabase.rawDatabase.rawQuery("ANALYZE $table").close()
    }
