...
import com.red.sovereign.apkupdate.ApkUpdateDownloadManagerReceiver
...
class ApkUpdateJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  public override fun onRun() {
    ...
    if (shouldUpdate(getCurrentAppVersionCode(), updateDescriptor, REDStore.apkUpdate.lastApkUploadTime, Environment.IS_WEBSITE)) {
      Log.i(TAG, "Newer version code available. Current: (versionCode: ${getCurrentAppVersionCode()}, uploadTime: ${REDStore.apkUpdate.lastApkUploadTime}), Update: (versionCode: ${updateDescriptor.versionCode}, uploadTime: ${updateDescriptor.uploadTimestamp})")
      ...
      Log.d(TAG, "Version code is the same or older than our own. Current: (versionCode: ${getCurrentAppVersionCode()}, uploadTime: ${REDStore.apkUpdate.lastApkUploadTime}), Update: (versionCode: ${updateDescriptor.versionCode}, uploadTime: ${updateDescriptor.uploadTimestamp})")
    }

    REDStore.apkUpdate.lastSuccessfulCheck = System.currentTimeMillis()
  }
  ...
  private fun getDownloadStatus(uri: String, remoteDigest: ByteArray): DownloadStatus {
    val pendingDownloadId: Long = REDStore.apkUpdate.downloadId
    val pendingDigest: ByteArray? = REDStore.apkUpdate.digest

    if (pendingDownloadId == -1L || pendingDigest == null || !MessageDigest.isEqual(pendingDigest, remoteDigest)) {
      REDStore.apkUpdate.clearDownloadAttributes()
  ...
  private fun handleDownloadStart(uri: String?, versionName: String?, digest: ByteArray, uploadTimestamp: Long) {
    ...
    val downloadRequest = DownloadManager.Request(Uri.parse(uri)).apply {
      ...
      setTitle("Downloading RED update")
      setDescription("Downloading RED $versionName")
    ...
    REDStore.apkUpdate.setDownloadAttributes(downloadId, digest, uploadTimestamp)
  }
