...
import com.red.sovereign.jobs.protos.ArchiveAttachmentReconciliationJobData
...
class ArchiveAttachmentReconciliationJob private constructor(
  ...
  companion object {
    ...
    fun enqueueIfRetryAllowed(forced: Boolean) {
      if (REDStore.backup.archiveAttachmentReconciliationAttempts < 3) {
        REDStore.backup.archiveAttachmentReconciliationAttempts++
  ...
  override fun run(): Result {
    ...
    if (!forced && REDStore.backup.lastAttachmentReconciliationTime < 0) {
    ...
    val timeSinceLastSync = System.currentTimeMillis() - REDStore.backup.lastAttachmentReconciliationTime
    ...
    snapshotVersion = snapshotVersion ?: REDDatabase.backupMediaSnapshots.getCurrentSnapshotVersion()
  ...
  private fun syncDataFromCdn(snapshotVersion: Long): Result? {
    ...
    Log.d(TAG, "BEFORE:\n" + REDDatabase.attachments.debugGetAttachmentStats().shortPrettyString(), true)
    stopwatch.split("stats-before")

    val mediaObjectsThatMayNeedReUpload = REDDatabase.backupMediaSnapshots.getMediaObjectsLastSeenOnCdnBeforeSnapshotVersion(snapshotVersion)
    ...
    if (mayNeedReUploadCount > 0) {
      ...
      mediaObjectsThatMayNeedReUpload.forEach { mediaObjectCursor ->
        ...
        if (entry.isThumbnail) {
          ...
          val wasReset = REDDatabase.attachments.resetArchiveThumbnailTransferStateByPlaintextHashAndRemoteKeyIfNecessary(entry.plaintextHash, entry.remoteKey)
          if (wasReset) {
          ...
          val wasReset = REDDatabase.attachments.resetArchiveTransferStateByPlaintextHashAndRemoteKeyIfNecessary(entry.plaintextHash, entry.remoteKey)
          if (wasReset) {
      ...
      if (internalUser && mediaIdsThatNeedUpload.isNotEmpty()) {
        ...
        val matchingAttachments = REDDatabase.attachments.getAttachmentDataForMediaIds(mediaIdsThatNeedUpload)
        ...
        matchingAttachments.take(250).forEach { match ->
          if (match.isThumbnail) {
            val thumbnailTransferState = REDDatabase.attachments.getArchiveThumbnailTransferState(match.attachment.attachmentId)
    ...
    REDStore.backup.remoteStorageGarbageCollectionPending = false
    REDStore.backup.lastAttachmentReconciliationTime = System.currentTimeMillis()
  ...
  private fun syncCdnPage(archivedItemPage: ArchiveGetMediaItemsResponse, currentSnapshotVersion: Long): Set<ArchivedMediaObject> {
    ...
    REDDatabase.backupMediaSnapshots.markSeenOnRemote(
      mediaIdBatch = mediaObjects.map { it.mediaId },
      snapshotVersion = currentSnapshotVersion
    )

    val mediaOnRemoteButNotLocal = REDDatabase.backupMediaSnapshots.getMediaObjectsThatCantBeFound(mediaObjects)
    val mediaObjectsOnBothRemoteAndLocal = mediaObjects - mediaOnRemoteButNotLocal

    val cdnMismatches = REDDatabase.backupMediaSnapshots.getMediaObjectsWithNonMatchingCdn(mediaObjectsOnBothRemoteAndLocal)
    if (cdnMismatches.isNotEmpty()) {
      ...
      for (mismatch in cdnMismatches) {
        REDDatabase.attachments.setArchiveCdnByPlaintextHashAndRemoteKey(mismatch.plaintextHash, mismatch.remoteKey, mismatch.cdn)
      }
    }

    if (REDStore.backup.localRestoreReconcilePending) {
      val markedFinished = REDDatabase.attachments.setArchiveFinishedForMatchingMediaObjects(mediaObjectsOnBothRemoteAndLocal.toSet())
      if (markedFinished > 0) {
  ...
  /**
   * Deletes attachments from the archive CDN, after verifying that they also can't be found anywhere in [com.red.sovereign.database.AttachmentTable]
  ...
  private fun validateAndDeleteFromRemote(deletes: Set<ArchivedMediaObject>): Result? {
    ...
    val validatedDeletes: MutableSet<ArchivedMediaObject> = REDDatabase.attachments.getMediaObjectsThatCantBeFound(deletes).toMutableSet()
    ...
    if (foundLocally.isNotEmpty()) {
      ...
      val matches = REDDatabase.attachments.getAttachmentDataForMediaIds(foundLocally.map { MediaId(it.mediaId) })
      for (match in matches) {
        if (match.messageRecord?.fromRecipient != null && match.messageRecord.fromRecipient.id == REDStore.releaseChannel.releaseChannelRecipientId) {
          ...
          val stringMediaId = match.mediaId.encode()
          validatedDeletes += foundLocally.first { it.mediaId == stringMediaId }
          REDDatabase.attachments.deleteAttachment(match.attachment.attachmentId)
    ...
    if (updatedFoundLocally.isNotEmpty()) {
      val fixedCount = REDDatabase.attachments.setArchiveFinishedForMatchingMediaObjects(updatedFoundLocally)
      if (fixedCount > 0) {
