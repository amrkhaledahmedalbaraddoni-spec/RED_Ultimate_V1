...
class ArchiveCommitAttachmentDeletesJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  companion object {
     ...
     */
    fun deleteMediaObjectsFromCdn(tag: String, attachmentsToDelete: Set<ArchivedMediaObject>, backoffGenerator: () -> Long, cancellationRED: () -> Boolean): Result? {
  ...
  override fun run(): Result {
    ...
    var mediaObjects = REDDatabase.backupMediaSnapshots.getPageOfOldMediaObjects(REMOTE_DELETE_BATCH_SIZE)
    ...
    while (mediaObjects.isNotEmpty()) {
      ...
      REDDatabase.backupMediaSnapshots.deleteOldMediaObjects(mediaObjects)

      mediaObjects = REDDatabase.backupMediaSnapshots.getPageOfOldMediaObjects(REMOTE_DELETE_BATCH_SIZE)
    }
