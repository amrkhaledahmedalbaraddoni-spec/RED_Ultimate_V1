...
import com.red.sovereign.jobmanager.Job.Parameters
...
import com.red.sovereign.s3.S3
import com.red.sovereign.transport.RetryLaterException
import com.red.sovereign.util.AttachmentUtil
...
class AttachmentDownloadJob private constructor(
    ...
    fun downloadAttachmentIfNeeded(databaseAttachment: DatabaseAttachment): String? {
      return when (val transferState = databaseAttachment.transferState) {
        ...
        AttachmentTable.TRANSFER_PROGRESS_FAILED -> {
          if (REDStore.backup.backsUpMedia && (databaseAttachment.remoteLocation == null || databaseAttachment.remoteDigest == null)) {
  ...
  override fun onAdded() {
    ...
    if (pending && (forceDownload || AttachmentUtil.isAutoDownloadPermitted(context, attachment))) {
      ...
      REDDatabase.attachments.setTransferState(messageId, attachmentId, AttachmentTable.TRANSFER_PROGRESS_STARTED)
    }
  }
  ...
  public override fun onRun() {
    ...
    if (!REDDatabase.messages.isStory(messageId)) {
  ...
  fun doWork() {
    ...
    if (!forceDownload && !AttachmentUtil.isAutoDownloadPermitted(context, attachment)) {
      ...
      REDDatabase.attachments.setTransferState(messageId, attachmentId, AttachmentTable.TRANSFER_PROGRESS_PENDING)
      return
    }

    if (REDStore.backup.backsUpMedia && attachment.remoteLocation == null) {
    ...
    if (REDStore.backup.backsUpMedia) {
      val isStory = REDDatabase.messages.isStory(messageId)
      when {
  ...
  private fun retrieveAttachment(
    ...
    val attachmentFile: File = REDDatabase.attachments.getOrCreateTransferFile(attachmentId)
    ...
    try {
      ...
      decryptingStream.use { input ->
        REDDatabase.attachments.finalizeAttachmentAfterDownload(messageId, attachmentId, input)
      }
      ...
      if (REDStore.backup.backsUpMedia && e.code == 404 && attachment.archiveTransferState === AttachmentTable.ArchiveTransferState.FINISHED) {
  ...
  private fun createAttachmentPointer(attachment: DatabaseAttachment): REDServiceAttachmentPointer {
    ...
    return try {
      ...
      val remoteId = REDServiceAttachmentRemoteId.from(attachment.remoteLocation, cdnNumber)
  ...
  private fun isReleaseChannelMessage(messageId: Long): Boolean {
    val releaseChannelRecipientId = REDStore.releaseChannel.releaseChannelRecipientId ?: return false
    val messageRecord = REDDatabase.messages.getMessageRecordOrNull(messageId) ?: return false
  ...
  private fun maybeRequestBackfill() {
    if (REDStore.account.isPrimaryDevice || requestSource != RequestSource.USER) {
      return
    }

    val attachment = REDDatabase.attachments.getAttachment(attachmentId) ?: return

    if (REDStore.backup.backsUpMedia && attachment.dataHash != null && attachment.transferState == AttachmentTable.TRANSFER_PROGRESS_FAILED) {
