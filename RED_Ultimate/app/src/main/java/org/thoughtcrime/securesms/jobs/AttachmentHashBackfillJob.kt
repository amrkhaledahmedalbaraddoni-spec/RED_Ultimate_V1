...
class AttachmentHashBackfillJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  override fun run(): Result {
    val (file: File?, attachmentId: AttachmentId?) = REDDatabase.attachments.getUnhashedDataFile() ?: (null to null)
    ...
    if (!file.exists()) {
      ...
      REDDatabase.attachments.clearUsagesOfDataFile(file)
      AppDependencies.jobManager.add(AttachmentHashBackfillJob())
      return Result.success()
    }

    try {
      val inputStream = REDDatabase.attachments.getAttachmentStream(attachmentId, 0)
      ...
      REDDatabase.attachments.setHashForDataFile(file, hash)
