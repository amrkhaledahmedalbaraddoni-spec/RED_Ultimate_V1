...
public class AutomaticSessionResetJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    ...
    if (RemoteConfig.automaticSessionReset()) {
      ...
      DeviceLastResetTime resetTimes         = REDDatabase.recipients().getLastSessionResetTimes(recipientId);
      ...
      if (timeSinceLastReset > resetInterval) {
        ...
        REDDatabase.recipients().setLastSessionResetTime(recipientId, setLastResetTime(resetTimes, deviceId, System.currentTimeMillis()));
  ...
  private void insertLocalMessage() {
    MessageTable.InsertResult result = REDDatabase.messages().insertChatSessionRefreshedMessage(recipientId, deviceId, sentTimestamp - 1);
