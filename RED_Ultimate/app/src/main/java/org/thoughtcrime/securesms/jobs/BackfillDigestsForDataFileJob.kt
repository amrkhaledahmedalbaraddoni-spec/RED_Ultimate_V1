...
import com.red.sovereign.jobs.protos.BackfillDigestsForDataFileJobData
...
class BackfillDigestsForDataFileJob private constructor(
  ...
  override fun run(): Result {
    val (originalKey, decryptingStream) = REDDatabase.rawDatabase.withinTransaction {
      val attachment = REDDatabase.attachments.getMostRecentValidAttachmentUsingDataFile(dataFile)
      if (attachment == null) {
      ...
      val stream = try {
        REDDatabase.attachments.getAttachmentStream(attachment.attachmentId, offset = 0)
      } catch (e: IOException) {
    ...
    REDDatabase.attachments.updateRemoteKeyAndDigestByDataFile(
      dataFile = dataFile,
      key = key,
      digest = digest
    )
