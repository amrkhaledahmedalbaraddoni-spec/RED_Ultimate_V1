...
import com.red.sovereign.jobmanager.impl.DeletionNotAwaitingMediaDownloadConstraint
...
class BackupDeleteJob private constructor(
  ...
  override fun run(): Result {
    ...
    if (REDStore.backup.deletionState.isIdle()) {
      Log.w(TAG, "Invalid state ${REDStore.backup.deletionState}. Exiting without local cleanup.")
      return Result.failure()
    }
  ...
  private fun doRun(): Result {
    ...
    val clearLocalStateResult = if (REDStore.backup.deletionState == DeletionState.CLEAR_LOCAL_STATE) {
      val results = listOf(
        deleteLocalState(),
    ...
    REDStore.backup.deletionState = DeletionState.DELETE_BACKUPS
    ...
    if (result.isSuccess) {
      ...
      BackupRepository.resetInitializedStateAndAuthCredentials()
      REDStore.backup.deletionState = DeletionState.COMPLETE
    }
  ...
  override fun onFailure() {
    ...
    } else if (REDStore.backup.deletionState == DeletionState.AWAITING_MEDIA_DOWNLOAD) {
  ...
  private fun isMediaRestoreRequired(): Boolean {
    ...
    val requiresMediaRestore = REDDatabase.attachments.getRemainingRestorableAttachmentSize() > 0L
    val hasOffloadedMedia = REDDatabase.attachments.getOptimizedMediaAttachmentSize() > 0L

    if ((requiresMediaRestore || hasOffloadedMedia) && !REDStore.backup.userManuallySkippedMediaRestore) {
  ...
  private fun clearLocalBackupState() {
    REDStore.backup.disableBackups()
