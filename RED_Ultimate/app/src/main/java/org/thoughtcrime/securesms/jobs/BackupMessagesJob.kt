...
import com.red.sovereign.backup.v2.ArchiveValidator
import com.red.sovereign.backup.v2.BackupRepository
import com.red.sovereign.backup.v2.ResumableMessagesBackupUploadSpec
import com.red.sovereign.backup.v2.util.ArchiveAttachmentInfo
import com.red.sovereign.backup.v2.util.getAllReferencedArchiveAttachmentInfos
import com.red.sovereign.database.BackupMediaSnapshotTable
import com.red.sovereign.database.REDDatabase
import com.red.sovereign.dependencies.AppDependencies
import com.red.sovereign.jobmanager.Job
import com.red.sovereign.jobmanager.impl.BackupMessagesConstraint
import com.red.sovereign.jobs.protos.BackupMessagesJobData
...
class BackupMessagesJob private constructor(
  ...
  override fun run(): Result {
    val result = doWork()
    if (result.isSuccess && !isCanceled && REDStore.backup.optimizeStorage && REDStore.backup.backsUpMedia) {
  ...
  private fun doWork(): Result {
    ...
    if (REDStore.backup.backupSecretRestoreRequired) {
      ...
      if (forwardSecrecyMetadata != null) {
        when (val result = REDNetwork.svrB.restore(auth, REDStore.backup.messageBackupKey, forwardSecrecyMetadata)) {
          ...
          SvrBApi.RestoreResult.InvalidDataError -> {
            ...
            REDStore.backup.nextBackupSecretData = null
          }
    ...
    val backupSecretData = REDStore.backup.nextBackupSecretData ?: run {
      Log.i(TAG, "First SVRB backup! Creating new backup chain.", true)
      val secretData = REDNetwork.svrB.createNewBackupChain(auth, REDStore.backup.messageBackupKey)
      REDStore.backup.nextBackupSecretData = secretData
      secretData
    }

    val svrBMetadata: SvrBStoreResponse = when (val result = REDNetwork.svrB.store(auth, REDStore.backup.messageBackupKey, backupSecretData)) {
    ...
    val createKeyResult = REDDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    ...
    REDDatabase.attachments.clearIncrementalMacsForAttachmentsThatNeedArchiveUpload().takeIf { it > 0 }?.let { count -> Log.w(TAG, "Needed to clear $count incrementalMacs.", true) }
    ...
    REDStore.backup.lastBackupProtoVersion = BackupRepository.VERSION
    ...
    val form: AttachmentUploadForm = if (existingSpec == null) {
      when (val result = BackupRepository.getMessageBackupUploadForm(tempBackupFile.length())) {
        ...
        is NetworkResult.StatusCodeError -> {
          when (result.code) {
            413 -> {
              Log.i(TAG, "Backup file is too large! Size: ${tempBackupFile.length()} bytes. Current threshold: ${REDStore.backup.messageCuttoffDuration}", result.getCause(), true)
              ...
              if (REDStore.backup.messageCuttoffDuration == null) {
                Log.i(TAG, "Setting message cuttoff duration to $TOO_LARGE_MESSAGE_CUTTOFF_DURATION", true)
                REDStore.backup.messageCuttoffDuration = TOO_LARGE_MESSAGE_CUTTOFF_DURATION
                return Result.retry(defaultBackoff())
              } else {
                return Result.failure()
              }
    ...
    val progressListener = object : REDServiceAttachment.ProgressListener {
    ...
    val uploadResult = FileInputStream(tempBackupFile).use { fileStream ->
      REDNetwork.archive.uploadBackupFile(
        uploadForm = form,
        data = fileStream,
    ...
    REDStore.backup.nextBackupSecretData = svrBMetadata.nextBackupSecretData

    REDStore.backup.lastBackupProtoSize = tempBackupFile.length()
    if (!tempBackupFile.delete()) {
      Log.e(TAG, "Failed to delete temp backup file", true)
    }

    REDStore.backup.lastBackupTime = System.currentTimeMillis()
    ...
    } else if (REDStore.backup.backsUpMedia && REDDatabase.attachments.doAnyAttachmentsNeedArchiveUpload()) {
      Log.i(TAG, "Enqueuing attachment backfill job.", true)
      AppDependencies.jobManager.add(ArchiveAttachmentBackfillJob())
    } else {
      Log.i(TAG, "No attachments need to be uploaded, we can finish. Tier: ${REDStore.backup.backupTier}", true)
      ArchiveUploadProgress.onMessageBackupFinishedEarly()
    }

    if (REDStore.backup.backsUpMedia && REDDatabase.attachments.doAnyThumbnailsNeedArchiveUpload()) {
      Log.i(TAG, "Enqueuing thumbnail backfill job.", true)
      AppDependencies.jobManager.add(ArchiveThumbnailBackfillJob())
    } else {
      Log.i(TAG, "No thumbnails need to be uploaded: ${REDStore.backup.backupTier}", true)
    }

    REDStore.backup.messageCuttoffDuration = null
    REDStore.backup.lastUsedMessageCutoffTime = messageCutoffTime
  ...
  private fun getOrCreateBackupFile(
    ...
    if (System.currentTimeMillis() > syncTime && syncTime > 0L && dataFile.isNotNullOrBlank()) {
      ...
      if (file.exists() && file.canRead() && elapsed < FILE_REUSE_TIMEOUT) {
        ...
        return BackupFileResult.Success(file, syncTime, messageInclusionCutoffTime = REDStore.backup.lastUsedMessageCutoffTime)
      }
    }
    ...
    val backupKey = REDStore.backup.messageBackupKey
    ...
    val messageInclusionCutoffTime = REDStore.backup.messageCuttoffDuration?.let { currentTime - it.inWholeMilliseconds } ?: 0

    try {
      BackupRepository.exportForREDBackup(
        ...
        cancellationRED = { this.isCanceled },
        ...
        if (attachmentInfoBuffer.size > ATTACHMENT_SNAPSHOT_BUFFER_SIZE) {
          REDDatabase.backupMediaSnapshots.writePendingMediaEntries(attachmentInfoBuffer.toFullSizeMediaEntries(mediaRootBackupKey))
          REDDatabase.backupMediaSnapshots.writePendingMediaEntries(attachmentInfoBuffer.toThumbnailMediaEntries(mediaRootBackupKey))
    ...
    when (val result = ArchiveValidator.validateREDBackup(tempBackupFile, backupKey, forwardSecrecyToken)) {
  ...
  private fun maybePostRemoteKeyMissingNotification() {
    if (!RemoteConfig.internalUser || !REDStore.backup.backsUpMedia) {
      return
    }
  ...
  private fun Set<ArchiveAttachmentInfo>.toThumbnailMediaEntries(mediaRootBackupKey: MediaRootBackupKey): Set<BackupMediaSnapshotTable.MediaEntry> {
    return this
      ...
      .toSet()
      .let { REDDatabase.attachments.filterThumbnailsWithoutEligibleAttachment(it) }
  }
