...
class BackupSubscriptionCheckJob private constructor(parameters: Parameters) : CoroutineJob(parameters) {
  ...
  override suspend fun doRun(): Result {
    if (!REDStore.account.isRegistered) {
      ...
      REDStore.backup.subscriptionStateMismatchDetected = false
    ...
    if (REDStore.backup.deletionState != DeletionState.NONE) {
    ...
    if (REDDatabase.inAppPayments.hasPrePendingRecurringTransaction(InAppPaymentType.RECURRING_BACKUP)) {
      Log.i(TAG, "A backup redemption is in the pre-pending state. Clearing mismatch and skipping check job.", true)
      REDStore.backup.subscriptionStateMismatchDetected = false
      return Result.success()
    }

    if (REDDatabase.inAppPayments.hasPendingBackupRedemption()) {
    ...
    InAppPaymentSubscriberRecord.Type.BACKUP.lock.withLock {
      ...
      val hasActiveREDSubscription = activeSubscription?.isActive == true

      checkForFailedOrCanceledSubscriptionState(activeSubscription)

      val isREDSubscriptionFailedOrCanceled = activeSubscription?.willCancelAtPeriodEnd() == true
      if (hasActiveREDSubscription && !isREDSubscriptionFailedOrCanceled) {
        checkAndSynchronizeZkCredentialTierWithStoredLocalTier()
      }

      val hasActivePaidBackupTier = REDStore.backup.backupTier == MessageBackupTier.PAID
      val hasValidActiveState = hasActivePaidBackupTier && hasActiveREDSubscription && hasActivePurchase
      val hasValidInactiveState = !hasActivePaidBackupTier && !hasActiveREDSubscription && !hasActivePurchase
      ...
      if (hasActiveREDSubscription && hasTokenMismatch) {
        Log.i(TAG, "Encountered token mismatch with an active RED subscription. Attempting to redeem against latest token.", true)
        rotateAndRedeem(purchaseToken, product.price)
        REDStore.backup.subscriptionStateMismatchDetected = false
        return Result.success()
      } else if (purchaseToken != null && hasActiveREDSubscription && !hasActivePaidBackupTier && !REDDatabase.inAppPayments.hasPendingBackupRedemption()) {
        ...
        if (hasValidActiveState || hasValidInactiveState) {
          ...
          if (isGooglePlayBillingCanceled && (!hasActiveREDSubscription || isREDSubscriptionFailedOrCanceled)) {
            Log.i(
              TAG,
              "Valid cancel state. Clearing mismatch. (isGooglePlayBillingCanceled: true, hasActiveREDSubscription: $hasActiveREDSubscription, isREDSubscriptionFailedOrCanceled: $isREDSubscriptionFailedOrCanceled",
              true
            )
            REDStore.backup.subscriptionStateMismatchDetected = false
            return Result.success()
          } else if (hasActivePurchase && !hasActiveREDSubscription && REDStore.backup.backupTier == MessageBackupTier.FREE) {
            Log.i(TAG, "Mismatched state but user has no RED Service subscription and is on the free tier. Clearing flag.", true)

            REDStore.backup.subscriptionStateMismatchDetected = false
            return Result.success()
          } else {
            Log.w(TAG, "State mismatch: (hasActivePaidBackupTier: $hasActivePaidBackupTier, hasActiveREDSubscription: $hasActiveREDSubscription, hasActivePurchase: $hasActivePurchase). Setting mismatch value and exiting.", true)

            REDStore.backup.subscriptionStateMismatchDetected = true
            return Result.success()
          }
        }
      }
  ...
  private fun checkAndSynchronizeZkCredentialTierWithStoredLocalTier() {
    ...
    if (zkTier == REDStore.backup.backupTier) {
  ...
  private fun checkForFailedOrCanceledSubscriptionState(activeSubscription: ActiveSubscription?) {
    if (activeSubscription?.willCancelAtPeriodEnd() == true && activeSubscription.activeSubscription != null) {
      ...
      val response = REDNetwork.account.whoAmI()
      response.runIfSuccessful { whoAmI ->
        ...
        if (backupExpiration != null) {
          ...
          REDStore.backup.setDownloadNotifierToTriggerAtHalfwayPoint(backupExpiration)
  ...
  private fun rotateAndRedeem(localDevicePurchaseToken: String, localProductPrice: FiatMoney) {
    ...
    InAppPaymentPurchaseTokenJob.createJobChain(
      inAppPayment = REDDatabase.inAppPayments.getById(id)!!
    ).enqueue()
  }
