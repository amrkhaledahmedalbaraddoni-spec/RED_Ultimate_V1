...
import com.red.sovereign.jobs.protos.CopyAttachmentToArchiveJobData
...
class CopyAttachmentToArchiveJob private constructor(private val attachmentId: AttachmentId, parameters: Parameters) : Job(parameters) {
  ...
  override fun onAdded() {
    val transferStatus = REDDatabase.attachments.getArchiveTransferState(attachmentId) ?: return
  ...
  override fun run(): Result {
    ...
    val attachment: DatabaseAttachment? = REDDatabase.attachments.getAttachment(attachmentId)
    ...
    val mediaIdLog: String = if (RemoteConfig.internalUser && attachment.remoteKey != null && attachment.dataHash != null) {
      val mediaId = MediaName.fromPlaintextHashAndRemoteKey(attachment.dataHash.decodeBase64()!!, attachment.remoteKey.decodeBase64()!!).toMediaId(REDStore.backup.mediaRootBackupKey)
      "[$mediaId]"
    } else {
      ""
    }
    ...
    if (REDDatabase.messages.isViewOnce(attachment.mmsId)) {
      Log.i(TAG, "[$attachmentId]$mediaIdLog Attachment is view-once. Resetting transfer state to none and skipping.")
      setArchiveTransferStateWithDelayedNotification(attachmentId, AttachmentTable.ArchiveTransferState.NONE)
      return Result.success()
    }

    if (REDDatabase.messages.willMessageExpireBeforeCutoff(attachment.mmsId)) {
      ...
      is NetworkResult.StatusCodeError -> {
        when (archiveResult.code) {
          ...
          413 -> {
            ...
            if (REDDatabase.attachments.getPaidEstimatedArchiveMediaSize() > remoteStorageQuota.inWholeBytes) {
            ...
            REDStore.backup.remoteStorageGarbageCollectionPending = true
    ...
    if (result.isSuccess) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        REDDatabase.attachments.setArchiveTransferState(attachmentId, attachment.remoteKey!!, attachment.dataHash!!, AttachmentTable.ArchiveTransferState.FINISHED, notify = false)
      ...
      REDStore.backup.archiveAttachmentReconciliationAttempts = 0
    }
  ...
  override fun onFailure() {
    if (this.isCanceled) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        REDDatabase.attachments.setArchiveTransferState(attachmentId, AttachmentTable.ArchiveTransferState.COPY_PENDING)
      }
    } else {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        REDDatabase.attachments.setArchiveTransferStateFailure(attachmentId, AttachmentTable.ArchiveTransferState.TEMPORARY_FAILURE)
      }
    }
  ...
  private fun setArchiveTransferStateWithDelayedNotification(attachmentId: AttachmentId, transferState: AttachmentTable.ArchiveTransferState) {
    ArchiveDatabaseExecutor.runBlocking {
      REDDatabase.attachments.setArchiveTransferState(attachmentId, transferState, notify = false)
      ArchiveDatabaseExecutor.throttledNotifyAttachmentObservers()
    }
  }
