...
import com.red.sovereign.database.model.EmojiSearchData;
...
public final class EmojiSearchIndexDownloadJob extends BaseJob {
  ...
  public static void scheduleIfNecessary() {
    long    timeSinceCheck = System.currentTimeMillis() - REDStore.emoji().getLastSearchIndexCheck();
    boolean needsCheck     = false;

    if (REDStore.emoji().hasSearchIndex() && !REDDatabase.emojiSearch().hasSearchIndexData()) {
      Log.w(TAG, "Emoji search data missing with metadata, clearing version to redownload.");
      REDStore.emoji().clearSearchIndexVersion();
      scheduleImmediately();
      return;
    }

    if (REDStore.emoji().hasSearchIndex()) {
  ...
  protected void onRun() throws Exception {
    ...
    if (manifest.getVersion() == REDStore.emoji().getSearchVersion() &&
        remoteLanguage.equals(REDStore.emoji().getSearchLanguage()))
    {
      Log.i(TAG, "Already using the latest version of " + manifest.getVersion() + " with the correct language " + remoteLanguage);
      REDStore.emoji().setLastSearchIndexCheck(System.currentTimeMillis());
      return;
    }
    ...
    REDDatabase.emojiSearch().setSearchIndex(localizedSearchIndex, englishSearchIndex);
    REDStore.emoji().onSearchIndexUpdated(manifest.getVersion(), remoteLanguage);
    REDStore.emoji().setLastSearchIndexCheck(System.currentTimeMillis());
