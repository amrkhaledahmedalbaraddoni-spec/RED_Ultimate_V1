...
public final class GroupV2UpdateSelfProfileKeyJob extends BaseJob {
  ...
  public static void enqueueForGroupsIfNecessary() {
    if (!REDStore.account().isRegistered() || REDStore.account().getAci() == null || !Recipient.self().isRegistered()) {
    ...
    long timeSinceLastCheck = System.currentTimeMillis() - REDStore.misc().getLastGv2ProfileCheckTime();
    ...
    REDStore.misc().setLastGv2ProfileCheckTime(System.currentTimeMillis());

    REDExecutors.BOUNDED.execute(() -> {
      boolean foundMismatch = false;

      for (GroupId.V2 id : REDDatabase.groups().getAllGroupV2Ids()) {
        Optional<GroupRecord> group = REDDatabase.groups().getGroup(id);
