...
class InAppPaymentAuthCheckJob private constructor(parameters: Parameters) : BaseJob(parameters), StripeApi.PaymentIntentFetcher, StripeApi.SetupIntentHelper {
  ...
  companion object {
    ...
    @JvmStatic
    fun enqueueIfNeeded() {
      if (REDDatabase.inAppPayments.hasWaitingForAuth()) {
  ...
  override fun onRun() {
    ...
    val unauthorizedInAppPayments = REDDatabase.inAppPayments.getAllWaitingForAuth()
  ...
  private fun migrateLegacyData() {
    val pending3DSData = REDStore.inAppPayments.consumePending3DSData()
  ...
  private fun checkOneTimePayment(inAppPayment: InAppPaymentTable.InAppPayment): CheckResult<Unit> {
    ...
    val updatedPayment = REDDatabase.inAppPayments.getById(inAppPayment.id)
  ...
  private fun checkRecurringPayment(inAppPayment: InAppPaymentTable.InAppPayment): CheckResult<Unit> {
    ...
    Log.d(TAG, "Set default payment method via RED service.", true)

    REDDatabase.inAppPaymentSubscribers.setPaymentMethod(subscriber.subscriberId, inAppPayment.data.paymentMethodType)
    ...
    try {
      ...
      if (updateLevelResponse.status in listOf(200, 204)) {
        ...
        with(REDDatabase.inAppPayments.getById(inAppPayment.id)!!) {
  ...
  private fun markChecked(inAppPayment: InAppPaymentTable.InAppPayment) {
    val fresh = REDDatabase.inAppPayments.getById(inAppPayment.id)
    REDDatabase.inAppPayments.update(
