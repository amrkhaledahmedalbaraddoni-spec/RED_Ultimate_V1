...
class InAppPaymentKeepAliveJob private constructor(
  ...
  companion object {
    ...
    fun enqueueAndTrackTimeIfNecessary() {
      ...
      // SIGNAL_INHERITED: TODO -- This should only be enqueued if we are completely drained of old subscription jobs. (No pending, no runnning)
      val lastKeepAliveTime = REDStore.inAppPayments.getLastKeepAliveLaunchTime().milliseconds
    ...
    fun enqueueAndTrackTime(now: Duration) {
      ...
      REDStore.inAppPayments.setLastKeepAliveLaunchTime(now.inWholeMilliseconds)
    }
  }
  ...
  private fun doRun() {
    ...
    if (REDDatabase.inAppPayments.hasPrePendingRecurringTransaction(type.inAppPaymentType)) {
    ...
    when (activeInAppPayment.data.redemption?.stage) {
      InAppPaymentData.RedemptionState.Stage.INIT -> {
        ...
        REDDatabase.inAppPayments.update(payment)
  ...
  private fun clearOldPendingPaymentsIfRequired() {
    ...
    val inAppPayments = REDDatabase.inAppPayments.getOldPendingPayments(type.inAppPaymentType)
  ...
  private fun getActiveInAppPayment(
    ...
    val current: InAppPaymentTable.InAppPayment? = REDDatabase.inAppPayments.getByEndOfPeriod(type.inAppPaymentType, endOfCurrentPeriod)

    val inAppPayment = if (current == null) {
      val oldInAppPayment = REDDatabase.inAppPayments.getByLatestEndOfPeriod(type.inAppPaymentType)
      ...
      if (type == InAppPaymentSubscriberRecord.Type.DONATION) {
        REDStore.inAppPayments.setLastEndOfPeriod(endOfCurrentPeriod.inWholeSeconds)
      }
      ...
      REDDatabase.inAppPaymentSubscribers.setPaymentMethod(subscriber.subscriberId, paymentMethodType = newInAppPaymentMethodType)
      ...
      val inAppPaymentId = REDDatabase.inAppPayments.insert(
        ...
        inAppPaymentData = InAppPaymentData(
          ...
          badge = badge,
          amount = FiatMoney.fromREDNetworkAmount(subscription.amount, Currency.getInstance(subscription.currency)).toFiatValue(),
          error = null,
      ...
      REDDatabase.inAppPayments.getById(current.id)
    ...
    if (
      ...
      return REDDatabase.inAppPayments.getById(inAppPayment.id)
    } else {
      return inAppPayment
    }
  }
