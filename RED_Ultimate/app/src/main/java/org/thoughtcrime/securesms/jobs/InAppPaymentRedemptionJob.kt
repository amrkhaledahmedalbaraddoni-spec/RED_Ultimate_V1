...
import com.red.sovereign.jobs.protos.InAppPaymentRedemptionJobData
import com.red.sovereign.keyvalue.REDStore
import com.red.sovereign.util.hasGiftBadge
import com.red.sovereign.util.requireGiftBadge
...
class InAppPaymentRedemptionJob private constructor(
  ...
  override fun onAdded() {
    if (jobData.giftMessageId != null) {
      ...
      REDDatabase.messages.markGiftRedemptionStarted(jobData.giftMessageId)
    }
  }
  ...
  override fun onFailure() {
    if (jobData.giftMessageId != null) {
      ...
      REDDatabase.messages.markGiftRedemptionFailed(jobData.giftMessageId)
    }

    if (jobData.inAppPaymentId != null) {
      Log.w(TAG, "A permanent failure occurred.")

      val inAppPayment = REDDatabase.inAppPayments.getById(InAppPaymentTable.InAppPaymentId(jobData.inAppPaymentId))
  ...
  private fun onRunForGiftMessageId(messageId: Long) {
    val giftBadge = REDDatabase.messages.getMessageRecord(messageId)
    ...
    val serviceResponse = AppDependencies
      ...
      .redeemDonationReceipt(
        ...
        REDStore.inAppPayments.getDisplayBadgesOnProfile(),
        jobData.makePrimary
      )
    ...
    REDDatabase.messages.markGiftRedemptionCompleted(messageId)
  }
