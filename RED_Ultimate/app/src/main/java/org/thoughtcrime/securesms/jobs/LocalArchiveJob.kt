...
import com.red.sovereign.backup.FullBackupExporter.BackupCanceledException
...
import com.red.sovereign.service.GenericForegroundService
import com.red.sovereign.service.NotificationController
...
class LocalArchiveJob internal constructor(parameters: Parameters) : Job(parameters) {
  ...
  override fun run(): Result {
    ...
    try {
      ...
      try {
        REDDatabase.attachmentMetadata.insertNewKeysForExistingAttachments()
        ...
        try {
          val result = LocalArchiver.export(snapshotFileSystem, archiveFileSystem.filesFileSystem, stopwatch, cancellationRED = { isCanceled })
      ...
      setProgress(LocalBackupCreationProgress(idle = LocalBackupCreationProgress.Idle()), notification)
      REDStore.backup.newLocalBackupsLastBackupTime = System.currentTimeMillis()
    } finally {
      notification?.close()
    }
