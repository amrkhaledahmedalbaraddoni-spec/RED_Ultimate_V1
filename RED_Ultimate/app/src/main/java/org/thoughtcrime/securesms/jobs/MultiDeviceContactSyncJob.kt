...
class MultiDeviceContactSyncJob(parameters: Parameters, private val attachmentPointer: ByteArray) : BaseJob(parameters) {
  ...
  constructor(contactsAttachment: REDServiceAttachmentPointer) : this(
    Parameters.Builder()
  ...
  override fun onRun() {
    ...
    val contactAttachment: REDServiceAttachmentPointer = AttachmentPointerUtil.createREDAttachmentPointer(attachmentPointer)
  ...
  private fun processContactFile(inputStream: InputStream) {
    ...
    while (contact != null) {
      val recipient: Recipient? = if (contact.aci.isPresent) {
        Recipient.externalPush(REDServiceAddress(contact.aci.get(), contact.e164.orElse(null)))
      } else {
