...
public class MultiDeviceContactUpdateJob extends BaseJob {
      ...
      throws IOException, UntrustedIdentityException, NetworkException, NoSessionException
  {
    ...
    try {
      ...
      Map<RecipientId, Integer> inboxPositions  = REDDatabase.threads().getInboxPositions();
      Set<RecipientId>          archived        = REDDatabase.threads().getArchivedRecipients();
      ...
      sendUpdate(AppDependencies.getREDServiceMessageSender(),
      ...
      throws IOException, UntrustedIdentityException, NetworkException, NoSessionException
  {
    ...
    try {
      DeviceContactsOutputStream out            = new DeviceContactsOutputStream(writeDetails.outputStream);
      List<Recipient>            recipients     = REDDatabase.recipients().getRecipientsForMultiDeviceSync();
      Map<RecipientId, Integer>  inboxPositions = REDDatabase.threads().getInboxPositions();
      Set<RecipientId>           archived       = REDDatabase.threads().getArchivedRecipients();
      ...
      if (profileKey != null) {
        out.write(new DeviceContact(Optional.of(REDStore.account().getAci()),
                                    Optional.of(REDStore.account().getE164()),
                                    Optional.empty(),
                                    Optional.empty(),
  ...
  private void sendUpdate(REDServiceMessageSender messageSender, InputStream stream, long length, boolean complete)
      throws UntrustedIdentityException, NetworkException, NoSessionException
  {
    if (length > 0) {
      try {
        ...
        REDServiceAttachmentStream.Builder attachmentStream = REDServiceAttachment.newStreamBuilder()
                                                                                        .withStream(stream)
        ...
        messageSender.sendSyncMessage(REDServiceSyncMessage.forContacts(new ContactsMessage(attachmentStream.build(), complete))
        );
      ...
      throws InvalidNumberException, IOException
  {
    ...
    REDServiceAddress destination = RecipientUtil.toREDServiceAddress(context, recipient);
