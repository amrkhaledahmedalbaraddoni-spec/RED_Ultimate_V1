...
public class MultiDeviceMessageRequestResponseJob extends BaseJob {
  ...
  public void onRun() throws IOException, UntrustedIdentityException, NoSessionException {
    ...
    if (!REDDatabase.recipients().containsId(threadRecipient)) {
    ...
    RecipientRecord            recipient     = REDDatabase.recipients().getRecord(threadRecipient);
    ...
    if (response != null) {
      messageSender.sendSyncMessage(REDServiceSyncMessage.forMessageRequestResponse(response)
      );
    } else {
