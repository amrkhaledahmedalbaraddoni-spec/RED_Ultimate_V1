...
public final class MultiDeviceOutgoingPaymentSyncJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    ...
    PaymentTable.PaymentTransaction payment = REDDatabase.payments().getPayment(uuid);
    ...
    if (!defrag && payment.getPayee().hasRecipientId()) {
      uuid = Optional.ofNullable(REDDatabase.recipients().getRecord(payment.getPayee().requireRecipientId()).getServiceId());
    } else {
      uuid = Optional.empty();
    }
    ...
    AppDependencies.getREDServiceMessageSender()
                   .sendSyncMessage(REDServiceSyncMessage.forOutgoingPayment(outgoingPaymentMessage)
                   );
  }
