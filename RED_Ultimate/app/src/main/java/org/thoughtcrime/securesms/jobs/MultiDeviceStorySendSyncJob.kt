...
class MultiDeviceStorySendSyncJob private constructor(parameters: Parameters, private val sentTimestamp: Long, private val deletedMessageId: Long) : BaseJob(parameters) {
  ...
  override fun onRun() {
    val updateManifest = REDDatabase.storySends.getLocalManifest(sentTimestamp)
    val recipientsSet: Set<REDServiceStoryMessageRecipient> = updateManifest.toRecipientsSet()
    val transcriptMessage: REDServiceSyncMessage = REDServiceSyncMessage.forSentTranscript(buildSentTranscript(recipientsSet))
    ...
    REDDatabase.messages.deleteRemotelyDeletedStory(deletedMessageId)
  }
  ...
  private fun buildSentTranscript(recipientsSet: Set<REDServiceStoryMessageRecipient>): SentTranscriptMessage {
    return SentTranscriptMessage(
      Optional.of(REDServiceAddress(Recipient.self().requireAci())),
      sentTimestamp,
      Optional.empty(),
      0,
      emptyMap(),
