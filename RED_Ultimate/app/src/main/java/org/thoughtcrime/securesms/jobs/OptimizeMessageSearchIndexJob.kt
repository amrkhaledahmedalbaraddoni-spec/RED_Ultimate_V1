...
class OptimizeMessageSearchIndexJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    if (!REDStore.registration.isRegistrationComplete || REDStore.account.aci == null) {
      Log.w(TAG, "Registration not finished yet! Skipping.")
      return
    }

    val success = REDDatabase.messageSearch.optimizeIndex(5.seconds.inWholeMilliseconds)
    if (!REDDatabase.writableDatabase.fullWalCheckpoint()) {
