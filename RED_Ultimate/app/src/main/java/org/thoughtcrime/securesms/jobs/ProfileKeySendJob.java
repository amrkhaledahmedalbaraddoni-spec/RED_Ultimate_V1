...
public class ProfileKeySendJob extends BaseJob {
  ...
  public static @Nullable ProfileKeySendJob create(@NonNull Recipient recipient, boolean queueLimits) {
    return create(REDDatabase.threads().getOrCreateThreadIdFor(recipient), queueLimits);
  }
  ...
  public static @Nullable ProfileKeySendJob create(long threadId, boolean queueLimits) {
    RecipientId conversationRecipientId = REDDatabase.threads().getRecipientIdForThreadId(threadId);
    ...
    List<RecipientId> recipients = conversationRecipient.getGroupId() != null
                                   ? RecipientUtil.getEligibleForSending(Recipient.resolvedList(REDDatabase.groups().getGroupMemberIds(conversationRecipient.getGroupId(), GroupTable.MemberSet.FULL_MEMBERS_INCLUDING_SELF)))
                                                  .stream()
  ...
  protected void onRun() throws Exception {
    ...
    if (threadId > 0 && REDDatabase.threads().getRecipientIdForThreadId(threadId) == null) {
  ...
  private List<Recipient> deliver(@NonNull List<Recipient> destinations) throws IOException, UntrustedIdentityException, NoSessionException {
    REDServiceDataMessage.Builder dataMessage = REDServiceDataMessage.newBuilder()
