...
import com.red.sovereign.database.model.ServiceMessageId
...
/**
 * A job that should be enqueued whenever we process a message that we think has arrived "early" (see [com.red.sovereign.util.EarlyMessageCache]).
...
class PushProcessEarlyMessagesJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    val earlyIds: List<ServiceMessageId> = AppDependencies.earlyMessageCache.allReferencedIds
      .filter { REDDatabase.messages.getMessageFor(it.sentTimestamp, it.sender) != null }
