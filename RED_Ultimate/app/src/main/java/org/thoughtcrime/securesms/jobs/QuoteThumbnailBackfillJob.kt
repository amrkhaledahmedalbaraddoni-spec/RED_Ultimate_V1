...
import com.red.sovereign.database.AttachmentTable.Companion.CONTENT_TYPE
...
import com.red.sovereign.database.AttachmentTable.Companion.DATA_HASH_START
import com.red.sovereign.database.AttachmentTable.Companion.DATA_RANDOM
import com.red.sovereign.database.AttachmentTable.Companion.DATA_SIZE
import com.red.sovereign.database.AttachmentTable.Companion.ID
...
class QuoteThumbnailBackfillJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  private fun peformSingleBackfill(): Boolean {
    val attachment = REDDatabase.attachments.getNextQuoteAttachmentForThumbnailProcessing()
    ...
    val thumbnail = REDDatabase.attachments.generateQuoteThumbnail(DecryptableUri(attachment.uri), attachment.contentType, quiet = true)
    if (thumbnail != null) {
      REDDatabase.attachments.migrationFinalizeQuoteWithData(attachment.dataFile, thumbnail, attachment.contentType)
    } else {
      Log.w(TAG, "Failed to generate thumbnail for attachment: ${attachment.id}. Clearing data.")
      REDDatabase.attachments.finalizeQuoteWithNoData(attachment.dataFile)
    }
