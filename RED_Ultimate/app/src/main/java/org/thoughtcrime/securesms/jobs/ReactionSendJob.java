...
public class ReactionSendJob extends BaseJob {
  ...
  {
    MessageRecord message = REDDatabase.messages().getMessageRecord(messageId.getId());
    ...
    List<RecipientId> recipients = conversationRecipient.getGroupId() != null ? RecipientUtil.getEligibleForSending(Recipient.resolvedList(REDDatabase.groups().getGroupMemberIds(conversationRecipient.getGroupId(), GroupTable.MemberSet.FULL_MEMBERS_EXCLUDING_SELF)))
                                                                                            .stream()
                                                                                            .map(Recipient::getId)
  ...
  protected void onRun() throws Exception {
    ...
    ReactionTable reactionTable = REDDatabase.reactions();
    MessageRecord message       = REDDatabase.messages().getMessageRecord(messageId.getId());

    Recipient targetAuthor        = message.getFromRecipient();
    long      targetSentTimestamp = message.getDateSent();

    if (targetAuthor.getId().equals(REDStore.releaseChannel().getReleaseChannelRecipientId())) {
      return;
    }
  ...
  private static REDServiceDataMessage.Reaction buildReaction(@NonNull ReactionRecord reaction,
                                                                 boolean remove,
                                                                 @NonNull Recipient targetAuthor,
                                                                 long targetSentTimestamp)
  {
    return new REDServiceDataMessage.Reaction(reaction.getEmoji(),
                                                 remove,
