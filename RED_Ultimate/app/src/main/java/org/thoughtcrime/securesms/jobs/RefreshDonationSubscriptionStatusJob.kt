 ...
 * [com.red.sovereign.keyvalue.InAppPaymentValues.setLastEndOfPeriod] when the
 ...
 * causing [com.red.sovereign.keyvalue.InAppPaymentValues.isLikelyASustainer] to
...
class RefreshDonationSubscriptionStatusJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    ...
    if (activeSubscription.isActive) {
      ...
      if (endOfCurrentPeriod > REDStore.inAppPayments.getLastEndOfPeriod()) {
        Log.i(TAG, "Server reports active subscription with newer end-of-period. Updating local state.")
        REDStore.inAppPayments.setLastEndOfPeriod(endOfCurrentPeriod)
      } else {
