...
class RefreshOwnProfileJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    if (!REDStore.account.isRegistered || REDStore.account.e164.isNullOrEmpty()) {
      Log.w(TAG, "Not yet registered!")
      return
    }

    if ((REDStore.svr.hasPin() || REDStore.account.restoredAccountEntropyPool || REDStore.account.restoredAccountEntropyPoolFromPrimary) && !REDStore.svr.hasOptedOut() && REDStore.storageService.lastSyncTime == 0L) {
      Log.i(TAG, "Registered with PIN or AEP but haven't completed storage sync yet.")
      return
    }

    if (!REDStore.registration.hasUploadedProfile && REDStore.account.isPrimaryDevice) {
    ...
    REDStore.registration.hasDownloadedProfile = true
  ...
  private fun setExpiringProfileKeyCredential(
    ...
    REDDatabase.recipients.setProfileKeyCredential(recipient.id, recipientProfileKey, credential)
  }
  ...
  private fun setProfileName(encryptedName: String?) {
    try {
      ...
      if (!profileName.isEmpty()) {
        ...
        REDDatabase.recipients.setProfileName(Recipient.self().id, profileName)
      } else {
  ...
  private fun setProfileAbout(encryptedAbout: String?, encryptedEmoji: String?) {
    try {
      ...
      REDDatabase.recipients.setAbout(Recipient.self().id, plaintextAbout, plaintextEmoji)
    } catch (e: InvalidCiphertextException) {
      Log.w(TAG, e)
    } catch (e: IOException) {
      Log.w(TAG, e)
  ...
  private fun setProfileCapabilities(capabilities: REDServiceProfile.Capabilities?) {
    if (capabilities == null) {
      return
    }

    REDDatabase.recipients.setCapabilities(Recipient.self().id, capabilities)
  }
  ...
  private fun ensurePhoneNumberSharingIsCorrect(phoneNumberSharingCiphertext: String?) {
    ...
    try {
      ...
      if (remotePhoneNumberSharing == PhoneNumberSharingState.UNKNOWN || remotePhoneNumberSharing.enabled != REDStore.phoneNumberPrivacy.isPhoneNumberSharingEnabled()) {
  ...
  private fun setProfileBadges(badges: List<REDServiceProfile.Badge>?) {
    if (badges == null) {
      return
    }
    ...
    if (!remoteHasSubscriptionBadges && localHasSubscriptionBadges) {
      ...
      REDStore.inAppPayments.setExpiredBadge(mostRecentExpiration)
      ...
      val badge = REDStore.inAppPayments.getExpiredBadge()
    ...
    if (!remoteHasGiftBadges && localHasGiftBadges) {
      ...
      REDStore.inAppPayments.setExpiredGiftBadge(mostRecentExpiration)
    ...
    if (userHasVisibleBadges && userHasInvisibleBadges) {
      val displayBadgesOnProfile = REDStore.inAppPayments.getDisplayBadgesOnProfile()
      Log.d(
        TAG,
      ...
      REDDatabase.recipients.setBadges(Recipient.self().id, updatedBadges)
    } else {
      REDDatabase.recipients.setBadges(Recipient.self().id, appBadges)
    }
  }
  ...
  private fun checkUsernameIsInSync() {
    if (REDStore.misc.needsUsernameRestore) {
      ...
      if (REDStore.account.isRegistered && REDStore.account.aci != null && REDStore.registration.restoreDecisionState.isTerminal) {
        AppDependencies.jobManager
    ...
    try {
      val localUsername = REDStore.account.username
    ...
    val localUsernameLink = REDStore.account.usernameLink ?: return

    when (val usernameFetchResult = REDNetwork.username.getDecryptedUsernameFromLinkServerIdAndEntropy(localUsernameLink.serverId, localUsernameLink.entropy)) {
      is RequestResult.Success -> {
        ...
        if (remoteUsername.getUsername() != REDStore.account.username) {
  ...
  private fun getRequestType(recipient: Recipient): REDServiceProfile.RequestType {
    return if (ExpiringProfileCredentialUtil.isValid(recipient.expiringProfileKeyCredential))
      REDServiceProfile.RequestType.PROFILE
    else
      REDServiceProfile.RequestType.PROFILE_AND_CREDENTIAL
  }
