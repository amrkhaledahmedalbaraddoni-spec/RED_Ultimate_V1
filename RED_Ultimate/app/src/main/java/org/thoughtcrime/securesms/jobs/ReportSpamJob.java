...
import com.red.sovereign.database.MessageTable.ReportSpamData;
...
public class ReportSpamJob extends BaseJob {
  ...
  public void onRun() throws IOException {
    ...
    Recipient threadRecipient = REDDatabase.threads().getRecipientForThreadId(threadId);
    ...
    if (threadRecipient.isGroup()) {
      Recipient inviter = REDDatabase.groups().getGroupInviter(threadRecipient.requireGroupId());
      if (inviter == null) {
        Log.w(TAG, "Unable to determine inviter to report");
        return;
      }

      reportSpamData = REDDatabase.messages().getGroupReportSpamMessageServerData(threadId, inviter.getId(), timestamp, MAX_MESSAGE_COUNT);
    } else {
      reportSpamData = REDDatabase.messages().getReportSpamMessageServerData(threadId, timestamp, MAX_MESSAGE_COUNT);
    }
    ...
    for (ReportSpamData data : reportSpamData) {
      ...
      if (serviceId != null && !serviceId.isUnknown()) {
        ...
        byte[] reportingTokenBytes = REDDatabase.recipients().getReportingToken(recipientId);
        if (reportingTokenBytes != null) {
          reportingTokenEncoded = Base64.encodeWithPadding(reportingTokenBytes);
        }

        NetworkResultUtil.toBasicLegacy(REDNetwork.message().reportSpam(serviceId, data.getServerGuid(), reportingTokenEncoded));
        count++;
      } else {
