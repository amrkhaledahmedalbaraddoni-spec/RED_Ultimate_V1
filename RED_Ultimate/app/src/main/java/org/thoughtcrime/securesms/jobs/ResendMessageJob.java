...
public class ResendMessageJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    if (REDStore.internal().getDelayResends()) {
    ...
    if (distributionId != null) {
      if (groupId != null) {
        ...
        Optional<GroupRecord> groupRecord = REDDatabase.groups().getGroupByDistributionId(distributionId);
        ...
        createGroupSendToken = () -> REDDatabase.groups().getGroupSendFullToken(groupId, recipientId);
      } else {
        Log.d(TAG, "GroupId is not present. Assuming this is a message for a distribution list.");
        DistributionListRecord listRecord = REDDatabase.distributionLists().getListByDistributionId(distributionId);
    ...
    try {
      ...
      AppDependencies.getProtocolStore().aci().sessions().archiveSessions(recipientId, REDServiceAddress.DEFAULT_DEVICE_ID);
      AppDependencies.getProtocolStore().aci().sessions().archiveSiblingSessions(recipient.getServiceId().toProtocolAddress(REDServiceAddress.DEFAULT_DEVICE_ID));
      AppDependencies.getProtocolStore().pni().sessions().archiveSessions(recipientId, REDServiceAddress.DEFAULT_DEVICE_ID);
      AppDependencies.getProtocolStore().pni().sessions().archiveSiblingSessions(recipient.getServiceId().toProtocolAddress(REDServiceAddress.DEFAULT_DEVICE_ID));
    ...
    if (result.isSuccess() && distributionId != null) {
      List<REDProtocolAddress> addresses = result.getSuccess()
                                                    .getDevices()
                                                    .stream()
