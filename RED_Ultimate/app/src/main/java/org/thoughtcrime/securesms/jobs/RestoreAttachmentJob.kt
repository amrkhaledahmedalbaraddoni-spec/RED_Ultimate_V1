...
import com.red.sovereign.backup.v2.ArchiveDatabaseExecutor
import com.red.sovereign.backup.v2.ArchiveRestoreProgress
import com.red.sovereign.backup.v2.BackupRepository
import com.red.sovereign.backup.v2.createArchiveAttachmentPointer
import com.red.sovereign.backup.v2.requireMediaName
...
import com.red.sovereign.jobmanager.JobLogger.format
import com.red.sovereign.jobmanager.impl.BackoffUtil
import com.red.sovereign.jobmanager.impl.BatteryNotLowConstraint
import com.red.sovereign.jobmanager.impl.DiskSpaceNotLowConstraint
import com.red.sovereign.jobmanager.impl.NetworkConstraint
import com.red.sovereign.jobmanager.impl.RestoreAttachmentConstraint
import com.red.sovereign.jobmanager.impl.StickersNotDownloadingConstraint
import com.red.sovereign.jobs.protos.RestoreAttachmentJobData
...
class RestoreAttachmentJob private constructor(
  ...
  private constructor(messageId: Long, attachmentId: AttachmentId, manual: Boolean, queue: String, priority: Int, stickerPackId: String? = null) : this(
    Parameters.Builder()
      ...
      .apply {
        ...
        if (stickerPackId != null && REDDatabase.stickers.isPackInstalled(stickerPackId)) {
  ...
  fun doWork() {
    ...
    if (attachment.stickerLocator.isValid()) {
      ...
      val stickerRecord = REDDatabase.stickers.getSticker(locator.packId, locator.stickerId, false)
      ...
      if (stickerRecord != null) {
        ...
        dataStream?.use { input ->
          ...
          ArchiveDatabaseExecutor.runBlocking {
            REDDatabase.attachments.finalizeAttachmentAfterDownload(messageId, attachmentId, input, if (manual) System.currentTimeMillis().milliseconds else null, notify = false)
    ...
    REDLocalMetrics.ArchiveAttachmentRestore.start(attachmentId)
    ...
    REDLocalMetrics.ArchiveAttachmentRestore.end(attachmentId)
  }

  override fun onFailure() {
    if (isCanceled) {
      ArchiveDatabaseExecutor.runBlocking {
        REDDatabase.attachments.setTransferState(messageId, attachmentId, AttachmentTable.TRANSFER_RESTORE_OFFLOADED)
      }
    } else {
  ...
  private fun retrieveAttachment(
    ...
    val attachmentFile: File = ArchiveDatabaseExecutor.runBlocking {
      REDDatabase.attachments.getOrCreateTransferFile(attachmentId)
    }
    ...
    try {
      ...
      useArchiveCdn = !forceTransitTier && REDStore.backup.backsUpMedia && attachment.dataHash != null

      if (!forceTransitTier && REDStore.backup.backsUpMedia && attachment.dataHash == null) {
      ...
      val decryptingStream = if (useArchiveCdn) {
        ...
        messageReceiver
          .retrieveArchivedAttachment(
            REDStore.backup.mediaRootBackupKey.deriveMediaSecrets(attachment.requireMediaName()),
      ...
      if (useArchiveCdn && attachment.archiveCdn == null) {
        ArchiveDatabaseExecutor.runBlocking {
          REDDatabase.attachments.setArchiveCdn(attachmentId, pointer.cdnNumber)
        }
      }
      ...
      when (e.code) {
        404 -> {
          if (forceTransitTier) {
            ...
            markPermanentlyFailed(attachmentId)
            return
          } else if (REDStore.backup.backsUpMedia && attachment.remoteLocation.isNotNullOrBlank()) {
          ...
          } else if (REDStore.backup.backsUpMedia) {
        ...
        401 -> {
          if (useArchiveCdn) {
            ...
            REDStore.backup.mediaCredentials.cdnReadCredentials = null
      ...
      ArchiveDatabaseExecutor.runBlocking {
        REDDatabase.attachments.clearIncrementalMacsForAttachmentAndAnyDuplicates(attachmentId, attachment.remoteKey, attachment.dataHash)
      }
      markFailed(attachmentId)
    } finally {
  ...
  private fun markFailed(attachmentId: AttachmentId) {
    ArchiveDatabaseExecutor.runBlocking {
      REDDatabase.attachments.setRestoreTransferState(attachmentId, AttachmentTable.TRANSFER_PROGRESS_FAILED)
    }
  ...
  private fun markPermanentlyFailed(attachmentId: AttachmentId) {
    ArchiveDatabaseExecutor.runBlocking {
      REDDatabase.attachments.setRestoreTransferState(attachmentId, AttachmentTable.TRANSFER_PROGRESS_PERMANENT_FAILURE)
    }
    maybeRequestBackfill()
  }

  private fun maybeRequestBackfill() {
    if (REDStore.account.isPrimaryDevice || !manual) {
      return
    }
    val attachment = REDDatabase.attachments.getAttachment(attachmentId) ?: return
