...
import com.red.sovereign.backup.v2.createArchiveThumbnailPointer
...
class RestoreAttachmentThumbnailJob private constructor(
  ...
  companion object {
    ...
    fun constructQueueString(attachmentId: AttachmentId): String {
      // SIGNAL_INHERITED: TODO: decide how many queues
  ...
  override fun onAdded() {
    REDDatabase.attachments.setThumbnailRestoreState(attachmentId, AttachmentTable.ThumbnailRestoreState.IN_PROGRESS)
  }
  ...
  private fun doWork() {
    ...
    val thumbnailTransferFile: File = REDDatabase.attachments.createArchiveThumbnailTransferFile()
    ...
    val decryptingStream = AppDependencies.signalServiceMessageReceiver
      .retrieveArchivedThumbnail(
        REDStore.backup.mediaRootBackupKey.deriveMediaSecrets(attachment.requireThumbnailMediaName()),
        cdnCredentials,
    ...
    decryptingStream.use { input ->
      ArchiveDatabaseExecutor.runBlocking {
        REDDatabase.attachments.finalizeAttachmentThumbnailAfterDownload(attachmentId, attachment.dataHash, attachment.remoteKey, input, thumbnailTransferFile)
      }
    }
  ...
  override fun onFailure() {
    ...
    ArchiveDatabaseExecutor.runBlocking {
      REDDatabase.attachments.setThumbnailRestoreProgressFailed(attachmentId, messageId)
    }
  }
