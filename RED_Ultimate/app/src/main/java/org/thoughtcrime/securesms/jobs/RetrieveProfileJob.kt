...
import com.red.sovereign.database.RecipientTable.Companion.maskCapabilitiesToLong
...
import org.whispersystems.signalservice.api.profiles.ProfileRepository.REDServiceProfileWithCredential
...
class RetrieveProfileJob private constructor(parameters: Parameters, private val recipientIds: MutableSet<RecipientId>, private val skipDebounce: Boolean) : BaseJob(parameters) {
  ...
  public override fun onRun() {
    ...
    val recipientsToFetch = REDDatabase
      .recipients
    ...
    val response: ProfileFetchResult<RecipientId> = runBlocking {
      withContext(Dispatchers.IO) {
        ProfileRepository(REDNetwork.profile).fetchProfiles(requests)
      }
    }
    stopwatch.split("responses")

    val localRecords = REDDatabase.recipients.getExistingRecords(fetchingRecipientIds)
    ...
    REDDatabase.recipients.markProfilesFetched(successIds, System.currentTimeMillis())
    ...
    if (newlyRegisteredIds.isNotEmpty()) {
      ...
      REDDatabase.recipients.bulkUpdatedRegisteredStatus(newlyRegisteredIds, emptySet())
    }
    if (response.unregistered.isNotEmpty()) {
      Log.i(TAG, "Marking ${response.unregistered.size} users as unregistered.")
      REDDatabase.recipients.markUnregistered(response.unregistered)
    }
    ...
    if (response.verificationFailures.isNotEmpty()) {
      ...
      for (recipientId in response.verificationFailures) {
        REDDatabase.recipients.clearProfileKeyData(recipientId)
      }
    }
  ...
  private fun isUpdated(localRecipientRecord: RecipientRecord, remoteProfile: REDServiceProfile, remoteExpiringProfileKeyCredential: ExpiringProfileKeyCredential?): Boolean {
  ...
  private fun process(recipient: Recipient, profileAndCredential: REDServiceProfileWithCredential, avatarJobs: MutableList<RetrieveProfileAvatarJob>) {
    ...
    if (recipientProfileKey != null) {
      ...
      REDDatabase.recipients.applyProfileUpdate(recipient.id, update)
  ...
  private fun resolveProfileName(recipient: Recipient, profileKey: ProfileKey, encryptedProfileName: String?): ProfileNameResult? {
    try {
      ...
      if (learnedFirstTime) {
        username = REDDatabase.recipients.getUsername(recipient.id)
        e164 = if (username == null) REDDatabase.recipients.getE164sForIds(listOf(recipient.id)).firstOrNull() else null
      }
  ...
  private fun handleProfileNameSideEffects(recipient: Recipient, result: ProfileNameResult) {
    if (result.learnedFirstTime) {
      if (result.username != null || result.e164 != null) {
        ...
        REDDatabase.messages.insertLearnedProfileNameChangeMessage(recipient, result.e164, result.username)
      } else {
    ...
    if (result.changed) {
      ...
      if (writeChangeEvent) {
        ...
        REDDatabase.messages.insertProfileNameChangeMessages(recipient, remoteDisplayName, localDisplayName)
      } else {
      ...
      if (individualCollisionsEnabled &&
        ...
        val threadId = REDDatabase.threads.getThreadIdFor(recipient.id)
        if (threadId != null && !RecipientUtil.isMessageRequestAccepted(threadId, recipient)) {
          REDDatabase.nameCollisions.handleIndividualNameCollision(recipient.id)
        }
      }
  ...
  companion object {
    ...
    fun forRecipients(recipientIds: Set<RecipientId>, skipDebounce: Boolean = false): List<Job> {
      ...
      for (recipientId in recipientIds) {
        ...
        when {
          ...
          recipient.isGroup -> combined += REDDatabase.groups.getGroupMemberIds(recipient.requireGroupId(), GroupTable.MemberSet.FULL_MEMBERS_EXCLUDING_SELF)
    ...
    fun enqueueRoutineFetchIfNecessary() {
      if (!REDStore.registration.isRegistrationComplete || !REDStore.account.isRegistered || REDStore.account.aci == null) {
        Log.i(TAG, "Registration not complete. Skipping.")
        return
      }

      val timeSinceRefresh = System.currentTimeMillis() - REDStore.misc.lastProfileRefreshTime
      ...
      REDExecutors.BOUNDED.execute {
        ...
        val ids: List<RecipientId> = REDDatabase.recipients.getRecipientsForRoutineProfileFetch(
        ...
        REDStore.misc.lastProfileRefreshTime = System.currentTimeMillis()
      }
    }
  }
}
