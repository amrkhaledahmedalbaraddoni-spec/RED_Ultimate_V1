...
class RetrieveRemoteAnnouncementsJob private constructor(private val force: Boolean, parameters: Parameters) : BaseJob(parameters) {
  ...
  companion object {
    ...
    fun enqueue(force: Boolean = false) {
      ...
      if (!force && System.currentTimeMillis() < REDStore.releaseChannel.nextScheduledCheck) {
  ...
  override fun onRun() {
    ...
    val values = REDStore.releaseChannel
  ...
  private fun updateReleaseNotes(announcements: List<ReleaseNote>) {
    ...
    if (REDDatabase.recipients.getRecord(values.releaseChannelRecipientId!!).isBlocked) {
    ...
    if (!values.hasMetConversationRequirement) {
      if ((REDDatabase.threads.getArchivedConversationListCount(ConversationFilter.OFF) + REDDatabase.threads.getUnarchivedConversationListCount(ConversationFilter.OFF)) < 6) {
    ...
    val threadId = REDDatabase.threads.getOrCreateThreadIdFor(Recipient.resolved(values.releaseChannelRecipientId!!))
    ...
    if (addedNewNotes) {
      ThreadUtil.sleep(5)
      REDDatabase.messages.insertBoostRequestMessage(values.releaseChannelRecipientId!!, threadId)
    }
  ...
  private fun updateMegaphones(megaphones: List<RemoteMegaphone>) {
    ...
    val existingMegaphones: Map<String, RemoteMegaphoneRecord> = REDDatabase.remoteMegaphones.getAll().associateBy { it.uuid }
    ...
    resolvedMegaphones
      ...
      .forEach { megaphone ->
        ...
        if (existingMegaphones.contains(uuid)) {
          REDDatabase.remoteMegaphones.update(
            uuid = uuid,
    ...
    if (megaphonesToDelete.isNotEmpty()) {
      ...
      for ((uuid, megaphone) in megaphonesToDelete) {
        REDDatabase.remoteMegaphones.clear(uuid)
  ...
  private fun String.getLocaleUrls(): List<String> {
    ...
    if (REDStore.settings.language != "zz") {
      potentialNoteUrls += "$this/${REDStore.settings.language}.json"
    }
