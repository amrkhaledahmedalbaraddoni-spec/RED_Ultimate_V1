...
import com.red.sovereign.util.ExceptionHelper;
...
public final class RotateCertificateJob extends BaseJob {
  ...
  public void onRun() throws IOException {
    ...
    synchronized (RotateCertificateJob.class) {
      Collection<CertificateType> certificateTypes = REDStore.phoneNumberPrivacy()
      ...
      for (CertificateType certificateType: certificateTypes) {
        ...
        switch (certificateType) {
          case ACI_AND_E164: certificate = NetworkResultUtil.toBasicLegacy(REDNetwork.certificate().getSenderCertificate()); break;
          case ACI_ONLY    : certificate = NetworkResultUtil.toBasicLegacy(REDNetwork.certificate().getSenderCertificateForPhoneNumberPrivacy()); break;
          default          : throw new AssertionError();
        }

        Log.i(TAG, String.format("Successfully got %s certificate", certificateType));
        REDStore.certificate()
