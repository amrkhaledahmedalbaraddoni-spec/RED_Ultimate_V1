...
public class SendDeliveryReceiptJob extends BaseJob {
  ...
  public void onRun() throws IOException, UntrustedIdentityException, UndeliverableMessageException {
    ...
    REDServiceReceiptMessage receiptMessage = new REDServiceReceiptMessage(REDServiceReceiptMessage.Type.DELIVERY,
    ...
    if (result != null && messageId != null) {
      REDDatabase.messageLog().insertIfPossible(recipientId, timestamp, result, ContentHint.IMPLICIT, messageId, false);
    }
  }
  ...
  private @Nullable GroupSendFullToken getGroupSendFullToken() {
    ...
    long threadId = REDDatabase.messages().getThreadIdForMessage(messageId.getId());
    if (threadId == -1) {
      return null;
    }

    return REDDatabase.groups().getGroupSendFullToken(threadId, recipientId);
  }
