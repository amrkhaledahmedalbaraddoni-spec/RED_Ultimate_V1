...
import static com.red.sovereign.jobs.SendReadReceiptJob.MAX_TIMESTAMPS;
...
public class SendViewedReceiptJob extends BaseJob {
  ...
  public void onRun() throws IOException, UntrustedIdentityException {
    ...
    boolean canSendStoryReceipts    = REDStore.story().getViewedReceiptsEnabled();

    List<MessageId> foundMessageIds       = new LinkedList<>();
    List<Long>      messageSentTimestamps = new LinkedList<>();
    List<StoryType> storyTypes            = REDDatabase.messages().getStoryTypes(this.messageIds);
    ...
    if (recipient.getId().equals(REDStore.releaseChannel().getReleaseChannelRecipientId())) {
    ...
    REDServiceAddress       remoteAddress = RecipientUtil.toREDServiceAddress(recipient);
    REDServiceReceiptMessage receiptMessage = new REDServiceReceiptMessage(REDServiceReceiptMessage.Type.VIEWED,
    ...
    SendMessageResult result = ReceiptSender.sendWithSessionRepair(recipientId, () -> messageSender.sendReceipt(remoteAddress,
                                                                                                                SealedSenderAccessUtil.getSealedSenderAccessFor(recipient,
                                                                                                                                                                () -> REDDatabase.groups().getGroupSendFullToken(threadId, recipientId)),
                                                                                                                receiptMessage,
                                                                                                                recipient.needsPniSignature()));

    if (result != null && Util.hasItems(foundMessageIds)) {
      REDDatabase.messageLog().insertIfPossible(recipientId, timestamp, result, ContentHint.IMPLICIT, foundMessageIds, false);
    }
  }
