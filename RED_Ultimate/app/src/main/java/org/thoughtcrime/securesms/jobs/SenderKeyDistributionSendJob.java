...
public final class SenderKeyDistributionSendJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    ...
    if (threadRecipient.isPushV2Group()) {
      ...
      createGroupSendFullToken = () -> REDDatabase.groups().getGroupSendFullToken(groupId, targetRecipientId);
    } else if (threadRecipient.isDistributionList()) {
      groupId        = null;
      distributionId = REDDatabase.distributionLists().getDistributionId(threadRecipientId);
    } else {
    ...
    if (groupId != null && !REDDatabase.groups().isCurrentMember(groupId, targetRecipientId)) {
      Log.w(TAG, targetRecipientId + " is no longer a member of " + groupId + "! Not sending.");
      return;
    } else if (groupId == null) {
      DistributionListRecord listRecord = REDDatabase.distributionLists().getList(threadRecipientId);
    ...
    List<REDServiceAddress>   address       = Collections.singletonList(RecipientUtil.toREDServiceAddress(context, targetRecipient));
