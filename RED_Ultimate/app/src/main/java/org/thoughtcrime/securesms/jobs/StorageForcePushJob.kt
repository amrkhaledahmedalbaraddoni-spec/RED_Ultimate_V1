...
class StorageForcePushJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    ...
    if (!REDStore.account.isRegistered || REDStore.account.e164 == null) {
    ...
    val storageServiceKey = REDStore.storageService.storageKey
    ...
    val oldContactStorageIds: Map<RecipientId, StorageId> = REDDatabase.recipients.getContactStorageSyncIdsMap()

    val newVersion = currentVersion + 1
    val newContactStorageIds = generateContactStorageIds(oldContactStorageIds)
    val inserts: MutableList<REDStorageRecord> = oldContactStorageIds.keys
    ...
    val oldChatFolderStorageIds = REDDatabase.chatFolders.getStorageSyncIdsMap()
    val newChatFolderStorageIds = generateChatFolderStorageIds(oldChatFolderStorageIds)
    val newChatFolderInserts: List<REDStorageRecord> = oldChatFolderStorageIds.keys
      .mapNotNull {
        val query = SqlUtil.buildQuery("${ChatFolderTable.CHAT_FOLDER_ID} = ?", it)
        REDDatabase.chatFolders.getChatFolder(query)
      }
    ...
    val oldNotificationProfileStorageIds = REDDatabase.notificationProfiles.getStorageSyncIdsMap()
    val newNotificationProfileStorageIds = generateNotificationProfileStorageIds(oldNotificationProfileStorageIds)
    val newNotificationProfileInserts: List<REDStorageRecord> = oldNotificationProfileStorageIds.keys
      .mapNotNull {
        val query = SqlUtil.buildQuery("${NotificationProfileTables.NotificationProfileTable.NOTIFICATION_PROFILE_ID} = ?", it)
        REDDatabase.notificationProfiles.getProfile(query)
      }
    ...
    val oldStickerPackStorageIds = REDDatabase.stickers.getStorageSyncIdsMap()
    val newStickerPackStorageIds = generateStickerPackStorageIds(oldStickerPackStorageIds)
    val newStickerPackInserts: List<REDStorageRecord> = oldStickerPackStorageIds.keys
      .mapNotNull { REDDatabase.stickers.getPackForStorageSync(it) }
    ...
    val manifest = REDStorageManifest(newVersion, REDStore.account.deviceId, recordIkm, allNewStorageIds)
    ...
    REDStore.storageService.manifest = manifest
    REDStore.svr.masterKeyForInitialDataRestore = null
    REDDatabase.recipients.applyStorageIdUpdates(newContactStorageIds)
    REDDatabase.recipients.applyStorageIdUpdates(Collections.singletonMap(Recipient.self().id, accountRecord.id))
    REDDatabase.chatFolders.applyStorageIdUpdates(newChatFolderStorageIds)
    REDDatabase.notificationProfiles.applyStorageIdUpdates(newNotificationProfileStorageIds)
    REDDatabase.stickers.applyStorageIdUpdates(newStickerPackStorageIds)
    REDDatabase.unknownStorageIds.deleteAll()
  }
