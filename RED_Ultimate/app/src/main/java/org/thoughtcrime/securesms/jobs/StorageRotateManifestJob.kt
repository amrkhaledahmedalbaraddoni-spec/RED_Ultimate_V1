...
class StorageRotateManifestJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  override fun run(): Result {
    ...
    val restoreKey: StorageKey? = REDStore.storageService.storageKeyForInitialDataRestore
    if (restoreKey == null) {
    ...
    val currentManifest: REDStorageManifest = when (val result = repository.getStorageManifest(restoreKey)) {
    ...
    return when (val result = repository.writeUnchangedManifest(storageServiceKey, manifestWithNewVersion)) {
      StorageServiceService.WriteStorageRecordsResult.Success -> {
        ...
        REDStore.storageService.manifest = manifestWithNewVersion
