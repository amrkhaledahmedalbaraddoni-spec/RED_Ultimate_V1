...
class StoryOnboardingDownloadJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  companion object {
    ...
    fun enqueueIfNeeded() {
      if (REDStore.story.hasDownloadedOnboardingStory || REDStore.story.userHasViewedOnboardingStory) {
  ...
  override fun onRun() {
    if (REDStore.story.hasDownloadedOnboardingStory) {
      Log.i(TAG, "Already downloaded onboarding story. Exiting.")
      return
    }

    val releaseChannelRecipientId = REDStore.releaseChannel.releaseChannelRecipientId
    ...
    REDDatabase.messages.getAllStoriesFor(releaseChannelRecipientId, -1).use { reader ->
      reader.forEach { messageRecord ->
        REDDatabase.messages.deleteMessage(messageRecord.id)
      }
    }
    ...
    val threadId = REDDatabase.threads.getOrCreateThreadIdFor(Recipient.resolved(releaseChannelRecipientId))
    ...
    if (insertResults.size != ONBOARDING_IMAGE_COUNT) {
      ...
      insertResults.forEach {
        REDDatabase.messages.deleteMessage(it.messageId)
      }
    ...
    REDStore.story.hasDownloadedOnboardingStory = true

    Log.i(TAG, "Enqueueing download jobs...")
    insertResults.forEach { insertResult ->
      REDDatabase.attachments.getAttachmentsForMessage(insertResult.messageId).forEach {
  ...
  private fun getLocaleCodes(): List<String> {
    ...
    if (REDStore.settings.language != "zz") {
      potentialOnboardingUrlLanguages += REDStore.settings.language
    }
