...
import com.red.sovereign.jobs.protos.Svr3MirrorJobData
  ...
  override fun run(): Result {
    ...
    SvrRepository.operationLock.withLock {
      ...
      val session: PinChangeSession = serializedChangeSession?.let { session ->
        svr3.resumePinChangeSession(pin, REDStore.svr.masterKey, session)
      } ?: svr3.setPin(pin, REDStore.svr.masterKey)
      ...
      return when (val response: BackupResponse = session.execute()) {
        is BackupResponse.Success -> {
          ...
          REDStore.svr.appendSvr3AuthTokenToList(response.authorization.asBasic())
