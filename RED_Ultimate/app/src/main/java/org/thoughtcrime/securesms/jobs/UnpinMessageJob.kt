...
import com.red.sovereign.jobs.protos.UnpinJobData
...
class UnpinMessageJob(
  ...
  override fun run(): Result {
    ...
    if (groupId != null && groupId.isV2) {
      val groupRecord = REDDatabase.groups.getGroup(conversationRecipient.id)
  ...
  private fun deliver(conversationRecipient: RecipientRecord, destinations: List<Recipient>, threadId: Long, targetAuthor: Recipient, targetSentTimestamp: Long): List<Recipient> {
    ...
    if (result.completed.isNotEmpty() || destinations.isEmpty()) {
      REDDatabase.messages.unpinMessage(
        messageId = messageId,
        threadId = threadId
      )
    }
