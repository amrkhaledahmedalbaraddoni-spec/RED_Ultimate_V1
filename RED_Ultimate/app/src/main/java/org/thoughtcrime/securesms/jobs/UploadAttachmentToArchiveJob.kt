...
import com.red.sovereign.jobs.protos.UploadAttachmentToArchiveJobData
...
class UploadAttachmentToArchiveJob private constructor(
  ...
  override fun onAdded() {
    ...
    if (transferStatus == AttachmentTable.ArchiveTransferState.NONE) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        REDDatabase.attachments.setArchiveTransferStateUnlessPermanentFailure(attachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
      }
    }
  }
  ...
  override fun run(): Result {
    ...
    progressServiceController.use {
      val uploadResult: AttachmentUploadResult = attachmentStream.use { stream ->
        when (
          val result = REDNetwork.attachments.uploadAttachmentV4(
            form = form,
            key = key,
            iv = iv,
          ...
          is NetworkResult.NetworkError -> {
            ...
            if (result.exception is ResumeLocationInvalidException) {
              ...
              val actualLength = REDDatabase.attachments.getAttachmentStream(attachmentId, 0)
                .use { it.readLength() }
      ...
      ArchiveDatabaseExecutor.runBlocking {
        REDDatabase.attachments.finalizeAttachmentAfterUpload(attachment.attachmentId, uploadResult)
      }
    }
  ...
  override fun onFailure() {
    if (this.isCanceled) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        REDDatabase.attachments.setArchiveTransferStateFailure(attachmentId, AttachmentTable.ArchiveTransferState.NONE)
      }
    } else {
