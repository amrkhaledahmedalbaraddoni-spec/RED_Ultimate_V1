...
import com.red.sovereign.crypto.MasterCipher
...
import com.red.sovereign.jobmanager.impl.RegisteredConstraint
...
import org.signal.libsignal.messagebackup.AccountEntropyPool as LibREDAccountEntropyPool

class AccountValues internal constructor(store: KeyValueStore, context: Context) : REDStoreValues(store) {
  ...
  val accountEntropyPool: AccountEntropyPool
    get() {
      AEP_LOCK.withLock {
        ...
        val newAep = LibREDAccountEntropyPool.generate()
  ...
  /** The password for communicating with the RED service. */
  ...
  /** Whether or not the user is registered with the RED service. */
  ...
  var deviceId: Int by integerValue(KEY_DEVICE_ID, REDServiceAddress.DEFAULT_DEVICE_ID)

  val isPrimaryDevice: Boolean
    get() = deviceId == REDServiceAddress.DEFAULT_DEVICE_ID
  ...
  private fun clearLocalCredentials() {
    ...
    REDDatabase.recipients.setProfileKey(self.id, newProfileKey)
