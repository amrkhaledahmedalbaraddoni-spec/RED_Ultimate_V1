...
import com.red.sovereign.components.settings.app.subscription.donate.stripe.Stripe3DSData
 ...
 */
class InAppPaymentValues internal constructor(store: KeyValueStore) : REDStoreValues(store) {
  ...
  companion object {
     ...
     * Needed to show donation pending sheet after returning to RED.
     */
  ...
  fun getRecurringDonationCurrency(): Currency {
    ...
    val currency: Currency? = if (currencyCode == null) {
      ...
      if (localeCurrency == null) {
        val e164: String? = REDStore.account.e164
        if (e164 == null) {
          null
        } else {
  ...
  fun updateLocalStateForManualCancellation(subscriberType: InAppPaymentSubscriberRecord.Type) {
    subscriberType.lock.withLock {
      ...
      if (subscriber != null) {
        REDDatabase.inAppPayments.markSubscriptionManuallyCanceled(subscriberId = subscriber.subscriberId)
      }
    }
  }
