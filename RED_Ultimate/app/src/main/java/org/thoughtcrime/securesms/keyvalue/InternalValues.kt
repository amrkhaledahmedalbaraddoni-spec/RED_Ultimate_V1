...
import com.red.sovereign.database.model.IssuePriority
import com.red.sovereign.util.Environment.Calling.defaultSfuUrl
import com.red.sovereign.util.RemoteConfig

class InternalValues internal constructor(store: KeyValueStore) : REDStoreValues(store) {
  companion object {
  ...
  /**
   * Show detailed recipient info in the [com.red.sovereign.components.settings.conversation.InternalConversationSettingsFragment].
   */
  ...
  /**
   * The minimum [IssuePriority] that an issue recorded by [com.red.sovereign.database.IssueReporter] must have
  ...
  private fun <T> REDStoreValueDelegate<T>.defaultForExternalUsers(): REDStoreValueDelegate<T> {
    return this.withPrecondition { RemoteConfig.internalUser }
  }

  private fun REDStoreValueDelegate<Boolean>.falseForExternalUsers(): REDStoreValueDelegate<Boolean> {
