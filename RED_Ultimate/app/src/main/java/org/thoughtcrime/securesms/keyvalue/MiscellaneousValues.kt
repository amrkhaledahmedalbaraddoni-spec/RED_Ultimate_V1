...
import com.red.sovereign.jobmanager.impl.ChangeNumberConstraintObserver
import com.red.sovereign.jobs.DeprecatedNotificationJob
import com.red.sovereign.keyvalue.protos.LeastActiveLinkedDevice

class MiscellaneousValues internal constructor(store: KeyValueStore) : REDStoreValues(store) {
  companion object {
  ...
  /**
   * Bypasses the timeout in [com.red.sovereign.jobs.PreKeysSyncJob] since otherwise we can hit a race.
   */
  ...
  /**
   * The last time we ran an account consistency check via [com.red.sovereign.jobs.AccountConsistencyWorkerJob]
   */
   ...
   *
   * estimatedServerTime = System.currentTimeMillis() - REDStore.misc.getLastKnownServerTimeOffset()
   */
  ...
  /**
   * The last time we prompted the user regarding a [com.red.sovereign.util.ConnectivityWarning].
   */
   ...
   * [com.red.sovereign.service.webrtc.CallingAssets.CURRENT_VERSION] to determine
