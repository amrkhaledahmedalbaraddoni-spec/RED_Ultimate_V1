...
import com.red.sovereign.payments.GeographicalRestrictions
import com.red.sovereign.payments.Mnemonic
import com.red.sovereign.payments.MobileCoinLedgerWrapper
import com.red.sovereign.payments.currency.CurrencyUtil
import com.red.sovereign.payments.proto.MobileCoinLedger
...
class PaymentsValues internal constructor(store: KeyValueStore) : REDStoreValues(store) {
    ...
    get() {
      ...
      return if (RemoteConfig.payments) {
        if (mobileCoinPaymentsEnabled()) {
          if (GeographicalRestrictions.e164Allowed(REDStore.account.e164)) {
            PaymentsAvailability.WITHDRAW_AND_SEND
          } else {
            return PaymentsAvailability.WITHDRAW_ONLY
          }
        } else if (GeographicalRestrictions.e164Allowed(REDStore.account.e164)) {
  ...
  fun showUpdatePinInfoCard(): Boolean {
    return if (userHasLargeBalance() &&
      ...
      REDStore.pin.keyboardType == PinKeyboardType.NUMERIC
    ) {
  ...
  private fun determineCurrency(): Currency {
    val localE164: String = REDStore.account.e164 ?: ""
