 ...
 */
class REDStore(context: Application, private val store: KeyValueStore) {
  ...
  companion object {
    ...
    private var instance: REDStore? = null

    @JvmStatic
    fun init(context: Application) {
      if (instance == null) {
        synchronized(REDStore::class.java) {
          if (instance == null) {
            instance = REDStore(context, KeyValueStore(KeyValueDatabase.getInstance(context)))
          }
        }
      }
    }
     ...
     */
    @VisibleForTesting
    fun testInject(instance: REDStore?) {
    ...
    val groupsV2AciAuthorizationCache: GroupsV2AuthorizationREDStoreCache
      get() = GroupsV2AuthorizationREDStoreCache.createAciCache(instance!!.store)

    val plaintext: PlainTextSharedPrefsDataStore
      get() = instance!!.plainTextValues

    fun getPreferenceDataStore(): PreferenceDataStore {
      return REDPreferenceDataStore(instance!!.store)
    }

    /**
     * Ensures any pending writes are finished. Only intended to be called by
     * [REDUncaughtExceptionHandler].
     */
    @JvmStatic
