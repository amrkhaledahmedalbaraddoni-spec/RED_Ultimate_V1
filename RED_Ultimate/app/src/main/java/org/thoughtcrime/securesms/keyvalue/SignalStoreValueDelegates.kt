...
internal fun REDStoreValues.longValue(key: String, default: Long): REDStoreValueDelegate<Long> {
  return LongValue(key, default, this.store)
}

internal fun REDStoreValues.booleanValue(key: String, default: Boolean): REDStoreValueDelegate<Boolean> {
  return BooleanValue(key, default, this.store)
}

internal fun <T : String?> REDStoreValues.stringValue(key: String, default: T): REDStoreValueDelegate<T> {
  return StringValue(key, default, this.store)
}

internal fun REDStoreValues.integerValue(key: String, default: Int): REDStoreValueDelegate<Int> {
  return IntValue(key, default, this.store)
}

internal fun REDStoreValues.floatValue(key: String, default: Float): REDStoreValueDelegate<Float> {
  return FloatValue(key, default, this.store)
}

internal fun REDStoreValues.blobValue(key: String, default: ByteArray): REDStoreValueDelegate<ByteArray> {
  return BlobValue(key, default, this.store)
}

internal fun REDStoreValues.nullableBlobValue(key: String, default: ByteArray?): REDStoreValueDelegate<ByteArray?> {
  return NullableBlobValue(key, default, this.store)
}

internal fun <T : Any?> REDStoreValues.enumValue(key: String, default: T, serializer: LongSerializer<T>): REDStoreValueDelegate<T> {
  return KeyValueEnumValue(key, default, serializer, this.store)
}

internal fun <M> REDStoreValues.protoValue(key: String, adapter: ProtoAdapter<M>): REDStoreValueDelegate<M?> {
  return KeyValueProtoValue(key, adapter, this.store)
}

internal fun <M> REDStoreValues.protoValue(key: String, default: M, adapter: ProtoAdapter<M>, onSet: ((M) -> Unit)? = null): REDStoreValueDelegate<M> {
  return KeyValueProtoWithDefaultValue(key, default, adapter, this.store, onSet)
}

internal fun REDStoreValues.durationValue(key: String, default: Duration?): REDStoreValueDelegate<Duration?> {
  return DurationValue(key, default, this.store)
}

internal fun <T> REDStoreValueDelegate<T>.withPrecondition(precondition: () -> Boolean): REDStoreValueDelegate<T> {
...
internal fun <T> REDStoreValueDelegate<T>.map(transform: (T) -> T): REDStoreValueDelegate<T> {
 ...
 */
sealed class REDStoreValueDelegate<T>(val store: KeyValueStore, open val default: T, private val onSet: ((T) -> Unit)? = null) {
...
private class LongValue(private val key: String, default: Long, store: KeyValueStore) : REDStoreValueDelegate<Long>(store, default) {
...
private class BooleanValue(private val key: String, default: Boolean, store: KeyValueStore) : REDStoreValueDelegate<Boolean>(store, default) {
...
private class StringValue<T : String?>(private val key: String, default: T, store: KeyValueStore) : REDStoreValueDelegate<T>(store, default) {
...
private class IntValue(private val key: String, default: Int, store: KeyValueStore) : REDStoreValueDelegate<Int>(store, default) {
...
private class FloatValue(private val key: String, default: Float, store: KeyValueStore) : REDStoreValueDelegate<Float>(store, default) {
...
private class BlobValue(private val key: String, default: ByteArray, store: KeyValueStore) : REDStoreValueDelegate<ByteArray>(store, default) {
...
private class NullableBlobValue(private val key: String, default: ByteArray?, store: KeyValueStore) : REDStoreValueDelegate<ByteArray?>(store, default) {
  override fun getValue(values: KeyValueStore): ByteArray? {
    return values.getBlob(key, default)
  }
...
private class DurationValue(private val key: String, default: Duration?, store: KeyValueStore) : REDStoreValueDelegate<Duration?>(store, default) {
  companion object {
...
private class KeyValueProtoWithDefaultValue<M>(
  ...
  store: KeyValueStore,
  onSet: ((M) -> Unit)? = null
) : REDStoreValueDelegate<M>(store, default, onSet) {
...
private class KeyValueProtoValue<M>(
  ...
  private val adapter: ProtoAdapter<M>,
  store: KeyValueStore
) : REDStoreValueDelegate<M?>(store, null) {
  override fun getValue(values: KeyValueStore): M? {
    return if (values.containsKey(key)) {
      adapter.decode(values.getBlob(key, null))
    } else {
...
private class KeyValueEnumValue<T>(private val key: String, default: T, private val serializer: LongSerializer<T>, store: KeyValueStore) : REDStoreValueDelegate<T>(store, default) {
  override fun getValue(values: KeyValueStore): T {
    return if (values.containsKey(key)) {
...
private class PreconditionDelegate<T>(
  private val delegate: REDStoreValueDelegate<T>,
  private val precondition: () -> Boolean
) : REDStoreValueDelegate<T>(delegate.store, delegate.default) {
