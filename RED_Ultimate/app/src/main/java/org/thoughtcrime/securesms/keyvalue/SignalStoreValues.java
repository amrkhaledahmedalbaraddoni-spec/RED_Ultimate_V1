...
import com.red.sovereign.database.model.databaseprotos.REDStoreList;
...
public abstract class REDStoreValues {

  private final KeyValueStore store;

  REDStoreValues(@NonNull KeyValueStore store) {
    this.store = store;
  }
  ...
  <T> List<T> getList(@NonNull String key, @NonNull StringSerializer<T> serializer) {
    ...
    try {
      REDStoreList signalStoreList = REDStoreList.ADAPTER.decode(blob);
  ...
  <T> void putList(@NonNull String key, @NonNull List<T> values, @NonNull StringSerializer<T> serializer) {
    putBlob(key, new REDStoreList.Builder()
