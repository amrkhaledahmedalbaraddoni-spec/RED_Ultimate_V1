...
class StorageServiceValues internal constructor(store: KeyValueStore) : REDStoreValues(store) {
  companion object {
  ...
  val storageKey: StorageKey
    get() {
      return REDStore.svr.masterKey.deriveStorageServiceKey()
    }
  ...
  var manifest: REDStorageManifest
    get() {
      val data = getBlob(MANIFEST, null)

      return if (data != null) {
        REDStorageManifest.deserialize(data)
      } else {
        REDStorageManifest.EMPTY
      }
    }
    set(manifest) {
  ...
  val storageKeyForInitialDataRestore: StorageKey?
    get() = REDStore.svr.masterKeyForInitialDataRestore?.deriveStorageServiceKey()
}
