...
import com.red.sovereign.lock.v2.SvrConstants;
...
public final class REDPinReminderDialog {

  private static final String TAG = Log.tag(REDPinReminderDialog.class);
  ...
  public static void show(@NonNull Context context, @NonNull Launcher launcher, @NonNull Callback mainCallback) {
    ...
    AlertDialog dialog = new MaterialAlertDialogBuilder(context, R.style.ThemeOverlay_RED_MaterialAlertDialog_Wide)
    ...
    REDStore.pin().getKeyboardType().applyInputTypeTo(pinEditText);
    ...
    pinEditText.addTextChangedListener(new SimpleTextWatcher() {
      ...
      private final String localHash = Objects.requireNonNull(REDStore.svr().getLocalPinHash());
  ...
  private static final class V2PinVerifier implements PinVerifier {
    ...
    V2PinVerifier() {
      localPinHash = REDStore.svr().getLocalPinHash();
