...
import com.red.sovereign.emoji.EmojiFiles.Version.Companion.readVersion
import com.red.sovereign.keyvalue.REDStore
import com.red.sovereign.net.StandardUserAgentInterceptor
import com.red.sovereign.notifications.SlowNotificationHeuristics.isHavingDelayedNotifications
import com.red.sovereign.recipients.Recipient.Companion.self
import com.red.sovereign.service.webrtc.AndroidTelecomUtil.telecomSupported
...
import com.red.sovereign.util.ScreenDensity
import com.red.sovereign.util.TextSecurePreferences
import com.red.sovereign.util.VersionTracker.getDaysSinceFirstInstalled
...
class LogSectionSystemInfo : LogSection {
  ...
  override fun getContent(context: Context): CharSequence {
    return """
      ...
      OS Host           : ${Build.HOST}
      RecipientId       : ${if (REDStore.registration.isRegistrationComplete) self().id else "N/A"}
      ACI               : ${getCensoredAci()}
      Device ID         : ${REDStore.account.deviceId}
      ...
      FCM               : ${REDStore.account.fcmEnabled}
      Locale            : ${Locale.getDefault()}
      Linked Devices    : ${REDStore.account.isMultiDevice}
      ...
      Server Time Offset: ${REDStore.misc.lastKnownServerTimeOffset} ms (last updated: ${REDStore.misc.lastKnownServerTimeOffsetUpdateTime})
  ...
  private fun getTimeRegistered(): String {
    if (REDStore.account.registeredAtTimestamp <= 0) {
      return "Unknown"
    }

    val timeSince = (System.currentTimeMillis() - REDStore.account.registeredAtTimestamp).milliseconds.toString()
    return "${REDStore.account.registeredAtTimestamp} ($timeSince ago)"
  }
}
