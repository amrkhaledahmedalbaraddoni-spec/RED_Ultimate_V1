...
public class SubmitDebugLogRepository {
  ...
  public SubmitDebugLogRepository() {
    ...
    this.executor = REDExecutors.SERIAL;
  }
  ...
  public void submitLogFromReader(DebugLogsViewer.LogReader logReader, @Nullable byte[] trace, Consumer<Optional<String>> callback) {
    REDExecutors.UNBOUNDED.execute(() -> callback.accept(submitLogFromReaderInternal(logReader, trace)));
  }
  ...
  private @NonNull List<LogLine> getPrefixLogLinesInternal() {
    ...
    for (LogSection section : SECTIONS) {
      futures.add(REDExecutors.BOUNDED.submit(() -> getLinesForSection(context, section, maxTitleLength)));
    }
