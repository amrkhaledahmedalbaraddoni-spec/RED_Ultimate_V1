...
import com.red.sovereign.database.MediaTable.Sorting
...
import com.red.sovereign.longmessage.resolveBody
...
class MediaPreviewRepository {
  ...
  fun resolveMessageBodies(context: Context, messageIds: Set<Long>): Single<Map<Long, SpannableString>> {
    return Single.fromCallable {
      REDDatabase.messages.getMessages(messageIds).toList().withAttachments()
  ...
  fun getMessagePositionIntent(context: Context, messageId: Long): Single<Intent> {
    return Single.fromCallable {
      ...
      val messageRecord: MessageRecord = REDDatabase.messages.getMessageRecord(messageId)
      stopwatch.split("get message record")

      val threadId: Long = messageRecord.threadId
      val messagePosition: Int = REDDatabase.messages.getMessagePositionInConversation(threadId, messageRecord.dateReceived)
      stopwatch.split("get message position")

      val recipientId: RecipientId = REDDatabase.threads.getRecipientForThreadId(threadId)?.id ?: throw IllegalStateException("Could not find recipient for thread ID $threadId")
