...
import com.red.sovereign.mediasend.MediaSendDocumentViewModel.DocumentInfo
    ...
    viewModel.documentInfo.observe(viewLifecycleOwner) { documentInfoOptional ->
      ...
      if (documentInfo != null) {
        ...
        if (documentInfo.extension.length <= 3) {
          ...
          extension.setTextAppearance(requireContext(), CoreUiR.style.RED_Text_BodySmall)
        } else if (documentInfo.extension.length == 4) {
          extension.text = documentInfo.extension
          extension.setTextAppearance(requireContext(), CoreUiR.style.RED_Text_Caption)
        }
      } else {
