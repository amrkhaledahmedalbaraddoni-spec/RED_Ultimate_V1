...
import com.red.sovereign.mediasend.v2.MediaSelectionRepository
import com.red.sovereign.mediasend.v2.MediaValidator
...
object MediaSendV3Repository : MediaSendRepository {
  ...
  override var isCameraFacingFront: Boolean
    get() = REDStore.misc.isCameraFacingFront
    set(value) {
      REDStore.misc.isCameraFacingFront = value
    }
    ...
    val recipients: List<Recipient> = contactIds
      ...
      .map { recipient ->
        when {
          ...
          recipient.isDistributionList -> Recipient.resolvedList(REDDatabase.distributionLists.getMembers(recipient.distributionListId.get()))
  ...
  private fun resolveSendType(sendType: Int): MessageSendType {
    return when (sendType) {
      else -> MessageSendType.REDMessageSendType
    }
  }
