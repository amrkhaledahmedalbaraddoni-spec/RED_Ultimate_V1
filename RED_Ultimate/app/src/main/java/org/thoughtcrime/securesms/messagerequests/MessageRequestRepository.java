...
import com.red.sovereign.jobs.ReportSpamJob;
...
public final class MessageRequestRepository {
  ...
  public MessageRequestRepository(@NonNull Context context) {
    ...
    this.executor = REDExecutors.BOUNDED;
  }

  @WorkerThread
  public @NonNull MessageRequestRecipientInfo getRecipientInfo(@NonNull RecipientId recipientId, long threadId) {
    List<String>          sharedGroups = REDDatabase.groups().getPushGroupNamesContainingMember(recipientId);
    Optional<GroupRecord> groupRecord  = REDDatabase.groups().getGroup(recipientId);
    ...
    if (sharedGroups.isEmpty() && recipient.getHasGroupsInCommon()) {
      REDDatabase.recipients().clearHasGroupsInCommon(recipient.getId());
    }
  ...
  public boolean threadContainsSms(long threadId) {
    return REDDatabase.messages().threadContainsSms(threadId);
  }

  private boolean reportedAsSpam(long threadId) {
    return REDDatabase.messages().hasReportSpamMessage(threadId) ||
           REDDatabase.messages().getOutgoingSecureMessageCount(threadId) > 0;
  }
                                   ...
                                   @NonNull GroupChangeErrorCallback error)
  {
    executor.execute(() -> {
      ...
      if (recipient.isPushV2Group()) {
        ...
        List<MessageTable.MarkedMessageInfo> messageIds = REDDatabase.threads().setEntireThreadRead(threadId);
        AppDependencies.getMessageNotifier().updateNotification(context);
        MarkReadReceiver.process(messageIds);

        List<MessageTable.MarkedMessageInfo> viewedInfos = REDDatabase.messages().getViewedIncomingMessages(threadId);
    ...
    executor.execute(() -> {
      ...
      if (resolved.isGroup() && resolved.requireGroupId().isPush()) {
        try {
          ...
          if (REDDatabase.groups().isCurrentMember(resolved.requireGroupId().requirePush(), Recipient.self().getId())) {
  ...
  private boolean isLegacyThread(@NonNull Recipient recipient) {
    Long threadId = REDDatabase.threads().getThreadIdFor(recipient.getId());
