...
abstract class BatchCache {
  ...
  protected fun flushIncomingMessageInsertThreadUpdate(threadId: Long) {
    REDDatabase.threads.updateForMessageInsert(threadId, unarchive = true)
  }

  protected fun flushMslDelete(recipientId: RecipientId, device: Int, timestamps: List<Long>) {
    REDDatabase.messageLog.deleteEntriesForRecipient(timestamps, recipientId, device)
  }
 ...
 * no batching is possible, mostly when the [com.red.sovereign.jobs.PushProcessMessageJob] runs.
 */
...
/**
 * This is intended to be used in [IncomingMessageObserver] to batch jobs (e.g., [com.red.sovereign.jobs.SendDeliveryReceiptJob])
 * and dedupe and batch calls to [REDDatabase.threads.updateForMessageInsert].
 *
