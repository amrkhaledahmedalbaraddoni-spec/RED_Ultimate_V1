...
import com.red.sovereign.database.model.MessageLogEntry
import com.red.sovereign.database.model.MessageRecord
import com.red.sovereign.database.model.PendingRetryReceiptModel
...
import com.red.sovereign.groups.v2.processing.GroupUpdateResult
import com.red.sovereign.groups.v2.processing.GroupUpdateResult.UpdateStatus
import com.red.sovereign.jobs.AutomaticSessionResetJob
import com.red.sovereign.jobs.NullMessageSendJob
import com.red.sovereign.jobs.ResendMessageJob
...
import com.red.sovereign.messages.REDServiceProtoUtil.hasDisallowedAnnouncementOnlyContent
import com.red.sovereign.messages.REDServiceProtoUtil.hasGroupContext
import com.red.sovereign.messages.REDServiceProtoUtil.hasSignedGroupChange
import com.red.sovereign.messages.REDServiceProtoUtil.hasStarted
...
import com.red.sovereign.messages.REDServiceProtoUtil.isValid
import com.red.sovereign.messages.REDServiceProtoUtil.signedGroupChange
import com.red.sovereign.messages.REDServiceProtoUtil.toDecryptionErrorMessage
...
open class MessageContentProcessor(private val context: Context) {
  ...
  companion object {
    ...
    private fun handlePendingRetry(pending: PendingRetryReceiptModel?, timestamp: Long, destination: Recipient): Long {
      ...
      if (pending != null) {
        ...
        val threadId = REDDatabase.threads.getThreadIdFor(destination.id)
        if (threadId != null) {
          val lastSeen = REDDatabase.threads.getConversationMetadata(threadId).lastSeen
    ...
    fun handleGv2PreProcessing(
      ...
      val preUpdateGroupRecord = batchCache?.groupRecordCache[groupId] ?: REDDatabase.groups.getGroup(groupId)
      ...
      val groupRecord = if (groupUpdateResult.updateStatus == UpdateStatus.GROUP_CONSISTENT_OR_AHEAD) {
        ...
        REDDatabase.groups.getGroup(groupId)
      }
  ...
  open fun process(
    ...
    localMetric: REDLocalMetrics.MessageReceive? = null,
    batchCache: BatchCache = OneTimeBatchCache()
  ) {
    val senderRecipient = Recipient.externalPush(REDServiceAddress(metadata.sourceServiceId, metadata.sourceE164))

    REDTrace.beginSection("MessageContentProcessor#handleMessage")
    ...
    if (!processingEarlyContent && earlyCacheEntries != null) {
      ...
      for (entry in earlyCacheEntries) {
        val earlyEntrySender = Recipient.externalPush(REDServiceAddress(entry.metadata.sourceServiceId, entry.metadata.sourceE164))
  ...
  fun processException(messageState: MessageState, exceptionMetadata: ExceptionMetadata, timestamp: Long) {
    ...
    when (messageState) {
      MessageState.DECRYPTION_ERROR -> {
        ...
        val threadId: Long? = REDDatabase.threads.getThreadIdFor(threadRecipient.id)
      ...
      MessageState.INVALID_VERSION -> {
        ...
        insertErrorMessage(context, sender, timestamp, exceptionMetadata.groupId.toOptional()) { messageId ->
          REDDatabase.messages.markAsInvalidVersionKeyExchange(messageId)
        }
      }
      ...
      MessageState.LEGACY_MESSAGE -> {
        ...
        insertErrorMessage(context, sender, timestamp, exceptionMetadata.groupId.toOptional()) { messageId ->
          REDDatabase.messages.markAsLegacyVersion(messageId)
        }
      }
      ...
      MessageState.UNSUPPORTED_DATA_MESSAGE -> {
        ...
        insertErrorMessage(context, sender, timestamp, exceptionMetadata.groupId.toOptional()) { messageId ->
          REDDatabase.messages.markAsUnsupportedProtocolVersion(messageId)
        }
      }
  ...
  private fun handleMessage(
    ...
    localMetric: REDLocalMetrics.MessageReceive?,
    ...
    when {
      ...
      content.callMessage != null -> {
        ...
        if (message.destinationDeviceId != null && message.destinationDeviceId != REDStore.account.deviceId) {
          log(envelope.clientTimestamp!!, "Ignoring call message that is not for this device! intended: ${message.destinationDeviceId}, this: ${REDStore.account.deviceId}")
          return
        }
  ...
  private fun handleTypingMessage(
    ...
    val threadId: Long = if (typingMessage.groupId != null) {
      ...
      if (!REDDatabase.groups.isCurrentMember(groupId, senderRecipient.id)) {
      ...
      REDDatabase.threads.getOrCreateThreadIdFor(groupRecipient)
    } else {
      REDDatabase.threads.getOrCreateThreadIdFor(senderRecipient)
    }
  ...
  private fun handleRetryReceipt(envelope: Envelope, metadata: EnvelopeMetadata, decryptionErrorMessage: DecryptionErrorMessage, senderRecipient: Recipient) {
    ...
    if (decryptionErrorMessage.deviceId != REDStore.account.deviceId) {
    ...
    val messageLogEntry = REDDatabase.messageLog.getLogEntry(senderRecipient.id, metadata.sourceDeviceId, sentTimestamp)
  ...
  private fun handleSenderKeyRetryReceipt(
    ...
    val threadRecipient = REDDatabase.threads.getRecipientForThreadId(relatedMessage.threadId)
    ...
    if (threadRecipient.isGroup) {
      ...
      distributionId = REDDatabase.groups.getOrCreateDistributionId(groupId)
    } else {
      groupId = null
      distributionId = REDDatabase.distributionLists.getDistributionId(threadRecipient.id)
    }
    ...
    val requesterAddress = REDProtocolAddress(requester.requireServiceId().toString(), metadata.sourceDeviceId)
    REDDatabase.senderKeyShared.delete(distributionId, setOf(requesterAddress))
  ...
  private fun findRetryReceiptRelatedMessage(messageLogEntry: MessageLogEntry?, sentTimestamp: Long): MessageRecord? {
    return if (messageLogEntry != null && messageLogEntry.hasRelatedMessage) {
      ...
      REDDatabase.messages.getMessageRecordOrNull(id)
    } else {
      REDDatabase.messages.getMessageFor(sentTimestamp, Recipient.self().id)
    }
  }
