...
import com.red.sovereign.jobs.SendRetryReceiptJob
...
import com.red.sovereign.messages.protocol.BufferedProtocolStore
...
import com.red.sovereign.util.asChain
...
import org.whispersystems.signalservice.api.crypto.REDServiceCipherResult
...
object MessageDecryptor {
  ...
  /**
   * Decrypts an envelope and provides a [Result]. This method has side effects, but all of them are limited to [REDDatabase].
   ...
   * These can vary from enqueueing jobs to inserting items into the [com.red.sovereign.database.PendingRetryReceiptCache].
   */
  fun decrypt(
    context: Context,
    ...
    val selfAci: ACI = REDStore.account.requireAci()
    val selfPni: PNI = REDStore.account.requirePni()
    ...
    if (destination == selfPni && Util.anyNotNull(envelope.sourceServiceId, envelope.sourceServiceIdBinary)) {
      ...
      if (sourceServiceId != null) {
        ...
        REDDatabase.recipients.markNeedsPniSignature(sender)
      } else {
    ...
    val localAddress = REDServiceAddress(destination, REDStore.account.e164)
    val cipher = REDServiceCipher(localAddress, REDStore.account.deviceId, bufferedStore, ReentrantSessionLock.INSTANCE, SealedSenderAccessUtil.getCertificateValidator())

    return try {
      val startTimeNanos = System.nanoTime()
      REDTrace.beginSection("MessageDecryptor#cipherDecrypt")
      val cipherResult: REDServiceCipherResult? = cipher.decrypt(envelope, serverDeliveredTimestamp)
      REDTrace.endSection()
      ...
      val validationResult: EnvelopeContentValidator.Result = EnvelopeContentValidator.validate(envelope, cipherResult.content, REDStore.account.aci!!, cipherResult.metadata.ciphertextMessageType)
      ...
      // SIGNAL_INHERITED: TODO We can move this to the "message processing" stage once we give it access to the envelope. But for now it'll stay here.
      if (envelope.report_spam_token != null && envelope.report_spam_token!!.size > 0) {
        ...
        REDDatabase.recipients.setReportingToken(sender, envelope.report_spam_token!!.toByteArray())
      }
  ...
  private fun buildResultForDecryptionError(
    ...
    if (ServiceId.parseOrNull(envelope.destinationServiceId, envelope.destinationServiceIdBinary) == REDStore.account.pni) {
      ...
      ContentHint.RESENDABLE -> {
        ...
        followUpOperations += FollowUpOperation {
          ...
          val threadId: Long? = if (groupId != null) {
            ...
            REDDatabase.threads.getThreadIdFor(groupRecipient.id)
          } else {
            REDDatabase.threads.getThreadIdFor(sender.id)
          }
  ...
  private fun handleSenderKeyDistributionMessage(envelope: Envelope, serviceId: ServiceId, deviceId: Int, message: SenderKeyDistributionMessage, senderKeyStore: SenderKeyStore) {
    ...
    val sender = REDProtocolAddress(serviceId.toString(), deviceId)
    REDGroupSessionBuilder(ReentrantSessionLock.INSTANCE, GroupSessionBuilder(senderKeyStore)).process(sender, message)
  }
  ...
  private fun handlePniSignatureMessage(envelope: Envelope, protocolStore: BufferedProtocolStore, aci: ACI, e164: String?, deviceId: Int, pniSignatureMessage: PniSignatureMessage) {
    ...
    if (REDDatabase.recipients.isAssociated(aci, pni)) {
      Log.i(TAG, "${logPrefix(envelope, aci)}[handlePniSignatureMessage] ACI ($aci) and PNI ($pni) are already associated.")
      return
    }

    val aciAddress = REDProtocolAddress(aci.toString(), deviceId)
    val pniAddress = REDProtocolAddress(pni.toString(), deviceId)
    ...
    if (pniIdentity == null) {
      ...
      if (deviceId != REDServiceAddress.DEFAULT_DEVICE_ID) {
        pniIdentity = protocolStore.getAciStore().getIdentity(REDProtocolAddress(pni.toString(), REDServiceAddress.DEFAULT_DEVICE_ID))
    ...
    if (pniIdentity.verifyAlternateIdentity(aciIdentity, pniSignatureMessage.signature!!.toByteArray())) {
      ...
      REDDatabase.recipients.getAndPossiblyMergePnpVerified(aci, pni, e164)
    } else {
  ...
  private fun logPrefix(envelope: Envelope, cipherResult: REDServiceCipherResult): String {
  ...
  private fun REDServiceCipherResult.toErrorMetadata(): ErrorMetadata {
  ...
  sealed interface Result {
    ...
    /** The envelope used an invalid version of the RED protocol. */
    ...
    /**
     * Indicates the that the [org.whispersystems.signalservice.internal.push.REDServiceProtos.DataMessage.getRequiredProtocolVersion]
