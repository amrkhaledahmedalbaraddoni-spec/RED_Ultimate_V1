...
import com.red.sovereign.database.model.databaseprotos.ChatColor
...
object StoryMessageProcessor {
  ...
  fun process(envelope: Envelope, content: Content, metadata: EnvelopeMetadata, senderRecipient: Recipient, threadRecipient: Recipient) {
    ...
    if (threadRecipient.isGroup && !REDDatabase.groups.isCurrentMember(threadRecipient.requireGroupId().requirePush(), senderRecipient.id)) {
      warn(envelope.clientTimestamp!!, "Dropping a group story from a user who's no longer a member.")
      return
    }

    if (threadRecipient.isGroup) {
      val groupRecord = REDDatabase.groups.getGroup(threadRecipient.requireGroupId()).orNull()
    ...
    try {
      ...
      insertResult = REDDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
