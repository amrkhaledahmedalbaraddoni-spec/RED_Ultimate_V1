...
internal class BadE164MigrationJob(
  ...
  override fun performMigration() {
    ...
    val invalidWithOtherIdentifiersCount = REDDatabase.recipients.removeInvalidE164sFromRecipientsWithOtherIdentifiers()
    ...
    val invalidWithNoMessagesCount = REDDatabase.recipients.deleteRecipientsWithInvalidE164sAndNoMessages()
    Log.d(TAG, "Deleted $invalidWithNoMessagesCount recipients with invalid e164's and no messages.")
    stopwatch.split("no-msg")

    val invalidEntries = REDDatabase.recipients.getRecipientsWithInvalidE164s()
    ...
    for (entry in invalidEntries) {
      ...
      val formattedNumber = REDE164Util.formatAsE164(entry.e164)
      ...
      REDDatabase.recipients.updateE164(entry.id, entry.e164)
