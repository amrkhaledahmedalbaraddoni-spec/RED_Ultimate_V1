...
internal class DuplicateE164MigrationJob(
    ...
    for ((_, entries) in dupes) {
      ...
      REDDatabase.rawDatabase.withinTransaction {
        ...
        for (entry in resolved.drop(1)) {
          ...
          REDDatabase.recipients.mergeForMigration(first.id, entry.id)
        }
      }
    }
  ...
  private fun attemptToGetRidOfE164(entries: List<E164Entry>): List<E164Entry> {
    ...
    for (invalidPrefix in entries) {
      if (REDDatabase.recipients.removeE164IfAnotherIdentifierIsPresent(invalidPrefix.id)) {
      ...
      if (REDDatabase.recipients.deleteRecipientIfItHasNoMessages(invalidPrefix.id)) {
