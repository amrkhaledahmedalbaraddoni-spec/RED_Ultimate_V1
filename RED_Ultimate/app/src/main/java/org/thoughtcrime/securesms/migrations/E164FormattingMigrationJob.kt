...
import com.red.sovereign.database.RecipientTable.Companion.ACI_COLUMN
import com.red.sovereign.database.RecipientTable.Companion.E164
import com.red.sovereign.database.RecipientTable.Companion.ID
import com.red.sovereign.database.RecipientTable.Companion.PNI_COLUMN
import com.red.sovereign.database.RecipientTable.Companion.TABLE_NAME
...
internal class E164FormattingMigrationJob(
  ...
  companion object {
    ...
    fun fixE164Formatting() {
      ...
      val e164sByRecipientId = REDDatabase.recipients.getAllE164sByRecipientId()
      val invalidE164s: MutableList<RecipientId> = mutableListOf()
      stopwatch.split("fetch")

      for ((id, e164) in e164sByRecipientId) {
        val formattedE164: String? = REDE164Util.formatNonShortCodeAsE164(e164)
        ...
        try {
          ...
          REDDatabase.recipients.setE164(id, formattedE164)
          ...
          val existing: Optional<RecipientId> = REDDatabase.recipients.getByE164(formattedE164)
          if (existing.isPresent) {
            Log.w(TAG, "Merging ${existing.get()} and $id", true)
            REDDatabase.rawDatabase.withinTransaction {
              REDDatabase.recipients.mergeForMigration(existing.get(), id)
            }
    ...
    private fun attemptToGetRidOfE164(entries: List<RecipientId>): Int {
      ...
      for (id in entries) {
        if (REDDatabase.recipients.removeE164IfAnotherIdentifierIsPresent(id)) {
        ...
        if (REDDatabase.recipients.deleteRecipientIfItHasNoMessages(id)) {
