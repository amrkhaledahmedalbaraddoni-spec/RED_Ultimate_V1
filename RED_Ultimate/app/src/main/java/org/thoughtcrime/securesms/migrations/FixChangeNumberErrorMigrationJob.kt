...
internal class FixChangeNumberErrorMigrationJob(
  ...
  override fun performMigration() {
    ...
    val pendingChangeNumberMetadata = REDStore.misc.pendingChangeNumberMetadata
    ...
    if (pendingChangeNumberMetadata.previousPni != REDStore.account.pni?.toByteString()) {
      Log.i(TAG, "Pending change number operation isn't for our current PNI, skipping.")
      return
    }

    when (val result = REDNetwork.account.whoAmI()) {
      is NetworkResult.Success<WhoAmIResponse> -> {
        val serverPni = result.result.pni?.let { ServiceId.PNI.parseOrNull(it) } ?: return

        if (result.result.number == REDStore.account.e164 && serverPni == REDStore.account.pni) {
  ...
  private fun pendingPniIdentityMatchesServer(metadata: PendingChangeNumberMetadata, pni: ServiceId.PNI): Boolean {
    ...
    val serverIdentityKey: IdentityKey = when (val profileResult = runBlocking { REDNetwork.profile.getUnversionedProfile(pni, null) }) {
