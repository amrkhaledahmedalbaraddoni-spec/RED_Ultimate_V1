...
import static com.red.sovereign.keyvalue.SettingsValues.THREAD_TRIM_ENABLED;
import static com.red.sovereign.keyvalue.SettingsValues.THREAD_TRIM_LENGTH;
...
public class TrimByLengthSettingsMigrationJob extends MigrationJob {
  ...
  void performMigration() throws Exception {
    ...
    if (preferences.contains(THREAD_TRIM_ENABLED)) {
      REDStore.settings().setThreadTrimByLengthEnabled(preferences.getBoolean(THREAD_TRIM_ENABLED, false));
      //noinspection ConstantConditions
      REDStore.settings().setThreadTrimLength(Integer.parseInt(preferences.getString(THREAD_TRIM_LENGTH, "500")));
