...
public class UserNotificationMigrationJob extends MigrationJob {
  ...
  void performMigration() {
    if (!REDStore.account().isRegistered()   ||
        REDStore.account().getE164() == null ||
        REDStore.account().getAci() == null)
    {
      Log.w(TAG, "Not registered! Skipping.");
      return;
    }

    if (!REDStore.settings().isNotifyWhenContactJoinsRED()) {
    ...
    Set<RecipientId>  registered               = REDDatabase.recipients().getRegistered();
    List<RecipientId> systemContacts           = REDDatabase.recipients().getSystemContacts();
