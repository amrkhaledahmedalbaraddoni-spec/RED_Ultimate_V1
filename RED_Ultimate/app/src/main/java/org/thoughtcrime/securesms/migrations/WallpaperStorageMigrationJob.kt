...
import com.red.sovereign.util.storage.FileStorage
import java.io.File
...
internal class WallpaperStorageMigrationJob(parameters: Parameters = Parameters.Builder().build()) : MigrationJob(parameters) {
  ...
  override fun performMigration() {
    ...
    val currentDefaultWallpaperUri = REDStore.wallpaper.currentRawWallpaper?.file_?.uri
    ...
    for (filename in wallpaperFileNames) {
      ...
      val inputStream = try {
        ...
        val updatedUserCount = REDDatabase.recipients.migrateWallpaperUri(
          legacyUri = legacyUri,
          newUri = null
        )
      ...
      val wallpaperAttachmentId = REDDatabase.attachments.insertWallpaper(inputStream)
      ...
      if (currentDefaultWallpaperUri == legacyUri.toString()) {
        ...
        REDStore.wallpaper.setRawWallpaperForMigration(Wallpaper(file_ = Wallpaper.File(uri = newUri.toString())))
      }
