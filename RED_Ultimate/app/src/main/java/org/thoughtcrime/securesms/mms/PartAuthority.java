...
import com.red.sovereign.providers.PartProvider;
...
public class PartAuthority {
      ...
      throws IOException
  {
    ...
    try {
      switch (match) {
      case PART_ROW:          return REDDatabase.attachments().getAttachmentStream(new PartUriParser(uri).getPartId(), 0);
      case STICKER_ROW:       return REDDatabase.stickers().getStickerStream(ContentUris.parseId(uri));
      ...
      case THUMBNAIL_ROW:     return REDDatabase.attachments().getAttachmentThumbnailStream(new PartUriParser(uri).getPartId(), 0);
  ...
  public static @Nullable String getAttachmentFileName(@NonNull Context context, @NonNull Uri uri) {
    ...
    switch (match) {
    case PART_ROW:
      Attachment attachment = REDDatabase.attachments().getAttachment(new PartUriParser(uri).getPartId());
  ...
  public static @Nullable TransformProperties getAttachmentTransformProperties(@NonNull Uri uri) {
    ...
    switch (match) {
      case PART_ROW:
        return REDDatabase.attachments().getTransformProperties(new PartUriParser(uri).getPartId());
      default:
        return null;
    }
  }
