...
class REDWebSocketHealthMonitor(
  private val sleepTimer: SleepTimer,
  private val sendKeepAlives: Boolean = true
) : HealthMonitor {

  companion object {
    private val TAG = Log.tag(REDWebSocketHealthMonitor::class)
  ...
  private var webSocket: REDWebSocket? = null
  ...
  fun monitor(webSocket: REDWebSocket) {
    executor.execute {
    ...
    executor.execute {
      ...
      when (connectionState) {
        ...
        WebSocketConnectionState.CONNECTED -> {
          if (webSocket is REDWebSocket.AuthenticatedWebSocket) {
  ...
  override fun onMessageError(status: Int, isIdentifiedWebSocket: Boolean) {
    executor.execute {
      if (status == 499 && !REDStore.misc.isClientDeprecated) {
  ...
  override fun onReceivedAlerts(alerts: Array<out String>, isIdentifiedWebSocket: Boolean) {
    ...
    executor.execute {
      REDStore.account.hasInactivePrimaryDeviceAlert = REDStore.account.isLinkedDevice && alerts.contains(ALERT_IDLE_PRIMARY_DEVICE)
    }
  }
