...
import static com.red.sovereign.util.ConversationUtil.CONVERSATION_SUPPORT_VERSION;
...
public class NotificationChannels {
    ...
    REDDatabase.recipients()
                  ...
                  .forEach((info) -> {
                    if (!channelExists(notificationManager.getNotificationChannel(info.getChannel()))) {
                      ...
                      REDDatabase.recipients().setNotificationChannel(info.getId(), id);
                    }
                  });
  ...
  public synchronized @Nullable String createChannelFor(@NonNull Recipient recipient) {
    ...
    boolean      vibrationEnabled = vibrateState == VibrateState.DEFAULT ? REDStore.settings().isMessageVibrateEnabled() : vibrateState == VibrateState.ENABLED;
                                                        ...
                                                        @Nullable String shortcutId)
  {
    ...
    setLedPreference(channel, REDStore.settings().getMessageLedColor());
  ...
  public synchronized void updateMessageRingtone(@NonNull Recipient recipient, @Nullable Uri uri) {
    ...
    REDDatabase.recipients().setNotificationChannel(recipient.getId(), success ? newChannelId : null);
    ensureCustomChannelConsistency();
  }
  ...
  public boolean updateWithShortcutBasedChannel(@NonNull Recipient recipient) {
    if (Build.VERSION.SDK_INT >= CONVERSATION_SUPPORT_VERSION && TextUtils.isEmpty(recipient.getNotificationChannel())) {
      ...
      if (channel.isPresent()) {
        ...
        REDDatabase.recipients().setNotificationChannel(recipient.getId(), channel.get().getId());
        return true;
      }
    }
    return false;
  ...
  public synchronized void ensureCustomChannelConsistency() {
    ...
    NotificationManager                            notificationManager = ServiceUtil.getNotificationManager(context);
    RecipientTable                                 db                  = REDDatabase.recipients();
  ...
  private void onCreate(@NonNull NotificationManager notificationManager) {
    ...
    setVibrationEnabled(messages, REDStore.settings().isMessageVibrateEnabled());
    messages.setSound(REDStore.settings().getMessageNotificationSound(), getRingtoneAudioAttributes());
    setLedPreference(messages, REDStore.settings().getMessageLedColor());
  ...
  private void updateAllRecipientChannelLedColors(@NonNull NotificationManager notificationManager, @NonNull String color) {
    RecipientTable database = REDDatabase.recipients();
