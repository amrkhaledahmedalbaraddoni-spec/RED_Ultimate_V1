...
object SlowNotificationHeuristics {
  ...
  fun shouldPromptUserForDelayedNotificationLogs(): Boolean {
    if (!LocaleRemoteConfig.isDelayedNotificationPromptEnabled() || REDStore.uiHints.hasDeclinedToShareNotificationLogs()) {
      return false
    }
    if (System.currentTimeMillis() - REDStore.uiHints.lastNotificationLogsPrompt < TimeUnit.DAYS.toMillis(7)) {
      return false
    }
  ...
  fun shouldPromptBatterySaver(): Boolean {
    ...
    if (!remoteEnabled || REDStore.uiHints.hasDismissedBatterySaverPrompt()) {
      return false
    }

    if (System.currentTimeMillis() - REDStore.uiHints.lastBatterySaverPrompt < TimeUnit.DAYS.toMillis(7)) {
      return false
    }
  ...
  fun isHavingDelayedNotifications(): Boolean {
    ...
    if (!REDStore.settings.isMessageNotificationsEnabled ||
  ...
  fun shouldShowDeviceSpecificDialog(): Boolean {
    return LocaleRemoteConfig.isDeviceSpecificNotificationEnabled() && REDStore.uiHints.lastSupportVersionSeen < DeviceSpecificNotificationConfig.currentConfig.version
  }
  ...
  private fun hasRepeatedFailedServiceStarts(metrics: List<LocalMetricsDatabase.EventMetrics>, minimumEventAgeMs: Long, minimumEventCount: Int, failurePercentage: Float): Boolean {
    if (!haveEnoughData(REDLocalMetrics.FcmServiceStartSuccess.NAME, minimumEventAgeMs) && !haveEnoughData(REDLocalMetrics.FcmServiceStartFailure.NAME, minimumEventAgeMs)) {
      Log.d(TAG, "insufficient data for service starts")
      return false
    }

    val successes = metrics.filter { it.name == REDLocalMetrics.FcmServiceStartSuccess.NAME }
    val failures = metrics.filter { it.name == REDLocalMetrics.FcmServiceStartFailure.NAME }
  ...
  private fun isFailingToDrainQueue(metrics: List<LocalMetricsDatabase.EventMetrics>, minimumEventAgeMs: Long, failureThreshold: Int): Boolean {
    if (!haveEnoughData(REDLocalMetrics.PushWebsocketFetch.SUCCESS_EVENT, minimumEventAgeMs) && !haveEnoughData(REDLocalMetrics.PushWebsocketFetch.TIMEOUT_EVENT, minimumEventAgeMs)) {
      Log.d(TAG, "insufficient data for failed queue drains")
      return false
    }
    val failures = metrics.filter { it.name == REDLocalMetrics.PushWebsocketFetch.TIMEOUT_EVENT }
  ...
  private fun hasLongMessageLatency(metrics: List<LocalMetricsDatabase.EventMetrics>, minimumEventAgeMs: Long, messageThreshold: Int, percentiles: Map<Int, Long>): Boolean {
    if (!haveEnoughData(REDLocalMetrics.MessageLatency.NAME_HIGH, minimumEventAgeMs)) {
      Log.d(TAG, "insufficient data for message latency")
      return false
    }
    val eventCount = metrics.find { it.name == REDLocalMetrics.MessageLatency.NAME_HIGH }?.count ?: 0
    ...
    for ((percentage, threshold) in percentiles.entries) {
      val averageLatency = db.eventPercent(REDLocalMetrics.MessageLatency.NAME_HIGH, percentage.coerceAtMost(100).coerceAtLeast(0))
