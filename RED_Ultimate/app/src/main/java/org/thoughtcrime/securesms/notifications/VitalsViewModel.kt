...
import com.red.sovereign.crash.CrashConfig
import com.red.sovereign.database.LogDatabase
import com.red.sovereign.dependencies.AppDependencies
import com.red.sovereign.keyvalue.REDStore
import com.red.sovereign.notifications.DeviceSpecificNotificationConfig.ShowCondition
import com.red.sovereign.util.ConnectivityWarning
...
class VitalsViewModel(private val context: Application) : AndroidViewModel(context) {
  ...
  private fun checkHeuristics(): Single<State> {
    return Single.fromCallable {
      ...
      val timeSinceLastConnection = System.currentTimeMillis() - REDStore.misc.lastWebSocketConnectTime
      val timeSinceLastConnectionWarning = System.currentTimeMillis() - REDStore.misc.lastConnectivityWarningTime
      val connectedToWebSocket = AppDependencies.webSocketObserver.value == WebSocketConnectionState.CONNECTED
      val isRegistered = REDStore.account.isRegistered && !TextSecurePreferences.isUnauthorizedReceived(context)
      ...
      if (LogDatabase.getInstance(context).crashes.anyMatch(patterns = CrashConfig.patterns, promptThreshold = System.currentTimeMillis() - 14.days.inWholeMilliseconds)) {
        val timeSinceLastPrompt = System.currentTimeMillis() - REDStore.uiHints.lastCrashPrompt
