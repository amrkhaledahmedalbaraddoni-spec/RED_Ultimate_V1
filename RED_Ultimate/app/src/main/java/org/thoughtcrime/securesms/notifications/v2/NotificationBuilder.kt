...
sealed class NotificationBuilder(protected val context: Context) {
  ...
  private val privacy: NotificationPrivacyPreference = REDStore.settings.messageNotificationsPrivacy
  ...
  fun addReplyActions(conversation: NotificationConversation) {
    if (privacy.isDisplayMessage && isNotLocked && !conversation.recipient.isPushV1Group && RecipientUtil.isMessageRequestAccepted(conversation.recipient)) {
      if (conversation.recipient.isPushV2Group) {
        val group: Optional<GroupRecord> = REDDatabase.groups.getGroup(conversation.recipient.requireGroupId())
  ...
  fun setLights() {
    val ledColor: String = REDStore.settings.messageLedColor

    if (ledColor != "none") {
      var blinkPattern = REDStore.settings.messageLedBlinkPattern
  ...
  private class NotificationBuilderCompat(context: Context) : NotificationBuilder(context) {
    ...
    override fun setAlarms(recipient: Recipient?) {
      ...
      val defaultRingtone: Uri = REDStore.settings.messageNotificationSound
      val defaultVibrate: Boolean = REDStore.settings.isMessageVibrateEnabled
