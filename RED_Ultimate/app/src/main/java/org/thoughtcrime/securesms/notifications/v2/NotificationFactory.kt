...
import com.red.sovereign.stories.my.MyStoriesActivity
...
object NotificationFactory {
  ...
  private fun notifyInThread(context: Context, recipient: Recipient, lastAudibleNotification: Long) {
    if (!NotificationChannels.getInstance().areNotificationsEnabled() ||
      !REDStore.settings.isMessageNotificationsInChatSoundsEnabled ||
    ...
    val uri: Uri = if (NotificationChannels.supported()) {
      ...
      recipient.messageRingtone ?: REDStore.settings.messageNotificationSound
    }
  ...
  fun notifyStoryDeliveryFailed(context: Context, recipient: Recipient, thread: ConversationId) {
    ...
    val contentTitle = if (REDStore.settings.messageNotificationsPrivacy.isDisplayContact) {
    ...
    val largeIcon = if (REDStore.settings.messageNotificationsPrivacy.isDisplayContact) {
    ...
    try {
      ...
      if (threadRecipient != null) {
        REDExecutors.BOUNDED.execute {
          REDDatabase.recipients.setMessageRingtone(threadRecipient.id, null)
