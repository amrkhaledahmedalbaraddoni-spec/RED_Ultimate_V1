...
import com.red.sovereign.database.ThreadBodyUtil
...
import com.red.sovereign.util.hasPoll
import com.red.sovereign.util.hasPollTerminate
import com.red.sovereign.util.hasSharedContact
import com.red.sovereign.util.hasSticker
import com.red.sovereign.util.isMediaMessage
...
sealed class NotificationItem(val threadRecipient: Recipient, protected val record: MessageRecord) : Comparable<NotificationItem> {
  ...
  fun getStyledPrimaryText(context: Context, trimmed: Boolean = false): CharSequence {
    return if (REDStore.settings.messageNotificationsPrivacy.isDisplayNothing) {
  ...
  fun getPersonUri(context: Context): String? {
    return if (REDStore.settings.messageNotificationsPrivacy.isDisplayContact && authorRecipient.isSystemContact && Permissions.hasAny(context, Manifest.permission.READ_CONTACTS, Manifest.permission.WRITE_CONTACTS)) {
  ...
  fun getPrimaryText(context: Context): CharSequence {
    return if (REDStore.settings.messageNotificationsPrivacy.isDisplayMessage) {
  ...
  fun getInboxLine(context: Context): CharSequence? {
    return when {
      REDStore.settings.messageNotificationsPrivacy.isDisplayNothing -> null
...
class MessageNotification(threadRecipient: Recipient, record: MessageRecord, val isUnread: Boolean = false) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return if (thread.groupStoryId != null) {
      REDDatabase.messages.getMessagePositionInConversation(thread.threadId, thread.groupStoryId, record.dateReceived)
    } else {
      -1
    }
  }
  ...
  override fun getThumbnailInfo(context: Context): ThumbnailInfo {
    return if (REDStore.settings.messageNotificationsPrivacy.isDisplayMessage && !KeyCachingService.isLocked(context)) {
...
class ReactionNotification(threadRecipient: Recipient, record: MessageRecord, val reaction: ReactionRecord) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return REDDatabase.messages.getMessagePositionInConversation(thread.threadId, thread.groupStoryId ?: 0L, record.dateReceived)
  }
...
class VoteNotification(threadRecipient: Recipient, record: MessageRecord, val vote: PollVote) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return REDDatabase.messages.getMessagePositionInConversation(threadId = thread.threadId, groupStoryId = 0L, receivedTimestamp = record.dateReceived)
  }
