...
import com.red.sovereign.BiometricDeviceAuthentication;
import com.red.sovereign.BiometricDeviceLockContract;
...
import com.red.sovereign.payments.preferences.PaymentsHomeFragmentDirections;
...
public class ConfirmPaymentFragment extends BottomSheetDialogFragment {
  ...
  private static CharSequence mono(Context context, CharSequence address) {
    ...
    spannable.setSpan(new TextAppearanceSpan(context, org.signal.core.ui.R.style.TextAppearance_RED_Mono),
                      0,
                      address.length(),
  ...
  private boolean isPaymentLockEnabled(Context context) {
    return REDStore.payments().isPaymentLockEnabled() && ServiceUtil.getKeyguardManager(context).isKeyguardSecure();
  }
