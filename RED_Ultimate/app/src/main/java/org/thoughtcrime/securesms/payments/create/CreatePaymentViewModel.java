package com.red.sovereign.payments.create;
...
import com.red.sovereign.payments.Balance;
...
public class CreatePaymentViewModel extends ViewModel {
  ...
  private CreatePaymentViewModel(@NonNull PayeeParcelable payee, @Nullable CharSequence note) {
    this.payee            = payee;
    this.spendableBalance = Transformations.map(REDStore.payments().liveMobileCoinBalance(), Balance::getTransferableAmount);
    ...
    this.enclaveFailure   = LiveDataUtil.mapDistinct(REDStore.payments().enclaveFailure(), isFailure -> isFailure);
    ...
    LiveData<Optional<CurrencyExchange.ExchangeRate>> liveExchangeRate = LiveDataUtil.mapAsync(REDStore.payments().liveCurrentCurrency(),
                                                                                               currency -> {
                                                                                                 try {
  ...
  private @NonNull InputState updateAmount(@NonNull Context context, @NonNull InputState inputState, @NonNull AmountKeyboardGlyph glyph) {
    switch (inputState.getInputTarget()) {
      case FIAT_MONEY:
        return updateFiatAmount(context, inputState, glyph, REDStore.payments().currentCurrency());
      case MONEY:
