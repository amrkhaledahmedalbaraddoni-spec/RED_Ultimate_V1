...
import com.red.sovereign.PaymentPreferencesDirections;
import com.red.sovereign.R;
import com.red.sovereign.banner.BannerManager;
import com.red.sovereign.banner.banners.EnclaveFailureBanner;
...
import com.red.sovereign.payments.MoneyView;
import com.red.sovereign.payments.backup.RecoveryPhraseStates;
import com.red.sovereign.payments.backup.confirm.PaymentsRecoveryPhraseConfirmFragment;
...
public class PaymentsHomeFragment extends LoggingFragment {
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    long    paymentLockTimestamp = REDStore.payments().getPaymentLockTimestamp();
    boolean enablePaymentLock    = PaymentsHomeFragmentArgs.fromBundle(getArguments()).getEnablePaymentLock();
    boolean showPaymentLock      = REDStore.payments().getPaymentLockSkipCount() < MAX_PAYMENT_LOCK_SKIP_COUNT &&
    ...
    if (enablePaymentLock && showPaymentLock) {
      ...
      REDStore.payments().setPaymentLockTimestamp(waitUntil);
  ...
  private void setSkipCount() {
      int skipCount = REDStore.payments().getPaymentLockSkipCount();
      REDStore.payments().setPaymentLockSkipCount(++skipCount);
  }
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    addMoney.setOnClickListener(v -> {
      ...
      } else if (REDStore.payments().getPaymentsAvailability().isSendAllowed()) {
    ...
    viewModel.getBalance().observe(getViewLifecycleOwner(), balanceAmount -> {
      ...
      if (REDStore.payments().getShowSaveRecoveryPhrase() &&
          !REDStore.payments().getUserConfirmedMnemonic() &&
        ...
        REDStore.payments().setShowSaveRecoveryPhrase(false);
      }
    });
    ...
    viewModel.getPaymentStateEvents().observe(getViewLifecycleOwner(), paymentStateEvent -> {
      ...
      switch (paymentStateEvent) {
        ...
        case DEACTIVATE_WITH_BALANCE:
          ...
          break;
        case ACTIVATED:
          if (!REDStore.payments().isPaymentLockEnabled()) {
  ...
  private boolean onMenuItemSelected(@NonNull MenuItem item) {
    if (item.getItemId() == R.id.payments_home_fragment_menu_transfer_to_exchange) {
      ...
      SafeNavigation.safeNavigate(NavHostFragment.findNavController(this),
                                  PaymentsHomeFragmentDirections.actionPaymentsHomeToPaymentsBackup().setRecoveryPhraseState(REDStore.payments().isMnemonicConfirmed() ?
