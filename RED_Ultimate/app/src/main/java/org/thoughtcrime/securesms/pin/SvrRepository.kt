...
import com.red.sovereign.jobs.Svr3MirrorJob
...
object SvrRepository {
        ...
        when (val response: RestoreResponse = operation()) {
          is RestoreResponse.Success -> {
            ...
            when (implementation.svrVersion) {
              SvrVersion.SVR2 -> REDStore.svr.appendSvr2AuthTokenToList(response.authorization.asBasic())
              SvrVersion.SVR3 -> REDStore.svr.appendSvr3AuthTokenToList(response.authorization.asBasic())
            }
        ...
        when (val response: RestoreResponse = implementation.restoreDataPostRegistration(userPin)) {
          is RestoreResponse.Success -> {
            ...
            REDStore.registration.localRegistrationMetadata?.let { metadata ->
              REDStore.registration.localRegistrationMetadata = metadata.copy(masterKey = response.masterKey.serialize().toByteString(), pin = userPin)
            }

            REDStore.svr.masterKeyForInitialDataRestore = response.masterKey
            REDStore.svr.setPin(userPin)
            REDStore.svr.isRegistrationLockEnabled = false
            REDStore.pin.resetPinReminders()
            REDStore.pin.keyboardType = pinKeyboardType
  ...
  fun setPin(userPin: String, keyboardType: PinKeyboardType): BackupResponse {
    return operationLock.withLock {
      val masterKey: MasterKey = REDStore.svr.masterKey
      ...
      if (overallResponse is BackupResponse.Success) {
        ...
        responses
          ...
          .forEach {
            when (it.svrVersion) {
              SvrVersion.SVR2 -> REDStore.svr.appendSvr2AuthTokenToList(it.authorization.asBasic())
              SvrVersion.SVR3 -> REDStore.svr.appendSvr3AuthTokenToList(it.authorization.asBasic())
            }
          }

        REDStore.pin.keyboardType = keyboardType
        REDStore.pin.resetPinReminders()
        ...
        if (hasNoRegistrationLock) {
          REDStore.svr.onPinCreateFailure()
        }
      }
  ...
  fun onRegistrationComplete(
    ...
    operationLock.withLock {
      ...
      if (masterKey != null && userPin != null) {
        ...
        REDStore.svr.clearRegistrationLockAndPin()
        REDStore.storageService.needsAccountRestore = true
      } else {
  ...
  fun optOutOfPin(rotateAep: Boolean) {
    operationLock.withLock {
      ...
      if (rotateAep) {
        REDStore.account.rotateAccountEntropyPool(AccountEntropyPool.generate())
  ...
  fun enableRegistrationLockForUserWithPin() {
    operationLock.withLock {
      check(REDStore.svr.hasPin() && !REDStore.svr.hasOptedOut()) { "Must have a PIN to set a registration lock!" }

      Log.i(TAG, "[enableRegistrationLockForUserWithPin] Enabling registration lock.", true)
      NetworkResultUtil.toBasicLegacy(REDNetwork.account.enableRegistrationLock(REDStore.svr.masterKey.deriveRegistrationLock()))
      REDStore.svr.isRegistrationLockEnabled = true
  ...
  fun disableRegistrationLockForUserWithPin() {
    operationLock.withLock {
      check(REDStore.svr.hasPin() && !REDStore.svr.hasOptedOut()) { "Must have a PIN to disable registration lock!" }

      Log.i(TAG, "[disableRegistrationLockForUserWithPin] Disabling registration lock.", true)
      NetworkResultUtil.toBasicLegacy(REDNetwork.account.disableRegistrationLock())
      REDStore.svr.isRegistrationLockEnabled = false
  ...
  fun refreshAndStoreAuthorization() {
    try {
      var newToken = if (Svr3Migration.shouldWriteToSvr3) {
        ...
        REDStore.svr.appendSvr3AuthTokenToList(credentials.asBasic())
      } else {
        false
      }
      ...
      newToken = newToken ||
        if (Svr3Migration.shouldWriteToSvr2) {
          ...
          REDStore.svr.appendSvr2AuthTokenToList(credentials.asBasic())
        } else {
          false
        }

      if (newToken && REDStore.svr.hasPin()) {
        BackupManager(AppDependencies.application).dataChanged()
      }
    } catch (e: Throwable) {
      if (e is IOException) {
  ...
  private val hasNoRegistrationLock: Boolean
    get() {
      return !REDStore.svr.isRegistrationLockEnabled &&
        !REDStore.svr.hasPin() &&
        !REDStore.svr.hasOptedOut()
    }
}
