...
import com.red.sovereign.backup.BackupDialog;
import com.red.sovereign.backup.BackupEvent;
import com.red.sovereign.dependencies.AppDependencies;
import com.red.sovereign.jobs.LocalBackupJob;
import com.red.sovereign.keyvalue.REDStore;
import com.red.sovereign.preferences.widgets.UpgradeLocalBackupCard;
...
public class BackupsPreferenceFragment extends Fragment {
  ...
  private void setBackupFolderName() {
    ...
    if (BackupUtil.canUserAccessBackupDirectory(requireContext())) {
      ...
      {
        Uri backupUri = Objects.requireNonNull(REDStore.settings().getREDBackupDirectory());

        folder.setVisibility(View.VISIBLE);
        folderName.setText(StorageUtil.getDisplayPath(requireContext(), backupUri));
      } else if (StorageUtil.canWriteInREDStorageDir()) {
        try {
          folder.setVisibility(View.VISIBLE);
  ...
  private void setUpdateState() {
    if (REDStore.settings().isBackupEnabled() && Environment.Backups.isNewFormatSupportedForLocalBackup()) {
  ...
  private void pickTime() {
    ...
    final MaterialTimePicker timePickerFragment = new MaterialTimePicker.Builder()
        ...
        .setHour(REDStore.settings().getBackupHour())
        .setMinute(REDStore.settings().getBackupMinute())
    ...
    timePickerFragment.addOnPositiveButtonClickListener(v -> {
      ...
      REDStore.settings().setBackupSchedule(hour, minute);
      updateTimeLabel();
  ...
  private void updateTimeLabel() {
    final int backupHour   = REDStore.settings().getBackupHour();
    final int backupMinute = REDStore.settings().getBackupMinute();
  ...
  private void updateToggle() {
    boolean userUnregistered          = TextSecurePreferences.isUnauthorizedReceived(AppDependencies.getApplication()) || !REDStore.account().isRegistered();
    boolean clientDeprecated          = REDStore.misc().isClientDeprecated();
    boolean legacyLocalBackupsEnabled = REDStore.settings().isBackupEnabled() && BackupUtil.canUserAccessBackupDirectory(AppDependencies.getApplication());
