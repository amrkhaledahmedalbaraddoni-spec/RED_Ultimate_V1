...
import com.red.sovereign.contactshare.SimpleTextWatcher;
...
import com.red.sovereign.util.views.CircularProgressMaterialButton;
import com.red.sovereign.util.views.LearnMoreTextView;
...
public class EditProxyFragment extends Fragment {
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    this.proxyText.setText(Optional.ofNullable(REDStore.proxy().getProxy()).map(REDProxy::getHost).orElse(""));
    this.proxySwitch.setChecked(REDStore.proxy().isProxyEnabled());
    ...
    description.setOnLinkClickListener(v -> CommunicationActions.openBrowserLink(requireContext(), "https://support.red.local/hc/articles/360056052052"));
  ...
  private void presentProxyState(@NonNull WebSocketConnectionState proxyState) {
    if (REDStore.proxy().getProxy() != null) {
      switch (proxyState) {
        case DISCONNECTED:
        case DISCONNECTING:
        case CONNECTING:
  ...
  private void presentEvent(@NonNull EditProxyViewModel.Event event) {
    switch (event) {
      case PROXY_SUCCESS:
        ...
        proxyText.setText(Optional.ofNullable(REDStore.proxy().getProxy()).map(REDProxy::getHost).orElse(""));
  ...
  private void onShareClicked() {
    String link = REDProxyUtil.generateProxyUrl(proxyText.getText().toString());
  ...
  private void onProxyTextChanged(@NonNull String text) {
    if (Util.isEmpty(text)) {
      ...
      String trueHost = REDProxyUtil.convertUserEnteredAddressToHost(proxyText.getText().toString());
      if (REDStore.proxy().isProxyEnabled() && trueHost.equals(REDStore.proxy().getProxyHost())) {
        proxyStatus.setVisibility(View.VISIBLE);
      } else {
        proxyStatus.setVisibility(View.INVISIBLE);
      }
