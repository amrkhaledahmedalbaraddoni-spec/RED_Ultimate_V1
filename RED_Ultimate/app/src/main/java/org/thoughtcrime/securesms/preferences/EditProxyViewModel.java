...
public class EditProxyViewModel extends ViewModel {
  ...
  public EditProxyViewModel() {
    ...
    this.pipeState = REDStore.account().getE164() == null ? Flowable.empty()
  ...
  void onToggleProxy(boolean enabled, String text) {
    if (enabled) {
      REDProxy currentProxy = REDStore.proxy().getProxy();

      if (currentProxy != null && !Util.isEmpty(currentProxy.getHost())) {
        REDProxyUtil.enableProxy(currentProxy);
      }
      uiState.onNext(UiState.ALL_ENABLED);
    } else if (Util.isEmpty(text)) {
        REDProxyUtil.disableAndClearProxy();
        uiState.onNext(UiState.ALL_DISABLED);
    } else {
        REDProxyUtil.disableProxy();
        uiState.onNext(UiState.ALL_DISABLED);
    }
  }

  public void onSaveClicked(@NonNull String host) {
    String trueHost = REDProxyUtil.convertUserEnteredAddressToHost(host);

    saveState.onNext(SaveState.IN_PROGRESS);

    REDExecutors.BOUNDED.execute(() -> {
      REDProxyUtil.enableProxy(new REDProxy(trueHost, 443));

      boolean success = REDProxyUtil.testWebsocketConnection(TimeUnit.SECONDS.toMillis(10));
