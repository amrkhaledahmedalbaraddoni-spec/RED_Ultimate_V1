...
import com.red.sovereign.profiles.manage.UsernameRepository.UsernameDeleteResult
import com.red.sovereign.profiles.manage.UsernameRepository.UsernameSetResult
import com.red.sovereign.util.NetworkUtil
import com.red.sovereign.util.UsernameUtil.InvalidReason
import com.red.sovereign.util.UsernameUtil.checkDiscriminator
import com.red.sovereign.util.UsernameUtil.checkNickname
...
internal class UsernameEditViewModel private constructor(private val mode: UsernameEditMode) : ViewModel() {
  ...
  private val uiState: RxStore<State> = RxStore(
    defaultValue = State(
      ...
      usernameState = REDStore.account.username?.let { UsernameState.Set(Username(it)) } ?: UsernameState.NoUsername
    ),
  ...
  private val stateMachineStore = RxStore<UsernameEditStateMachine.State>(
    defaultValue = UsernameEditStateMachine.NoUserEntry(
      nickname = REDStore.account.username?.split(Usernames.DELIMITER)?.first() ?: "",
      discriminator = REDStore.account.username?.split(Usernames.DELIMITER)?.last() ?: "",
  ...
  init {
    ...
    if (mode == UsernameEditMode.RECOVERY) {
      onNicknameUpdated(REDStore.account.username?.split(Usernames.DELIMITER)?.first() ?: "")
      onDiscriminatorUpdated(REDStore.account.username?.split(Usernames.DELIMITER)?.last() ?: "")
    }
  }
  ...
  fun onNicknameUpdated(nickname: String) {
    uiState.update { state: State ->
      if (nickname.isBlank() && REDStore.account.username != null) {
        return@update State(
  ...
  fun onDiscriminatorUpdated(discriminator: String) {
    uiState.update { state: State ->
      if (discriminator.isBlank() && REDStore.account.username != null) {
        return@update State(
          buttonState = ButtonState.DELETE,
          usernameStatus = UsernameStatus.NONE,
          usernameState = UsernameState.NoUsername
  ...
  fun onUsernameSkipped() {
    REDStore.uiHints.markHasSetOrSkippedUsernameCreation()
  ...
  fun isSameUsernameRecovery(): Boolean {
    ...
    return mode == UsernameEditMode.RECOVERY &&
      ...
      usernameState.requireUsername().username.lowercase() == REDStore.account.username?.lowercase()
  }
  ...
  fun onUsernameSubmitted(userConfirmedResetOk: Boolean) {
    ...
    if (!isCaseChange && REDStore.account.username.isNotNullOrBlank() && !userConfirmedResetOk) {
    ...
    if (usernameState.requireUsername().username == REDStore.account.username && mode != UsernameEditMode.RECOVERY) {
  ...
  private fun isCaseChange(state: UsernameEditStateMachine.State): Boolean {
    ...
    val oldLower = REDStore.account.username?.split(Usernames.DELIMITER)?.firstOrNull()?.lowercase()
  ...
  private fun onUsernameStateUpdateDebounced(state: UsernameEditStateMachine.State) {
    ...
    if (isCaseChange(state)) {
      val discriminator = REDStore.account.username?.split(Usernames.DELIMITER)?.lastOrNull() ?: error("Unexpected case change, no discriminator!")
      uiState.update {
        State(
