...
import com.red.sovereign.components.settings.app.usernamelinks.main.UsernameLinkResetResult
import com.red.sovereign.database.REDDatabase
import com.red.sovereign.dependencies.AppDependencies
import com.red.sovereign.jobs.MultiDeviceUsernameChangeSyncJob
import com.red.sovereign.keyvalue.AccountValues
import com.red.sovereign.keyvalue.REDStore
import com.red.sovereign.net.REDNetwork
import com.red.sovereign.profiles.manage.UsernameRepository.confirmUsernameAndCreateNewLink
import com.red.sovereign.profiles.manage.UsernameRepository.reserveUsername
import com.red.sovereign.profiles.manage.UsernameRepository.updateUsernameDisplayForCurrentLink
...
object UsernameRepository {
  ...
  private val accountManager: REDServiceAccountManager get() = AppDependencies.signalServiceAccountManager
  ...
  fun reclaimUsernameIfNecessary(): UsernameReclaimResult {
    ...
    when (result) {
      ...
      UsernameReclaimResult.PERMANENT_ERROR -> {
        ...
        REDStore.account.usernameSyncState = AccountValues.UsernameSyncState.USERNAME_AND_LINK_CORRUPTED
        REDStore.misc.needsUsernameRestore = false
      }
  ...
  fun createOrResetUsernameLink(): Single<UsernameLinkResetResult> {
    ...
    val usernameString = REDStore.account.username
    ...
    return Single
      .fromCallable {
        ...
        when (val result = REDNetwork.account.createUsernameLink(usernameLink)) {
          is NetworkResult.Success -> {
            REDStore.account.usernameLink = result.result

            if (REDStore.account.usernameSyncState == AccountValues.UsernameSyncState.LINK_CORRUPTED) {
  ...
  fun fetchUsernameAndAciFromLink(url: String): Single<UsernameLinkConversionResult> {
    ...
    return Single
      .fromCallable {
        val username = when (val result = REDNetwork.username.getDecryptedUsernameFromLinkServerIdAndEntropy(components.serverId, components.entropy)) {
        ...
        when (val result = REDNetwork.username.getAciByUsername(username)) {
          is RequestResult.Success -> {
            result.result?.let {
  ...
  fun fetchAciForUsername(usernameString: String): UsernameAciFetchResult {
    ...
    return when (val result = REDNetwork.username.getAciByUsername(username)) {
      is RequestResult.Success -> {
        result.result?.let {
  ...
  fun onUsernameConsistencyValidated() {
    ...
    if (REDStore.account.usernameSyncErrorCount > 0) {
      Log.i(TAG, "Username consistency validated. There were previously ${REDStore.account.usernameSyncErrorCount} error(s).")
      REDStore.account.usernameSyncErrorCount = 0
    }
  }
  ...
  fun onUsernameMismatchDetected() {
    REDStore.account.usernameSyncErrorCount++

    if (REDStore.account.usernameSyncErrorCount >= USERNAME_SYNC_ERROR_THRESHOLD) {
      Log.w(TAG, "We've now seen ${REDStore.account.usernameSyncErrorCount} mismatches in a row. Marking username and link as corrupted.")
      REDStore.account.usernameSyncState = AccountValues.UsernameSyncState.USERNAME_AND_LINK_CORRUPTED
      REDStore.account.usernameSyncErrorCount = 0
    } else {
      Log.w(TAG, "Username mismatch reported. At ${REDStore.account.usernameSyncErrorCount} / $USERNAME_SYNC_ERROR_THRESHOLD tries.")
    }
  }
  ...
  fun onUsernameLinkMismatchDetected() {
    ...
    if (REDStore.account.usernameSyncErrorCount >= USERNAME_SYNC_ERROR_THRESHOLD) {
      Log.w(TAG, "We've now seen ${REDStore.account.usernameSyncErrorCount} mismatches in a row. Marking link as corrupted.")
      REDStore.account.usernameSyncState = AccountValues.UsernameSyncState.LINK_CORRUPTED
      REDStore.account.usernameLink = null
      REDStore.account.usernameSyncErrorCount = 0
      StorageSyncHelper.scheduleSyncForDataChange()
    } else {
      Log.w(TAG, "Link mismatch reported. At ${REDStore.account.usernameSyncErrorCount} / $USERNAME_SYNC_ERROR_THRESHOLD tries.")
    }
  }
  ...
  private fun reserveUsernameInternal(nickname: String, discriminator: String?): Result<UsernameState.Reserved, UsernameSetResult> {
    ...
    return when (val result = REDNetwork.account.reserveUsername(hashes)) {
      is NetworkResult.Success -> {
  ...
  private fun updateUsernameDisplayForCurrentLinkInternal(updatedUsername: Username): UsernameSetResult {
    ...
    val oldUsernameLink = REDStore.account.usernameLink ?: return UsernameSetResult.USERNAME_INVALID
    val newUsernameLink = updatedUsername.generateLink(oldUsernameLink.entropy)

    return when (val result = REDNetwork.account.updateUsernameLink(newUsernameLink)) {
      is NetworkResult.Success -> {
        REDStore.account.username = updatedUsername.username
        REDStore.account.usernameLink = result.result
        REDDatabase.recipients.setUsername(Recipient.self().id, updatedUsername.username)
        REDStore.account.usernameSyncState = AccountValues.UsernameSyncState.IN_SYNC
        REDStore.account.usernameSyncErrorCount = 0
        REDStore.misc.needsUsernameRestore = false
  ...
  private fun confirmUsernameAndCreateNewLinkInternal(username: Username): UsernameSetResult {
    ...
    return when (val result = REDNetwork.account.confirmUsername(username, link)) {
      is NetworkResult.Success -> {
        REDStore.account.username = username.username
        REDStore.account.usernameLink = UsernameLinkComponents(link.entropy, result.result)
        REDDatabase.recipients.setUsername(Recipient.self().id, username.username)
        REDStore.account.usernameSyncState = AccountValues.UsernameSyncState.IN_SYNC
        REDStore.account.usernameSyncErrorCount = 0
        REDStore.misc.needsUsernameRestore = false
  ...
  private fun deleteUsernameInternal(): UsernameDeleteResult {
    ...
    return when (val result = REDNetwork.account.deleteUsernameHash()) {
      is RequestResult.Success -> {
        REDDatabase.recipients.setUsername(Recipient.self().id, null)
        REDStore.account.username = null
        REDStore.account.usernameLink = null
        REDStore.account.usernameSyncState = AccountValues.UsernameSyncState.IN_SYNC
        REDStore.account.usernameSyncErrorCount = 0
        REDStore.misc.needsUsernameRestore = false
  ...
  private fun reclaimUsernameIfNecessaryInternal(username: Username, usernameLinkComponents: UsernameLinkComponents): UsernameReclaimResult {
    ...
    return when (val result = REDNetwork.account.confirmUsername(username, link)) {
      is NetworkResult.Success -> {
        REDStore.account.usernameLink = UsernameLinkComponents(usernameLinkComponents.entropy, result.result)
        REDDatabase.recipients.markNeedsSync(Recipient.self().id)
        StorageSyncHelper.scheduleSyncForDataChange()
        UsernameReclaimResult.SUCCESS
      }
