...
import com.red.sovereign.databinding.ReviewCardBinding;
...
class ReviewCardViewHolder extends RecyclerView.ViewHolder {
  ...
  private final Runnable                        onREDConnectionClicked;
                              ...
                              @NonNull Callbacks callbacks)
  {
    ...
    onREDConnectionClicked = callbacks::onREDConnectionClicked;
  }
  ...
  private void presentReviewTextRows(@NonNull List<ReviewTextRow> reviewTextRows, @NonNull Context context, @NonNull ReviewCard reviewCard) {
    ...
    for (int i = 0; i < Math.min(reviewTextRows.size(), subtextGroups.size()); i++) {
      ...
      switch (row) {
        case SIGNAL_CONNECTION -> presentREDConnection(group.getFirst(), group.getSecond(), context, reviewCard);
  ...
  private void presentREDConnection(@NonNull TextView line, @NonNull ImageView icon, @NonNull Context context, @NonNull ReviewCard reviewCard) {
    ...
    line.setOnClickListener(v -> onREDConnectionClicked.run());
  }
