...
public class RecaptchaProofActivity extends PassphraseRequiredActivity {
  ...
  protected void onCreate(Bundle savedInstanceState, boolean ready) {
    ...
    REDStore.misc().setCaptchaLastViewedAt(System.currentTimeMillis());
  ...
  private void handleToken(@NonNull String token) {
    ...
    SimpleTask.run(() -> {
      String challenge = REDStore.rateLimit().getChallenge();
      ...
      try {
        for (int i = 0; i < 3; i++) {
          try {
            NetworkResultUtil.toBasicLegacy(REDNetwork.rateLimitChallenge().submitCaptchaChallenge(challenge, token));
