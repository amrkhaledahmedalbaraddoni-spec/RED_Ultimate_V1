...
public final class LiveRecipientCache {
  ...
  public LiveRecipientCache(@NonNull Context context) {
    this(context, new FilteredExecutor(REDExecutors.newCachedBoundedExecutor("signal-recipients", ThreadUtil.PRIORITY_UI_BLOCKING_THREAD, 1, 4, 15), () -> !REDDatabase.inTransaction()));
  }
  ...
  public void refresh(@NonNull Collection<RecipientId> recipientIds) {
    ...
    if (!cachedIds.isEmpty()) {
      Set<Recipient> recipients = REDDatabase
          .recipients()
  ...
  @NonNull Recipient getSelf() {
    ...
    if (selfId == null) {
      ...
      String localE164 = REDStore.account().getE164();
  ...
  public void warmUp() {
    ...
    REDExecutors.BOUNDED.execute(() -> {
      ...
      if (REDStore.registration().isRegistrationComplete() && REDStore.account().getAci() != null) {
        try (Cursor cursor = REDDatabase.recipients().getNonGroupContacts(RecipientTable.IncludeSelfMode.Exclude.INSTANCE)) {
          int count = 0;
