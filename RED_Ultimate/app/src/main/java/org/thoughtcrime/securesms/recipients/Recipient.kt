...
import com.red.sovereign.contacts.avatars.GroupRecordContactPhoto
import com.red.sovereign.contacts.avatars.ProfileContactPhoto
import com.red.sovereign.contacts.avatars.SystemContactPhoto
import com.red.sovereign.conversation.colors.AvatarColor
import com.red.sovereign.conversation.colors.ChatColors
import com.red.sovereign.conversation.colors.ChatColors.Id.Auto
import com.red.sovereign.conversation.colors.ChatColorsPalette
import com.red.sovereign.database.RecipientTable.MissingRecipientException
...
import com.red.sovereign.phonenumbers.NumberUtil
import com.red.sovereign.profiles.ProfileName
import com.red.sovereign.recipients.Recipient.Companion.external
...
import com.red.sovereign.util.UsernameUtil.isValidUsernameForSearch
...
class Recipient(
  ...
  /** Whether the recipient represents a group. It could be a RED group or MMS group. */
  val isGroup: Boolean
  ...
  /** Whether the recipient represents a RED group. */
  ...
  /** Whether the recipient represents a V1 RED group. These types of groups were deprecated in 2020. */
  ...
  /** Whether the recipient represents a V2 RED group. */
  ...
  val contactPhoto: ContactPhoto?
    ...
    } else if (systemContactPhoto != null && REDStore.settings.isPreferSystemContactPhotos) {
  ...
  /** The [RegisteredState] of this recipient. RED groups/lists are always registered. */
  ...
  val callNotificationSetting: NotificationSetting
    get() = if (REDStore.labs.muteBreakthroughNotifications) callNotificationSettingValue else NotificationSetting.ALWAYS_NOTIFY
  ...
  val replyNotificationSetting: NotificationSetting
    get() = if (groupIdValue == null) NotificationSetting.DO_NOT_NOTIFY else if (REDStore.labs.muteBreakthroughNotifications) replyNotificationSettingValue else mentionSetting
  ...
  val wallpaper: ChatWallpaper?
    get() {
      return if (wallpaperValue != null) {
        ...
        REDStore.wallpaper.getWallpaper()
      }
    }
  ...
  val hasWallpaper: Boolean
    get() = wallpaperValue != null || REDStore.wallpaper.hasWallpaperSet() || isReleaseNotes
  ...
  val chatColors: ChatColors
    get() {
      return if (chatColorsValue != null && chatColorsValue.id !is Auto) {
        ...
        val global = REDStore.chatColors.chatColors
   ...
   * This value respects the local user's [REDStore.inAppPayments.getDisplayBadgesOnProfile()] preference.
   */
  val featuredBadge: Badge?
    get() {
      return if (isSelf && !REDStore.inAppPayments.getDisplayBadgesOnProfile()) {
        null
      } else {
        badges.firstOrNull()
      }
  ...
  private fun getNameFromLocalData(context: Context): String? {
    ...
    if (name.isBlank() && e164Value.isNotNullOrBlank()) {
      name = REDE164Util.prettyPrint(e164Value)
    }
  ...
  fun getDisplayNameForHeadline(context: Context): CharSequence {
    ...
    return buildSpannedString {
      ...
      if (showVerified) {
        ...
        val systemContactGlyph = REDSymbols
          .getSpannedString(context, REDSymbols.Weight.BOLD, REDSymbols.Glyph.PERSON_CIRCLE)
      ...
      if (isIndividual && !isSelf) {
        ...
        val chevronGlyph = REDSymbols.getSpannedString(context, REDSymbols.Weight.BOLD, if (isLtr) REDSymbols.Glyph.CHEVRON_RIGHT else REDSymbols.Glyph.CHEVRON_LEFT, CoreUiR.color.signal_colorOutline)
  ...
  companion object {
    ...
    fun distributionList(distributionListId: DistributionListId): Recipient {
      val id = REDDatabase.recipients.getOrInsertFromDistributionListId(distributionListId)
      return resolved(id)
    }
    ...
    fun externalUsername(serviceId: ServiceId, username: String): Recipient {
      ...
      REDDatabase.recipients.setUsername(recipient.id, username)
      return recipient
    }

    /**
     * Returns a fully-populated [Recipient] based off of a [REDServiceAddress], creating one in the database if necessary.
     */
    @JvmStatic
    @WorkerThread
    fun externalPush(signalServiceAddress: REDServiceAddress): Recipient {
    ...
    private fun externalPush(serviceId: ServiceId, e164: String?): Recipient {
      ...
      if (!resolved.isRegistered) {
        ...
        REDDatabase.recipients.markRegistered(recipientId, serviceId)
      }
    ...
    fun trustedPush(aci: ACI, pni: PNI?, e164: String?): Recipient {
      ...
      val recipientId = REDDatabase.recipients.getAndPossiblyMergePnpVerified(aci, pni, e164)
      val resolved = resolved(recipientId)
    ...
    fun externalContact(identifier: String): Recipient? {
      val id: RecipientId = if (UuidUtil.isUuid(identifier)) {
        ...
        REDDatabase.recipients.getOrInsertFromEmail(identifier)
      } else {
        val e164 = REDE164Util.formatNonShortCodeAsE164(identifier) ?: return null
        REDDatabase.recipients.getOrInsertFromE164(e164)
      }
    ...
    fun externalGroupExact(groupId: GroupId): Recipient {
      return resolved(REDDatabase.recipients.getOrInsertFromGroupId(groupId))
    }
    ...
    fun externalPossiblyMigratedGroup(groupId: GroupId): Recipient {
      ...
      return try {
        ...
        resolved(REDDatabase.recipients.getOrInsertFromPossiblyMigratedGroupId(groupId))
      }
    }
     ...
     *
     * If the identifier is a UUID of a RED user, prefer using
    ...
    fun external(identifier: String): Recipient? {
      ...
      val id: RecipientId = if (serviceId != null) {
        ...
        REDDatabase.recipients.getOrInsertFromGroupId(GroupId.parseOrThrow(identifier))
      } else if (NumberUtil.isValidEmail(identifier)) {
        REDDatabase.recipients.getOrInsertFromEmail(identifier)
      ...
      } else if (REDE164Util.isPotentialE164(identifier)) {
        REDDatabase.recipients.getOrInsertFromE164(identifier)
      } else {
        return null
      }
