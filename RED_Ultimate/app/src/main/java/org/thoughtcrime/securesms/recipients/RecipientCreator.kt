...
import com.red.sovereign.wallpaper.UriChatWallpaper
...
object RecipientCreator {
  ...
  fun forIndividual(context: Context, record: RecipientRecord): Recipient {
    val isSelf = record.e164 != null && record.e164 == REDStore.account.e164 || record.aci != null && record.aci == REDStore.account.aci
    val isReleaseChannel = record.id == REDStore.releaseChannel.releaseChannelRecipientId
    var registeredState = record.registered

    if (isSelf) {
      registeredState = if (REDStore.account.isRegistered && !TextSecurePreferences.isUnauthorizedReceived(context)) {
  ...
  private fun getGroupRecipientDetails(record: RecipientRecord): Recipient {
    val groupRecord = REDDatabase.groups.getGroup(record.id)
  ...
  private fun getDistributionListRecipientDetails(record: RecipientRecord): Recipient {
    val groupRecord = REDDatabase.distributionLists.getList(record.distributionListId!!)

    // SIGNAL_INHERITED: TODO [stories] We'll have to see what the perf is like for very large distribution lists. We may not be able to support fetching all the members.
    if (groupRecord != null) {
  ...
  private fun getCallLinkRecipientDetails(record: RecipientRecord): Recipient {
    val callLink = REDDatabase.callLinks.getCallLinkByRoomId(record.callLinkRoomId!!)
