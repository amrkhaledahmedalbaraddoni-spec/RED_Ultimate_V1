...
final class RecipientDialogViewModel extends ViewModel {
                                   ...
                                   @NonNull RecipientDialogRepository recipientDialogRepository)
  {
    ...
    this.isDeprecatedOrUnregistered = REDStore.misc().isClientDeprecated() || TextSecurePreferences.isUnauthorizedReceived(context);
    ...
    if (groupId != null && groupId.isV2() && recipient.isIndividual() && !recipient.isSelf()) {
      REDExecutors.BOUNDED.execute(() -> {
        ...
        if (label != null) {
          ...
          Optional<GroupRecord> groupRecord = REDDatabase.groups().getGroup(v2GroupId);
  ...
  public void onTapToViewAvatar(@NonNull Recipient recipient) {
    REDExecutors.BOUNDED.execute(() -> REDDatabase.recipients().manuallyUpdateShowAvatar(recipient.getId(), true));
  ...
  public void onResetBlurAvatar(@NonNull Recipient recipient) {
    REDExecutors.BOUNDED.execute(() -> REDDatabase.recipients().manuallyUpdateShowAvatar(recipient.getId(), false));
  }

  public void refreshGroupId(@Nullable GroupId groupId) {
    if (groupId != null) {
      REDExecutors.BOUNDED.execute(() -> {
        RecipientId groupRecipientId = REDDatabase.groups().getGroup(groupId).get().getRecipientId();
