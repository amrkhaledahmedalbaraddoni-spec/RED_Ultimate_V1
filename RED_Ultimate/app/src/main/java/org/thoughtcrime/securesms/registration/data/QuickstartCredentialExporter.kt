...
object QuickstartCredentialExporter {
  ...
  fun export(context: Context): File {
    ...
    val e164 = REDStore.account.requireE164()
    val servicePassword = REDStore.account.servicePassword ?: error("No service password")

    val aciIdentityKeyPair = REDStore.account.aciIdentityKey
    val pniIdentityKeyPair = REDStore.account.pniIdentityKey

    val aciSignedPreKey = AppDependencies.protocolStore.aci().loadSignedPreKey(REDStore.account.aciPreKeys.activeSignedPreKeyId)
    val aciLastResortKyberPreKey = AppDependencies.protocolStore.aci().loadKyberPreKey(REDStore.account.aciPreKeys.lastResortKyberPreKeyId)
    val pniSignedPreKey = AppDependencies.protocolStore.pni().loadSignedPreKey(REDStore.account.pniPreKeys.activeSignedPreKeyId)
    val pniLastResortKyberPreKey = AppDependencies.protocolStore.pni().loadKyberPreKey(REDStore.account.pniPreKeys.lastResortKyberPreKeyId)
