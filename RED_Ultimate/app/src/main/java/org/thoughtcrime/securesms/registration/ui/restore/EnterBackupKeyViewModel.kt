...
class EnterBackupKeyViewModel : ViewModel() {
  ...
  private fun verifyKey(selectedTimestamp: Long): Boolean {
    try {
      ...
      val dirUri = REDStore.backup.newLocalBackupsDirectory ?: return false
  ...
  fun handleRegistrationFailure(registerAccountResult: RegisterAccountResult) {
    store.update {
      if (it.isRegistering) {
        ...
        if (incorrectKeyError && REDStore.account.restoredAccountEntropyPool) {
  ...
  fun handleBackupTimestampNotRestored() {
    store.update {
      it.copy(
        showBackupTierNotRestoreError = if (REDStore.backup.isBackupTimestampRestored) TierRestoreError.NOT_FOUND else TierRestoreError.NETWORK_ERROR
      )
    }
  }
