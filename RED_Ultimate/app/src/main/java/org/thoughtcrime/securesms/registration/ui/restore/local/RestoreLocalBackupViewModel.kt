...
class RestoreLocalBackupViewModel : ViewModel() {
  ...
  suspend fun backupBelongsToCurrentAccount(context: Context, backupKey: String, timestamp: Long): Boolean {
    return withContext(Dispatchers.IO) {
      ...
      val dirUri = requireNotNull(REDStore.backup.newLocalBackupsDirectory) { "Backup directory must be set" }
      ...
      val expectedBackupId = messageBackupKey.deriveBackupId(REDStore.account.requireAci())
