...
class AppRegistrationNetworkController(
  ...
  override suspend fun enableRegistrationLock(): RequestResult<Unit, SetRegistrationLockError> = withContext(Dispatchers.IO) {
    ...
    when (val result = REDNetwork.account.enableRegistrationLock(masterKey.deriveRegistrationLock())) {
  ...
  override suspend fun disableRegistrationLock(): RequestResult<Unit, SetRegistrationLockError> = withContext(Dispatchers.IO) {
    when (val result = REDNetwork.account.disableRegistrationLock()) {
      is NetworkResult.Success -> RequestResult.Success(Unit)
      is NetworkResult.StatusCodeError -> {
        when (result.code) {
          401 -> RequestResult.NonSuccess(SetRegistrationLockError.Unauthorized)
  ...
  override suspend fun setAccountAttributes(
    ...
    when (val result = REDNetwork.account.setAccountAttributes(attributes.toServiceAccountAttributes())) {
      is NetworkResult.Success -> RequestResult.Success(Unit)
      is NetworkResult.StatusCodeError -> {
        when (result.code) {
  ...
  override suspend fun getRemoteBackupInfo(aep: AccountEntropyPool): RequestResult<NetworkController.GetBackupInfoResponse, NetworkController.GetBackupInfoError> = withContext(Dispatchers.IO) {
    val aci = REDStore.account.aci ?: return@withContext RequestResult.ApplicationError(IllegalStateException("ACI not available"))
    ...
    val access = if (messageCredential != null) {
      ...
      when (val credResult = REDNetwork.archive.getServiceCredentials(currentTime)) {
        is NetworkResult.Success -> {
          REDStore.backup.messageCredentials.add(credResult.result.messageCredentials)
          REDStore.backup.messageCredentials.clearOlderThan(currentTime)
          val credential = REDStore.backup.messageCredentials.byDay.getForCurrentTime(currentTime.milliseconds)
    ...
    when (val result = REDNetwork.archive.getBackupInfo(aci, access)) {
      is NetworkResult.Success -> {
  ...
  override suspend fun setProfile(
    ...
    if (avatar != null) {
      ...
      REDStore.misc.hasEverHadAnAvatar = true
    }

    REDStore.phoneNumberPrivacy.phoneNumberDiscoverabilityMode = if (discoverableByPhoneNumber) {
      com.red.sovereign.keyvalue.PhoneNumberPrivacyValues.PhoneNumberDiscoverabilityMode.DISCOVERABLE
    } else {
      com.red.sovereign.keyvalue.PhoneNumberPrivacyValues.PhoneNumberDiscoverabilityMode.NOT_DISCOVERABLE
    }
  ...
  override suspend fun getBackupFileLastModified(
    ...
    val cdnCredentials = when (val cdnResult = REDNetwork.archive.getCdnReadCredentials(cdn, aci, access)) {
  ...
  override fun startNewDeviceTransferServer(context: Context, aep: AccountEntropyPool) {
    val pendingIntent = android.app.PendingIntent.getActivity(
      context,
      0,
      com.red.sovereign.MainActivity.clearTop(context),
    ...
    val notificationData = org.signal.devicetransfer.DeviceToDeviceTransferService.TransferNotificationData(
      com.red.sovereign.notifications.NotificationIds.DEVICE_TRANSFER,
      com.red.sovereign.notifications.NotificationChannels.getInstance().BACKUPS,
      org.signal.core.ui.R.drawable.ic_signal_backup
    )
    org.signal.devicetransfer.DeviceToDeviceTransferService.startServer(
      context,
      com.red.sovereign.devicetransfer.newdevice.NewDeviceServerTask(),
      notificationData,
      pendingIntent
    )
  }
