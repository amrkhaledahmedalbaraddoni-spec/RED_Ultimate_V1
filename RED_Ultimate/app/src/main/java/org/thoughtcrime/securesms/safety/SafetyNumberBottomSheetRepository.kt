...
class SafetyNumberBottomSheetRepository {
    ...
    return filterForDistributionLists(destinations).flatMapCompletable { distributionRecipients ->
      Completable.fromAction {
        distributionRecipients
          .mapNotNull { REDDatabase.distributionLists.getList(it.requireDistributionListId()) }
          .filter { it.members.contains(recipientId) }
          .forEach {
            REDDatabase.distributionLists.excludeFromStory(recipientId, it)
  ...
  fun removeAllFromStory(recipientIds: List<RecipientId>, distributionList: DistributionListId): Completable {
    return Completable.fromAction {
      val record = REDDatabase.distributionLists.getList(distributionList)
      if (record != null) {
        REDDatabase.distributionLists.excludeAllFromStory(recipientIds, record)
  ...
  private fun observeDistributionList(recipient: Recipient): Observable<DistributionListRecord> {
    return Recipient.observable(recipient.id).map { REDDatabase.distributionLists.getList(it.requireDistributionListId())!! }
  }
