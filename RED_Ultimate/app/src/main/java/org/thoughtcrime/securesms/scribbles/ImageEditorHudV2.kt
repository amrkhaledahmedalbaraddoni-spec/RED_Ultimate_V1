...
import com.red.sovereign.components.TooltipPopup
import com.red.sovereign.keyvalue.REDStore
import com.red.sovereign.mediasend.v2.MediaAnimations
import com.red.sovereign.scribbles.HSVColorSlider.getColor
import com.red.sovereign.scribbles.HSVColorSlider.setColor
import com.red.sovereign.scribbles.HSVColorSlider.setUpForColor
...
class ImageEditorHudV2 @JvmOverloads constructor(
  ...
  private fun setupWidthSeekBar() {
    ...
    widthSeekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
      override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        when (currentMode) {
          Mode.DRAW -> REDStore.imageEditor.setMarkerPercentage(progress)
          Mode.BLUR -> REDStore.imageEditor.setBlurPercentage(progress)
          Mode.HIGHLIGHT -> REDStore.imageEditor.setHighlighterPercentage(progress)
          else -> Unit
        }
      }
  ...
  private fun presentModeDraw() {
    ...
    widthSeekBar.progress = REDStore.imageEditor.getMarkerPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
  ...
  private fun presentModeHighlight() {
    ...
    widthSeekBar.progress = REDStore.imageEditor.getHighlighterPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
      inSet = drawButtonRow + drawTools,
  ...
  private fun presentModeBlur() {
    ...
    widthSeekBar.progress = REDStore.imageEditor.getBlurPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
  ...
  companion object {
    ...
    private val DRAW_WIDTH_BOUNDARIES: Map<Mode, Pair<Float, Float>> = mapOf(
      Mode.DRAW to REDStore.imageEditor.getMarkerWidthRange(),
      Mode.HIGHLIGHT to REDStore.imageEditor.getHighlighterWidthRange(),
      Mode.BLUR to REDStore.imageEditor.getBlurWidthRange()
    )
