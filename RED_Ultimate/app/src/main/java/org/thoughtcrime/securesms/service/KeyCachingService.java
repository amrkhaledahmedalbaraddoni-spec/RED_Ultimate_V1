...
import com.red.sovereign.DummyActivity;
import com.red.sovereign.MainActivity;
import com.red.sovereign.R;
import com.red.sovereign.crypto.InvalidPassphraseException;
import com.red.sovereign.crypto.MasterSecret;
...
import com.red.sovereign.util.DynamicLanguage;
...
public class KeyCachingService extends Service {
  ...
  public static synchronized boolean isLocked(Context context) {
    boolean locked = masterSecret == null && (!REDStore.settings().getPassphraseDisabled() || REDStore.settings().getScreenLockEnabled());

    if (locked) {
      Log.d(TAG, "Locked! PasswordDisabled: " + REDStore.settings().getPassphraseDisabled() + ", ScreenLock: " + REDStore.settings().getScreenLockEnabled());
    }
  ...
  public static synchronized @Nullable MasterSecret getMasterSecret(Context context) {
    if (masterSecret == null && (REDStore.settings().getPassphraseDisabled() && !REDStore.settings().getScreenLockEnabled())) {
      try {
  ...
  public void onCreate() {
    ...
    if (REDStore.settings().getPassphraseDisabled() && !REDStore.settings().getScreenLockEnabled()) {
      try {
  ...
  private void handleDisableService() {
    if (REDStore.settings().getPassphraseDisabled() &&
        !REDStore.settings().getScreenLockEnabled())
    {
      stopForeground(true);
    }
  }
  ...
  private static void startTimeoutIfAppropriate(@NonNull Context context) {
    ...
    boolean timeoutEnabled   = REDStore.settings().getPassphraseTimeoutEnabled();
    boolean passLockActive   = timeoutEnabled && !REDStore.settings().getPassphraseDisabled();

    long    screenTimeout       = REDStore.settings().getScreenLockTimeout();
    boolean screenLockActive    = REDStore.settings().getScreenLockEnabled();
    ...
    if (immediateScreenLock) {
      ...
      REDStore.settings().setScreenLockTimeout(60);
    ...
    if (!appVisible && secretSet && (passLockActive || screenLockActive)) {
      if (immediateScreenLock) {
        ...
        long passphraseTimeoutMinutes = REDStore.settings().getPassphraseTimeout();
        long screenLockTimeoutSeconds = REDStore.settings().getScreenLockTimeout();

        long timeoutMillis;

        if (!REDStore.settings().getPassphraseDisabled()) timeoutMillis = TimeUnit.MINUTES.toMillis(passphraseTimeoutMinutes);
