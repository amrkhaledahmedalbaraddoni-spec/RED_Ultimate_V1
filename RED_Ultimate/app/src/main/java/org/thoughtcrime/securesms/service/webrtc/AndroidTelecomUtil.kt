...
object AndroidTelecomUtil {
  ...
  fun getActiveAudioDevice(recipientId: RecipientId): REDAudioManager.AudioDevice {
    return synchronized(controllers) {
      controllers[recipientId]?.currentAudioDevice ?: REDAudioManager.AudioDevice.NONE
    }
  }

  fun selectAudioDevice(recipientId: RecipientId, device: REDAudioManager.AudioDevice) {
    if (telecomSupported) {
      synchronized(controllers) {
  ...
  @JvmStatic
  fun getCurrentActiveAudioDevice(): REDAudioManager.AudioDevice {
    return synchronized(controllers) {
      controllers.values.firstOrNull()?.currentAudioDevice ?: REDAudioManager.AudioDevice.NONE
    }
  }
  ...
  private fun isTelecomAllowedForDevice(): Boolean {
    if (RemoteConfig.internalUser) {
      return !REDStore.internal.callingDisableTelecom
    }
