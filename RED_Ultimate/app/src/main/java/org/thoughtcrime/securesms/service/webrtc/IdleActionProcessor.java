...
public class IdleActionProcessor extends WebRtcActionProcessor {
                                                                  ...
                                                                  @NonNull CallManager.RingUpdate ringUpdate)
  {
    ...
    if (ringUpdate != CallManager.RingUpdate.REQUESTED) {
      REDDatabase.calls().insertOrUpdateGroupCallFromRingState(ringId, remotePeerGroup.getId(), sender, System.currentTimeMillis(), ringUpdate);
      return currentState;
    } else if (REDDatabase.calls().isRingCancelled(ringId, remotePeerGroup.getId())) {
      try {
    ...
    if (activeProfile != null && !(activeProfile.isRecipientAllowed(remotePeerGroup.getId()) || activeProfile.getAllowAllCalls())) {
      try {
        ...
        REDDatabase.calls().insertOrUpdateGroupCallFromRingState(ringId, remotePeerGroup.getId(), sender, System.currentTimeMillis(), CallManager.RingUpdate.EXPIRED_REQUEST, true);
  ...
  protected @NonNull WebRtcServiceState handleReceivedGroupCallPeekForRingingCheck(@NonNull WebRtcServiceState currentState, @NonNull GroupCallRingCheckInfo info, @NonNull PeekInfo peekInfo) {
    ...
    if (REDDatabase.calls().isRingCancelled(info.getRingId(), info.getRecipientId())) {
      try {
    ...
    if (peekInfo.getDeviceCount() == 0) {
      ...
      REDDatabase.calls().insertOrUpdateGroupCallFromRingState(info.getRingId(), info.getRecipientId(), info.getRingerAci(), System.currentTimeMillis(), CallManager.RingUpdate.EXPIRED_REQUEST);
      return currentState;
      ...
      REDDatabase.calls().insertOrUpdateGroupCallFromRingState(info.getRingId(), info.getRecipientId(), info.getRingerAci(), System.currentTimeMillis(), CallManager.RingUpdate.ACCEPTED_ON_ANOTHER_DEVICE);
      return currentState;
    }
