...
import com.red.sovereign.notifications.DoNotDisturbUtil;
...
import static com.red.sovereign.webrtc.CallNotificationBuilder.TYPE_INCOMING_RINGING;
...
public final class IncomingGroupCallActionProcessor extends DeviceAwareActionProcessor {
                                                                  ...
                                                                  @NonNull CallManager.RingUpdate ringUpdate)
  {
    ...
    if (REDDatabase.calls().isRingCancelled(ringId, remotePeerGroup.getId()) && !updateForCurrentRingId) {
      try {
    ...
    if (!shouldDisturbUserWithCall) {
      ...
      boolean isCallNotificationsEnabled = REDStore.settings().isCallNotificationsEnabled() && NotificationChannels.getInstance().areNotificationsEnabled();
      if (isCallNotificationsEnabled) {
        Uri                         ringtone     = recipient.resolve().getCallRingtone();
        RecipientTable.VibrateState vibrateState = recipient.resolve().getCallVibrate();

        if (ringtone == null) {
          ringtone = REDStore.settings().getCallRingtone();
        }

        webRtcInteractor.startIncomingRinger(ringtone, vibrateState == RecipientTable.VibrateState.ENABLED || (vibrateState == RecipientTable.VibrateState.DEFAULT && REDStore.settings().isCallVibrateEnabled()));
      }
  ...
  protected @NonNull WebRtcServiceState handleAcceptCall(@NonNull WebRtcServiceState currentState, boolean answerWithVideo) {
    ...
    GroupCall groupCall    = webRtcInteractor.getCallManager().createGroupCall(groupId,
                                                                               REDStore.internal().getGroupCallingServer(),
                                                                               new byte[0],
