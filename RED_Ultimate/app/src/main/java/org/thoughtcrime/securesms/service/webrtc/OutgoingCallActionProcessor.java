...
import com.red.sovereign.service.webrtc.state.CallSetupState;
import com.red.sovereign.service.webrtc.state.VideoState;
...
import static com.red.sovereign.webrtc.CallNotificationBuilder.TYPE_OUTGOING_RINGING;
...
public class OutgoingCallActionProcessor extends DeviceAwareActionProcessor {
  ...
  protected @NonNull WebRtcServiceState handleStartOutgoingCall(@NonNull WebRtcServiceState currentState, @NonNull RemotePeer remotePeer, @NonNull OfferMessage.Type offerType) {
    ...
    webRtcInteractor.setDefaultAudioDevice(remotePeer.getId(),
                                           isVideoCall ? REDAudioManager.AudioDevice.SPEAKER_PHONE : REDAudioManager.AudioDevice.EARPIECE,
                                           false);
    ...
    RecipientUtil.setAndSendUniversalExpireTimerIfNecessary(context, Recipient.resolved(remotePeer.getId()), REDDatabase.threads().getThreadIdIfExistsFor(remotePeer.getId()));

    REDDatabase.calls().insertOneToOneCall(remotePeer.getCallId().longValue(),
