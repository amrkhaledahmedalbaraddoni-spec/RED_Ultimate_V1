...
public final class WebRtcUtil {
  ...
  public static void enableSpeakerPhoneIfNeeded(@NonNull WebRtcInteractor webRtcInteractor, WebRtcServiceState currentState) {
    ...
    if (currentState.getLocalDeviceState().getActiveDevice() == REDAudioManager.AudioDevice.EARPIECE ||
        currentState.getLocalDeviceState().getActiveDevice() == REDAudioManager.AudioDevice.NONE &&
        currentState.getCallInfoState().getActivePeer() != null)
    {
      webRtcInteractor.setDefaultAudioDevice(currentState.getCallInfoState().requireActivePeer().getId(), REDAudioManager.AudioDevice.SPEAKER_PHONE, true);
    }
  }
