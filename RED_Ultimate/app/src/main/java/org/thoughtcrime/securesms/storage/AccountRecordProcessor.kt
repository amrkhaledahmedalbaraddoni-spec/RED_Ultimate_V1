...
import com.red.sovereign.storage.StorageSyncHelper.applyAccountStorageSyncUpdates
...
/**
 * Processes [REDAccountRecord]s. Unlike some other [StorageRecordProcessor]s, this
...
class AccountRecordProcessor(
  ...
  private val localAccountRecord: REDAccountRecord
) : DefaultStorageRecordProcessor<REDAccountRecord>() {
  ...
  constructor(context: Context, self: Recipient) : this(
    context = context,
    self = self,
    localAccountRecord = StorageSyncHelper.buildAccountRecord(context, self).let { it.proto.account!!.toREDAccountRecord(it.id) }
  )
   ...
   */
  override fun isInvalid(remote: REDAccountRecord): Boolean {
  ...
  override fun getMatching(remote: REDAccountRecord, keyGenerator: StorageKeyGenerator): Optional<REDAccountRecord> {
    return Optional.of(localAccountRecord)
  }

  override fun merge(remote: REDAccountRecord, local: REDAccountRecord, keyGenerator: StorageKeyGenerator): REDAccountRecord {
    ...
    val merged = REDAccountRecord.newBuilder(unknownFields).apply {
    ...
    }.toREDAccountRecord(StorageId.forAccount(keyGenerator.generate()))
  ...
  override fun insertLocal(record: REDAccountRecord) {
  ...
  override fun updateLocal(update: StorageRecordUpdate<REDAccountRecord>) {
    applyAccountStorageSyncUpdates(context, self, update, true)
  }

  override fun compare(lhs: REDAccountRecord, rhs: REDAccountRecord): Int {
    return 0
  }
}
