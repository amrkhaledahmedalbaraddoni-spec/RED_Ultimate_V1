...
/**
 * Record processor for [REDChatFolderRecord].
 * Handles merging and updating our local store when processing remote chat folder storage records.
 */
class ChatFolderRecordProcessor : DefaultStorageRecordProcessor<REDChatFolderRecord>() {

  companion object {
    private val TAG = Log.tag(ChatFolderRecordProcessor::class)
  }

  override fun compare(o1: REDChatFolderRecord, o2: REDChatFolderRecord): Int {
   ...
   */
  override fun isInvalid(remote: REDChatFolderRecord): Boolean {
  ...
  override fun getMatching(remote: REDChatFolderRecord, keyGenerator: StorageKeyGenerator): Optional<REDChatFolderRecord> {
    ...
    val folder = REDDatabase.chatFolders.getChatFolder(query)
    ...
    return if (folder?.storageServiceId != null) {
      ...
      REDDatabase.chatFolders.applyStorageIdUpdate(folder.chatFolderId, storageId)
      ...
      Optional.empty<REDChatFolderRecord>()
    }
  }
   ...
   */
  override fun merge(remote: REDChatFolderRecord, local: REDChatFolderRecord, keyGenerator: StorageKeyGenerator): REDChatFolderRecord {
  ...
  override fun insertLocal(record: REDChatFolderRecord) {
    REDDatabase.chatFolders.insertChatFolderFromStorageSync(record)
  }

  override fun updateLocal(update: StorageRecordUpdate<REDChatFolderRecord>) {
    REDDatabase.chatFolders.updateChatFolderFromStorageSync(update.new)
  }
