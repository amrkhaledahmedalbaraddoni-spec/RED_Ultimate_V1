...
public final class StorageSyncValidations {
  ...
  public static void validate(@NonNull StorageSyncHelper.WriteOperationResult result,
                              @NonNull REDStorageManifest previousManifest,
  ...
  public static void validateForcePush(@NonNull REDStorageManifest manifest, @NonNull List<REDStorageRecord> inserts, @NonNull Recipient self) {
    validateManifestAndInserts(manifest, inserts, self);
  }

  private static void validateManifestAndInserts(@NonNull REDStorageManifest manifest, @NonNull List<REDStorageRecord> inserts, @NonNull Recipient self) {
    int accountCount = 0;
    ...
    Set<StorageId>  insertSet = new HashSet<>(inserts.stream().map(REDStorageRecord::getId).collect(Collectors.toList()));
