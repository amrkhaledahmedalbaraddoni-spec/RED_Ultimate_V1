...
/**
 * Record processor for [REDStoryDistributionListRecord].
 * Handles merging and updating our local store when processing remote dlist storage records.
 */
class StoryDistributionListRecordProcessor : DefaultStorageRecordProcessor<REDStoryDistributionListRecord>() {
   ...
   */
  override fun isInvalid(remote: REDStoryDistributionListRecord): Boolean {
  ...
  override fun getMatching(remote: REDStoryDistributionListRecord, keyGenerator: StorageKeyGenerator): Optional<REDStoryDistributionListRecord> {
    Log.d(TAG, "Attempting to get matching record...")
    val matching = REDDatabase.distributionLists.getRecipientIdForSyncRecord(remote)
    ...
    if (matching != null) {
      ...
      val recordForSync = REDDatabase.recipients.getRecordForSync(matching)
      ...
      return StorageSyncModels.localToRemoteRecord(recordForSync).let { it.proto.storyDistributionList!!.toREDStoryDistributionListRecord(it.id) }.asOptional()
    } else {
  ...
  override fun merge(remote: REDStoryDistributionListRecord, local: REDStoryDistributionListRecord, keyGenerator: StorageKeyGenerator): REDStoryDistributionListRecord {
    val merged = REDStoryDistributionListRecord.newBuilder(remote.serializedUnknowns).apply {
    ...
    }.build().toREDStoryDistributionListRecord(StorageId.forStoryDistributionList(keyGenerator.generate()))
  ...
  override fun insertLocal(record: REDStoryDistributionListRecord) {
    REDDatabase.distributionLists.applyStorageSyncStoryDistributionListInsert(record)
  }

  override fun updateLocal(update: StorageRecordUpdate<REDStoryDistributionListRecord>) {
    REDDatabase.distributionLists.applyStorageSyncStoryDistributionListUpdate(update)
  }

  override fun compare(o1: REDStoryDistributionListRecord, o2: REDStoryDistributionListRecord): Int {
