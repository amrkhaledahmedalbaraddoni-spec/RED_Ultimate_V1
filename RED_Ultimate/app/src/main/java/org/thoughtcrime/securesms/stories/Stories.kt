...
object Stories {
  ...
  fun isFeatureEnabled(): Boolean {
    return !REDStore.story.isFeatureDisabled
  }
  ...
  fun getRecipientsToSendTo(messageId: Long, sentTimestamp: Long, allowsReplies: Boolean): SendData {
    val recipientIds: List<RecipientId> = REDDatabase.storySends.getRecipientsToSendTo(messageId, sentTimestamp, allowsReplies)
  ...
  fun onStorySettingsChanged(distributionListId: DistributionListId) {
    val recipientId = REDDatabase.distributionLists.getRecipientId(distributionListId) ?: error("Cannot find recipient id for distribution list.")
    onStorySettingsChanged(recipientId)
  }

  @WorkerThread
  fun onStorySettingsChanged(storyRecipientId: RecipientId) {
    REDDatabase.recipients.markNeedsSync(storyRecipientId)
  ...
  fun enqueueNextStoriesForDownload(recipientId: RecipientId, force: Boolean = false, limit: Int) {
    ...
    REDDatabase.messages.getUnreadStories(recipientId, limit).use { reader ->
      reader.forEach {
  ...
  fun enqueueAttachmentsFromStoryForDownloadSync(record: MmsMessageRecord, ignoreAutoDownloadConstraints: Boolean) {
    REDDatabase.attachments.getAttachmentsForMessage(record.id).filterNot { it.isSticker }.forEach {
