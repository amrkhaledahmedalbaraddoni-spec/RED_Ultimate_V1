...
import com.red.sovereign.jobs.MultiDeviceReadUpdateJob
...
class StoriesLandingRepository(context: Context) {
  ...
  fun getStories(): Observable<List<StoriesLandingItemData>> {
    val storyRecipients: Observable<Map<Recipient, List<StoryResult>>> = RxDatabaseObserver
      ...
      .map {
        ...
        val stories = REDDatabase.messages.getOrderedStoryRecipientsAndIds(false)
    ...
    return storyRecipients.switchMap { map ->
      val observables = map.map { (recipient, results) ->
        val messages = results
          ...
          .mapNotNull {
            try {
              REDDatabase.messages.getMessageRecord(it.messageId).withAttachments()
        ...
        if (recipient.isMyStory) {
          REDDatabase.messages.getMessages(results.map { it.messageId }).use { reader ->
    ...
    val itemDataObservable = Observable.create<StoriesLandingItemData> { emitter ->
      fun refresh(sender: Recipient) {
        ...
        val itemData = StoriesLandingItemData(
          ...
          hasReplies = messageRecords.any { REDDatabase.messages.getNumberOfStoryReplies(it.id) > 0 },
          hasRepliesFromSelf = messageRecords.any { REDDatabase.messages.hasSelfReplyInStory(it.id) },
  ...
  fun setHideStory(recipientId: RecipientId, hideStory: Boolean): Completable {
    return Completable.fromAction {
      REDDatabase.recipients.setHideStory(recipientId, hideStory)
    }.subscribeOn(Schedulers.io())
  }
  ...
  fun markStoriesRead() {
    REDExecutors.BOUNDED_IO.execute {
      val messageInfos: List<MessageTable.MarkedMessageInfo> = REDDatabase.messages.markAllIncomingStoriesRead()
      val releaseThread: Long? = REDStore.releaseChannel.releaseChannelRecipientId?.let { REDDatabase.threads.getThreadIdIfExistsFor(it) }
  ...
  fun markFailedStoriesNotified() {
    REDExecutors.BOUNDED_IO.execute {
      REDDatabase.messages.markAllFailedStoriesNotified()
    }
  }
}
