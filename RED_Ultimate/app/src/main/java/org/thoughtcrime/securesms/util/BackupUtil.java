...
public class BackupUtil {
  ...
  public static boolean canUserAccessBackupDirectory(@NonNull Context context) {
    if (isUserSelectionRequired(context)) {
      Uri backupDirectoryUri = REDStore.settings().getREDBackupDirectory();
  ...
  public static boolean canUserAccessUnifiedBackupDirectory(@NonNull Context context) {
    if (isUserSelectionRequired(context)) {
      String backupDirectoryPath = REDStore.backup().getNewLocalBackupsDirectory();
  ...
  public static void deleteUnifiedBackups(@NonNull Context context, @Nullable String backupDirectoryPath, @Nullable com.red.sovereign.backup.v2.local.AllFilesProgressListener progressListener) {
    if (backupDirectoryPath != null) {
      Uri          backupDirectoryUri = Uri.parse(backupDirectoryPath);
      DocumentFile backupDirectory    = DocumentFile.fromTreeUri(context, backupDirectoryUri);
  ...
  public static void disableBackups(@NonNull Context context) {
    ...
    REDStore.settings().setBackupEnabled(false);
    BackupUtil.deleteAllBackups();

    if (BackupUtil.isUserSelectionRequired(context)) {
      Uri backupLocationUri = REDStore.settings().getREDBackupDirectory();

      if (backupLocationUri == null) {
        return;
      }

      REDStore.settings().clearREDBackupDirectory();
