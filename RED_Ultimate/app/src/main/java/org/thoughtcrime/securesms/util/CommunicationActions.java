...
import com.red.sovereign.groups.ui.invitesandrequests.joining.GroupJoinBottomSheetDialogFragment;
import com.red.sovereign.groups.ui.invitesandrequests.joining.GroupJoinUpdateRequiredBottomSheetDialogFragment;
...
import com.red.sovereign.profiles.manage.UsernameRepository.UsernameLinkConversionResult;
import com.red.sovereign.proxy.ProxyBottomSheetFragment;
import com.red.sovereign.recipients.Recipient;
import com.red.sovereign.recipients.RecipientId;
import com.red.sovereign.service.webrtc.ActiveCallData;
...
public class CommunicationActions {
  ...
  private static void startVoiceCall(@NonNull CallContext callContext, @NonNull Recipient recipient, @NonNull OnUserAlreadyInAnotherCall onUserAlreadyInAnotherCall) {
    ...
    if (recipient.isRegistered()) {
      AppDependencies.getREDCallManager().isCallActive(new ResultReceiver(new Handler(Looper.getMainLooper())) {
        @Override
                                       ...
                                       @Nullable TaskStackBuilder backStack)
  {
    new AsyncTask<Void, Void, Long>() {
      ...
      protected Long doInBackground(Void... voids) {
        return REDDatabase.threads().getOrCreateThreadIdFor(recipient);
      }
   ...
   * If the url is a malformed group link, it will assume RED needs to update.
  ...
  public static boolean handlePotentialProxyLinkUrl(@NonNull FragmentActivity activity, @NonNull String potentialProxyLinkUrl) {
    String proxy = REDProxyUtil.parseHostFromProxyDeepLink(potentialProxyLinkUrl);
   ...
   */
  public static void handlePotentialREDMeUrl(@NonNull FragmentActivity activity, @NonNull String potentialUrl) {
    String                 e164     = REDMeUtil.parseE164FromLink(potentialUrl);
  ...
  private static void startVideoCall(@NonNull CallContext callContext, @NonNull CallLinkRootKey rootKey, @NonNull OnUserAlreadyInAnotherCall onUserAlreadyInAnotherCall) {
    SimpleTask.run(() -> {
      ...
      CallLinkTable.CallLink callLink = REDDatabase.callLinks().getOrCreateCallLinkByRootKey(rootKey);

      if (callLink.getState().hasBeenRevoked()) {
        return Optional.<Recipient>empty();
      }

      return REDDatabase.recipients().getByCallLinkRoomId(roomId).map(Recipient::resolved);
    }, callLinkRecipient -> {
  ...
  private static void startAudioCallInternal(@NonNull CallContext callContext, @NonNull Recipient recipient) {
    callContext.getPermissionsBuilder()
               ...
               .onAllGranted(() -> {
                 AppDependencies.getREDCallManager().startOutgoingAudioCall(recipient);
  ...
  private static void startVideoCallInternal(@NonNull CallContext callContext, @NonNull Recipient recipient, boolean fromCallLink) {
    AppDependencies.getREDCallManager().startPreJoinCall(recipient);
    ...
    SimpleTask.run(() -> {
      ...
      if (!recipient.isRegistered() || !recipient.getHasServiceId()) {
        try {
          ...
          Log.w(TAG, "[handlePotentialREDMeUrl] Failed to refresh directory for new contact.");
        }
      }
  ...
  private static void handleUsernameLink(Activity activity, String link) {
    ...
    SimpleTask.run(() -> {
      try {
        ...
        // SIGNAL_INHERITED: TODO we could be better here and report different types of errors to the UI
