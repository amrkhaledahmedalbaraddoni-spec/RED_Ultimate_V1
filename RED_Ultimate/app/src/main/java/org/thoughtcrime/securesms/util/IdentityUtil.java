...
public final class IdentityUtil {
  ...
  public static void markIdentityVerified(Context context, Recipient recipient, boolean verified, boolean remote)
  {
    ...
    MessageTable smsDatabase   = REDDatabase.messages();
      ...
      while ((groupRecord = reader.getNext()) != null) {
        if (groupRecord.getMembers().contains(recipient.getId()) && groupRecord.isActive() && !groupRecord.isMms()) {
          ...
          if (remote) {
            ...
            RecipientId recipientId    = REDDatabase.recipients().getOrInsertFromGroupId(groupRecord.getId());
            ...
            try {
              REDDatabase.messages().insertMessageOutbox(outgoing, threadId, false, null);
            } catch (MmsException e) {
              throw new AssertionError(e);
            }
    ...
    if (remote) {
      ...
      long threadId = REDDatabase.threads().getOrCreateThreadIdFor(recipient);
  ...
  public static void markIdentityUpdate(@NonNull Context context, @NonNull RecipientId recipientId) {
    ...
    REDDatabase.messageLog().deleteAllForRecipient(recipientId);
  }
  ...
  public static void saveIdentity(String user, IdentityKey identityKey) {
    try(REDSessionLock.Lock unused = ReentrantSessionLock.INSTANCE.acquire()) {
      ...
      REDProtocolAddress address          = new REDProtocolAddress(user, REDServiceAddress.DEFAULT_DEVICE_ID);
  ...
  public static void processVerifiedMessage(Context context, Verified verified) throws InvalidKeyException {
    REDServiceAddress          destination = new REDServiceAddress(ServiceId.ACI.parseOrThrow(verified.destinationAci, verified.destinationAciBinary));
