...
import com.red.sovereign.mms.MmsSlide;
...
import com.red.sovereign.mms.ViewOnceSlide;
    ...
    try {
      ...
      try {
        if (AppDependencies.getBlobs().isAuthority(uri) && MediaUtil.isVideo(AppDependencies.getBlobs().getMimeType(uri))) {
          ...
          MediaDataSource source = REDDatabase.attachments().mediaDataSourceFor(PartAuthority.requireAttachmentId(uri), false);
          if (source == null) {
  ...
  public static @Nullable Bitmap getVideoThumbnail(@NonNull Context context, @Nullable Uri uri, long timeUs) {
               ...
               MediaUtil.isVideoType(PartAuthority.getAttachmentContentType(context, uri)))
    {
      try {
        ...
        MediaDataSource source       = REDDatabase.attachments().mediaDataSourceFor(attachmentId, false);
        return extractFrame(source, timeUs);
      } catch (IOException e) {
        Log.w(TAG, "Failed to extract frame for URI: " + uri, e);
      }
