...
import com.red.sovereign.util.RemoteConfig.REMOTE_VALUES
import com.red.sovereign.util.RemoteConfig.asBoolean
import com.red.sovereign.util.RemoteConfig.asInteger
import com.red.sovereign.util.RemoteConfig.remoteBoolean
import com.red.sovereign.util.RemoteConfig.remoteValue
import com.red.sovereign.util.RemoteConfig.retryReceiptMaxCount
import com.red.sovereign.util.RemoteConfig.retryReceiptMaxCountResetAge
...
object RemoteConfig {
  ...
  fun init() {
    initLock.withLock {
      val current = parseStoredConfig(REDStore.remoteConfig.currentConfig)
      val pending = parseStoredConfig(REDStore.remoteConfig.pendingConfig)
      val changes = computeChanges(current, pending)

      REDStore.remoteConfig.currentConfig = mapToJson(pending)
  ...
  fun refreshIfNecessary() {
    val timeSinceLastFetch = System.currentTimeMillis() - REDStore.remoteConfig.lastFetchTime
  ...
  fun refreshSync() {
    val result = NetworkResultUtil.toBasicLegacy(REDNetwork.remoteConfig.getRemoteConfig())
  ...
  fun update(config: Map<String, Any?>) {
    ...
    val disk = parseStoredConfig(REDStore.remoteConfig.pendingConfig)
    ...
    REDStore.remoteConfig.pendingConfig = mapToJson(result.disk)
    REMOTE_VALUES.clear()
  ...
  val debugDiskValues: Map<String, Any>
    get() = TreeMap(parseStoredConfig(REDStore.remoteConfig.currentConfig))

  /** Only for rendering debug info.  */
  @JvmStatic
  @get:Synchronized
  val debugPendingDiskValues: Map<String, Any>
    get() = TreeMap(parseStoredConfig(REDStore.remoteConfig.pendingConfig))
  ...
  private fun remoteStringSet(
    ...
    return remoteValue(
      ...
      onChangeListener = onChangeListener,
      transformer = { value ->
        value?.let { REDJson.decode<Set<String>>(it.toString()).getOrNull() } ?: defaultValue
      }
    )
  }
   ...
   * [REDServiceMessageSender] send path.
   */
  @JvmStatic
