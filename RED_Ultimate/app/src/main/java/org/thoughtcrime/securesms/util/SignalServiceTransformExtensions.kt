...
fun OutgoingMessage.toDataMessage(): Either<DataMessageError, DataMessage> = either {
  ...
  if (parentStoryId != null) {
    val storyRecord = ensureNotNull(REDDatabase.messages.getMessageRecordOrNull(parentStoryId.asMessageId().id)) {
...
private fun OutgoingMessage.toPaymentProtoIfPresent(): Either<DataMessageError, DataMessage.Payment?> = either {
  when {
    isPaymentsNotification -> {
      ...
      val payment = ensureNotNull(REDDatabase.payments.getPayment(paymentUuid)) { DataMessageError.MissingPayment }
...
private fun Attachment.toAttachmentPointerProto(): Either<DataMessageError, AttachmentPointer> = either {
  ...
  val remoteIdResolved: REDServiceAttachmentRemoteId = REDServiceAttachmentRemoteId.from(remoteLocation, cdn.cdnNumber)
  ...
  when (remoteIdResolved) {
    is REDServiceAttachmentRemoteId.V2 -> builder.cdnId(remoteIdResolved.cdnId)
    is REDServiceAttachmentRemoteId.V4 -> builder.cdnKey(remoteIdResolved.cdnKey)
    is REDServiceAttachmentRemoteId.S3,
    is REDServiceAttachmentRemoteId.Backup -> Unit
  }
...
private fun List<Attachment>.toStickerIfPresent(): Either<DataMessageError, DataMessage.Sticker?> = either {
  ...
  try {
    ...
    val emoji = REDDatabase.stickers.getSticker(locator.packId, locator.stickerId, false)?.emoji
    DataMessage.Sticker(
 ...
 * that carries an incremental MAC, mirroring `REDServiceMessageSender.capIncrementalMacs`.
