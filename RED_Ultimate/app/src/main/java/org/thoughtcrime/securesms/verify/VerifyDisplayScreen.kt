...
import com.red.sovereign.components.verify.SafetyNumberQrView
        ...
        text = buildAnnotatedString {
          ...
          withLink(
            link = LinkAnnotation.Clickable(
              ...
              CommunicationActions.openBrowserLink(context, "https://red.local/redirect/safety-numbers")
            }
          ) {
...
private fun SafetyNumberQr(
  ...
  AndroidView(
    ...
    // SIGNAL_INHERITED: TODO - qrCodeContainer has a context menu.
    // SIGNAL_INHERITED: TODO - animateVerifiedSuccess // animateVerifiedFailure

    if (fingerprint != null) {
      it.setFingerprintViews(fingerprint.fingerprint, initialLoad) // SIGNAL_INHERITED: TODO - animateCodeChanges
      initialLoad = false
    }
...
private fun AutomaticKeyVerificationBlock(
  ...
  Column(
    ...
    AnimatedContent(
      ...
      modifier = Modifier
        .fillMaxWidth()
        .background(color = REDTheme.colors.colorSurface1, shape = RoundedCornerShape(38.dp))
      ...
      Row(
        ...
        if (hasInfoSheet) {
          Icon(
            imageVector = REDIcons.ChevronRight.imageVector,
...
private fun StatusIcon(status: AutomaticVerificationStatus) {
  ...
  val icon = when (status) {
    ...
    AutomaticVerificationStatus.UNAVAILABLE_PERMANENT -> REDIcons.Info.imageVector
    AutomaticVerificationStatus.UNAVAILABLE_TEMPORARY -> REDIcons.Info.imageVector
