...
import com.red.sovereign.backup.v2.DatabaseAttachmentArchiveUtil;
    ...
    if (inProgress && !hasData && hasIncrementalDigest && attachmentKey != null) {
      ...
      if (attachment.transferState == AttachmentTable.TRANSFER_RESTORE_IN_PROGRESS && attachment.archiveTransferState == AttachmentTable.ArchiveTransferState.FINISHED) {
        ...
        try {
          ...
          String mediaId   = MediaName.toMediaIdString(mediaName, REDStore.backup().getMediaRootBackupKey());

          MediaRootBackupKey.MediaKeyMaterial mediaKeyMaterial     = REDStore.backup().getMediaRootBackupKey().deriveMediaSecretsFromMediaId(mediaId);
