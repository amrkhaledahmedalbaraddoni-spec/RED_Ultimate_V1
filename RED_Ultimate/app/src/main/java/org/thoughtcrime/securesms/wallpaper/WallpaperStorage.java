...
public final class WallpaperStorage {
  ...
  public static @NonNull ChatWallpaper save(@NonNull InputStream wallpaperStream) throws IOException {
    AttachmentId attachmentId = REDDatabase.attachments().insertWallpaper(wallpaperStream);
  ...
  public static @NonNull List<ChatWallpaper> getAll() {
    return REDDatabase.attachments()
                         .getAllWallpapers()
                         .stream()
  ...
  public static boolean isWallpaperUriUsed(@NonNull Uri uri) {
    Uri globalUri = REDStore.wallpaper().getWallpaperUri();
    if (Objects.equals(uri, globalUri)) {
      return true;
    }

    int recipientCount = REDDatabase.recipients().getWallpaperUriUsageCount(uri);
    if (recipientCount > 0) {
      return true;
    }
