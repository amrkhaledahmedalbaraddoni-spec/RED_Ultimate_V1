...
/**
 * Commands that can be issued to [REDAudioManager] to perform various tasks.
 *
 * Additional context: The audio management is tied closely with the Android audio and thus benefits from being
 * tied to the [com.red.sovereign.service.webrtc.ActiveCallManager] lifecycle. Because of this, all
...
sealed class AudioManagerCommand : Parcelable {
  ...
  class SetDefaultDevice(val recipientId: RecipientId?, val device: REDAudioManager.AudioDevice, val clearUserEarpieceSelection: Boolean) : AudioManagerCommand() {
    override fun writeToParcel(parcel: Parcel, flags: Int) {
      parcel.writeParcelable(recipientId, flags)
    ...
    companion object {
      ...
      val CREATOR: Parcelable.Creator<SetDefaultDevice> = ParcelCheat { parcel ->
        SetDefaultDevice(
          ...
          device = parcel.readSerializableCompat(REDAudioManager.AudioDevice::class.java)!!,
