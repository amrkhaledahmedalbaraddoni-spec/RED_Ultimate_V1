...
import com.red.sovereign.audio.AudioDeviceUpdatedListener
import com.red.sovereign.audio.REDBluetoothManager
import com.red.sovereign.dependencies.AppDependencies
import com.red.sovereign.recipients.RecipientId
import com.red.sovereign.service.webrtc.AndroidTelecomUtil

private val TAG = Log.tag(REDAudioManager::class.java)

sealed class REDAudioManager(protected val context: Context, protected val eventListener: EventListener?) {

  private var commandAndControlThread = REDExecutors.getAndStartHandlerThread("call-audio", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
  protected val handler = REDAudioHandler(commandAndControlThread.looper)
  ...
  companion object {
    @SuppressLint("NewApi")
    @JvmStatic
    fun create(context: Context, eventListener: EventListener?, canUseTelecom: Boolean): REDAudioManager {
      return if (canUseTelecom && AndroidTelecomUtil.telecomSupported) {
        TelecomAudioManager(context, eventListener)
      } else if (Build.VERSION.SDK_INT >= 31) {
        FullREDAudioManagerApi31(context, eventListener)
      } else {
        FullREDAudioManager(context, eventListener)
      }
    }
  }
 ...
 */
class FullREDAudioManager(context: Context, eventListener: EventListener?) : REDAudioManager(context, eventListener), AudioDeviceUpdatedListener {
  private val signalBluetoothManager = REDBluetoothManager(context, this, handler)
  ...
  private var previousBluetoothState: REDBluetoothManager.State? = null
  ...
  override fun onAudioDeviceUpdated() {
    ...
    if (signalBluetoothManager.state == REDBluetoothManager.State.UNAVAILABLE && userSelectedAudioDevice == AudioDevice.BLUETOOTH) {
    ...
    val needBluetoothAudioStart = signalBluetoothManager.state == REDBluetoothManager.State.AVAILABLE &&
    ...
    val needBluetoothAudioStop = (signalBluetoothManager.state == REDBluetoothManager.State.CONNECTED || signalBluetoothManager.state == REDBluetoothManager.State.CONNECTING) &&
    ...
    if (!autoSwitchToBluetooth && signalBluetoothManager.state == REDBluetoothManager.State.UNAVAILABLE) {
    ...
    if (autoSwitchToBluetooth && signalBluetoothManager.state == REDBluetoothManager.State.CONNECTED) {
      userSelectedAudioDevice = AudioDevice.BLUETOOTH
      autoSwitchToBluetooth = false
    }

    if (previousBluetoothState != null && previousBluetoothState != REDBluetoothManager.State.PERMISSION_DENIED && signalBluetoothManager.state == REDBluetoothManager.State.PERMISSION_DENIED) {
