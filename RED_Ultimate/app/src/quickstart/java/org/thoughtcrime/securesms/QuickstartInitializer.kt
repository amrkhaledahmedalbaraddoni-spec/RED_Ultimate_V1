...
import com.red.sovereign.registration.data.QuickstartCredentials
...
object QuickstartInitializer {
  ...
  fun initialize(context: Context) {
    ...
    if (credentials.accountEntropyPool != null) {
      REDStore.account.restoreAccountEntropyPool(AccountEntropyPool(credentials.accountEntropyPool))
    ...
    REDStore.account.registrationId = credentials.registrationId
    REDStore.account.pniRegistrationId = credentials.pniRegistrationId
    ...
    REDStore.account.fcmEnabled = true

    // Finalize registration state
    REDStore.svr.optOut()
    REDStore.registration.restoreDecisionState = RestoreDecisionState.Skipped
    REDDatabase.recipients.setProfileName(Recipient.self().id, ProfileName.fromParts(credentials.profileGivenName, credentials.profileFamilyName))
    RegistrationUtil.maybeMarkRegistrationComplete()

    // Check for a local backup to restore
    val backupsDir = File(DISK_CREDENTIALS_DIR, "REDBackups")
  ...
  private val diskCredentialFileName: String
    get() = if (com.red.sovereign.util.Environment.IS_STAGING) "staging_credentials.json" else "prod_credentials.json"
