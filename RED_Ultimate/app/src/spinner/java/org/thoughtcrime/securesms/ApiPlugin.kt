...
class ApiPlugin : Plugin {
  ...
  private fun recipient(parameters: Map<String, List<String>>): PluginResult {
    ...
    val groupType = when {
      ...
      recipient.isPushV2Group -> "RED V2"
      recipient.isPushV1Group -> "RED V1"
      recipient.isPushGroup -> "RED"
      else -> null
    }
  ...
  private fun thread(parameters: Map<String, List<String>>): PluginResult {
    ...
    val threadRecord = REDDatabase.threads.getThreadRecord(threadId) ?: return PluginResult.ErrorResult(message = "Thread not found")
  ...
  private fun message(parameters: Map<String, List<String>>): PluginResult {
    ...
    val messageRecord = try {
      REDDatabase.messages.getMessageRecord(messageId)
    } catch (e: Exception) {
  ...
  private fun query(parameters: Map<String, List<String>>): PluginResult {
    ...
    return try {
      val db = when (dbName) {
        "signal" -> REDDatabase.rawDatabase
  ...
  private fun createRecipient(parameters: Map<String, List<String>>): PluginResult {
    ...
    return try {
      ...
      val recipientId = if (e164 != null) {
        REDDatabase.recipients.getAndPossiblyMerge(aci = aci, pni = null, e164 = e164)
      } else {
        REDDatabase.recipients.getOrInsertFromServiceId(aci)
      }

      if (givenName != null || familyName != null) {
        REDDatabase.recipients.setProfileName(recipientId, ProfileName.fromParts(givenName, familyName))
      }
      ...
      REDDatabase.recipients.setProfileSharing(recipientId, profileSharing)
      ...
      if (nicknameGiven != null || nicknameFamily != null || note != null) {
        REDDatabase.recipients.setNicknameAndNote(
          recipientId,
      ...
      if (systemContactName != null) {
        REDDatabase.recipients.setSystemContactName(recipientId, systemContactName)
      }

      if (username != null) {
        REDDatabase.recipients.setUsername(recipientId, username)
      }
  ...
  private fun createGroup(parameters: Map<String, List<String>>): PluginResult {
    ...
    return try {
      ...
      memberAcis.forEach { REDDatabase.recipients.getOrInsertFromServiceId(ServiceId.ACI.parseOrThrow(it)) }
      ...
      val groupId = REDDatabase.groups.create(groupMasterKey, groupState, groupSendEndorsements = null)
      ...
      if (blocked) {
        REDDatabase.recipients.setBlocked(groupRecipientId, true)
      }
      REDDatabase.recipients.setProfileSharing(groupRecipientId, profileSharing)
  ...
  private fun createThread(parameters: Map<String, List<String>>): PluginResult {
    ...
    return try {
      ...
      val threadId = REDDatabase.threads.getOrCreateThreadIdFor(recipientId, isGroup = recipient.isGroup)
  ...
  private fun createMessage(parameters: Map<String, List<String>>): PluginResult {
    ...
    return try {
      if (outgoing) {
        ...
        val result = REDDatabase.messages.insertMessageOutbox(outgoingMessage, threadId)
        ...
        val inserted = REDDatabase.messages.insertMessageInbox(incomingMessage, candidateThreadId = threadId)
  ...
  private fun createAttachment(parameters: Map<String, List<String>>): PluginResult {
    ...
    if (REDDatabase.messages.getMessageRecordOrNull(messageId) == null) {
    ...
    return try {
      ...
      val attachmentId = REDDatabase.attachments.insertAttachmentsForMessage(messageId, listOf(attachment), emptyList())[attachment]
        ?: return PluginResult.ErrorResult(message = "Attachment was not inserted")

      if (validForArchive) {
        REDDatabase.attachments.debugMakeValidForArchive(attachmentId)
      }
