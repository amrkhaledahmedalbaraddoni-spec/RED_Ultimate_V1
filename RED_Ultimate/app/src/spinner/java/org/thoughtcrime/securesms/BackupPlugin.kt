...
class BackupPlugin : Plugin {
  ...
  private fun getSelectedRemoteBackup(): String {
    ...
    val frameReader = EncryptedBackupReader.createForREDBackup(
      key = backupKey,
      aci = selfData.aci,
  ...
  private fun getRemoteBackup(): String {
    return if (REDStore.backup.hasBackupBeenUploaded) {
      """<a href="$PATH?remoteBackup=true">${formatTimestamp(REDStore.backup.lastBackupTime)} - ${REDStore.backup.lastBackupProtoSize.bytes.toUnitString()}</a>"""
    } else {
      "No remote backup"
    }
  }
