...
import com.red.sovereign.backup.ArchiveUploadProgress.ArchiveBackupProgressListener
...
import com.red.sovereign.keyvalue.protos.ArchiveUploadProgressState.BackupPhase
import com.red.sovereign.keyvalue.protos.ArchiveUploadProgressState.State
...
class ArchiveUploadProgressTest {
  ...
  companion object {
    ...
    fun setUpClass() {
      ...
      val signalDatabase = mockk<REDDatabase>(relaxed = true)
      ...
      mockkStatic(REDLocalMetrics.ArchiveAttachmentUpload::class)
      every { REDLocalMetrics.ArchiveAttachmentUpload.start(any()) } just Runs
      every { REDLocalMetrics.ArchiveAttachmentUpload.end(any()) } just Runs
    }
    ...
    fun tearDownClass() {
      ...
      unmockkStatic(REDLocalMetrics.ArchiveAttachmentUpload::class)
    }
  }
  ...
  fun `onAttachmentStarted - registers attachment and starts metric`() {
    ...
    verify { REDLocalMetrics.ArchiveAttachmentUpload.start(attachmentId) }
  ...
  fun `onAttachmentFinished - removes attachment and ends metric`() {
    ...
    verify { REDLocalMetrics.ArchiveAttachmentUpload.end(attachmentId) }
