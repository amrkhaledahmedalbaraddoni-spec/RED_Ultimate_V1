...
package com.red.sovereign.backup.v2.stream
...
class EncryptedBackupReaderWriterTest {
  ...
  fun `can read back all of the frames we write - forward secrecy`() {
    ...
    val frameCount = 10_000
    EncryptedBackupWriter.createForREDBackup(
      key = key,
      aci = aci,
    ...
    val frames: List<Frame> = EncryptedBackupReader.createForREDBackup(key, aci, forwardSecrecyToken, ciphertext.size.toLong()) { ciphertext.inputStream() }.use { reader ->
      assertEquals(reader.backupInfo?.version, 1L)
      assertEquals(reader.backupInfo?.backupTimeMs, 1000L)
      reader.asSequence().toList()
    }
  ...
  fun `can read back all frames using BackupId directly - forward secrecy`() {
    ...
    val frames: List<Frame> = EncryptedBackupReader.createForREDBackup(key, backupId, forwardSecrecyToken, ciphertext.size.toLong()) { ciphertext.inputStream() }.use { reader ->
      assertEquals(reader.backupInfo?.version, 1L)
      assertEquals(reader.backupInfo?.backupTimeMs, 1000L)
      reader.asSequence().toList()
    }
