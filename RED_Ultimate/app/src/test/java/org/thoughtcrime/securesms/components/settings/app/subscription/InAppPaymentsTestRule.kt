...
class InAppPaymentsTestRule : ExternalResource() {
  ...
  override fun before() {
    ...
    every { REDDatabase.Companion.donationReceipts } returns mockk {
      every { REDDatabase.Companion.donationReceipts.addReceipt(any()) } returns Unit
    }

    every { REDDatabase.Companion.inAppPayments } returns mockk {
      every { REDDatabase.Companion.inAppPayments.insert(any(), any(), any(), any(), any()) } answers {
      ...
      every { REDDatabase.Companion.inAppPayments.update(any()) } answers {
        val inAppPayment = firstArg<InAppPaymentTable.InAppPayment>()
        inAppPaymentCache[inAppPayment.id] = inAppPayment
      }

      every { REDDatabase.Companion.inAppPayments.getById(any()) } answers {
        val inAppPaymentId = firstArg<InAppPaymentTable.InAppPaymentId>()
        inAppPaymentCache[inAppPaymentId]
      }
    }

    mockkObject(REDStore.Companion)
    every { REDStore.Companion.inAppPayments } returns mockk {
      every { setLastEndOfPeriod(any()) } returns Unit
    }
    every { REDStore.Companion.backup } returns mockk {
  ...
  override fun after() {
    ...
    unmockkObject(InAppDonations, REDDatabase.Companion, REDStore.Companion)
  }
  ...
  companion object {
    fun mockLocalSubscriberAccess(initialSubscriber: InAppPaymentSubscriberRecord? = null): AtomicReference<InAppPaymentSubscriberRecord?> {
      ...
      every { REDDatabase.inAppPaymentSubscribers.getBySubscriberId(any()) } answers {
        ref.get()
      }
      every { REDDatabase.inAppPaymentSubscribers.insertOrReplace(any()) } answers {
        ref.set(firstArg())
      }
