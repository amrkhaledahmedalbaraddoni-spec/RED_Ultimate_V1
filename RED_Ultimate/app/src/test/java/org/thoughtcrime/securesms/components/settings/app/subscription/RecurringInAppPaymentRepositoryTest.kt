...
import com.red.sovereign.testutil.RxPluginsRule
...
class RecurringInAppPaymentRepositoryTest {
  ...
  fun setUp() {
    ...
    every { REDStore.Companion.inAppPayments } returns mockk {
      every { REDStore.Companion.inAppPayments.getRecurringDonationCurrency() } returns Currency.getInstance("USD")
      every { REDStore.Companion.inAppPayments.updateLocalStateForManualCancellation(any()) } returns Unit
      every { REDStore.Companion.inAppPayments.updateLocalStateForLocalSubscribe(any()) } returns Unit
    }

    every { REDDatabase.Companion.recipients } returns mockk {
      every { REDDatabase.Companion.recipients.markNeedsSync(any<RecipientId>()) } returns Unit
    }
