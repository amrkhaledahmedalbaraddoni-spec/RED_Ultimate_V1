...
class AttachmentTableTest_createRemoteKeyForAttachmentsThatNeedArchiveUpload {
  ...
  fun whenNoEligibleAttachments_returnsZero() {
    val result = REDDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
  ...
  fun whenAttachmentHasArchiveTransferStateInProgress_returnsZero() {
    ...
    REDDatabase.attachments.setArchiveTransferState(attachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
  ...
  fun whenTransferStateNotDone_returnsZero() {
    val attachmentId = insertWithData()
    REDDatabase.attachments.setTransferState(1L, attachmentId, 1) // Not TRANSFER_PROGRESS_DONE (0)
  ...
  fun whenOneEligibleAttachment_returnsOneAndCreatesRemoteKey() {
    ...
    val attachmentBefore = REDDatabase.attachments.getAttachment(attachmentId)
    ...
    val attachmentAfter = REDDatabase.attachments.getAttachment(attachmentId)
  ...
  fun whenMultipleEligibleAttachments_returnsCorrectCountAndCreatesKeys() {
    ...
    assertThat(REDDatabase.attachments.getAttachment(attachmentId1)?.remoteKey).isNull()
    assertThat(REDDatabase.attachments.getAttachment(attachmentId2)?.remoteKey).isNull()
    assertThat(REDDatabase.attachments.getAttachment(attachmentId3)?.remoteKey).isNull()

    val result = REDDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    assertThat(result.totalCount).isEqualTo(3)

    // Verify all remote keys were created
    assertThat(REDDatabase.attachments.getAttachment(attachmentId1)?.remoteKey).isNotNull()
    assertThat(REDDatabase.attachments.getAttachment(attachmentId2)?.remoteKey).isNotNull()
    assertThat(REDDatabase.attachments.getAttachment(attachmentId3)?.remoteKey).isNotNull()
  }
  ...
  fun whenMixedScenarios_returnsCorrectCount() {
    ...
    REDDatabase.attachments.setArchiveTransferState(inProgressAttachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
    ...
    assertThat(REDDatabase.attachments.getAttachment(eligibleAttachmentId)?.remoteKey).isNotNull()
    assertThat(REDDatabase.attachments.getAttachment(attachmentWithKeyId)?.remoteKey).isNotNull() // Already had one
    assertThat(REDDatabase.attachments.getAttachment(inProgressAttachmentId)?.remoteKey).isNull()
    assertThat(REDDatabase.attachments.getAttachment(noDataFileAttachmentId)?.remoteKey).isNull()
  }
  ...
  fun whenCalledTwice_secondCallReturnsZero() {
    ...
    val firstResult = REDDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    assertThat(firstResult.totalCount).isEqualTo(1)

    // Second call should find no eligible attachments since remote key now exists
    val secondResult = REDDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    assertThat(secondResult.totalCount).isEqualTo(0)

    // Verify attachment still has remote key
    assertThat(REDDatabase.attachments.getAttachment(attachmentId)?.remoteKey).isNotNull()
  }
  ...
  fun whenMatchingDataFileAndHashExists_reusesRemoteKey() {
    ...
    val targetAttachment = REDDatabase.attachments.getAttachment(targetAttachmentId)!!
  ...
  fun whenSourceHasNoRemoteData_generatesNewKey() {
    ...
    assertThat(REDDatabase.attachments.getAttachment(sourceAttachmentId)?.remoteKey).isNotNull()
    assertThat(REDDatabase.attachments.getAttachment(targetAttachmentId)?.remoteKey).isNotNull()
  }
  ...
  fun whenTwoAttachmentsShareDataFileAndBothNeedNewKey_bothGetSameKey() {
    ...
    val attachment1 = REDDatabase.attachments.getAttachment(attachmentId1)!!
    val attachment2 = REDDatabase.attachments.getAttachment(attachmentId2)!!
  ...
  private fun insertAttachmentDirectly(
    ...
    val db = REDDatabase.attachments.writableDatabase
