...
class AttachmentTableTest_localRestoreArchiveState {
  ...
  fun hasArchiveFinishedLocalBackupMedia_trueForFinishedLocalBackupMedia() {
    ...
    assertThat(REDDatabase.attachments.hasArchiveFinishedLocalBackupMedia()).isTrue()
  }
  ...
  fun hasArchiveFinishedLocalBackupMedia_falseWhenFinishedButNotFromLocalBackup() {
    ...
    assertThat(REDDatabase.attachments.hasArchiveFinishedLocalBackupMedia()).isFalse()
  }
  ...
  fun hasArchiveFinishedLocalBackupMedia_falseOnceLocalBackupMediaIsNoLongerFinished() {
    ...
    REDDatabase.attachments.resetArchiveTransferStateForLocalBackupMedia()
  ...
  private fun insertArchivedAttachment(archiveCdn: Int?, localBackupKey: ByteArray?): AttachmentId {
    ...
    val threadId = REDDatabase.threads.getOrCreateThreadIdFor(Recipient.resolved(from))
    ...
    val messageId = REDDatabase.messages.insertMessageInbox(message, threadId).get().messageId
    return REDDatabase.attachments.getAttachmentsForMessage(messageId).first().attachmentId
  }
