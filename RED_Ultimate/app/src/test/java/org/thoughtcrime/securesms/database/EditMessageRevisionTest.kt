...
class EditMessageRevisionTest {
  ...
  fun setUp() {
    ...
    threadId = REDDatabase.threads.getOrCreateThreadIdFor(senderId, false, ThreadTable.DistributionTypes.DEFAULT)
  }
  ...
  fun removeDuplicatesRepairsOrphanedLatestRevisionIdInsteadOfLeavingForeignKeyViolation() {
    ...
    REDDatabase.writableDatabase.execSQL("PRAGMA foreign_keys=OFF")
    REDDatabase.writableDatabase.execSQL("DELETE FROM ${MessageTable.TABLE_NAME} WHERE ${MessageTable.ID} = ?", arrayOf(edit2Id))
    assertThat(getLatestRevisionId(originalId)).isNotNull().isEqualTo(edit2Id)
    ...
    REDDatabase.writableDatabase.execSQL("PRAGMA foreign_keys=ON")
  }

  private fun countDanglingLatestRevisionIds(): Int {
    return REDDatabase.writableDatabase
  ...
  private fun insertEdit(originalSentTimestamp: Long, editSentTimeMillis: Long): Long {
    val targetMessage = REDDatabase.messages.getMessageFor(originalSentTimestamp, senderId) as MmsMessageRecord
    ...
    return REDDatabase.messages.insertEditMessageInbox(editMessage, targetMessage).get().messageId
  }
  ...
  private fun getNotificationStateMessageIds(): List<Long> {
    return REDDatabase.messages.getMessagesForNotificationState(emptyList()).use { cursor ->
  ...
  private fun markAsRead(messageId: Long) {
    REDDatabase.writableDatabase.execSQL(
