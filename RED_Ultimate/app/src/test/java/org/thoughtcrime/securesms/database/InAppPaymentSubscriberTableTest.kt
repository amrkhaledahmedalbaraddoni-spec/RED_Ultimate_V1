...
class InAppPaymentSubscriberTableTest {
  ...
  @Before
  fun setUp() {
    REDDatabase.inAppPaymentSubscribers.writableDatabase.deleteAll(InAppPaymentTable.TABLE_NAME)
  }
  ...
  fun givenABackupSubscriberWithAppleIAPData_whenITryToInsertAGoogleSubscriber_thenIExpectSuccess() {
    ...
    REDDatabase.inAppPaymentSubscribers.insertOrReplace(appleSubscriber)
    ...
    REDDatabase.inAppPaymentSubscribers.insertOrReplace(googleSubscriber)

    val subscriberCount = REDDatabase.inAppPaymentSubscribers.readableDatabase.count()
