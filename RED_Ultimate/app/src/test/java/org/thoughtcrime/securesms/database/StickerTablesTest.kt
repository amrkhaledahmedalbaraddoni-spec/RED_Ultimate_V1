...
import com.red.sovereign.database.model.StickerPackRecord as LocalStickerPackRecord
...
class StickerTablesTest {
  ...
  fun setUp() {
    ...
    REDDatabase.stickers.writableDatabase.deleteAll(StickerTables.Sticker.TABLE_NAME)
    REDDatabase.stickers.writableDatabase.deleteAll(StickerTables.Pack.TABLE_NAME)
  }
  ...
  fun `given an installed pack, when I get it for storage sync, then I expect a storage id and no tombstone`() {
    ...
    val pack = REDDatabase.stickers.getPackForStorageSync(StickerPackId(packId1))!!
  ...
  fun `given two installed packs, when I install them, then I expect increasing positions`() {
    ...
    val pack1 = REDDatabase.stickers.getPackForStorageSync(StickerPackId(packId1))!!
    val pack2 = REDDatabase.stickers.getPackForStorageSync(StickerPackId(packId2))!!
  ...
  fun `given an installed pack, when I uninstall it, then I expect a tombstone with a rotated storage id`() {
    installPack(packId1, packKey1)
    val originalId = REDDatabase.stickers.getPackForStorageSync(StickerPackId(packId1))!!.storageServiceId!!

    REDDatabase.stickers.uninstallPack(packId1)
  ...
  fun `given a tombstoned pack, when I reinstall it, then I expect the tombstone cleared and the max position`() {
    ...
    REDDatabase.stickers.markPackAsInstalled(packId1, notify = false)
  ...
  fun `given a remote record, when I insert it locally, then I expect an installed pack with the remote storage id`() {
    val remoteRecord = REDStickerPackRecord(
    ...
    REDDatabase.stickers.insertStickerPackFromStorageSync(remoteRecord)
  ...
  fun `given an installed pack, when I apply a deleted remote record, then I expect it to be uninstalled`() {
    ...
    REDDatabase.stickers.updateStickerPackFromStorageSync(remoteRecord)
  ...
  fun `given installed packs, when I update their storage sync ids, then I expect an updated map`() {
    ...
    val existingMap = REDDatabase.stickers.getStorageSyncIdsMap()
    existingMap.forEach { (id, _) ->
      REDDatabase.stickers.applyStorageIdUpdate(id, StorageId.forStickerPack(StorageSyncHelper.generateKey()))
    }
    val updatedMap = REDDatabase.stickers.getStorageSyncIdsMap()
  ...
  fun `given a pack deleted longer than the message queue time, when I clean up, then I expect it to not have a storage id`() {
    ...
    REDDatabase.stickers.removeStorageIdsFromOldDeletedPacks(System.currentTimeMillis() + TimeUnit.DAYS.toMillis(46))

    assertThat(REDDatabase.stickers.getStorageSyncIds()).isEmpty()
    assertThat(REDDatabase.stickers.getPackForStorageSync(StickerPackId(packId1))!!.storageServiceId).isNull()
  }
  ...
  fun `given installed packs, when I update the pack positions, then I expect display-order positions and rotated ids`() {
    ...
    val originalIds = REDDatabase.stickers.getStorageSyncIdsMap()

    REDDatabase.stickers.updatePackPositions(
      listOf(
    ...
    val pack1 = REDDatabase.stickers.getPackForStorageSync(StickerPackId(packId1))!!
    val pack2 = REDDatabase.stickers.getPackForStorageSync(StickerPackId(packId2))!!
    val pack3 = REDDatabase.stickers.getPackForStorageSync(StickerPackId(packId3))!!

    assertThat(pack3.position).isEqualTo(0)
    assertThat(pack1.position).isEqualTo(1)
    assertThat(pack2.position).isEqualTo(2)

    val updatedIds = REDDatabase.stickers.getStorageSyncIdsMap()
  ...
  private fun installedPackIds(): List<String> {
    return StickerTables.StickerPackRecordReader(REDDatabase.stickers.getInstalledStickerPacks()).use { reader ->
      reader.asSequence().map { it.packId }.toList()
    }
  }

  private fun installPack(packId: String, packKey: String) {
    REDDatabase.stickers.insertSticker(
