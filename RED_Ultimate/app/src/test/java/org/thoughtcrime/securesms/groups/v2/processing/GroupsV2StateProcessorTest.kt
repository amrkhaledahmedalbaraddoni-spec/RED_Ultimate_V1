...
import com.red.sovereign.database.GroupStateTestData
...
import com.red.sovereign.database.model.databaseprotos.member
import com.red.sovereign.database.model.databaseprotos.pendingMember
import com.red.sovereign.database.model.databaseprotos.requestingMember
import com.red.sovereign.database.setNewDescription
import com.red.sovereign.database.setNewTitle
...
import com.red.sovereign.groups.v2.processing.GroupsV2StateProcessor.ProfileAndMessageHelper
...
class GroupsV2StateProcessorTest {
  ...
  fun setUp() {
    ...
    every { REDStore.internal.gv2IgnoreP2PChanges } returns false
    ...
    every { REDDatabase.groups } returns groupTable
    every { REDDatabase.recipients } returns recipientTable
    every { REDDatabase.threads } returns threadTable
  ...
  fun updateFromServerIfUnableToApplyP2PChange() {
    given {
      ...
      serverState(
        revision = 2,
        title = "Breaking RED for Science",
        members = selfAndOthers
      )
      changeSet {
        changeLog(2) {
    ...
    val signedChange = DecryptedGroupChange(
      revision = 2,
      newTitle = DecryptedString("Breaking RED for Science")
    )
  ...
  fun `when freshly added to a group, with additional group changes after being added, then only update from server at the revision we were added, and then schedule pulling additional changes later`() {
    given {
      changeSet {
        ...
        changeLog(3) {
          change {
            setNewTitle("Breaking RED for Science")
          }
        }
      }
      apiCallParameters(2, true)
    ...
    assertThat(result.latestServer)
      ...
      .given {
        ...
        assertThat(it.title, "title matches that as it was in revision added").isEqualTo("Baking RED for Science")
      }
   ...
   */
  @Test
  fun ignore403sWithoutREDTimestampHeader() {
    given {
      localState(
        active = false,
        revision = 5,
