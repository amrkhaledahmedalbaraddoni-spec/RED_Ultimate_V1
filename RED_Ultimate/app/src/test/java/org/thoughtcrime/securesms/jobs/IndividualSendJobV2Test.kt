...
import com.red.sovereign.database.PendingPniSignatureMessageTable
...
import com.red.sovereign.util.DataMessageError
 ...
 * coroutine entry point) with all of its collaborators mocked: [REDStore], [REDDatabase] tables,
...
class IndividualSendJobV2Test {
  ...
  @get:Rule
  val signalStore = MockREDStoreRule()
  ...
  fun setUp() {
    ...
    every { REDDatabase.messages } returns messages
    every { REDDatabase.messageLog } returns messageLog
    every { REDDatabase.threads } returns threads
    every { REDDatabase.recipients } returns recipientsTable
    every { REDDatabase.attachments } returns attachments
    every { REDDatabase.pendingPniSignatureMessages } returns pendingPniSignatureMessages
    ...
    mockkStatic("com.red.sovereign.util.REDServiceTransformExtensionsKt")
  ...
  fun `Given client is deprecated, when run, then return failure and skip send`() {
    every { REDStore.misc.isClientDeprecated } returns true
