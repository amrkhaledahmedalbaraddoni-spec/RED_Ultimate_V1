...
class PreKeysSyncJobTest {
  ...
  private val aciProtocolStore: REDServiceAccountDataStoreImpl = mockk(relaxed = true)
  private val pniProtocolStore: REDServiceAccountDataStoreImpl = mockk(relaxed = true)
  ...
  fun setUp() {
    ...
    every { mockREDStore.account.aci } returns ServiceId.ACI.from(UUID.randomUUID())
    every { mockREDStore.account.pni } returns ServiceId.PNI.from(UUID.randomUUID())
    every { mockREDStore.account.aciPreKeys } returns aciMetadataStore
    every { mockREDStore.account.pniPreKeys } returns pniMetadataStore
