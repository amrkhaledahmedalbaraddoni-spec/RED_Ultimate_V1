...
import com.red.sovereign.testutil.FakeStorageServiceRule
...
class StorageSyncJobTest {
  ...
  fun `given no access to storage service, when I run, then I touch nothing`() {
    ...
    REDDatabase.recipients.rotateStorageId(recipients.createRecipient("Local Contact"))
  ...
  fun `given a remote-only contact, when I run, then I insert it locally`() {
    ...
    val inserted = REDDatabase.recipients.getByAci(aci)
  ...
  fun `given a local-only contact, when I run, then I write it remotely`() {
    ...
    REDDatabase.recipients.rotateStorageId(local)
  ...
  fun `given an unknown ID for a sticker pack I do not have, when I run, then I clear it out`() {
    ...
    assertTrue(result.isSuccess)
    assertTrue(REDDatabase.unknownStorageIds.allUnknownIds.isEmpty())
    assertEquals(0, remoteStorage.writeCount)
    assertEquals(BASE_MANIFEST_VERSION, remoteStorage.manifest!!.version)
  }
  ...
  fun `given an unknown ID for a sticker pack that exists remotely, when I run, then I promote it to a real sticker pack`() {
    ...
    val pack = REDDatabase.stickers.getPackForStorageSync(StickerPackId(Hex.toStringCondensed(packId)))
    assertNotNull(pack)
  ...
  private fun runInitialSync() {
    REDDatabase.recipients.updateStorageId(recipients.self, StorageSyncHelper.generateKey())
  ...
  private fun contactRecord(aci: ACI, profileName: ProfileName): REDStorageRecord {
    return REDStorageRecord(
  ...
  private fun stickerPackRecord(packId: ByteArray): REDStorageRecord {
    return REDStorageRecord(
  ...
  private fun insertUnknownStorageId(id: StorageId) {
    REDDatabase.writableDatabase.withinTransaction {
      REDDatabase.unknownStorageIds.insert(listOf(REDStorageRecord.forUnknown(id)))
    }
  }
