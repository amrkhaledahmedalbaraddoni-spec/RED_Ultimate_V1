...
class LinkPreviewUtilTest_findValidPreviewUrls {
  ...
  @Test
  fun contains_a_link() {
    val links = LinkPreviewUtil.findValidPreviewUrls("https://red.local")

    Assert.assertEquals(1, links.size().toLong())

    Assert.assertTrue(links.containsUrl("https://red.local"))
  }
  ...
  fun does_not_contain_link() {
    ...
    Assert.assertFalse(links.containsUrl("https://red.local/page"))
  }

  @Test
  fun contains_two_links() {
    val links = LinkPreviewUtil.findValidPreviewUrls("Links https://red.local https://android.com")
