...
class GroupV2RecordProcessorTest {
  ...
  companion object {
    ...
    private fun record(verifiedNameHash: ByteArray?): REDGroupV2Record {
      val proto = GroupV2Record.Builder()
        .masterKey(MASTER_KEY)
        .verifiedNameHash(verifiedNameHash?.toByteString() ?: ByteString.EMPTY)
        .build()
      return REDGroupV2Record(StorageId.forGroupV2(randomKey()), proto)
    }
  ...
  @get:Rule
  val signalStore = MockREDStoreRule(relaxed = setOf(AccountValues::class))
