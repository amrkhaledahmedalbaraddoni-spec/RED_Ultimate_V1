...
/**
 * An in-memory stand-in for the storage service, installed over [REDNetwork.storageService].
 *
...
class FakeStorageServiceRule(val storageKey: StorageKey = StorageKey(Util.getSecretBytes(32))) : ExternalResource() {
  ...
  val manifest: REDStorageManifest?
  ...
  val records: List<REDStorageRecord>
    get() {
  ...
  override fun before() {
    ...
    mockkObject(REDNetwork)
    every { REDNetwork.storageService } returns api
  }

  override fun after() {
    unmockkObject(REDNetwork)
    unmockkObject(RemoteConfig)
  }

  /**
   * Stubs every [REDStore] and [RemoteConfig] value that a storage service sync touches, with the storage
   ...
   * Must be called after [MockREDStoreRule] has been applied, i.e. from your test's setup.
   */
  fun stubDefaults(store: MockREDStoreRule) {
    var localManifest = REDStorageManifest.EMPTY
