...
private typealias LibREDAccountEntropyPool = org.signal.libsignal.messagebackup.AccountEntropyPool
...
class AccountEntropyPool(value: String) {
  ...
  companion object {
    ...
    fun generate(): AccountEntropyPool {
      return AccountEntropyPool(LibREDAccountEntropyPool.generate())
    }
    ...
    fun isFullyValid(input: String): Boolean {
      return LibREDAccountEntropyPool.isValid(input)
    }
  ...
  fun deriveMasterKey(): MasterKey {
    return MasterKey(LibREDAccountEntropyPool.deriveSvrKey(value))
  }

  fun deriveMessageBackupKey(): MessageBackupKey {
    val libREDBackupKey = LibREDAccountEntropyPool.deriveBackupKey(value)
    return MessageBackupKey(libREDBackupKey.serialize())
  }
}
