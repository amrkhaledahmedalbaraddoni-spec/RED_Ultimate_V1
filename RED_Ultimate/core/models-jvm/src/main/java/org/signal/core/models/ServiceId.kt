 ...
 */
sealed class ServiceId(val libREDServiceId: org.signal.libsignal.protocol.ServiceId) {
  companion object {
    private const val TAG = "ServiceId"

    @JvmStatic
    fun fromLibRED(serviceId: org.signal.libsignal.protocol.ServiceId): ServiceId {
    ...
    fun parseOrNull(raw: String?, logFailures: Boolean = true): ServiceId? {
      ...
      return try {
        fromLibRED(org.signal.libsignal.protocol.ServiceId.parseFromString(raw))
    ...
    fun parseOrNull(raw: ByteArray?): ServiceId? {
      ...
      return try {
        if (raw.size == 17) {
          fromLibRED(org.signal.libsignal.protocol.ServiceId.parseFromFixedWidthBinary(raw))
        } else {
          fromLibRED(org.signal.libsignal.protocol.ServiceId.parseFromBinary(raw))
        }
      } catch (e: IllegalArgumentException) {
  ...
  val rawUuid: UUID = libREDServiceId.rawUUID

  val isUnknown: Boolean = rawUuid == UuidUtil.UNKNOWN_UUID

  val isValid: Boolean = !isUnknown

  fun toProtocolAddress(deviceId: Int): REDProtocolAddress = REDProtocolAddress(libREDServiceId.toServiceIdString(), deviceId)

  fun toByteString(): ByteString = ByteString.Companion.of(*libREDServiceId.toServiceIdBinary())

  fun toByteArray(): ByteArray = libREDServiceId.toServiceIdBinary()

  fun logString(): String = libREDServiceId.toLogString()
   ...
   */
  override fun toString(): String = libREDServiceId.toServiceIdString()

  data class ACI(val libREDAci: org.signal.libsignal.protocol.ServiceId.Aci) : ServiceId(libREDAci) {
    companion object {
      @JvmField
      ...
      @JvmStatic
      fun fromLibRED(aci: org.signal.libsignal.protocol.ServiceId.Aci): ACI = ACI(aci)
  ...
  data class PNI(val libREDPni: org.signal.libsignal.protocol.ServiceId.Pni) : ServiceId(libREDPni) {
    companion object {
      @JvmField
