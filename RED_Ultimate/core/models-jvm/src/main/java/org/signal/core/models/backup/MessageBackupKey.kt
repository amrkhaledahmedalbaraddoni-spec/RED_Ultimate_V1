...
private typealias LibREDBackupKey = org.signal.libsignal.messagebackup.BackupKey
...
class MessageBackupKey(override val value: ByteArray) : BackupKey {
  ...
  override fun deriveAnonymousCredentialPrivateKey(aci: ServiceId.ACI): ECPrivateKey {
    return LibREDBackupKey(value).deriveEcKey(aci.libREDAci)
  }
  ...
  fun deriveBackupSecrets(backupId: BackupId, forwardSecrecyToken: BackupForwardSecrecyToken?): BackupKeyMaterial {
    val libsignalBackupKey = LibREDBackupKey(value)
  ...
  fun deriveBackupId(aci: ServiceId.ACI): BackupId {
    return BackupId(
      LibREDBackupKey(value).deriveBackupId(aci.libREDAci)
    )
  }
  ...
  fun deriveLocalBackupMetadataKey(): ByteArray {
    return LibREDBackupKey(value).deriveLocalBackupMetadataKey()
  }
