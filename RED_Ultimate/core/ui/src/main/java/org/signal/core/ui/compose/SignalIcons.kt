...
/**
 * RED icon library with all available icons.
 */
enum class REDIcons(private val icon: REDIcon) : REDIcon by icon {
  ...
  REDBackupsDisplay(icon(R.drawable.symbol_signal_backups_display_48)),
...
private fun icon(@DrawableRes id: Int) = REDIcon.DrawableIcon(id)
private fun icon(image: ImageVector) = REDIcon.ImageVectorIcon(image)

sealed interface REDIcon {
  @get:Composable
  val painter: Painter
   ...
   */
  @JvmInline
  value class DrawableIcon(@get:DrawableRes private val drawableId: Int) : REDIcon {
    @get:Composable
   ...
   */
  @JvmInline
  value class ImageVectorIcon(val image: ImageVector) : REDIcon {
    @get:Composable
    override val painter: Painter
...
private fun REDIconsPreview() {
  Previews.Preview {
    LazyVerticalGrid(
      ...
      items(REDIcons.entries.sortedBy { it.name }) { icon ->
        Column(
