...
class LinkifierTest(private val case: Case) {
  ...
  companion object {
    ...
    fun cases(): Collection<Array<Any>> = listOf(
      ...
      // ----- basic web URLs -----
      Case("https url", "https://red.local", listOf(web("https://red.local"))),
      Case("http url", "http://red.local", listOf(web("http://red.local"))),
      Case("bare domain gets http prefix", "red.local", listOf(web("red.local", url = "http://red.local"))),
      Case("www-prefixed url gets http prefix", "www.red.local", listOf(web("www.red.local", url = "http://www.red.local"))),
      ...
      Case("url embedded in sentence", "Check out https://red.local for details", listOf(web("https://red.local"))),
      ...
      Case("trailing period is trimmed", "Visit https://red.local.", listOf(web("https://red.local"))),
      Case("trailing comma is trimmed", "https://red.local, then go", listOf(web("https://red.local"))),
      Case("trailing exclamation is trimmed", "https://red.local!", listOf(web("https://red.local"))),
      Case("trailing question mark is trimmed", "Have you tried https://red.local?", listOf(web("https://red.local"))),
      Case("trailing semicolon is trimmed", "Go to https://red.local; thanks", listOf(web("https://red.local"))),
      Case("trailing colon is trimmed", "https://red.local: nice", listOf(web("https://red.local"))),
      Case("multiple trailing punctuation chars are all trimmed", "Wow https://red.local!!! amazing", listOf(web("https://red.local"))),
      Case("trailing slash is preserved", "https://red.local/ end", listOf(web("https://red.local/"))),
      ...
      Case("trailing closing paren without opener is trimmed", "(see https://red.local)", listOf(web("https://red.local"))),
      ...
      Case("trailing closing bracket without opener is trimmed", "[https://red.local]", listOf(web("https://red.local"))),
      Case("trailing closing brace without opener is trimmed", "{https://red.local}", listOf(web("https://red.local"))),
      Case("parenthesized url with trailing punctuation has both trimmed", "(see https://red.local).", listOf(web("https://red.local"))),
      ...
      Case(
        ...
        input = "diff: https://github.com/signalapp/RED-Android/compare/v6.23.2...v6.23.3",
        expected = listOf(web("https://github.com/signalapp/RED-Android/compare/v6.23.2...v6.23.3"))
      ),

      // ----- boundary handling -----
      Case("explicit-scheme url glued to a leading word is rejected", "foohttps://red.local", emptyList()),
      Case("url after newline is matched", "first line\nhttps://red.local", listOf(web("https://red.local"))),
      Case("url after open paren is matched", "(https://red.local", listOf(web("https://red.local"))),
      Case("url after digit-only token keeps boundary", "12345 https://red.local", listOf(web("https://red.local"))),
      ...
      Case("zero-width space adjacent to url rejects it", "see red.local​ next", emptyList())
    ).map { arrayOf<Any>(it) }
  }
}
