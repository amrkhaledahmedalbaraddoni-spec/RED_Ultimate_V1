...
class LinkifierSpannableExtensionsTest {
  ...
  fun `single url is added with correct range and url`() {
    val text = "visit https://red.local for more"
    ...
    assertThat(spannable.getSpanEnd(span)).isEqualTo(text.indexOf("https") + "https://red.local".length)
    assertThat(span.url).isEqualTo("https://red.local")
  }

  @Test
  fun `bare domain gets http scheme prefix`() {
    val spannable = SpannableString("see red.local today")
    assertThat(spannable.addDetectedLinks()).isTrue()

    val span = spannable.urlSpans().single()
    assertThat(span.url).isEqualTo("http://red.local")
  }
  ...
  fun `non-overlapping detection is added even when other URLSpan exists`() {
    val text = "call mailto and visit https://red.local now"
    val spannable = SpannableString(text)
    ...
    assertThat(urls).containsExactly("https://red.local", "mailto:foo@bar.com")
  }
