...
import org.signal.archive.stream.EncryptedBackupReader.Companion.createForREDBackup
...
class EncryptedBackupWriter private constructor(
  ...
  companion object {
    ...
    fun createForREDBackup(
      ...
      return createForREDBackup(key, key.deriveBackupId(aci), forwardSecrecyToken, forwardSecrecyMetadata, outputStream, append)
    }
    ...
    /**
     * Create a writer for a local backup or for a transfer to a linked device. Basically everything that isn't [createForREDBackup].
