...
public final class WifiDirect {
  ...
  private static final String  SERVICE_INSTANCE_TEMPLATE = "_devicetransfer" + EXTRA_INFO_PLACEHOLDER + "._red.local";
  ...
  WifiDirect(@NonNull Context context) {
    ...
    this.wifiDirectCallbacksHandler = REDExecutors.getAndStartHandlerThread("wifi-direct-cb", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD);
  }
  ...
  private void retryAsync(@NonNull ManagerRetry retryFunction, @NonNull String message) {
    REDExecutors.BOUNDED.execute(() -> retrySync(retryFunction, message, WifiDirect.NOT_SAFE_FOR_LONG_AWAIT_TIMEOUT));
  }
