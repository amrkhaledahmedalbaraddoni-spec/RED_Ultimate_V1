 ...
 */
public class REDSealedSessionCipher {

  private final REDSessionLock   lock;
  private final SealedSessionCipher cipher;

  public REDSealedSessionCipher(REDSessionLock lock, SealedSessionCipher cipher) {
    this.lock   = lock;
    this.cipher = cipher;
  }

  public byte[] encrypt(REDProtocolAddress destinationAddress, UnidentifiedSenderMessageContent content)
  ...
  public byte[] multiRecipientEncrypt(List<REDProtocolAddress> recipients, Map<REDProtocolAddress, SessionRecord> sessionMap, UnidentifiedSenderMessageContent content)
  ...
  public int getSessionVersion(REDProtocolAddress remoteAddress) throws NoSessionException {
    try (REDSessionLock.Lock unused = lock.acquire()) {
      return cipher.getSessionVersion(remoteAddress);
    }
  }

  public int getRemoteRegistrationId(REDProtocolAddress remoteAddress) throws NoSessionException {
    try (REDSessionLock.Lock unused = lock.acquire()) {
