...
class KeysApi(
   ...
   * - 429: Rate limited
   */
  fun getPreKeySync(destination: REDServiceAddress, deviceId: Int): NetworkResult<PreKeyBundle> {
    return getPreKeysBySpecifierSync(destination, null, deviceId.toString())
      .then { bundles ->
        if (bundles.isNotEmpty()) {
          NetworkResult.Success(bundles[0])
        } else {
          val address = REDProtocolAddress(destination.identifier, deviceId)
   ...
   * - 429: Rate limited
   */
  private fun getPreKeysBySpecifierSync(destination: REDServiceAddress, sealedSenderAccess: SealedSenderAccess?, deviceSpecifier: String): NetworkResult<List<PreKeyBundle>> {
  ...
  private fun preKeysRequest(destination: REDServiceAddress, deviceSpecifier: String): WebSocketRequestMessage {
  ...
  private fun NetworkResult<PreKeyResponse>.toPreKeyBundles(destination: REDServiceAddress): NetworkResult<List<PreKeyBundle>> {
