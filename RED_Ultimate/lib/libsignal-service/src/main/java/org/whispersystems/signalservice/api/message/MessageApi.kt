...
class MessageApi(
   ...
   * - 404: Message is not a story and recipient is not a registered RED user
...
fun MultiRecipientMessageResponse.unsentTargets(): Set<ServiceId> {
  return unregisteredIds.mapTo(HashSet(unregisteredIds.size)) { ServiceId.fromLibRED(it) }
}
