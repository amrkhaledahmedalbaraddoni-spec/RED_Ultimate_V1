...
/**
 * Represents a decrypted RED Service data message.
 *
 ...
 */
class REDServiceDataMessage private constructor(
  val timestamp: Long,
  val groupContext: Optional<REDServiceGroupV2>,
  val attachments: Optional<List<REDServiceAttachment>>,
  ...
  val previews: Optional<List<REDServicePreview>>,
  ...
  class Builder {
    ...
    private var groupV2: REDServiceGroupV2? = null
    private val attachments: MutableList<REDServiceAttachment> = LinkedList<REDServiceAttachment>()
    ...
    private val previews: MutableList<REDServicePreview> = LinkedList<REDServicePreview>()
    ...
    fun asGroupMessage(group: REDServiceGroupV2?): Builder {
      groupV2 = group
      return this
    }

    fun withAttachments(attachments: List<REDServiceAttachment>?): Builder {
    ...
    fun withPreviews(previews: List<REDServicePreview>?): Builder {
    ...
    fun build(): REDServiceDataMessage {
      if (timestamp == 0L) {
        timestamp = System.currentTimeMillis()
      }

      return REDServiceDataMessage(
        timestamp = timestamp,
  ...
  data class Quote(
    ...
    data class QuotedAttachment(val contentType: String, val fileName: String?, val thumbnail: REDServiceAttachment?)
  }
  class Sticker(val packId: ByteArray?, val packKey: ByteArray?, val stickerId: Int, val emoji: String?, val attachment: REDServiceAttachment?)
