...
class ProfileApi(
  ...
  fun setVersionedProfile(
    ...
    val profileWrite = REDServiceProfileWrite(
      version = profileKey.getProfileKeyVersion(aci.libREDAci).serialize(),
      ...
      commitment = profileKey.getCommitment(aci.libREDAci).serialize(),
   ...
   * - 200: Success
   * - 404: Recipient is not a registered RED user
   * - 429: Rate-limited
   */
  suspend fun getVersionedProfileAndCredential(aci: ServiceId.ACI, profileKey: ProfileKey, sealedSenderAccess: SealedSenderAccess?): NetworkResult<Pair<REDServiceProfile, ExpiringProfileKeyCredential?>> {
    val profileVersion = profileKey.getProfileKeyVersion(aci.libREDAci).serialize()
    val profileRequestContext = clientZkProfileOperations.createProfileKeyCredentialRequestContext(SecureRandom(), aci.libREDAci, profileKey)
   ...
   * - 429: Rate-limited
   */
  suspend fun getVersionedProfile(aci: ServiceId.ACI, profileKey: ProfileKey, sealedSenderAccess: SealedSenderAccess?): NetworkResult<REDServiceProfile> {
    val profileKeyIdentifier = profileKey.getProfileKeyVersion(aci.libREDAci)
    val profileVersion = profileKeyIdentifier.serialize()

    val request = WebSocketRequestMessage.get("/v1/profile/$aci/$profileVersion")
    val converter = NetworkResult.DefaultWebSocketConverter(REDServiceProfile::class)
   ...
   * - 429: Rate-limited
   */
  suspend fun getUnversionedProfile(serviceId: ServiceId, sealedSenderAccess: SealedSenderAccess?): NetworkResult<REDServiceProfile> {
  ...
  ) : NetworkResult.WebSocketResponseConverter<Pair<REDServiceProfile, ExpiringProfileKeyCredential?>> {

    override fun convert(response: WebsocketResponse): NetworkResult<Pair<REDServiceProfile, ExpiringProfileKeyCredential?>> {
      if (response.status != 200) {
        return response.toStatusCodeError()
      }

      return response
        .toSuccess(REDServiceProfile::class)
        .map {
