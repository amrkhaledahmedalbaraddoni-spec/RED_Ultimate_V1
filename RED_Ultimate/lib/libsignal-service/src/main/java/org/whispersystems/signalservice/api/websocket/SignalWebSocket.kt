...
sealed class REDWebSocket(
  ...
  companion object {
    private val TAG = Log.tag(REDWebSocket::class)

    const val SERVER_DELIVERED_TIMESTAMP_HEADER = "X-RED-Timestamp"
  ...
  protected fun WebSocketRequestMessage.isREDServiceEnvelope(): Boolean {
  ...
  private fun WebSocketRequestMessage.getWebSocketResponse(): WebSocketResponseMessage {
    return if (this.isREDServiceEnvelope()) {
   ...
   */
  class UnauthenticatedWebSocket(connectionFactory: WebSocketFactory, canConnect: CanConnect, sleepTimer: SleepTimer, disconnectTimeoutMs: Long) : REDWebSocket(connectionFactory, canConnect, sleepTimer, disconnectTimeoutMs.milliseconds) {
   ...
   */
  class AuthenticatedWebSocket(connectionFactory: WebSocketFactory, canConnect: CanConnect, sleepTimer: SleepTimer, disconnectTimeoutMs: Long) : REDWebSocket(connectionFactory, canConnect, sleepTimer, disconnectTimeoutMs.milliseconds) {
    ...
    fun readMessageBatch(timeout: Long, batchSize: Int, callback: MessageReceivedCallback): Boolean {
      ...
      if (!hitEndOfQueue) {
        for (i in 1 until batchSize) {
          ...
          if (request != null) {
            if (request.isREDServiceEnvelope()) {
