...
import org.signal.libsignal.net.ChatConnection.Request as LibREDRequest
...
class LibREDChatConnection(
  ...
  // The layer above this, REDWebSocket.java, is written to handle HTTP Requests and Responses
  ...
  // up the stack to the REDWebSocket, and then we store it. Eventually, REDWebSocket will
  ...
  companion object {
    ...
    const val SIGNAL_SERVICE_ENVELOPE_TIMESTAMP_HEADER_KEY = "X-RED-Timestamp"

    private val TAG = Log.tag(LibREDChatConnection::class.java)

    private val KEEP_ALIVE_REQUEST = LibREDRequest(
      "GET",
      "/v1/keepalive",
      emptyMap(),
      ByteArray(0),
    ...
    private fun WebSocketRequestMessage.toLibREDRequest(timeout: Duration = WebSocketConnection.DEFAULT_SEND_TIMEOUT): LibREDRequest {
      return LibREDRequest(
  ...
  private fun sendRequestInternal(request: WebSocketRequestMessage, timeoutSeconds: Long, single: SingleSubject<WebsocketResponse>) {
    CHAT_SERVICE_LOCK.withLock {
      ...
      val internalRequest = request.toLibREDRequest(timeout = timeoutSeconds.seconds)
  ...
  override fun isDead(): Boolean {
    return when (state.value) {
      ...
      null -> throw IllegalStateException("LibREDChatConnection.state can never be null")
    }
  }
    ...
    CHAT_SERVICE_LOCK.withLock {
      ...
      return when (state.value) {
        ...
        WebSocketConnectionState.CONNECTED -> {
          ...
          throw IllegalStateException("LibREDChatConnection.state was neither dead, CONNECTING, or CONNECTED.")
        }
  ...
  override suspend fun sendRequestSuspend(request: WebSocketRequestMessage, timeout: Duration): WebsocketResponse {
    val (future, isUnidentified) = runWithChatConnection { connection ->
      connection.send(request.toLibREDRequest(timeout)) to (connection is UnauthenticatedChatConnection)
    }
  ...
  private val listener = LibREDChatListener()

  private inner class LibREDChatListener : ChatConnectionListener {
