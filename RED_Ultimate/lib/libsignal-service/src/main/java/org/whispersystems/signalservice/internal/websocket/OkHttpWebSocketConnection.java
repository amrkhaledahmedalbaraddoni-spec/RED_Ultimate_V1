...
public class OkHttpWebSocketConnection extends WebSocketListener implements WebSocketConnection {
  ...
  public OkHttpWebSocketConnection(String name,
                                   REDServiceConfiguration serviceConfiguration,
                                   ...
                                   boolean allowStories)
  {
    ...
    this.trustStore          = serviceConfiguration.getREDServiceUrls()[0].getTrustStore();
    this.credentialsProvider = credentialsProvider;
    this.signalAgent         = signalAgent;
    this.interceptors        = serviceConfiguration.getNetworkInterceptors();
    this.dns                 = serviceConfiguration.getDns();
    this.signalProxy         = serviceConfiguration.getREDProxy();
    ...
    this.serviceUrls         = serviceConfiguration.getREDServiceUrls();
  ...
  private Pair<REDServiceUrl, String> getConnectionInfo() {
    REDServiceUrl serviceUrl = serviceUrls[random.nextInt(serviceUrls.length)];
  ...
  public synchronized Observable<WebSocketConnectionState> connect() {
    ...
    if (client == null) {
      Pair<REDServiceUrl, String> connectionInfo = getConnectionInfo();
      REDServiceUrl               serviceUrl     = connectionInfo.getFirst();
      ...
      if (signalAgent != null) {
        requestBuilder.addHeader("X-RED-Agent", signalAgent);
      }
      ...
      requestBuilder.addHeader("X-RED-Receive-Stories", allowStories ? "true" : "false");
