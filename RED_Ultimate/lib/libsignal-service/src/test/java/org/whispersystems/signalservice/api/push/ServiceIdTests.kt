...
import org.signal.libsignal.protocol.ServiceId.Aci as LibREDAci
import org.signal.libsignal.protocol.ServiceId.Pni as LibREDPni
...
class ServiceIdTests {
  ...
  fun `ServiceId parseOrNull ByteArray`() {
    ...
    assertEquals(PNI.from(UUID.fromString(uuidString)), ServiceId.parseOrNull(LibREDPni(uuid).toServiceIdBinary()))
    assertEquals(ACI.from(UUID.fromString(uuidString)), ServiceId.parseOrNull(LibREDAci(uuid).toServiceIdFixedWidthBinary()))
    assertEquals(PNI.from(UUID.fromString(uuidString)), ServiceId.parseOrNull(LibREDPni(uuid).toServiceIdFixedWidthBinary()))
  }
  ...
  fun `ACI parseOrNull String`() {
    ...
    assertNull(ACI.parseOrNull(LibREDPni(uuid).toServiceIdString()))
  ...
  fun `ACI parseOrNull ByteArray`() {
    ...
    assertNull(ACI.parseOrNull(LibREDPni(uuid).toServiceIdBinary()))

    assertEquals(ACI.from(UUID.fromString(uuidString)), ACI.parseOrNull(uuidBytes))
    assertEquals(ACI.from(UUID.fromString(uuidString)), ACI.parseOrNull(LibREDAci(uuid).toServiceIdBinary()))
    assertEquals(ACI.from(UUID.fromString(uuidString)), ACI.parseOrNull(LibREDAci(uuid).toServiceIdFixedWidthBinary()))
  }
  ...
  fun `PNI parseOrNull ByteArray`() {
    ...
    assertNull(PNI.parseOrNull(LibREDAci(uuid).toServiceIdFixedWidthBinary()))

    assertEquals(PNI.from(UUID.fromString(uuidString)), PNI.parseOrNull(uuidBytes))
    assertEquals(PNI.from(UUID.fromString(uuidString)), PNI.parseOrNull(LibREDPni(uuid).toServiceIdBinary()))
  }
