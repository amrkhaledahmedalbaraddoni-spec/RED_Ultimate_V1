...
class MessageApiV2(
  ...
  suspend fun sendSealedSenderMessage(
    ...
    return unauthWebSocket.runCatchingWithChatConnection { connection ->
      UnauthMessagesService(connection).sendMessage(serviceId.libREDServiceId, timestamp, contents, auth, onlineOnly, urgent)
    }
  }
  ...
  suspend fun sendUnsealedSenderMessage(
    ...
    return authWebSocket.runCatchingWithChatConnection { connection ->
      AuthMessagesService(connection).sendMessage(serviceId.libREDServiceId, timestamp, contents, onlineOnly, urgent)
    }
  }
