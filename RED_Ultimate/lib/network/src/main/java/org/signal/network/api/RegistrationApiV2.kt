...
import org.signal.core.util.serialization.REDJson
 ...
 */
class RegistrationApiV2(private val restClient: REDRestClient) {
  ...
  suspend fun createVerificationSession(e164: String, fcmToken: String?, mcc: String?, mnc: String?): RequestResult<SessionMetadata, CreateSessionError> {
    ...
    return result.toTypedResult(
      parseSuccess = { REDJson.json.decodeFromString<SessionMetadata>(it.bodyString()) },
      mapError = { error ->
    ...
    return result.toTypedResult(
      ...
      mapError = { error ->
        when (error.statusCode) {
          ...
          429 -> UpdateSessionError.RateLimited(error.retryAfter(), REDJson.json.decodeFromString<SessionMetadata>(error.bodyString()))
          else -> null
        }
      }
    )
    ...
    return result.toTypedResult(
      ...
      mapError = { error ->
        when (error.statusCode) {
          ...
          409 -> RequestVerificationCodeError.MissingRequestInformationOrAlreadyVerified(REDJson.json.decodeFromString<SessionMetadata>(error.bodyString()))
          418 -> RequestVerificationCodeError.CouldNotFulfillWithRequestedTransport(REDJson.json.decodeFromString<SessionMetadata>(error.bodyString()))
          429 -> RequestVerificationCodeError.RateLimited(error.retryAfter(), REDJson.json.decodeFromString<SessionMetadata>(error.bodyString()))
          440 -> RequestVerificationCodeError.ThirdPartyServiceError(REDJson.json.decodeFromString<ThirdPartyServiceErrorResponse>(error.bodyString()))
          else -> null
        }
      }
    )
    ...
    return result.toTypedResult(
      ...
      mapError = { error ->
        when (error.statusCode) {
          ...
          409 -> SubmitVerificationCodeError.SessionAlreadyVerifiedOrNoCodeRequested(REDJson.json.decodeFromString<SessionMetadata>(error.bodyString()))
          429 -> SubmitVerificationCodeError.RateLimited(error.retryAfter(), REDJson.json.decodeFromString<SessionMetadata>(error.bodyString()))
          else -> null
        }
      }
    )
  ...
  suspend fun registerAccount(
    ...
    return result.toTypedResult(
      parseSuccess = { REDJson.json.decodeFromString<RegisterAccountResponse>(it.bodyString()) },
      mapError = { error ->
        when (error.statusCode) {
          ...
          423 -> RegisterAccountError.RegistrationLock(REDJson.json.decodeFromString<RegistrationLockResponse>(error.bodyString()))
  ...
  suspend fun checkSvr2AuthCredentials(e164: String, credentials: List<SvrCredentials>): RequestResult<CheckSvrCredentialsResponse, CheckSvrCredentialsError> {
    ...
    return result.toTypedResult(
      parseSuccess = { REDJson.json.decodeFromString<CheckSvrCredentialsResponse>(it.bodyString()) },
      mapError = { error ->
        when (error.statusCode) {
  ...
  suspend fun registerAsSecondaryDevice(
    ...
    return result.toTypedResult(
      parseSuccess = { REDJson.json.decodeFromString<LinkDeviceResponse>(it.bodyString()) },
      mapError = { error ->
        when (error.statusCode) {
  ...
  private inline fun <reified T> T.toJsonRequestBody(): RequestBody {
    return REDJson.json.encodeToString(this).toRequestBody(APPLICATION_JSON)
  }
