 ...
 */
class RemoteConfigApi(val authWebSocket: REDWebSocket.AuthenticatedWebSocket, val pushServiceSocket: PushServiceSocket) {
    ...
    override fun convert(response: WebsocketResponse): NetworkResult<RemoteConfigResult> {
      return if (response.status < 200 || response.status > 299) {
        ...
        NetworkResult.Success(
          RemoteConfigResult(
            config = transformed,
            serverEpochTimeMilliseconds = response.getHeader(REDWebSocket.SERVER_DELIVERED_TIMESTAMP_HEADER).toLongOrNull() ?: System.currentTimeMillis(),
