...
/**
 * Description of a single REST request to be executed by [REDRestClient].
 */
data class RequestSpec(
  ...
  /** Which set of RED URLs the request should be routed through. */
  sealed interface Host {
    /** The standard RED service URLs. */
    data object Service : Host

    /** A specific RED CDN. [number] selects the entry in [REDServiceConfiguration.signalCdnUrlMap]. */
    data class Cdn(val number: Int) : Host

    /** The RED storage service URLs. */
  ...
  sealed interface Auth {
    ...
    /** Basic auth derived from the CredentialsProvider passed to the [REDRestClient] constructor. */
