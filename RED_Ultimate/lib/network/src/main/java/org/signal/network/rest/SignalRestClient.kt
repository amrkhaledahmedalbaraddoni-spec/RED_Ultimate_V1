 ...
 */
class REDRestClient @JvmOverloads constructor(
  private val configuration: REDServiceConfiguration,
  ...
  companion object {
    private val TAG = Log.tag(REDRestClient::class)
    ...
    private fun createConnectionHolders(
      urls: Array<out REDUrl>,
      interceptors: List<Interceptor>,
      dns: Optional<Dns>,
      proxy: Optional<REDProxy>,
  ...
  private fun buildHttpRequest(
    ...
    when (val auth = spec.auth) {
      ...
      is RequestSpec.Auth.Standard -> {
        val provider = checkNotNull(credentialsProvider) { "RequestSpec.Auth.Basic requires a CredentialsProvider on REDRestClient" }
    ...
    if (signalAgent != null) {
      builder.addHeader("X-RED-Agent", signalAgent)
    }
  ...
  private fun basicAuthHeader(provider: CredentialsProvider): String {
    ...
    val identifier = if (provider.deviceId != REDServiceAddress.DEFAULT_DEVICE_ID) {
