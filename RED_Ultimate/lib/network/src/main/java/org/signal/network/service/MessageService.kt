...
open class MessageService(
  private val localAddress: REDServiceAddress,
  private val localDeviceId: Int,
  private val messageApi: MessageApiV2,
  private val keysApi: KeysApiV2,
  private val protocolStore: REDServiceAccountDataStore,
  private val sessionLock: REDSessionLock,
  private val cipher: REDServiceCipher,
  ...
  private val localProtocolAddress: REDProtocolAddress = REDProtocolAddress(localAddress.identifier, localDeviceId)
  ...
  suspend fun Raise<SendError>.handleMismatched(error: MismatchedDeviceException, sealedSenderAccess: SealedSenderAccess?) {
    ...
    for (entry in error.entries) {
      for (staleDeviceId in entry.staleDevices) {
        ...
        protocolStore.archiveSession(REDProtocolAddress(entry.account, staleDeviceId))
      }

      for (extraDeviceId in entry.extraDevices) {
        Log.w(TAG, "Archiving extra session: (${entry.account}, $extraDeviceId)")
        protocolStore.archiveSession(REDProtocolAddress(entry.account, extraDeviceId))
      }
      ...
      for (missingDeviceId in entry.missingDevices) {
        ...
        val address = REDProtocolAddress(entry.account, missingDeviceId)
        initializeSession(ServiceId.fromLibRED(entry.account), address, sealedSenderAccess)
      }
    }
  }
  ...
  private suspend fun Raise<SendError>.encryptForAllDevices(
    ...
    return targetDeviceIds(serviceId).map { deviceId ->
      val address = REDProtocolAddress(serviceId.libREDServiceId, deviceId)
  ...
  private fun OutgoingPushMessage.toUnsealedMessage(): SingleOutboundUnsealedMessage {
    ...
    val message = when (type) {
      Envelope.Type.PREKEY_MESSAGE.value -> PreKeyREDMessage(bytes)
      Envelope.Type.DOUBLE_RATCHET.value -> REDMessage(bytes)
  ...
  private fun targetDeviceIds(serviceId: ServiceId): List<Int> {
    val devices: MutableSet<Int> = protocolStore.getSubDeviceSessions(serviceId.toString())
      .filter { protocolStore.containsSession(REDProtocolAddress(serviceId.libREDServiceId, it)) }
      .toMutableSet()

    devices += REDServiceAddress.DEFAULT_DEVICE_ID
  ...
  internal open suspend fun Raise<SendError>.initializeSession(
    ...
    try {
      REDSessionBuilder(sessionLock, SessionBuilder(protocolStore, address, localProtocolAddress)).process(bundle)
    } catch (e: UntrustedIdentityException) {
      raise(SendError.IdentityMismatch(serviceId, e))
    } catch (e: InvalidKeyException) {
      raise(SendError.ApplicationError(e))
   ...
   * a [com.red.sovereign.database.MessageSendLogTables] entry (or a pending PNI signature record)
