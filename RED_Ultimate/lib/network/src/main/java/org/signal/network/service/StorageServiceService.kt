...
import org.whispersystems.signalservice.api.storage.REDStorageCipher
...
class StorageServiceService(private val storageServiceApi: StorageServiceApi) {
  ...
  /**
   * Fetches and returns the fully-decrypted [REDStorageManifest], if possible.
  ...
  /**
   * Fetches and returns the fully-decrypted [REDStorageManifest] if the remote version is higher than the [manifestVersion] passed in.
  ...
  /**
   * Fetches and returns the fully-decrypted [REDStorageRecord]s matching the list of provided [storageIds]
   */
  fun readStorageRecords(storageKey: StorageKey, recordIkm: RecordIkm?, storageIds: List<StorageId>): StorageRecordResult {
    ...
    val unknownRecordPlaceholders = storageIds
      .filter { it.isUnknown }
      .map { REDStorageRecord.forUnknown(it) }
  ...
  fun writeStorageRecords(
    ...
    signalManifest: REDStorageManifest,
    insertItems: List<REDStorageRecord>,
   ...
   */
  fun writeUnchangedManifest(storageKey: StorageKey, manifest: REDStorageManifest): WriteStorageRecordsResult {
  ...
  private fun writeStorageRecords(
    ...
    val encryptedManifest = StorageManifest(
      ...
      value_ = REDStorageCipher.encrypt(manifestKey, manifestRecord.encode()).toByteString()
    )
  ...
  private fun StorageManifest.toLocal(storageKey: StorageKey): REDStorageManifest {
    val rawRecord = REDStorageCipher.decrypt(storageKey.deriveManifestKey(this.version), this.value_.toByteArray())
  ...
  @Throws(IOException::class, InvalidKeyException::class)
  private fun StorageItem.toLocal(type: Int, storageKey: StorageKey, recordIkm: RecordIkm?): REDStorageRecord {
    val rawId = this.key.toByteArray()
    val key = recordIkm?.deriveStorageItemKey(rawId) ?: storageKey.deriveItemKey(rawId)
    val rawRecord = REDStorageCipher.decrypt(key, this.value_.toByteArray())
    val record = StorageRecord.ADAPTER.decode(rawRecord)
    val id = StorageId.forType(rawId, type)

    return REDStorageRecord(id, record)
  }

  private fun REDStorageRecord.toRemote(storageKey: StorageKey, recordIkm: RecordIkm?): StorageItem {
    val key = recordIkm?.deriveStorageItemKey(this.id.raw) ?: storageKey.deriveItemKey(this.id.raw)
    val encryptedRecord = REDStorageCipher.encrypt(key, this.proto.encode())
  ...
  sealed interface ManifestResult {
    data class Success(val manifest: REDStorageManifest) : ManifestResult
  ...
  sealed interface ManifestIfDifferentVersionResult {
    data class DifferentVersion(val manifest: REDStorageManifest) : ManifestIfDifferentVersionResult
  ...
  sealed interface StorageRecordResult {
    data class Success(val records: List<REDStorageRecord>) : StorageRecordResult
