...
class REDRestClientTest {
  ...
  private fun client(random: Random = ScriptedRandom(0)): REDRestClient {
    return REDRestClient(
  ...
  private fun testConfiguration(): REDServiceConfiguration {
    return REDServiceConfiguration(
      signalServiceUrls = arrayOf(
        REDServiceUrl("https://service-a.test", DUMMY_TRUST_STORE),
        REDServiceUrl("https://service-b.test", DUMMY_TRUST_STORE),
        REDServiceUrl("https://service-c.test", DUMMY_TRUST_STORE)
      ),
      signalCdnUrlMap = mapOf(
        2 to arrayOf(REDCdnUrl("https://cdn2.test", DUMMY_TRUST_STORE)),
        3 to arrayOf(REDCdnUrl("https://cdn3.test", DUMMY_TRUST_STORE))
      ),
      signalStorageUrls = arrayOf(REDStorageUrl("https://storage.test", DUMMY_TRUST_STORE)),
      signalCdsiUrls = emptyArray<REDCdsiUrl>(),
      signalSvr2Urls = emptyArray<REDSvr2Url>(),
