...
import org.signal.libsignal.protocol.state.impl.InMemoryREDProtocolStore
...
class MessageServiceTest {
  ...
  private val protocolStore: REDServiceAccountDataStore = mockk(relaxUnitFun = true)
  private val sessionLock: REDSessionLock = mockk()
  private val cipher: REDServiceCipher = mockk()

  private val localAci = ServiceId.ACI.from(UUID.fromString("aaaaaaaa-0000-0000-0000-000000000001"))
  private val localAddress = REDServiceAddress(localAci)

  private val recipientAci = ServiceId.ACI.from(UUID.fromString("bbbbbbbb-0000-0000-0000-000000000002"))
  private val recipient = REDServiceAddress(recipientAci)
  ...
  fun `missing default-device session initializes session before sending`() = runTest {
    val service = newService()
    val defaultAddress = REDProtocolAddress(recipientAci.libREDServiceId, REDServiceAddress.DEFAULT_DEVICE_ID)
    every { protocolStore.getSubDeviceSessions(recipientAci.toString()) } returns emptyList()
    every { protocolStore.containsSession(defaultAddress) } returns false
    every { cipher.encrypt(any(), any(), any()) } returns OutgoingPushMessage(1, 1, 100, "AAAA")
    coEvery { keysApi.getPreKey(recipientAci.toString(), REDServiceAddress.DEFAULT_DEVICE_ID, null) } returns
    ...
    val success = (result as Either.Right).value
    assertThat(success.devices).isEqualTo(listOf(REDServiceAddress.DEFAULT_DEVICE_ID))
    coVerifyOrder {
      keysApi.getPreKey(recipientAci.toString(), REDServiceAddress.DEFAULT_DEVICE_ID, null)
  ...
  fun `sub-device without session is excluded from target devices`() = runTest {
    ...
    every { protocolStore.containsSession(REDProtocolAddress(recipientAci.libREDServiceId, 2)) } returns true
    every { protocolStore.containsSession(REDProtocolAddress(recipientAci.libREDServiceId, 3)) } returns false
    every { cipher.encrypt(any(), any(), any()) } returns OutgoingPushMessage(1, 1, 100, "AAAA")
    ...
    verify { cipher.encrypt(REDProtocolAddress(recipientAci.libREDServiceId, 1), any(), any()) }
    verify { cipher.encrypt(REDProtocolAddress(recipientAci.libREDServiceId, 2), any(), any()) }
    verify(exactly = 0) { cipher.encrypt(REDProtocolAddress(recipientAci.libREDServiceId, 3), any(), any()) }
  }

  @Test
  fun `local send with no other devices sends to own device`() = runTest {
    val service = newService()
    val localProtocolAddress = REDProtocolAddress(localAci.libREDServiceId, REDServiceAddress.DEFAULT_DEVICE_ID)
  ...
  fun `MismatchedDeviceException archives extras and fetches missing prekeys`() = runTest {
    ...
    verify { protocolStore.archiveSession(REDProtocolAddress(recipientAci.libREDServiceId, 5)) }
  ...
  fun `MismatchedDeviceException archives stale devices`() = runTest {
    ...
    verify { protocolStore.archiveSession(REDProtocolAddress(recipientAci.libREDServiceId, 3)) }
  }
  ...
  fun `sync send to self with no other devices stops instead of looping`() = runTest {
    ...
    every { protocolStore.getSubDeviceSessions(localAci.toString()) } returns emptyList()
    every { cipher.encrypt(any(), any(), any()) } returns OutgoingPushMessage(1, 1, 100, validSerializedREDMessageBase64())
    ...
    verify { protocolStore.archiveSession(REDProtocolAddress(localAci.libREDServiceId, 1)) }
  ...
  fun `sync send that reached a real linked device is not treated as no other devices`() = runTest {
    ...
    every { cipher.encrypt(any(), any(), any()) } returns OutgoingPushMessage(1, 2, 100, validSerializedREDMessageBase64())
  ...
  fun `NoSessionException during encryption archives the session and surfaces a retryable error`() = runTest {
    ...
    verify { protocolStore.archiveSession(REDProtocolAddress(recipientAci.libREDServiceId, REDServiceAddress.DEFAULT_DEVICE_ID)) }
    val app = (result as Either.Left).value as MessageService.SendError.ApplicationError
  ...
  private fun mismatchedException(
    ...
    val entry = MismatchedDeviceException.Entry(
      account = account.libREDServiceId,
  ...
  /**
   * Produces a genuinely valid, native-parseable serialized [org.signal.libsignal.protocol.message.REDMessage]
   * (a WHISPER_TYPE message) encoded as base64. The unsealed send path wraps the encrypted bytes in
   * `REDMessage(bytes)`, whose native constructor rejects arbitrary input — so the fallback test needs real
   * ciphertext here rather than a placeholder like "AAAA". We get one by establishing a real session between two
   * in-memory stores and capturing a post-handshake reply (the initiator's first message is a PreKeyREDMessage,
   * so we round-trip once to reach a plain REDMessage).
   */
  private fun validSerializedREDMessageBase64(): String {
    val aliceAddress = REDProtocolAddress("alice", 1)
    val bobAddress = REDProtocolAddress("bob", 1)

    val aliceStore = InMemoryREDProtocolStore(IdentityKeyPair.generate(), 1)
    val bobStore = InMemoryREDProtocolStore(IdentityKeyPair.generate(), 2)
    ...
    bobCipher.decrypt(PreKeyREDMessage(preKeyMessage.serialize()))

    val reply = bobCipher.encrypt("reply".toByteArray())
    check(reply.type == CiphertextMessage.WHISPER_TYPE) { "Expected a WHISPER_TYPE REDMessage but got ${reply.type}" }

    return Base64.encodeWithPadding(reply.serialize())
  }

  private fun createPreKeyBundle(store: InMemoryREDProtocolStore, deviceId: Int): PreKeyBundle {
  ...
  private fun newService(maxContentSizeBytes: Long = 0L): MessageService {
    ...
    coEvery {
      ...
      val addressArg = arg<REDProtocolAddress>(2)
