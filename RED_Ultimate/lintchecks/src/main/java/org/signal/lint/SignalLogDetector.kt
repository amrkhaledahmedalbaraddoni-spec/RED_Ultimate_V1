...
class REDLogDetector : Detector(), Detector.UastScanner {
  ...
  override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
    ...
    if (evaluator.isMemberInClass(method, "android.util.Log")) {
      context.report(
        ...
        message = "Using 'android.util.Log' instead of a RED Logger",
    ...
    if (evaluator.isMemberInClass(method, "org.signal.glide.Log")) {
      context.report(
        ...
        scope = node,
        location = context.getLocation(node),
        message = "Using 'org.signal.glide.Log' instead of a RED Logger",
        quickfixData = quickFixIssueLog(node)
      )
    }
    ...
    if (evaluator.isMemberInClass(method, "org.signal.libsignal.protocol.logging.Log")) {
      context.report(
        ...
        scope = node,
        location = context.getLocation(node),
        message = "Using RED server logger instead of app level Logger",
        quickfixData = quickFixIssueLog(node)
      )
    }
  ...
  companion object {
    val LOG_NOT_SIGNAL: Issue = Issue.create(
      id = "LogNotRED",
      briefDescription = "Logging call to Android Log instead of RED's Logger",
      explanation = "RED has its own logger which must be used.",
      category = MESSAGES,
      priority = 5,
      severity = ERROR,
      implementation = Implementation(REDLogDetector::class.java, JAVA_FILE_SCOPE)
    )

    val LOG_NOT_APP: Issue = Issue.create(
      id = "LogNotAppRED",
      briefDescription = "Logging call to RED Service Log instead of App level Logger",
      explanation = "RED app layer has its own logger which must be used.",
      category = MESSAGES,
      priority = 5,
      severity = ERROR,
      implementation = Implementation(REDLogDetector::class.java, JAVA_FILE_SCOPE)
