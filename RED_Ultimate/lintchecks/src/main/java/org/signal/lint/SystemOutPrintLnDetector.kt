...
class SystemOutPrintLnDetector : Detector(), Detector.UastScanner {
  ...
  override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
    ...
    if (evaluator.isMemberInClass(method, "java.io.PrintStream")) {
      if (isSystemOutCall(node.receiver)) {
        context.report(
          ...
          message = "Using 'System.out.${method.name}' instead of RED Logger",
    ...
    if (method.name == "println" && evaluator.isMemberInClass(method, "kotlin.io.ConsoleKt")) {
      context.report(
        ...
        scope = node,
        location = context.getLocation(node),
        message = "Using 'kotlin.io.println' instead of RED Logger.",
        quickfixData = createQuickFix(node)
      )
    }
  }
  ...
  companion object {
    val SYSTEM_OUT_PRINTLN_USAGE: Issue = Issue.create(
      ...
      explanation = "System.out.println/print should not be used in production code. Use RED Logger instead.",
      category = MESSAGES,
      priority = 5,
      severity = ERROR,
