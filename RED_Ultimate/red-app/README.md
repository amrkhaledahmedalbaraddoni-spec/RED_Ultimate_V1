...
Registration, approval, public-only libsignal enrollment keys, certificate verification, rotating sessions, foreground WebSocket transport, persistent libsignal protocol records, one-to-one PQXDH/Double-Ratchet encryption, durable ciphertext storage with SENT/DELIVERED/READ ACKs, synchronized feed, MinIO uploads, 24-hour story cleanup and authorized PSTN dialing are implemented in source. Private identity/pre-key/session/Sender-Key records are encrypted before SQLite with a non-exportable Android Keystore AES-GCM key. SQLCipher protection for routing metadata, safety-number/QR UI, one-time pre-key replenishment, Sender Keys group-message encryption, secure image/video rendering and Android WebRTC/SFU media remain gated and require successful APK/backend builds; incomplete controls stay visibly disabled.
