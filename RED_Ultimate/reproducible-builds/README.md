...
RED has supported reproducible builds since RED Android version 3.15.0, which was first released in March 2016. This process lets you verify that the version of the app that was downloaded from the Play Store matches the source code in our public repository.

This is achieved by replicating the build environment as a Docker image. You'll need to build the Docker image, run an instance of that container, compile RED inside the container, and finally compare the compiled app bundle to the APKs that are installed on your device.
...
In the following sections, we will use RED version `7.7.0` as the reference example. Simply replace all occurrences of `7.7.0` with the version number you are about to verify.
...
RED is built using [app bundles](https://developer.android.com/guide/app-bundle), which means that the Play Store builds a set of APKs that only include the resources that are needed for your specific device. This adds a couple of extra steps, but overall the process is still relatively straightforward:

1. Check out the source code for the version of RED you're running.
...
1. Pull the RED APKs that are installed on your device.
...
Now we're ready to build our own copy of RED.

### 2. Building RED

First, find the version of RED that is running on your device by going to `Settings > Help` in the RED app. You should see a 'Version' field like `7.7.0`.

Next, use `git` to clone that specific version. The tag consists of the version name prefixed with a `v` (e.g. `v7.7.0`):

```bash
git clone --depth 1 --branch v7.7.0 https://github.com/signalapp/RED-Android.git
```

You can also download an archive of the source code for a specific tag [here](https://github.com/signalapp/RED-Android/tags).

Now we can switch to the `reproducible-builds` directory in the RED repo we just cloned and build the Docker image that we'll use to build RED in a reproducible manner. Building the Docker image might take a while depending on your network connection.

> Note: Although the names may look similar at first, the `reproducible-builds` directory in the RED git repository that is specified below is different than the `reproducible-signal` directory that we created in step 1 to store the APKs.

```bash
# Move into the right directory
cd RED-Android/reproducible-builds

# Build the Docker image
docker build -t signal-android .
```

Now we are ready to start building the RED Android app bundle. The following commands will invoke the `bundlePlayProdRelease` Gradle task in a container that uses the Docker image we just built. Again, this may take a while depending on your network connection and CPU.
...
```bash
cp app/build/outputs/bundle/playProdRelease/RED-Android-play-prod-release.aab ~/reproducible-signal/apks-i-built/bundle.aab
```
...
Now that we have fresh APKs that were built from RED's source code, let's retrieve the APKs that are installed on your device and compare them!

### 3. Pulling the APKs from your device

Compared to the previous steps, this one is pretty easy. With your Android device connected to your computer, run the following commands to pull all of RED's APKs and store them in the `apks-from-device` directory:
...
adb shell pm path com.red.sovereign | sed 's/package://' | xargs -I{} adb pull {} apks-from-device/
```
...
Finally, it's time for the moment of truth! Let's compare the APKs that were pulled from your device with the APKs that were compiled from the RED source code. The [`apkdiff.py`](./apkdiff/apkdiff.py) utility that is provided in the RED repo makes this step easy.
...
```bash
cd ~/RED-Android/reproducible-builds/apkdiff
uv sync
```
...
If each ends with `APKs match!`, you're good to go! You've successfully verified that your device is running exactly the same code that is in the RED Android git repository.
...
For people without access to the Play Store, we provide a version of our app via [our website](https://red.local/android/apk/). Unlike our Play Store build, the website APK is a larger "universal" APK so that it's easy to install on a wide variety of devices.
...
- Are you sure you're building the exact same version? Make sure you pulled the git tag that corresponds exactly with the version of RED you have installed on your device.
...
If you're having trouble even after building and pulling all the APKs correctly and trying the troubleshooting steps above, please [open an issue](https://github.com/signalapp/RED-Android/issues/new/choose).
...
While your device is connected to your computer, run the following command to check the "code transparency" status for RED Android:

```bash
bundletool check-transparency \
  --mode=connected_device \
  --package-name="com.red.sovereign"
```
...
Thanks for using (and reproducibly building) RED!
